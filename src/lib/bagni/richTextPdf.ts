/**
 * richTextPdf — converte il testo dei campi "prosa" del template Bagni
...
export interface BgnRichRun {
  text: string;
  bold?: boolean;
  italic?: boolean;
}

export interface BgnRichBlock {
...
function parseInlineRuns(html: string): BgnRichRun[] {
  const runs: BgnRichRun[] = [];
  let bold = 0;
  let italic = 0;
...
function htmlToBlocks(html: string): BgnRichBlock[] {
  const blocks: BgnRichBlock[] = [];
...
function plainTextToBlocks(text: string): BgnRichBlock[] {
  const blocks: BgnRichBlock[] = [];
 ...
 */
export function htmlToRichBlocks(input: string | null | undefined): BgnRichBlock[] {
...
export function richBlocksToPlainText(blocks: BgnRichBlock[]): string {
  return blocks
