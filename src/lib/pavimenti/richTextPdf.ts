/**
 * richTextPdf — converte il testo dei campi "prosa" del template Pavimenti
...
export interface PavRichRun {
  text: string;
  bold?: boolean;
  italic?: boolean;
}

export interface PavRichBlock {
...
function parseInlineRuns(html: string): PavRichRun[] {
  const runs: PavRichRun[] = [];
  let bold = 0;
  let italic = 0;
...
function htmlToBlocks(html: string): PavRichBlock[] {
  const blocks: PavRichBlock[] = [];
...
function plainTextToBlocks(text: string): PavRichBlock[] {
  const blocks: PavRichBlock[] = [];
 ...
 */
export function htmlToRichBlocks(input: string | null | undefined): PavRichBlock[] {
...
export function richBlocksToPlainText(blocks: PavRichBlock[]): string {
  return blocks
