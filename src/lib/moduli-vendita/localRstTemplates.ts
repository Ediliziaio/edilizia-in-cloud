...
export function localRstTemplateKey(companyId: string, moduleId: FullRstModuleId) {
  if (!companyId || !isFullRstModuleId(moduleId)) throw new Error("Azienda o modulo non valido.");
  return `eic:full-rst-module:v1:${encodeURIComponent(companyId)}:${moduleId}`;
}

function assertRecord(value: unknown, companyId: string, moduleId: FullRstModuleId): asserts value is LocalRstTemplate {
  const r = value as LocalRstTemplate | null;
  const t = r?.template;
  ...
  if (!r || r.version !== 1 || r.companyId !== companyId || r.moduleId !== moduleId ||
    ...
    !t || t.company_id !== companyId || t.id !== `local-ristrutturazioni-${moduleId}` ||
...
export function saveLocalRstTemplate(companyId: string, moduleId: FullRstModuleId, template: RstTemplatePdf, expectedSavedAt: string | null, storage: StoragePort = localStorage): LocalRstTemplate {
  ...
  const record: LocalRstTemplate = { version: 1, companyId, moduleId,
    savedAt: new Date(Math.max(Date.now(), existing ? Date.parse(existing.savedAt) + 1 : 0)).toISOString(),
    template: { ...template, id: `local-ristrutturazioni-${moduleId}`, company_id: companyId } };
  assertRecord(record, companyId, moduleId);
  try { storage.setItem(localRstTemplateKey(companyId, moduleId), JSON.stringify(record)); }
