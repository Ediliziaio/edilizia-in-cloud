...
import { IDR_EDITORIAL, IDR_MODULE_TITLES, type FullIdrModuleId } from "./fullIdrModules";

export function idrCopyChoices(id: FullIdrModuleId, template: IdrTemplatePdf): InterventionCopyChoice[] {
  const content = IDR_EDITORIAL[id];
  ...
  return [
    ...
    { id: "cover-diretta", section: "Copertina", label: "Proposta essenziale", preview: `${IDR_MODULE_TITLES[id]}\nLa configurazione, le opere e le condizioni della tua proposta.`, patch: { cover_title: IDR_MODULE_TITLES[id], pdf_cover_hero: IDR_MODULE_TITLES[id], cover_subtitle: "La configurazione, le opere e le condizioni della tua proposta.", pdf_cover_subhero: "La configurazione, le opere e le condizioni della tua proposta." } },
