/**
 * richTextPdf — converte il testo dei campi "prosa" del template Piscine
...
export interface PisRichRun {
  text: string;
  bold?: boolean;
  italic?: boolean;
}

export interface PisRichBlock {
...
function parseInlineRuns(html: string): PisRichRun[] {
  const runs: PisRichRun[] = [];
  let bold = 0;
  let italic = 0;
...
function htmlToBlocks(html: string): PisRichBlock[] {
  const blocks: PisRichBlock[] = [];
...
function plainTextToBlocks(text: string): PisRichBlock[] {
  const blocks: PisRichBlock[] = [];
 ...
 */
export function htmlToRichBlocks(input: string | null | undefined): PisRichBlock[] {
...
export function richBlocksToPlainText(blocks: PisRichBlock[]): string {
  return blocks
