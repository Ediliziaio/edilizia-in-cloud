/**
 * richTextPdf — converte il testo dei campi "prosa" del template Tetti
...
export interface TetRichRun {
  text: string;
  bold?: boolean;
  italic?: boolean;
}

export interface TetRichBlock {
...
function parseInlineRuns(html: string): TetRichRun[] {
  const runs: TetRichRun[] = [];
  let bold = 0;
  let italic = 0;
...
function htmlToBlocks(html: string): TetRichBlock[] {
  const blocks: TetRichBlock[] = [];
...
function plainTextToBlocks(text: string): TetRichBlock[] {
  const blocks: TetRichBlock[] = [];
 ...
 */
export function htmlToRichBlocks(input: string | null | undefined): TetRichBlock[] {
...
export function richBlocksToPlainText(blocks: TetRichBlock[]): string {
  return blocks
