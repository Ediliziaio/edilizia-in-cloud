/**
 * richTextPdf — converte il testo dei campi "prosa" del template Ristrutturazione
...
export interface RstRichRun {
  text: string;
  bold?: boolean;
  italic?: boolean;
}

export interface RstRichBlock {
...
function parseInlineRuns(html: string): RstRichRun[] {
  const runs: RstRichRun[] = [];
  let bold = 0;
  let italic = 0;
...
function htmlToBlocks(html: string): RstRichBlock[] {
  const blocks: RstRichBlock[] = [];
...
function plainTextToBlocks(text: string): RstRichBlock[] {
  const blocks: RstRichBlock[] = [];
 ...
 */
export function htmlToRichBlocks(input: string | null | undefined): RstRichBlock[] {
...
export function richBlocksToPlainText(blocks: RstRichBlock[]): string {
  return blocks
