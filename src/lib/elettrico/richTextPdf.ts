/**
 * richTextPdf — converte il testo dei campi "prosa" del template Elettrico
...
export interface EleRichRun {
  text: string;
  bold?: boolean;
  italic?: boolean;
}

export interface EleRichBlock {
...
function parseInlineRuns(html: string): EleRichRun[] {
  const runs: EleRichRun[] = [];
  let bold = 0;
  let italic = 0;
...
function htmlToBlocks(html: string): EleRichBlock[] {
  const blocks: EleRichBlock[] = [];
...
function plainTextToBlocks(text: string): EleRichBlock[] {
  const blocks: EleRichBlock[] = [];
 ...
 */
export function htmlToRichBlocks(input: string | null | undefined): EleRichBlock[] {
...
export function richBlocksToPlainText(blocks: EleRichBlock[]): string {
  return blocks
