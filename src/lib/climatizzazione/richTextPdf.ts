/**
 * richTextPdf — converte il testo dei campi "prosa" del template Climatizzazione
...
export interface ClmRichRun {
  text: string;
  bold?: boolean;
  italic?: boolean;
}

export interface ClmRichBlock {
...
function parseInlineRuns(html: string): ClmRichRun[] {
  const runs: ClmRichRun[] = [];
  let bold = 0;
  let italic = 0;
...
function htmlToBlocks(html: string): ClmRichBlock[] {
  const blocks: ClmRichBlock[] = [];
...
function plainTextToBlocks(text: string): ClmRichBlock[] {
  const blocks: ClmRichBlock[] = [];
 ...
 */
export function htmlToRichBlocks(input: string | null | undefined): ClmRichBlock[] {
...
export function richBlocksToPlainText(blocks: ClmRichBlock[]): string {
  return blocks
