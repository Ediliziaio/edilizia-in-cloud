...
import { Phone, Clock, MessageSquare, User, Calendar, ExternalLink, Euro } from "lucide-react";
...
export function CallDetailDrawer({ callId, open, onOpenChange }: Props) {
  ...
  // Fetch credit usage for this call
  const { data: creditUsage } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        ...
        .select("cost_billed_total, duration_min")
        .eq("conversation_id", callId!)
        .maybeSingle();
      return data as { cost_billed_total: number; duration_min: number } | null;
    },
  });
        ...
        {isLoading || !call ? (
          ...
          <div className="mt-4 space-y-4">
            ...
            <div className="flex items-center gap-2 text-sm text-muted-foreground">
              ...
              <span>{format(new Date(call.started_at), "dd MMMM yyyy 'alle' HH:mm", { locale: it })}</span>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="flex items-center gap-2 text-sm">
                ...
                <div className="flex-1 min-w-0">
                  <p className="truncate">{call.contact_name ?? "Sconosciuto"}</p>
                  {call.contact_id && (
                    <button
                      type="button"
                      className="text-[10px] text-primary flex items-center gap-0.5 hover:underline"
                      onClick={() => { onOpenChange(false); navigate(`/azienda/marketing/contatti/${call.contact_id}`); }}
                    >
                      <ExternalLink className="h-2.5 w-2.5" /> Apri nel CRM
                    </button>
                  )}
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 text-sm">
                ...
                <span>{call.messages_count} msg</span>
              </div>
              {creditUsage?.cost_billed_total != null && (
                <div className="flex items-center gap-2 text-sm col-span-2">
                  <Euro className="h-4 w-4 text-muted-foreground" />
                  <span>Costo: <strong>€{creditUsage.cost_billed_total.toFixed(4)}</strong> ({creditUsage.duration_min.toFixed(2)} min)</span>
                </div>
              )}
            </div>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <Badge variant={call.status === "completed" ? "default" : call.status === "failed" ? "destructive" : "outline"}>
                {call.status}
              </Badge>
            ...
            {call.summary && (
              <div className="rounded-lg border bg-muted/40 px-3 py-2.5">
                <p className="text-xs font-medium text-muted-foreground mb-1">Riepilogo AI</p>
            ...
            <Tabs defaultValue="actions" className="w-full">
              <TabsList className="w-full">
                <TabsTrigger value="actions" className="flex-1">
                  Azioni CRM {actions.length > 0 && <span className="ml-1 text-[10px] bg-primary/20 text-primary rounded-full px-1">{actions.length}</span>}
                </TabsTrigger>
