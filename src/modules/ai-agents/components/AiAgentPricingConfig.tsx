...
export function AiAgentPricingConfig() {
  const queryClient = useQueryClient();
  const [globalMarkup, setGlobalMarkup] = useState("3.0");
  const [editedPricing, setEditedPricing] = useState<AiAgentPricingRow[]>([]);
  const [dirtyRows, setDirtyRows] = useState<Set<string>>(new Set());

  const { data: pricing, isLoading } = useQuery({
    queryKey: ["ai-agent-pricing"],
    queryFn: async (): Promise<AiAgentPricingRow[]> => {
      const { data, error } = await supabase
        .from("ai_agent_pricing" as never)
        .select("*")
        .order("cost_real_per_unit", { ascending: true });
      if (error) throw error;
      return (data as unknown as AiAgentPricingRow[]) ?? [];
    },
  });
  ...
  const updateRow = (id: string, field: keyof AiAgentPricingRow, value: unknown) => {
      ...
      prev.map((r) => {
        ...
        if (field === "cost_real_per_unit" || field === "markup_multiplier") {
          updated.cost_billed_per_unit = Number(
            ((updated.cost_real_per_unit || 0) * (updated.markup_multiplier || 3)).toFixed(6)
          );
        }
        return updated;
      })
  ...
  const savePricingRow = async (row: AiAgentPricingRow) => {
    ...
    toast.success(`Tariffa "${row.label || row.model_tier}" aggiornata`);
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          <Bot className="h-4 w-4" /> Prezzi Agenti AI & Markup
        </CardTitle>
        <CardDescription>Costo reale per tier e prezzo addebitato alle aziende per chiamata/minuto/token.</CardDescription>
      </CardHeader>
        ...
        <div className="bg-primary/5 border border-primary/20 rounded-lg p-5">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-muted-foreground">Applica a tutte le tariffe Agenti AI</p>
            </div>
            <Input
          ...
          <div className="bg-background border rounded-lg px-5 py-3 mt-4 grid grid-cols-3 gap-4 text-center">
            <div>
              ...
              <p className="text-sm font-mono">{formatEur(previewReal, 4)}/unit</p>
            </div>
            <div>
            ...
            <div>
              ...
              <p className="text-xl font-extrabold">{formatEur(previewBilled, 4)}/unit</p>
            </div>
          </div>
          <p className="text-xs text-primary font-mono mt-2">
            Margine: {previewMarginPct}% · {formatEur(previewBilled - previewReal, 4)} per unità
          </p>
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>Costo Reale (€/unit)</TableHead>
                  <TableHead>Markup</TableHead>
                  <TableHead>Costo Azienda (€/unit)</TableHead>
                ...
                {editedPricing.map((row) => {
                  ...
                  return (
                    <TableRow key={row.id}>
                      ...
                      <TableCell>
                        <Badge className={`text-[10px] ${TIER_COLORS[row.model_tier] ?? "bg-gray-100 text-gray-800"}`}>
                          {TIER_LABELS[row.model_tier] ?? row.model_tier}
                        </Badge>
                      </TableCell>
                      <TableCell>
                        <span className="text-xs font-mono">{row.unit_label}</span>
                      </TableCell>
                      <TableCell>
