...
describe("tassonomia area → intervento", () => {
  it("definisce 14 aree e 109 interventi senza identificatori duplicati", () => {
    expect(SALES_AREAS).toHaveLength(14);
    expect(new Set(SALES_AREAS.map(area => area.id)).size).toBe(14);
    expect(SALES_AREAS.flatMap(area => area.interventions)).toHaveLength(109);
