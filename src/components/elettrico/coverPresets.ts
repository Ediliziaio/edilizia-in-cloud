...
/**
 * coverPresets.ts (Elettrico) — Preset layout cover PDF.
 *
 * Clone di src/components/serramenti/coverPresets.ts, adattato al modulo
 * Elettrico. I NOMI dei campi pdf_cover_* sono IDENTICI a Serramenti (così la
 * migration `ele_template_pdf` ↔ `sr_template_pdf` resta in parità e il PDF
 * legge gli stessi campi). Cambiano solo i `sampleTitle` (tema impianti
 * elettrici/casa) — puro testo decorativo della mini-anteprima.
 *
 ...
 *
 * NOTA TIPI: `ele_template_pdf` non è nei tipi generati di Supabase e i campi
 * pdf_cover_* non fanno parte di `EleTemplatePdf`. Qui definiamo un'interfaccia
 * locale `EleCoverFields` con i tipi/union dei campi cover, così il patch è
 * tipizzato e combacia col tipo del form dell'editor (che riusa `EleCoverFields`).
 */

/** Sottoinsieme cover dei campi `pdf_cover_*` (parità con sr_template_pdf). */
export interface EleCoverFields {
...
export type CoverPresetPatch = Partial<Pick<EleCoverFields,
  | "pdf_cover_bg_color"
  | "pdf_cover_image_url"
...
const STOCK_FALLBACK_HOUSE = "/cover-stock/elettrico/1.jpg";
