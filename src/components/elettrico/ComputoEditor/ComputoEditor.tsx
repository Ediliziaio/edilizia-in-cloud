 ...
 *
 * Assembla le `CapitoloSection` raggruppando le `EleComputoVoce[]` per
 ...
 *
 * Contratto: stato controllato `value: EleComputoVoce[]` + `onChange`. I capitoli
...
export default function ComputoEditor({
  ...
  const capitoli = useMemo<CapitoloGroup[]>(() => {
    const order: string[] = [];
    const map = new Map<string, EleComputoVoce[]>();
  ...
  const replaceCapitoloVoci = useCallback(
    (nome: string, nextVoci: EleComputoVoce[]) => {
