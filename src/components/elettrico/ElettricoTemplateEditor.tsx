...
import { createFullEltTemplate, buildEltModulePreview, ELT_MODULE_TITLES, type FullEltModuleId } from "@/lib/moduli-vendita/fullEltModules";
...
type FormState = Required<Pick<EleTemplatePdf,
  ...
  | "gallery_lavori" | "finanziamento_promo"
>> & EleCoverFields;
...
interface Props {
  ...
  embedded?: boolean;
  localModule?: { id: FullEltModuleId; template: EleTemplatePdf; saved: boolean; save: (template: EleTemplatePdf) => void; onDirtyChange: (dirty: boolean) => void };
}

export function ElettricoTemplateEditor({ embedded = false, localModule }: Props) {
  ...
  const [needsFirstLocalSave, setNeedsFirstLocalSave] = useState(() => !!localModule && !localModule.saved);
  // A new local copy can be saved without being an edited document. Reverting
  // edits also clears the leave guard by comparing the actual form snapshot.
  const dirty = localModule ? JSON.stringify(form) !== localBaseline : legacyDirty;
  const [moduleDefaults] = useState(() => localModule ? createFullEltTemplate(localModule.template, localModule.id) : null);
  ...
  const previewTemplate = useMemo<EleTemplatePdf | null>(
    () => (form ? ({ ...localModule?.template, id: localModule?.template.id ?? "preview", company_id: companyId ?? "", ...form } as EleTemplatePdf) : null),
    [form, companyId, localModule?.template],
  );
  ...
  const applyStockImage = useCallback((img: Pick<CoverStockImage, "url">) => {
  ...
  }, [companyId, setCoverImage, localModule]);
  ...
  const handlePreview = async () => {
    ...
    const template: EleTemplatePdf = { ...localModule?.template, id: localModule?.template.id ?? "preview", company_id: companyId, ...form };
    if (localModule) { await previewPDF(buildEltModulePreview(companyId, template, localModule.id)); return; }
    const row = (
  ...
  const sectionFromUrl = (searchParams.get("section") ?? (localModule ? "page_cover" : "brand")) as EleSection;
    ...
    <div className={cn("space-y-4", embedded ? "" : "mx-auto max-w-4xl p-4")}>
      ...
      <div className="rounded-xl border border-orange-200 bg-gradient-to-br from-orange-50 to-amber-50 p-4 dark:border-orange-900/40 dark:from-orange-950/30 dark:to-amber-950/20">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-[12px] text-muted-foreground">
                {localModule ? "Personalizza testi e immagini delle singole pagine, poi verifica il PDF a destra." : "Genera una bozza dei testi, poi rifinisci e salva."}
              </p>
            </div>
          </div>
          {!localModule && <Button
          ...
          {localModule && moduleDefaults ? <InterventionTextPicker title={ELT_MODULE_TITLES[localModule.id]} choices={eltCopyChoices(localModule.id, moduleDefaults)} onApply={patch => { setForm(prev => prev ? { ...prev, ...patch } : prev); setDirty(true); }} /> : <StandardTextTemplatePicker
            module="elettrico"
      ...
      <TemplateEditorWorkspace moduleId={localModule?.id}>
        ...
        <TemplateEditorNavigation>
          <nav className={templateEditorLayout.navigationPanel}>
            <TemplateSectionNavigation groups={ELE_SECTION_GROUPS} activeSection={activeSection} onSelect={setActiveSection} isExcluded={id => edileSectionExcluded(form, id)} />
...
if (field === "subtitle") {
                        ...
                        setDirty(true);
                      } }}   placeholders={(value, onChange) => <PlaceholderChips value={value} onChange={onChange}  />} />
...
<ImageUploadField localOnly={!!localModule}
                  ...
                  />
<TemplateImageFieldView label="Immagine copertina" value={form.pdf_cover_image_url ?? null} busy={uploadingCover} disabled={!companyId} localOnly={!!localModule} inputRef={coverInputRef} onFile={handleCoverUpload} onRemove={() => setCoverImage(null)} />
...
{ id: "eyebrowSize", kind: "range", value: form.pdf_cover_eyebrow_size ?? 11, min: 8, max: 20, step: 1, unit: "pt", onChange: value => set("pdf_cover_eyebrow_size", value) },
{ id: "titleSize", kind: "range", value: form.pdf_cover_title_size ?? 40, min: 22, max: 64, step: 1, unit: "pt", onChange: value => set("pdf_cover_title_size", value) },
{ id: "subtitleSize", kind: "range", value: form.pdf_cover_subtitle_size ?? 13, min: 9, max: 22, step: 1, unit: "pt", onChange: value => set("pdf_cover_subtitle_size", value) },
...
{ id: "showClientCard", kind: "toggle", value: !!(form.pdf_cover_show_client_card !== false), onChange: value => set("pdf_cover_show_client_card", value) }
          ...
          {activeSection === "opzioni" && (
            <>
              ...
              <FinanziamentoPromoField
                ...
                onChange={(v) => set("finanziamento_promo", { ...v })}
              />
              </SectionCard>
          ...
          <TemplateEditorSaveBar>
            <span className={cn("text-[11px]", dirty ? "text-amber-600" : "text-muted-foreground")}>
              {dirty ? "Modifiche non salvate" : needsFirstLocalSave ? "Nuovo modulo · non ancora salvato" : "Tutto salvato"}
            </span>
            <div className="flex items-center gap-2">
              ...
              <Button
                ...
                disabled={(!dirty && !needsFirstLocalSave) || upsert.isPending}
        ...
        <aside data-template-preview className={templateEditorLayout.preview}>
          <div className={templateEditorLayout.previewPanel}>
            <ElettricoLivePreviewPanel activeSection={activeSection} moduleId={localModule?.id} template={previewTemplate} companyId={companyId} />
          </div>
        </aside>
        ...
        <DialogContent className="max-w-4xl max-h-[85vh] flex flex-col p-0 gap-0">
          ...
          <div className="flex-1 overflow-y-auto p-4">
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
              {(localModule && moduleDefaults ? fotoDellaLibreria("elettrico", moduleDefaults.pdf_blocchi).map((f, i) => ({ id: `modulo-${i}`, url: f.url, thumb: f.url, label: f.nome })) : stockFiltered).map((img) => {
