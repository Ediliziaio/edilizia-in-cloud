...
import { ElettricoTemplateEditor } from "./ElettricoTemplateEditor";
import type { EleTemplatePdf } from "@/types/elettrico";
import { createFullEltTemplate, ELT_MODULE_TITLES, type FullEltModuleId } from "@/lib/moduli-vendita/fullEltModules";
import { loadLocalEltTemplate, saveLocalEltTemplate } from "@/lib/moduli-vendita/localEltTemplates";

export function EltModuleTemplatePanel({ moduleId }: { moduleId: FullEltModuleId }) {
  const companyId = useEffectiveCompanyId();
  const { data: base, isLoading, isError } = useEleTemplatePdf();
  if (!companyId || isLoading) return <p role="status">Caricamento modello Elettrico…</p>;
...
function Workspace({ companyId, moduleId, base }: { companyId: string; moduleId: FullEltModuleId; base: EleTemplatePdf }) {
  const [params, setParams] = useSearchParams();
  const [initial] = useState(() => {
    try { const record = loadLocalEltTemplate(companyId, moduleId); return { record, template: record?.template ?? createFullEltTemplate(base, moduleId), error: "" }; }
  ...
  useEffect(() => {
    const guard = (event: MouseEvent) => {
      ...
      if (!dirtyRef.current || !el?.closest("a,button,[role=tab]") || el.closest("[data-elt-module-editor]")) return;
