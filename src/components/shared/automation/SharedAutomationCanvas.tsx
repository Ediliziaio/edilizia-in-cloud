...
/** Minimal node shape the canvas needs for pan/zoom/drag */
...
interface Props<N extends CanvasNode> {
  ...
  onUpdateNode: (id: string, updates: Partial<N>) => void;
  onOpenTriggerPicker: () => void;
  onOpenActionPicker: () => void;
  /** Show settings button in bottom toolbar */
  showSettingsButton?: boolean;
  /** Render connections SVG content — receives display-adjusted nodes (drag offset applied) */
  renderConnections: (displayNodes: N[]) => ReactNode;
  /** Render a single node — receives display-adjusted node */
  renderNode: (displayNode: N, handlers: {
    isSelected: boolean;
    onDragStart: (id: string, e: React.MouseEvent) => void;
  }) => ReactNode;
}

export function SharedAutomationCanvas<N extends CanvasNode>({
  nodes, selectedNodeId, onSelectNode, onUpdateNode,
  onOpenTriggerPicker, onOpenActionPicker,
  showSettingsButton, renderConnections, renderNode,
}: Props<N>) {
  ...
  // Compute display nodes (with drag offset applied)
  const getDisplayNodes = useCallback((): N[] => {
    if (!dragNodeId || !dragOffset) return nodes;
    return nodes.map(n =>
      n.id === dragNodeId
        ? { ...n, position_x: dragStart.current.nodeX + dragOffset.x, position_y: dragStart.current.nodeY + dragOffset.y }
        : n
    );
  }, [nodes, dragNodeId, dragOffset]);
