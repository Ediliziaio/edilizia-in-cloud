...
export function ContactAppointmentsPanel({ contactId, companyId, contactName, calendars, users }: Props) {
  ...
  const upcoming = appointments.filter((a: any) => new Date(a.appointment_date).getTime() >= todayStart.getTime() && a.status !== "annullato");
  const past = appointments.filter((a: any) => new Date(a.appointment_date).getTime() < todayStart.getTime() || a.status === "annullato");
    ...
    return (
      ...
      >
        ...
        <div className="flex items-center gap-1">
          <Badge variant={statusMeta.variant} className="text-[9px] h-4 px-1">
            {statusMeta.label}
          </Badge>
