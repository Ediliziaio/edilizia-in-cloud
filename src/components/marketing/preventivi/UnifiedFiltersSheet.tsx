...
const TIPO_OPTIONS: Array<{ value: PreventivoTipo; label: string; emoji: string }> = [
  ...
  { value: "termoidraulico",   label: "Termoidraulico",   emoji: "🔥" },
  { value: "pavimenti",        label: "Pavimenti",        emoji: "🟫" },
...
interface Props {
  ...
  termoidraulicoEnabled: boolean;
  pavimentiEnabled: boolean;
...
export function UnifiedFiltersSheet({
  ...
  serramentiEnabled, fotovoltaicoEnabled, ristrutturazioneEnabled, bagniEnabled, tettiEnabled, climatizzazioneEnabled, elettricoEnabled, termoidraulicoEnabled, pavimentiEnabled, piscineEnabled, totalResults,
}: Props) {
          ...
          <Section icon={<Layers className="h-3.5 w-3.5" />} title="Tipo preventivo">
            <div className="flex flex-wrap gap-2">
              {TIPO_OPTIONS.map((t) => {
                ...
                if (t.value === "termoidraulico" && !termoidraulicoEnabled) return null;
                if (t.value === "pavimenti" && !pavimentiEnabled) return null;
                if (t.value === "piscine" && !piscineEnabled) return null;
