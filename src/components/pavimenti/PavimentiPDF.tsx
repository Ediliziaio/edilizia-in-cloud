/**
 * PavimentiPDF — documento PDF nativo A4 del preventivo Pavimenti
 * (Task 21), generato lato client con @react-pdf/renderer.
 *
 * Struttura pagine (modellata su SerramentoPDF, scala verticale Pavimenti):
...
function richRunStyle(r: PavRichRun): Record<string, unknown> {
...
function PageFooter({ styles, company, code, footerText, anagrafica, showVersion, showLegal }: {
  styles: Styles;
  company: PavPdfEnriched["company"];
  code: string | null;
...
function PageHeader({ styles, companyName, logoUrl, code }: {
  ...
  return (
    <View style={styles.header} fixed>
      ...
      <View>
        <Text style={styles.headerRight}>Preventivo pavimenti</Text>
...
function CapitoloTable({ styles, cap, showMargine, mostraPrezzi, mostraQta, mostraSubtotali }: {
  styles: Styles; cap: PavPdfCapitolo; showMargine: boolean;
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Footer cover: riferimento + totale (value-prop Pavimenti, sempre
            in basso, distinto dal blocco testo). */}
        <View style={{ position: "absolute", bottom: 70, left: 44, right: 44 }}>
          <View style={styles.coverTotalBox}>
            <Text style={styles.coverTotalLabel}>
              Investimento totale (IVA inclusa) · Rif. {p.code ?? "—"} · {dateStr()}
            </Text>
