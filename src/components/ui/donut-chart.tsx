...
// Donut/ring chart animato con Framer Motion: i segmenti si disegnano in
// sequenza e si evidenziano con un glow all'hover. Usa le CSS var del tema.
...
export interface DonutChartSegment {
  value: number;
  color: string; // colore CSS valido (es. hsl(var(--chart-1)))
  label: string;
  [key: string]: unknown; // dati extra ammessi
}
...
interface DonutChartProps extends React.HTMLAttributes<HTMLDivElement> {
  ...
  /** Segmento attivo controllato dall'esterno (es. legenda) */
  activeLabel?: string | null;
  /** Callback all'hover di un segmento */
        ...
        <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} className="overflow-visible -rotate-90">
          ...
          <AnimatePresence>
            {data.map((segment, index) => {
              ...
              const isActive = (hoveredSegment?.label ?? activeLabel) === segment.label;
