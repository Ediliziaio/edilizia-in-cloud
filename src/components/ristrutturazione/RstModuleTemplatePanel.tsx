...
import { RistrutturazioneTemplateEditor } from "./RistrutturazioneTemplateEditor";
import type { RstTemplatePdf } from "@/types/ristrutturazione";
import { createFullRstTemplate, RST_MODULE_TITLES, type FullRstModuleId } from "@/lib/moduli-vendita/fullRstModules";
import { loadLocalRstTemplate, saveLocalRstTemplate } from "@/lib/moduli-vendita/localRstTemplates";

export function RstModuleTemplatePanel({ moduleId }: { moduleId: FullRstModuleId }) {
  const companyId = useEffectiveCompanyId();
  const { data: base, isLoading, isError } = useRstTemplatePdf();
  if (!companyId || isLoading) return <p role="status">Caricamento modello Ristrutturazioni…</p>;
...
function Workspace({ companyId, moduleId, base }: { companyId: string; moduleId: FullRstModuleId; base: RstTemplatePdf }) {
  const [params, setParams] = useSearchParams();
  const [initial] = useState(() => {
    try { const record = loadLocalRstTemplate(companyId, moduleId); return { record, template: record?.template ?? createFullRstTemplate(base, moduleId), error: "" }; }
  ...
  useEffect(() => {
    const guard = (event: MouseEvent) => {
      ...
      if (!dirtyRef.current || !el?.closest("a,button,[role=tab]") || el.closest("[data-rst-module-editor]")) return;
  ...
  return <section className="space-y-4">
    ...
    <div className="flex flex-wrap justify-between gap-2"><Button variant="outline" onClick={() => navigate(false)}>← Moduli Ristrutturazioni</Button><Button variant="ghost" onClick={() => navigate(true)}>Apri la bozza precedente</Button></div>
    <div><p className="text-xs font-semibold uppercase tracking-wider text-orange-700">Area Ristrutturazioni · Modulo PDF</p><h2 className="text-2xl font-semibold">{RST_MODULE_TITLES[moduleId]}</h2><p className="text-sm text-muted-foreground">Le pagine del modello originale, con testi e immagini dedicati a questo intervento.</p></div>
    ...
    <div data-rst-module-editor><RistrutturazioneTemplateEditor embedded localModule={{ id: moduleId, template: initial.template, saved: !!initial.record, onDirtyChange,
      save: template => { revision.current = saveLocalRstTemplate(companyId, moduleId, template, revision.current).savedAt; },
    }} /></div>
  </section>;
}
