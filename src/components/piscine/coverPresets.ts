...
const STOCK_FALLBACK_POOL =
  "https://images.unsplash.com/photo-1572331165267-854da2b10ccc?w=1600&q=80&auto=format&fit=crop";
...
export function detectActiveCoverPreset(
  ...
  for (const preset of COVER_PRESETS) {
    ...
    const allMatch = (Object.keys(patch) as (keyof CoverPresetPatch)[]).every((k) => {
      // Per preset photo, skippiamo cover_image_url (vedi commento sopra).
      if (preset.category === "photo" && k === "cover_image_url") return true;
