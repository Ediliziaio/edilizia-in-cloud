 ...
 *
 * Assembla le `CapitoloSection` raggruppando le `PisComputoVoce[]` per
 ...
 *
 * Contratto: stato controllato `value: PisComputoVoce[]` + `onChange`. I capitoli
...
interface Props {
  value: PisComputoVoce[];
  onChange: (voci: PisComputoVoce[]) => void;
  progettoId: string;
  companyId: string;
...
export default function ComputoEditor({
  ...
  const capitoli = useMemo<CapitoloGroup[]>(() => {
    const order: string[] = [];
    const map = new Map<string, PisComputoVoce[]>();
  ...
  const replaceCapitoloVoci = useCallback(
    (nome: string, nextVoci: PisComputoVoce[]) => {
