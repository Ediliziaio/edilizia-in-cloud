  ...
  return (
    <section className="py-16 md:py-24 bg-white">
      <div ref={ref} className="max-w-3xl mx-auto px-6 text-center">
        <h2
          className={`text-2xl md:text-4xl font-extrabold text-[#111111] mb-10 md:mb-14 transition-all duration-700 ${
            isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"
          }`}
        >
          Se richiedi ora la tua Dimostrazione{" "}
          <span className="text-[#F97415]">riceverai subito questo regalo:</span>
        </h2>

        <div
          className={`relative border-2 border-[#F97415] rounded-2xl p-6 md:p-10 bg-white shadow-[0_0_30px_rgba(249,116,21,0.1)] transition-all duration-700 delay-200 ${
        ...
        >
          ...
          <h3 className="text-xl md:text-2xl font-bold text-[#111111] mb-4 flex items-center justify-center gap-2">
            <BookOpen className="w-5 h-5 text-[#F97415]" />
            Corso Metodo Vendita Edile
          </h3>

          <p className="text-[#111111]/60 text-sm md:text-base leading-relaxed max-w-lg mx-auto mb-6">
            4 lezioni in cui ho racchiuso il metodo di vendita per il Settore Edile, come aumentare del 40% le vendite in Edilizia
          </p>

          <span className="inline-flex items-center gap-1.5 px-4 py-2 rounded-full bg-[#F97415] text-white text-xs font-bold tracking-wide">
            +40% VENDITE
          </span>
          ...
          <div className="flex items-center justify-center gap-2 mt-6">
            {units.map((u) => (
              <div key={u.short} className="flex flex-col items-center">
                <span className="inline-block min-w-[44px] rounded-lg border border-[#111111]/10 bg-[#111111] px-2 py-1.5 text-center font-mono text-lg md:text-xl font-bold leading-tight text-white">
                  {String(u.value).padStart(2, "0")}
                </span>
                <span className="text-[10px] text-[#111111]/50 mt-1 hidden md:block">{u.label}</span>
                <span className="text-[10px] text-[#111111]/50 mt-1 md:hidden">{u.short}</span>
              </div>
            ))}
          </div>
