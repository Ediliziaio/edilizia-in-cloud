 ...
 *
 * Assembla le `CapitoloSection` raggruppando le `ClmComputoVoce[]` per
 ...
 *
 * Contratto: stato controllato `value: ClmComputoVoce[]` + `onChange`. I capitoli
...
interface Props {
  value: ClmComputoVoce[];
  onChange: (voci: ClmComputoVoce[]) => void;
  progettoId: string;
  companyId: string;
...
export default function ComputoEditor({
  ...
  const capitoli = useMemo<CapitoloGroup[]>(() => {
    const order: string[] = [];
    const map = new Map<string, ClmComputoVoce[]>();
  ...
  const replaceCapitoloVoci = useCallback(
    (nome: string, nextVoci: ClmComputoVoce[]) => {
