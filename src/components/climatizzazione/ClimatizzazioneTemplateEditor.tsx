...
import { buildClmModulePreview, createFullClmTemplate, CLM_MODULE_TITLES, type FullClmModuleId } from "@/lib/moduli-vendita/fullClmModules";
...
interface Props {
  ...
  embedded?: boolean;
  localModule?: { id: FullClmModuleId; template: ClmTemplatePdf; saved: boolean; save: (template: ClmTemplatePdf) => void; onDirtyChange: (dirty: boolean) => void };
}

export function ClimatizzazioneTemplateEditor({ embedded = false, localModule }: Props) {
  ...
  // A fresh factory template is not an edit, even before its first local save.
  const [dirty, setDirty] = useState(false);
  const [savedLocally, setSavedLocally] = useState(() => !!localModule?.saved);
  const newLocalModule = !!localModule && !savedLocally;
  const [moduleDefaults] = useState(() => localModule ? createFullClmTemplate(localModule.template, localModule.id) : null);
  ...
  const previewTemplate = useMemo<ClmTemplatePdf | null>(
    () => (form ? ({
      id: "preview", company_id: companyId ?? "", ...form,
      // Use the already-read company snapshot for local previews. These
      // fallbacks are not persisted and never replace fields explicitly edited.
      ...(localModule ? Object.fromEntries(
        (["ragione_sociale", "indirizzo_completo", "telefono", "email", "partita_iva", "logo_url"] as const)
          .map(key => [key, form[key] || companyAnagrafica?.[key] || null]),
      ) : {}),
    } as ClmTemplatePdf) : null),
    [form, companyId, localModule, companyAnagrafica],
  );
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.success(localModule ? "Modello salvato" : "Template salvato", {
        description: localModule ? "Salvato per l'azienda, lo vedono i colleghi. Il preventivatore usa ancora il template aziendale." : "Verrà applicato ai nuovi preventivi climatizzazione.",
      });
    } catch (e) {
  ...
  const handlePreview = async () => {
    ...
    if (localModule) { await previewPDF(buildClmModulePreview(companyId, previewTemplate ?? template, localModule.id)); return; }
    const row = (
    ...
    const progetto: ClmProgetto = {
      ...
      tipo_intervento: "Climatizzazione completa", numero_unita_interne: null, tipologia_impianto: null, massimale_detrazione: null,
  ...
  const sectionFromUrl = (searchParams.get("section") ?? (localModule ? "page_cover" : "brand")) as ClmSection;
    ...
    <div className={cn("space-y-4", embedded ? "" : "mx-auto max-w-4xl p-4")}>
      ...
      <div className="rounded-xl border border-orange-200 bg-gradient-to-br from-orange-50 to-amber-50 p-4 dark:border-orange-900/40 dark:from-orange-950/30 dark:to-amber-950/20">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          {localModule && moduleDefaults ? <InterventionTextPicker title={CLM_MODULE_TITLES[localModule.id]} choices={clmCopyChoices(localModule.id, moduleDefaults)} onApply={patch => { setForm(prev => prev ? { ...prev, ...patch } : prev); setDirty(true); }} /> : <StandardTextTemplatePicker
            module="climatizzazione"
      ...
      <TemplateEditorWorkspace moduleId={localModule?.id}>
        ...
        <TemplateEditorNavigation>
          <nav className={templateEditorLayout.navigationPanel}>
            <TemplateSectionNavigation groups={CLM_SECTION_GROUPS} activeSection={activeSection} onSelect={setActiveSection} isExcluded={id => edileSectionExcluded(form, id)} />
...
if (field === "eyebrow") { set("pdf_cover_eyebrow", (value ?? "") || null); } }}   placeholders={(value, onChange) => <PlaceholderChips value={value} onChange={onChange}  />} />
...
<ImageUploadField localOnly={!!localModule}
                      ...
                      hint={localModule ? "Foto locale PNG/JPG/WEBP, massimo 1 MB, oppure immagine illustrativa dedicata." : "Carica una foto (PNG/JPG max 8 MB) o scegli dalla galleria stock."}
...
{ id: "textAlign", kind: "choice", value: form.pdf_cover_text_align ?? "left", choices: COVER_DESIGN_CHOICES.textAlign, onChange: value => { set("pdf_cover_text_align", value as typeof form.pdf_cover_text_align); set("cover_text_align", value as typeof form.cover_text_align); } },
...
{ id: "logoPosition", kind: "choice", value: form.pdf_cover_logo_position ?? "top_left", choices: COVER_DESIGN_CHOICES.logoPosition, onChange: value => { set("pdf_cover_logo_position", value as typeof form.pdf_cover_logo_position); set("cover_logo_position", value as typeof form.cover_logo_position); } },
...
{ id: "textColor", kind: "color", value: form.pdf_cover_text_color, fallback: "#FFFFFF", onChange: value => { set("pdf_cover_text_color", value); set("cover_text_color", value); } },
...
{ id: "showClientCard", kind: "toggle", value: !!(form.pdf_cover_show_client_card !== false), onChange: value => set("pdf_cover_show_client_card", value) }
          ...
          <TemplateEditorSaveBar>
            <span className={cn("text-[11px]", dirty ? "text-amber-600" : "text-muted-foreground")}>
              {dirty ? "Modifiche non salvate" : newLocalModule ? "Nuovo modulo · non ancora salvato" : "Tutto salvato"}
            </span>
            <div className="flex items-center gap-2">
              ...
              <Button
                ...
                disabled={(!dirty && !newLocalModule) || upsert.isPending}
        ...
        <aside data-template-preview className={templateEditorLayout.preview}>
          <div className={templateEditorLayout.previewPanel}>
            <ClimatizzazioneLivePreviewPanel activeSection={activeSection} moduleId={localModule?.id} template={previewTemplate} companyId={companyId} />
          </div>
        </aside>
        ...
        <DialogContent className="max-w-4xl max-h-[85vh] flex flex-col p-0 gap-0">
          ...
          <div className="flex-1 overflow-y-auto p-4">
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
              {(localModule && moduleDefaults ? fotoDellaLibreria("climatizzazione", moduleDefaults.pdf_blocchi).map((f, i) => ({ id: `modulo-${i}`, url: f.url, thumb: f.url, label: f.nome })) : stockFiltered).map((img) => {
...
function ImageUploadField({ label, hint, value, companyId, onChange, aspect = "aspect-[4/3]", localOnly = false }: ImageUploadFieldProps) {
  ...
  const handleFile = async (file: File | null) => {
    ...
    try {
      ...
      toast.error("Immagine non caricata", { description: error instanceof Error ? error.message : "Riprova." });
    } finally {
      setUploading(false);
