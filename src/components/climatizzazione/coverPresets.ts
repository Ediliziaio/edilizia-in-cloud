...
/**
 * coverPresets.ts (climatizzazione) — Preset layout cover PDF.
 *
 * Clone 1:1 di src/components/serramenti/coverPresets.ts, adattato al verticale
 * Climatizzazione. I preset NON sono solo "stili di colore" ma combinazioni
 ...
 *
 * Ogni preset applica in batch tutti questi campi sui `pdf_cover_*` del form
 * dell'editor (ClimatizzazioneTemplateEditor). I NOMI dei campi sono identici a
 * Serramenti per riuso del render PDF; i sampleTitle sono a tema clima.
 *
 * Per i preset 'photo': il preset imposta `pdf_cover_image_url` con una stock
 * image suggerita (l'utente può poi cambiarla via galleria/upload).
 */

export type CoverPresetCategory = "solid" | "photo";

/**
 * Patch tipizzato sui SOLI campi `pdf_cover_*` del form dell'editor. I tipi
 * (union literal per gli enum) combaciano con quelli aggiunti a `FormState` in
 * ClimatizzazioneTemplateEditor.tsx. Tenuto locale per non creare import circolari
 * editor↔preset.
 */
...
// URL di una stock image usata come "suggested image" per i preset photo che non
// hanno una propria foto specifica (l'utente la cambierà poi). Tema residenziale,
// coerente con un preventivo di climatizzazione.
const STOCK_FALLBACK_HOME = "/cover-stock/climatizzazione/1.jpg";
...
export const COVER_PRESETS: CoverPreset[] = [
  ...
  // ═══ PHOTO — Background immagine + overlay + testo ═════════════════════
  {
    ...
    patch: {
      ...
      pdf_cover_image_url: STOCK_FALLBACK_HOME,
 ...
 * l'utente potrebbe aver caricato una sua immagine (e va benissimo, il preset
 * comunque "funziona" come layout).
 */
export function detectActiveCoverPreset(
  form: Partial<Record<keyof CoverPresetPatch, unknown>>,
