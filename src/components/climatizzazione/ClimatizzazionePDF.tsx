/**
 * ClimatizzazionePDF — documento PDF nativo A4 del preventivo Climatizzazione
 * (Task 21), generato lato client con @react-pdf/renderer.
 *
 * Struttura pagine (modellata su SerramentoPDF, scala verticale Climatizzazione):
...
import type { ClmProgetto, ClmTemplatePdf } from "@/types/climatizzazione";
import { htmlToRichBlocks, type ClmRichRun } from "@/lib/climatizzazione/richTextPdf";
...
function richRunStyle(r: ClmRichRun): Record<string, unknown> {
...
function makePalette(t: ClmTemplatePdf) {
...
function PageFooter({ styles, company, code, footerText, anagrafica, showVersion, showLegal }: {
  styles: Styles;
  company: ClmPdfEnriched["company"];
  code: string | null;
...
function PageHeader({ styles, companyName, logoUrl, code }: {
  ...
  return (
    <View style={styles.header} fixed>
      ...
      <View>
        <Text style={styles.headerRight}>Preventivo climatizzazione</Text>
...
function CapitoloTable({ styles, cap, showMargine, mostraPrezzi, mostraQta, mostraSubtotali }: {
  styles: Styles; cap: ClmPdfCapitolo; showMargine: boolean;
...
function TotalsBlock({ styles, totali, detrazioneText, showMargine, promo, pc }: {
  styles: Styles; totali: ClmPdfTotali; detrazioneText: string | null; showMargine: boolean;
      ...
      <Page size="A4" style={[styles.cover, { backgroundColor: coverBgColor }]}>
        ...
        {!coverImageUrl && (
          <View style={{ position: "absolute", top: 0, left: 0, width: 595, height: 841 }}>
            <Svg width={595} height={841} viewBox="0 0 595 841">
              <Defs>
                <LinearGradient id="clm-cover-depth" x1="0" y1="0" x2="0" y2="1">
                ...
                <RadialGradient id="clm-cover-glow" cx="0.24" cy="0.74" r="0.55" fx="0.24" fy="0.74">
                  <Stop offset="0" stopColor={C.secondary} stopOpacity={0.22} />
                  <Stop offset="1" stopColor={C.secondary} stopOpacity={0} />
                </RadialGradient>
              </Defs>
              <Rect x={0} y={0} width={595} height={841} fill="url(#clm-cover-depth)" />
              <Rect x={0} y={0} width={595} height={841} fill="url(#clm-cover-glow)" />
            </Svg>
          </View>
        )}
