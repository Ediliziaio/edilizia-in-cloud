/**
 * BagniPDF — documento PDF nativo A4 del preventivo Bagni
 * (Task 21), generato lato client con @react-pdf/renderer.
 *
 * Struttura pagine (modellata su SerramentoPDF, scala verticale Bagni):
...
function richRunStyle(r: BgnRichRun): Record<string, unknown> {
...
function makePalette(t: BgnTemplatePdf) {
...
function PageFooter({ styles, company, code, footerText, anagrafica, showVersion, showLegal }: {
  styles: Styles;
  company: BgnPdfEnriched["company"];
  code: string | null;
...
function PageHeader({ styles, companyName, logoUrl, code }: {
  ...
  return (
    <View style={styles.header} fixed>
      ...
      <View>
        <Text style={styles.headerRight}>Preventivo bagni</Text>
...
function CapitoloTable({ styles, cap, showMargine, mostraPrezzi, mostraQta, mostraSubtotali }: {
  styles: Styles; cap: BgnPdfCapitolo; showMargine: boolean;
...
function TotalsBlock({ styles, totali, detrazioneText, showMargine, promo, pc }: {
  styles: Styles; totali: BgnPdfTotali; detrazioneText: string | null; showMargine: boolean;
          ...
          <View style={{ position: "absolute", top: 0, left: 0, width: 595, height: 841 }}>
            <Svg width={595} height={841} viewBox="0 0 595 841">
              <Defs>
                ...
                <RadialGradient id="bgn-cover-grad-glow" cx="0.24" cy="0.74" r="0.55" fx="0.24" fy="0.74">
              ...
              <Rect x={0} y={0} width={595} height={841} fill="url(#bgn-cover-grad-glow)" />
            </Svg>
          </View>
        )}
