...
import { useBgnTemplatePdf } from "@/hooks/useBagniProgetto";
import { BagniTemplateEditor } from "./BagniTemplateEditor";
import type { BgnTemplatePdf } from "@/types/bagni";
import { createFullBgnTemplate, BGN_MODULE_TITLES, type FullBgnModuleId } from "@/lib/moduli-vendita/fullBgnModules";
import { loadLocalBgnTemplate, saveLocalBgnTemplate } from "@/lib/moduli-vendita/localBgnTemplates";

export function BgnModuleTemplatePanel({ moduleId }: { moduleId: FullBgnModuleId }) {
  const companyId = useEffectiveCompanyId();
  const { data: base, isLoading, isError } = useBgnTemplatePdf();
  if (!companyId || isLoading) return <p role="status">Caricamento modello Bagni…</p>;
...
function Workspace({ companyId, moduleId, base }: { companyId: string; moduleId: FullBgnModuleId; base: BgnTemplatePdf }) {
  const [params, setParams] = useSearchParams();
  const [initial] = useState(() => {
    try { const record = loadLocalBgnTemplate(companyId, moduleId); return { record, template: record?.template ?? createFullBgnTemplate(base, moduleId), error: "" }; }
  ...
  useEffect(() => {
    const guard = (event: MouseEvent) => {
      ...
      if (!dirtyRef.current || !el?.closest("a,button,[role=tab]") || el.closest("[data-bgn-module-editor]")) return;
  ...
  return <section className="space-y-4">
    ...
    <div className="flex flex-wrap justify-between gap-2"><Button variant="outline" onClick={() => navigate(false)}>← Moduli Bagni</Button><Button variant="ghost" onClick={() => navigate(true)}>Apri la bozza precedente</Button></div>
    <div><p className="text-xs font-semibold uppercase tracking-wider text-orange-700">Area Bagni · Modulo PDF</p><h2 className="text-2xl font-semibold">{BGN_MODULE_TITLES[moduleId]}</h2><p className="text-sm text-muted-foreground">Le pagine del modello originale, con testi e immagini dedicati a questo intervento.</p></div>
    ...
    <div data-bgn-module-editor><BagniTemplateEditor embedded localModule={{ id: moduleId, template: initial.template, saved: !!initial.record, onDirtyChange,
      save: template => { revision.current = saveLocalBgnTemplate(companyId, moduleId, template, revision.current).savedAt; },
    }} /></div>
  </section>;
}
