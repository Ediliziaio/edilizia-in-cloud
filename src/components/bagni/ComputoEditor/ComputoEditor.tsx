 ...
 *
 * Assembla le `CapitoloSection` raggruppando le `BgnComputoVoce[]` per
 ...
 *
 * Contratto: stato controllato `value: BgnComputoVoce[]` + `onChange`. I capitoli
...
interface Props {
  value: BgnComputoVoce[];
  onChange: (voci: BgnComputoVoce[]) => void;
  progettoId: string;
  companyId: string;
...
export default function ComputoEditor({
  ...
  const capitoli = useMemo<CapitoloGroup[]>(() => {
    const order: string[] = [];
    const map = new Map<string, BgnComputoVoce[]>();
  ...
  const replaceCapitoloVoci = useCallback(
    (nome: string, nextVoci: BgnComputoVoce[]) => {
