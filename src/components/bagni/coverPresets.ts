 ...
 *
 * Clone 1:1 di `src/components/serramenti/coverPresets.ts` (stesso stack
 * @react-pdf, stessi nomi colonna pdf_cover_*), con i `sampleTitle` adattati al
 * tema bagni/ristrutturazione. I preset NON sono solo "stili di colore" ma
 * combinazioni complete di LAYOUT che includono:
 ...
 * `pdf_cover_image_url` SOLO se esplicitamente fornita (stock fallback).
 */
...
export type CoverPresetPatch = Partial<Pick<BgnTemplatePdf,
  | "pdf_cover_bg_color"
  | "pdf_cover_image_url"
...
export const COVER_PRESETS: CoverPreset[] = [
  ...
  // ═══ PHOTO — Background immagine + overlay + testo ═════════════════════
  {
    ...
    patch: {
      ...
      pdf_cover_image_url: STOCK_FALLBACK_BATHROOM,
