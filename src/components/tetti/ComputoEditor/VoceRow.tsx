...
/** Unità di misura selezionabili (allineate a `TetUnitaMisura`). */
const UM_OPTIONS: TetUnitaMisura[] = ["mq", "ml", "cad", "corpo", "kg", "h", "a corpo"];

interface Props {
  voce: TetComputoVoce;
  onChange: (patch: Partial<TetComputoVoce>) => void;
  onDelete: () => void;
  onDuplicate: () => void;
  showMargine: boolean;
}
        ...
        <div className="min-w-0 flex-1 max-sm:flex max-sm:flex-wrap max-sm:items-center max-sm:gap-x-1 max-sm:gap-y-1.5">
          ...
          <div className="flex items-center gap-2 max-sm:contents">
            ...
            <Select
              ...
              onValueChange={(v) => onChange({ unita_misura: v as TetUnitaMisura })}
            >
