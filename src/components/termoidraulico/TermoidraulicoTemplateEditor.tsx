...
/**
 * TermoidraulicoTemplateEditor — editor del template PDF del verticale
 * Termoidraulico (Task 20).
 *
 * Modellato su `SerramentiTemplateEditor` ma alla scala di `idr_template_pdf`
 * (un record/azienda, upsert via `useUpsertIdrTemplatePdf`). Configura:
 ...
 * path `{company_id}/termoidraulico/template/{uuid}.{ext}` → URL pubblico
...
import { buildIdrModulePreview, createFullIdrTemplate, IDR_MODULE_TITLES, type FullIdrModuleId } from "@/lib/moduli-vendita/fullIdrModules";
...
// I preset cover 1-click ora vivono in `./coverPresets` (parità Serramenti:
// layout completo, non solo colore testo). Importati in cima al file come
// COVER_PRESETS + detectActiveCoverPreset. Il vecchio array locale "solo stile"
// è stato rimosso a favore della gallery completa.

// Forma del form locale: stesso shape del patch persistito.
// I campi legacy `cover_*` (titolo/sottotitolo/immagine + posizione logo/colore
// testo/opacità) restano la fonte dei CONTENUTI. I campi `pdf_cover_*` (parità con
// Serramenti, aggiunti dalla migration 20271110040000) sono il LAYER di STILE
// applicato dai preset 1-click e dai controlli avanzati di copertina.
type FormState = Required<Pick<IdrTemplatePdf,
...
function templateToForm(t: IdrTemplatePdf): FormState {
  return {
    ...
    pdf_cover_hero: strOr((t as unknown as Record<string, unknown>).pdf_cover_hero, null),
    pdf_cover_subhero: strOr((t as unknown as Record<string, unknown>).pdf_cover_subhero, null),
    pdf_cover_subhero_template: strOr((t as unknown as Record<string, unknown>).pdf_cover_subhero_template, null),
    pdf_cover_eyebrow: strOr((t as unknown as Record<string, unknown>).pdf_cover_eyebrow, null),
    gallery_lavori: t.gallery_lavori ?? [],
  };
}

/** Numero finito o fallback (per smallint/integer letti dalla riga grezza). */
function numOr(v: unknown, fb: number | null): number | null {
  return typeof v === "number" && Number.isFinite(v) ? v : fb;
}
function strOr(v: unknown, fb: string | null): string | null {
  return typeof v === "string" && v.length > 0 ? v : fb;
}
function boolOr(v: unknown, fb: boolean | null): boolean | null {
  return typeof v === "boolean" ? v : fb;
}

/** Estrae il layer pdf_cover_* dalla riga grezza con default da sr_template_pdf. */
function readCoverStyle(r: Record<string, unknown>): IdrCoverFields & { pdf_cover_logo_size: number | null } {
  return {
    pdf_cover_bg_color: strOr(r.pdf_cover_bg_color, null),
    pdf_cover_image_url: strOr(r.pdf_cover_image_url, null),
    pdf_cover_overlay_opacity: numOr(r.pdf_cover_overlay_opacity, null),
    pdf_cover_overlay_style: strOr(r.pdf_cover_overlay_style, "flat") ?? "flat",
    pdf_cover_text_color: strOr(r.pdf_cover_text_color, null),
    pdf_cover_text_align: strOr(r.pdf_cover_text_align, null),
    pdf_cover_text_vertical: strOr(r.pdf_cover_text_vertical, "bottom") ?? "bottom",
    pdf_cover_eyebrow_size: numOr(r.pdf_cover_eyebrow_size, null),
    pdf_cover_title_size: numOr(r.pdf_cover_title_size, null),
    pdf_cover_subtitle_size: numOr(r.pdf_cover_subtitle_size, null),
    pdf_cover_show_decoration: boolOr(r.pdf_cover_show_decoration, null),
    pdf_cover_decoration_style: strOr(r.pdf_cover_decoration_style, "square") ?? "square",
    pdf_cover_show_client_card: boolOr(r.pdf_cover_show_client_card, null),
    pdf_cover_logo_position: strOr(r.pdf_cover_logo_position, "top_left") ?? "top_left",
    pdf_cover_logo_size: numOr(r.pdf_cover_logo_size, null),
  };
}

// ─── WCAG contrast helper (per badge leggibilità testo cover) ──────────────────
...
function wcagLevel(ratio: number, largeText: boolean): "AAA" | "AA" | "FAIL" {
  if (largeText) return ratio >= 4.5 ? "AAA" : ratio >= 3 ? "AA" : "FAIL";
  return ratio >= 7 ? "AAA" : ratio >= 4.5 ? "AA" : "FAIL";
}
function suggestBestTextColor(bg: string): string {
...
interface GeneratedTemplateTexts {
  ...
  esigenze?: IdrListItem[];
  soluzione?: IdrListItem[];
  usp?: IdrListItem[];
  garanzie?: IdrListItem[];
  percorso?: IdrListItem[];
  cronoprogramma?: IdrCronoFase[];
  faq?: IdrFaqItem[];
...
interface Props {
  ...
  embedded?: boolean;
  localModule?: { id: FullIdrModuleId; template: IdrTemplatePdf; saved: boolean; save: (template: IdrTemplatePdf) => void; onDirtyChange: (dirty: boolean) => void };
}

export function TermoidraulicoTemplateEditor({ embedded = false, localModule }: Props) {
  ...
  const { data: companyAnagrafica } = useQuery({
    queryKey: ["idr-template-company-anagrafica", companyId],
    enabled: !!companyId,
    staleTime: 5 * 60 * 1000,
    queryFn: async () => {
  ...
  const { data: template, isLoading } = useIdrTemplatePdf();
  const upsert = useUpsertIdrTemplatePdf();
  // Probe: il modulo è pubblicato sul DB? Se no, l'editor mostra comunque i default
  // (vedi getIdrTemplatePdf) + un banner, e il salvataggio segnala che serve pubblicare.
  const { data: backendReady } = useIdrBackendReady();
  const { previewPDF, isGenerating: isPreviewing } = useTermoidraulicoPDF();
  ...
  const [moduleDefaults] = useState(() => localModule ? createFullIdrTemplate(localModule.template, localModule.id) : null);
  ...
  const previewTemplate = useMemo<IdrTemplatePdf | null>(
    () => (form ? ({ id: "preview", company_id: companyId ?? "", ...form } as IdrTemplatePdf) : null),
    [form, companyId],
  );
  ...
  // ─── Preset cover 1-click (parità Serramenti) ─────────────────────────────
  // Applica in BATCH tutta la patch pdf_cover_* del preset (layout completo:
  // bg/immagine + posizione testo + overlay + decorazione + font). L'immagine
  // di sfondo del preset photo è una stock suggerita: NON sovrascrive una foto
  // già caricata dall'utente (detectActiveCoverPreset ignora image_url sui photo).
  ...
  const handleSave = async () => {
    if (!form) return;
    // `form` include sia i campi legacy cover_* sia il layer pdf_cover_* (non
    // presente in IdrTemplatePatch perché la tabella non è nei types generati):
    // l'upsert (sb() = supabase as any) li scrive comunque sulle colonne aggiunte
    // dalla migration. Cast a IdrTemplatePatch via unknown per soddisfare TS.
    const patch = { ...form } as unknown as IdrTemplatePatch;
    try {
      if (localModule) {
  ...
  // esempio. `previewPDF` non rilegge le tabelle idr_* (passiamo il template) →
  // funziona anche con il modulo non ancora pubblicato sul DB.
  const handlePreview = async () => {
    if (!form || !companyId) return;
    const template: IdrTemplatePdf = { id: "preview", company_id: companyId, ...form };
    if (localModule) { await previewPDF(buildIdrModulePreview(companyId, template, localModule.id)); return; }
    const row = (
      i: number, cap: string, descrizione: string,
      um: IdrComputoVoce["unita_misura"], q: number, p: number, cm: number, cl: number,
    ): IdrComputoVoce => ({
    ...
    const progetto: IdrProgetto = {
      id: "preview", company_id: companyId, code: "ANTEPRIMA", stato: "bozza",
      tipo_intervento: "Impianto termoidraulico completo", tipo_generatore: null, numero_terminali: null, massimale_detrazione: null,
  ...
  const IDR_SECTION_GROUPS: Array<{
    label: string;
    items: Array<{ id: IdrSection; label: string; emoji: string; descr?: string }>;
  }> = [
    {
      label: "AZIENDA",
      items: [
    ...
    },
    {
      ...
      items: PAGINE_EDITOR_EDILI.map((p) => ({ id: p.id as IdrSection, label: p.voce, emoji: p.emoji, descr: p.descrizione })),
    },
    {
  ...
  const IDR_SECTIONS = IDR_SECTION_GROUPS.flatMap((g) => g.items);
  const [searchParams, setSearchParams] = useSearchParams();
  const sectionFromUrl = (searchParams.get("section") ?? (localModule ? "page_cover" : "brand")) as IdrSection;
  const activeSection: IdrSection = IDR_SECTIONS.some((s) => s.id === sectionFromUrl)
    ? sectionFromUrl
    : ("brand" as IdrSection);
  const setActiveSection = (id: IdrSection) => {
    ...
    <div className={cn("space-y-4", embedded ? "" : "mx-auto max-w-4xl p-4")}>
      {!localModule && backendReady === false && (
        <div className="flex items-start gap-2 rounded-lg border border-amber-300 bg-amber-50 p-3 text-sm text-amber-900 dark:border-amber-900/50 dark:bg-amber-950/30 dark:text-amber-200">
          ...
          <div className="space-y-0.5">
            <p className="font-medium">Modulo Termoidraulico non ancora pubblicato sul database</p>
      ...
      <div className="rounded-xl border border-orange-200 bg-gradient-to-br from-orange-50 to-amber-50 p-4 dark:border-orange-900/40 dark:from-orange-950/30 dark:to-amber-950/20">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          {localModule && moduleDefaults ? <InterventionTextPicker title={IDR_MODULE_TITLES[localModule.id]} choices={idrCopyChoices(localModule.id, moduleDefaults)} onApply={patch => { setForm(prev => prev ? { ...prev, ...patch } : prev); setDirty(true); }} /> : <StandardTextTemplatePicker
            module="termoidraulico"
      ...
      <TemplateEditorWorkspace moduleId={localModule?.id}>
        ...
        <TemplateEditorNavigation>
          <nav className={templateEditorLayout.navigationPanel}>
            <TemplateSectionNavigation groups={IDR_SECTION_GROUPS} activeSection={activeSection} onSelect={setActiveSection} isExcluded={id => edileSectionExcluded(form, id)} />
        ...
        <div data-template-content className={templateEditorLayout.content}>
            ...
            <SectionCard icon={Building2} title="Anagrafica azienda" description="Dati che compaiono nell'header e nel footer di ogni preventivo PDF.">
              ...
              <div className="mb-3 flex items-start gap-2 rounded-lg border border-blue-200 bg-blue-50/60 p-2.5 text-[11px] text-blue-900 dark:border-blue-900/40 dark:bg-blue-950/30 dark:text-blue-200">
                ...
                <span>
                  ...
                  sovrascriverli nei preventivi termoidraulico.
                </span>
              </div>
          ...
          {activeSection === "page_ordine" && (
            <SectionCard icon={ListOrdered} title="Ordine e pagine" description="In che ordine escono i capitoli, quali nascondere, e le pagine scritte da voi.">
              <OrdineCapitoli visibilitySettings={form} onVisibilityChange={setPageVisibility}
                ...
                apriSezione={(sezione) => setActiveSection(sezione as IdrSection)}
          ...
          {activeSection === "page_cover" && (
            <SectionCard icon={FileText} title="Copertina" description="Titolo, sottotitolo, immagine e stile della prima pagina.">
              ...
              <TemplateCoverTextFields value={{ title: form.cover_title, subtitle: form.cover_subtitle, eyebrow: form.pdf_cover_eyebrow }} onChange={(field, value) => { if (field === "title") { set("cover_title", (value ?? "")); }
if (field === "subtitle") { set("cover_subtitle", (value ?? "")); }
if (field === "eyebrow") { set("pdf_cover_eyebrow", value); } }}   placeholders={(value, onChange) => <PlaceholderChips value={value} onChange={onChange}  />} />
...
<ImageUploadField localOnly={!!localModule}
                    ...
                    hint="Per la copertina scegli una foto verticale con il soggetto centrale. Controlla sempre il taglio nell'anteprima PDF."
...
{ id: "titleSize", kind: "range", value: form.pdf_cover_title_size ?? 30, min: 22, max: 64, step: 1, unit: "pt", onChange: value => set("pdf_cover_title_size", value) },
...
{ id: "showClientCard", kind: "toggle", value: !!(form.pdf_cover_show_client_card !== false), onChange: value => set("pdf_cover_show_client_card", value) }
          ...
          {activeSection === "opzioni" && (
            <>
              ...
              <FinanziamentoPromoField
                ...
                onChange={(v) => set("finanziamento_promo", v ? { ...v } : null)}
              />
              </SectionCard>
        ...
        <aside data-template-preview className={templateEditorLayout.preview}>
          <div className={templateEditorLayout.previewPanel}>
            <TermoidraulicoLivePreviewPanel activeSection={activeSection} moduleId={localModule?.id} template={previewTemplate} companyId={companyId} />
          </div>
        </aside>
      </TemplateEditorWorkspace>
      <TermoidraulicoTemplatePreviewDialog
        ...
        <DialogContent className="max-w-4xl max-h-[85vh] flex flex-col p-0 gap-0">
          <DialogHeader className="p-4 pb-3 border-b">
            ...
            <DialogDescription className="text-xs">
              {localModule ? "Immagini illustrative dedicate: non documentano installazioni aziendali né identificano il prodotto venduto." : "Scegli una foto dalla libreria del modulo per la copertina."}
            </DialogDescription>
          ...
          <div className="flex-1 overflow-y-auto p-4">
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
              {(localModule && moduleDefaults ? fotoDellaLibreria("termoidraulico", moduleDefaults.pdf_blocchi).map((f, i) => ({ id: `modulo-${i}`, url: f.url, thumb: f.url, label: f.nome })) : stockFiltered).map((img) => {
...
function ImageUploadField({ label, hint, value, companyId, onChange, aspect = "aspect-[4/3]", localOnly = false }: ImageUploadFieldProps) {
  ...
  const handleFile = async (file: File | null) => {
    ...
    } catch (error) { toast.error(error instanceof Error ? error.message : "Immagine non caricata"); } finally {
      setUploading(false);
...
export default TermoidraulicoTemplateEditor;
