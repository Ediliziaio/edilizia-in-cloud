/**
 * TermoidraulicoTemplatePreviewDialog — anteprima LIVE del PDF cliente in un
 ...
 * funziona anche col modulo Termoidraulico non ancora pubblicato sul DB.
 *
...
export function TermoidraulicoTemplatePreviewDialog({
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      void (async () => {
        ...
        setError(null);
        try {
          const blobUrl = await renderIdrPreviewBlobUrl(moduleId ? buildIdrModulePreview(companyId, template, moduleId) : {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="flex h-[88vh] max-w-5xl flex-col gap-0 p-0">
        ...
        <div className="relative min-h-0 flex-1 bg-muted/30">
          ...
          {url && (
            <iframe title="Anteprima PDF Termoidraulico" src={url} className="h-full w-full border-0" />
          )}
        </div>
      </DialogContent>
    </Dialog>
