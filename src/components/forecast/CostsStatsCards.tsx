import { AlertCircle, Check, Clock, Calculator, Truck } from "lucide-react";
...
export function CostsStatsCards({ stats, vatStats, monthlyDistribution }: CostsStatsCardsProps) {
  return (
    <>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4">
        <div className="p-4 rounded-lg bg-red-50 dark:bg-red-900/10 border border-red-200 dark:border-red-800">
          ...
          <p className="text-xl font-bold text-red-600">{formatCurrency(stats.totalUnpaidThisMonth)}</p>
          <p className="text-[10px] text-muted-foreground">{stats.unpaidCount} da pagare</p>
        </div>
        <div className="p-4 rounded-lg bg-green-50 dark:bg-green-900/10 border border-green-200 dark:border-green-800">
          ...
          <p className="text-xl font-bold text-green-600">{formatCurrency(stats.totalPaidThisMonth)}</p>
        ...
        <div className="p-4 rounded-lg bg-orange-50 dark:bg-orange-900/10 border border-orange-200 dark:border-orange-800">
          ...
          <p className="text-xl font-bold text-orange-600">{formatCurrency(stats.totalOverdue)}</p>
