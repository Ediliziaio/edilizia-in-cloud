...
export default function CompanyCostsManager() {
  ...
  const supplierPaymentsData = useMemo(() => {
    ...
    supplierPaymentItems.forEach((item: any) => {
      ...
      if (method === "50_50" || method === "30_70") {
        ...
        items.push({
          ...
          type: "Acconto",
          amount: depositAmt,
          isPaid: !!item.deposit_paid,
          date: item.deposit_paid_date ? new Date(item.deposit_paid_date) : null,
          paidDate: item.deposit_paid_date,
        });
        items.push({
          id: `${item.id}-bal`,
          supplierName,
          ...
          type: "Saldo",
          amount: balanceAmt,
          isPaid: !!item.balance_paid,
          date: item.balance_expected_date ? new Date(item.balance_expected_date) : null,
          paidDate: item.balance_paid_date,
        });
      } else if (totalCost > 0) {
        items.push({
          id: item.id,
          ...
          amount: totalCost,
          isPaid: !!item.is_paid,
          date: item.paid_date ? new Date(item.paid_date) : (item.balance_expected_date ? new Date(item.balance_expected_date) : null),
    ...
    return { items, paid, unpaid, totalPaid, totalUnpaid };
              ...
              <div className="space-y-4">
                ...
                {supplierPaymentsData.items.length > 0 && (
                  <div className="space-y-3">
                    {/* Group by supplier with progress */}
                    {(() => {
                      ...
                      <TableBody>
                        {supplierPaymentsData.items.map((item) => (
                          <TableRow key={item.id}>
                            ...
                            <TableCell>
                              {item.orderCode ? (
                                <Link to={`/azienda/ordini/${item.orderId}`} className="text-primary hover:underline text-sm">
                                  {item.orderCode}
                                </Link>
                              ) : "—"}
                            </TableCell>
                            ...
                            <TableCell>
                              {item.date ? format(item.date, "dd/MM/yyyy", { locale: it }) : "—"}
                            </TableCell>
                            <TableCell>
                              {item.isPaid ? (
