...
export function InterventoScelto({ intervento }: { intervento: SalesIntervention }) {
  return <div className="mt-1 rounded-lg border bg-muted/30 p-3">
    <p className="font-medium">{intervento.title}</p>
    <p className="mt-1 text-xs text-muted-foreground">Intervento scelto all'inizio del preventivo. {intervento.summary}</p>
  </div>;
}
...
export function LavorazioniDelModello({ intervento }: { intervento: SalesIntervention }) {
  return <section className="rounded-xl border bg-muted/20 p-4" aria-label={`Lavorazioni per ${intervento.title}`}>
