...
const mobileNodes = graphNodes.filter((node) => node.id !== "silvio" && node.size !== "small");
...
export function AziendaBrain() {
  ...
  useGSAP(() => {
    ...
    motion.add("(prefers-reduced-motion: reduce)", () => {
      gsap.set(".brain-shell, .brain-graph-node, .brain-path, .brain-synapse", {
        opacity: 1,
        scale: 1,
        x: 0,
        y: 0,
    ...
    <section ref={sectionRef} className="relative overflow-hidden bg-eic-navy px-5 py-20 text-white md:px-8 lg:py-28">
      ...
      <div className="relative mx-auto max-w-6xl">
        <FadeUp className="mx-auto max-w-4xl text-center">
          ...
          <h2 className="mt-5 text-[clamp(2.35rem,5vw,5.4rem)] font-black leading-[1.01] tracking-[-0.055em]">
        ...
        <FadeUp transition={{ delay: 0.1 }} className="mt-10">
          ...
          >
            ...
            <div className="relative z-[3] grid grid-cols-2 gap-2 md:hidden">
              {mobileNodes.map((node) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      "rounded-md border p-3 text-left shadow-xl shadow-black/18 backdrop-blur transition",
                      activeNodeId === node.id ? "border-eic-orange bg-eic-orange/20" : "border-white/12 bg-eic-navy-90/82",
                    )}
                  >
                    <div className="flex items-center gap-2">
                      <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-md bg-eic-orange text-white shadow-lg shadow-eic-orange/20">
                        <Icon className="h-4 w-4" strokeWidth={1.5} />
                      </span>
                      <div className="min-w-0">
                        <p className="truncate text-sm font-black text-white">{node.label}</p>
                        <p className="truncate text-[11px] text-white/54">{node.detail}</p>
                      </div>
                    </div>
                  </button>
                );
            ...
            <div className="relative z-[3] mt-4 rounded-md border border-emerald-300/18 bg-emerald-400/10 p-4 text-sm leading-6 text-emerald-50 md:hidden">
              <div className="flex items-start gap-3">
                <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-md bg-emerald-400/16 text-emerald-200">
                  <Bot className="h-5 w-5" strokeWidth={1.5} />
                </span>
                <p>
                  <strong className="text-white">{activeNode.label}:</strong> {activeNode.detail}. Silvio collega questi dati al resto dell'azienda.
                </p>
              </div>
            </div>
          </div>
