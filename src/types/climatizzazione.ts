...
export type ClmUnitaMisura = "mq" | "ml" | "cad" | "corpo" | "kg" | "h" | "a corpo";
export type ClmStato = "bozza" | "da_consegnare" | "consegnato" | "in_valutazione" | "accettato" | "rifiutato" | "scaduto" | "archiviato";

export interface ClmListinoCapitolo { id: string; company_id: string; nome: string; ordine: number; }
export interface ClmListinoVoce {
  id: string; company_id: string; capitolo_id: string | null; codice: string | null;
  descrizione: string; unita_misura: ClmUnitaMisura;
...
export interface ClmComputoVoce {
  id: string; progetto_id: string; company_id: string; capitolo_nome: string;
  descrizione: string; unita_misura: ClmUnitaMisura; quantita: number;
...
export interface ClmProgetto {
  /** Il modello della libreria congelato nel preventivo (lib/moduli/modelloPreventivo). */
  modello_snapshot?: import("@/lib/moduli/modelloPreventivo").ModelloPreventivo<ClmTemplatePdf> | null;
  id: string; company_id: string; code: string | null; stato: ClmStato; tipo_intervento: string | null;
  numero_unita_interne: number | null; tipologia_impianto: string | null;
...
export interface ClmProgettoMedia { id: string; progetto_id: string; company_id: string; tipo: string; url: string; caption: string | null; ordine: number; }
...
// la stessa forma.
export interface ClmListItem { titolo: string; descrizione?: string | null; }
export interface ClmTestimonianza { autore: string; ruolo?: string | null; testo: string; }
export interface ClmCronoFase { fase: string; durata?: string | null; descrizione?: string | null; }
export interface ClmFaqItem { domanda: string; risposta: string; }

export interface ClmTemplatePdf {
  id: string;
  company_id: string;
  ...
  usp: ClmListItem[];
  testimonianze: ClmTestimonianza[];
  ...
  /** I blocchi del preventivo: solo i campi cambiati dall'azienda (vedi _shared/blocchiPreventivo.ts). */
  pdf_blocchi?: Record<string, unknown> | null;
  /** La rata di esempio nella pagina dell'investimento: { attivo, rate, tan_pct } (vedi lib/preventivi/finanziamentoLite). */
  finanziamento_promo?: Record<string, unknown> | null;
