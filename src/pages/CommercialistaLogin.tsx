...
const allowedAccountantRoles = new Set(["accountant", "super_admin"]);

const accountantHighlights = [
  {
    icon: Building2,
    title: "Clienti in un cruscotto unico",
    text: "Apri ogni azienda cliente con il perimetro giusto, senza cambiare strumento.",
  },
  {
    icon: FileText,
    title: "Documenti e finanza sotto controllo",
    text: "Vedi fatture, tesoreria, costi, magazzino e documenti da verificare.",
  },
  {
    icon: BarChart3,
    title: "Consulenza attiva sui dati",
    text: "Controllo di gestione, commesse e margini sono gia dentro il lavoro quotidiano.",
  },
];

async function getLoggedUserHasAccountantAccess() {
  const {
    data: { user },
...
export default function CommercialistaLogin() {
  const { user, role, isLoading, signIn, signOut } = useAuth();
  const { toast } = useToast();
  const navigate = useNavigate();
  useSEO({ title: "Portale Commercialista", noindex: true });
  ...
  // Signup state (firm + owner)
  const [signupName, setSignupName] = useState("");
  const [signupPhone, setSignupPhone] = useState("");
  const [signupFirmName, setSignupFirmName] = useState("");
  const [signupFirmVat, setSignupFirmVat] = useState("");
  const [signupFirmFiscalCode, setSignupFirmFiscalCode] = useState("");
  const [signupFirmEmail, setSignupFirmEmail] = useState("");
  const [signupTermsAccepted, setSignupTermsAccepted] = useState(false);
  const [signupSuccessMessage, setSignupSuccessMessage] = useState<string | null>(null);

  if (!isLoading && user && role && allowedAccountantRoles.has(role)) {
    return <Navigate to="/commercialista" replace />;
  }

  if (!isLoading && user && role && !allowedAccountantRoles.has(role)) {
    return (
      <div className="flex min-h-dvh items-center justify-center bg-[#07111f] p-4">
        <div className="w-full max-w-md rounded-2xl border border-white/10 bg-white p-8 text-center shadow-2xl">
          ...
          <div className="mt-5 space-y-2">
            ...
            <p className="text-sm leading-relaxed text-slate-600">
              Questo portale e riservato agli studi commercialisti abilitati. Se hai un account aziendale,
    ...
    return (
      <div className="flex min-h-dvh items-center justify-center bg-[#07111f]">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-sm text-white/70">Caricamento portale commercialista...</p>
        </div>
      </div>
    );
  }
  ...
  const handleLogin = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const hasAccountantAccess = await getLoggedUserHasAccountantAccess();
      if (!hasAccountantAccess) {
        await supabase.auth.signOut();
        setFormError("Questo account non e abilitato al portale commercialista.");
        return;
      }
      ...
      toast({
        ...
        description: "Benvenuto nel portale commercialista.",
      });
      navigate("/commercialista", { replace: true });
    } catch {
  ...
  const handleSignup = async (event: React.FormEvent) => {
    ...
    if (signupFirmName.trim().length < 2) {
      setFormError("Nome studio richiesto (minimo 2 caratteri).");
      return;
    }
    ...
    try {
      ...
      const response = await fetchWithTimeout(
        `${supabaseUrl}/functions/v1/accountant-self-signup`,
        {
          method: "POST",
          timeoutMs: 15_000,
          context: "commercialista-login.self-signup",
          ...
          body: JSON.stringify({
            ...
            firm_vat_number: signupFirmVat.trim() || null,
            firm_fiscal_code: signupFirmFiscalCode.trim() || null,
            firm_email: signupFirmEmail.trim() || null,
      ...
      <aside className="hidden overflow-hidden bg-[#07111f] p-12 lg:flex lg:flex-col lg:justify-between">
        <div className="space-y-12">
          ...
          <div className="max-w-md space-y-5">
            <div className="inline-flex items-center gap-2 rounded-full border border-blue-300/25 bg-blue-400/10 px-3 py-1 text-xs font-semibold uppercase tracking-widest text-blue-100">
              <Landmark className="h-3.5 w-3.5" />
              Portale commercialista
            </div>
            <div className="space-y-4">
              <h1 className="text-4xl font-bold leading-tight text-white">
                Accedi allo studio e governa le aziende clienti dai dati reali.
              </h1>
              <p className="text-base leading-relaxed text-white/70">
                Entra nella dashboard dello studio, scegli il cliente e apri le stesse aree operative
                dell'azienda: controllo di gestione, cantieri, documenti, tesoreria e magazzino.
              </p>
            </div>
          </div>

          <div className="grid max-w-lg gap-4">
            {accountantHighlights.map((item) => {
        ...
        <div className="rounded-2xl border border-white/10 bg-white/[0.05] p-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-semibold text-white">Consulenza evoluta</p>
              <p className="text-xs text-white/60">
                Accesso dedicato agli studi approvati da Edilizia in Cloud.
              </p>
            </div>
          </div>
        </div>
          ...
          {view === "2fa" && (
            <section className="rounded-2xl border border-slate-200 bg-white p-7 shadow-xl shadow-slate-200/50">
              <div className="mb-6 text-center">
                ...
                <p className="mt-1 text-sm text-slate-500">Completa l'accesso al portale commercialista.</p>
              </div>
            ...
            <section className="rounded-2xl border border-slate-200 bg-white p-7 shadow-xl shadow-slate-200/50">
              <div className="mb-7 space-y-3">
                ...
                <div>
                  <h2 className="text-2xl font-bold tracking-tight text-slate-950">
                    Accedi al portale commercialista
                  </h2>
                  <p className="mt-1 text-sm leading-relaxed text-slate-500">
                    Usa le credenziali dello studio per entrare nella dashboard clienti.
                  </p>
                </div>
              </div>
              ...
              <form onSubmit={handleLogin} className="space-y-5">
                <div className="space-y-2">
                  <Label htmlFor="commercialista-email">Email</Label>
                  <div className="relative">
                    <Mail className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" />
                    <Input
                      id="commercialista-email"
                      type="email"
                      inputMode="email"
                      autoComplete="email"
                ...
                <div className="space-y-2">
                  <Label htmlFor="commercialista-password">Password</Label>
                  <div className="relative">
                    <Lock className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" />
                    <Input
                      id="commercialista-password"
                ...
                <Button
                  ...
                  className="h-12 w-full bg-blue-700 text-base font-semibold text-white shadow-lg shadow-blue-200 hover:bg-blue-800"
                >
                  {isSubmitting ? (
                    <>
              ...
              <div className="mt-7 rounded-xl border border-slate-200 bg-slate-50 p-4">
                <div className="flex gap-3">
                  <Users2 className="mt-0.5 h-4 w-4 shrink-0 text-blue-700" />
                  <p className="text-xs leading-relaxed text-slate-600">
                    Sei un nuovo studio commercialista? Registrati in 60 secondi: crei lo studio
                    e diventi automaticamente owner. Gli account aziendali restano separati.
                  </p>
                </div>
              </div>
            </section>
            ...
            <section className="rounded-2xl border border-slate-200 bg-white p-7 shadow-xl shadow-slate-200/50">
              ...
              <div className="mb-6 space-y-3">
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm leading-relaxed text-slate-500">
                    Crea l'account commercialista e accedi subito alla dashboard dello studio
                    per gestire le aziende clienti.
                  </p>
                </div>
              </div>
              ...
              <form onSubmit={handleSignup} className="space-y-4">
                ...
                <div className="space-y-2">
                  <Label htmlFor="acc-signup-name">Nome e cognome</Label>
                  <div className="relative">
                ...
                <div className="space-y-2">
                  <Label htmlFor="acc-signup-email">Email personale</Label>
                  <div className="relative">
                    <Mail className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" />
                    <Input
                      id="acc-signup-email"
                ...
                <div className="space-y-2">
                  <Label htmlFor="acc-signup-password">Password (min. 8 caratteri)</Label>
                  <div className="relative">
                    <Lock className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" />
                    <Input
                      id="acc-signup-password"
                      type={showPassword ? "text" : "password"}
                ...
                <div className="space-y-2">
                  <Label htmlFor="acc-signup-phone">Telefono (opzionale)</Label>
                  <div className="relative">
                ...
                <div className="border-t border-slate-200 pt-4 space-y-4">
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="acc-signup-firm-name">Nome studio / Ragione sociale</Label>
                    <div className="relative">
                      <Building2 className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" />
                      <Input
                        id="acc-signup-firm-name"
                        type="text"
                        placeholder="Studio Rossi & Associati"
                        value={signupFirmName}
                        onChange={(event) => setSignupFirmName(event.target.value)}
                        className="h-11 pl-10"
                        required
                        minLength={2}
                        maxLength={200}
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-2">
                      <Label htmlFor="acc-signup-firm-vat">P.IVA (opzionale)</Label>
                      <div className="relative">
                        <Hash className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" />
                        <Input
                          id="acc-signup-firm-vat"
                          type="text"
                          inputMode="numeric"
                          placeholder="12345678901"
                          value={signupFirmVat}
                          onChange={(event) => setSignupFirmVat(event.target.value.replace(/\s/g, ""))}
                          className="h-11 pl-10"
                          maxLength={11}
                          pattern="\d{11}"
                        />
                    ...
                    <div className="space-y-2">
                      <Label htmlFor="acc-signup-firm-cf">Cod. Fiscale</Label>
                      <Input
                        id="acc-signup-firm-cf"
                        type="text"
                        placeholder="RSSMRA80A01H501Z"
                        value={signupFirmFiscalCode}
                        onChange={(event) => setSignupFirmFiscalCode(event.target.value.toUpperCase())}
                        className="h-11"
                        maxLength={16}
                      />
                    </div>
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="acc-signup-firm-email">
                      Email studio (se diversa dalla personale)
                    </Label>
                    <div className="relative">
                      <Mail className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" />
                      <Input
                        id="acc-signup-firm-email"
                        type="email"
                        inputMode="email"
                        placeholder="info@studio.it"
                        value={signupFirmEmail}
                        onChange={(event) => setSignupFirmEmail(event.target.value)}
                        className="h-11 pl-10"
                      />
                    </div>
                  </div>
                ...
                <label className="flex items-start gap-3 cursor-pointer text-sm text-slate-600 pt-2">
                  <input
                    type="checkbox"
              ...
              <div className="space-y-5 text-center">
                ...
                <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-left">
                  <p className="text-xs leading-relaxed text-emerald-900">
                    ...
                    <br />
                    4. Invita le aziende clienti dal cruscotto studio
                  </p>
                </div>
                <Button
                  onClick={switchToLogin}
            ...
            <section className="rounded-2xl border border-slate-200 bg-white p-7 shadow-xl shadow-slate-200/50">
              ...
              {resetSent ? (
                <div className="space-y-5 text-center">
                  ...
                  <Button onClick={switchToLogin} className="w-full bg-blue-700 text-white hover:bg-blue-800">
                    Torna al login
                  </Button>
                </div>
              ) : (
                <>
                  <div className="mb-7 space-y-2">
                    ...
                    <p className="text-sm leading-relaxed text-slate-500">
                      Inserisci l'email dello studio. Ti invieremo le istruzioni di reset.
                    </p>
                  </div>
                  ...
                  <form onSubmit={handleForgotPassword} className="space-y-5">
                    <div className="space-y-2">
                      <Label htmlFor="commercialista-reset-email">Email</Label>
                      <div className="relative">
                        <Mail className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" />
                        <Input
                          id="commercialista-reset-email"
                          type="email"
                          inputMode="email"
                          autoComplete="email"
                          placeholder="studio@commercialista.it"
                    ...
                    <Button
                      type="submit"
                      disabled={isSubmitting}
                      className="h-12 w-full bg-blue-700 text-white hover:bg-blue-800"
                    >
                      {isSubmitting ? (
                        <>
                          <Loader2 className="mr-2 h-4 w-4 animate-spin" />
