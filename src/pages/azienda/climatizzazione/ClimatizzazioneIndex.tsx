/**
 * ClimatizzazioneIndex — landing del modulo Climatizzazione.
 *
 ...
 * Climatizzazione non richiede ancora i 20+ filtri avanzati dei serramenti).
 */
...
export default function ClimatizzazioneIndex() {
        ...
        <div className="relative max-w-[1400px] mx-auto px-3 sm:px-8 py-4 sm:py-8 flex items-center justify-between flex-wrap gap-3 sm:gap-4 max-sm:flex-nowrap max-sm:py-3">
          <div className="min-w-0 flex-1">
            ...
            <h1 className="text-xl sm:text-3xl font-bold tracking-tight flex items-center gap-2 max-sm:text-lg">
              ...
              <span className="truncate">Climatizzazione</span>
            </h1>
        ...
        >
          <PilloleFiltro
            ...
            scelte={[
              ...
              ...(Object.keys(CLM_STATI_LABEL) as ClmStato[]).map((k) => ({ value: k, label: CLM_STATI_LABEL[k].label, n: progetti.filter((p) => p.stato === k).length })),
            ]}
          />
        </PannelloFiltri>
          ...
          <CardContent className="p-2.5 sm:p-3 flex flex-wrap items-center gap-2">
            <div className="relative flex-1 min-w-[160px] sm:min-w-[200px] w-full sm:w-auto">
              ...
              <Input
                ...
                aria-label="Cerca progetto climatizzazione"
              />
            </div>
            <Select value={filtroStato} onValueChange={setFiltroStato}>
              ...
              <SelectContent>
                ...
                {(Object.keys(CLM_STATI_LABEL) as ClmStato[]).map((k) => (
                  <SelectItem key={k} value={k}>{CLM_STATI_LABEL[k].label}</SelectItem>
                ))}
              </SelectContent>
            </Select>
        ...
        <Card>
          <CardContent className="p-0">
            {isLoading ? (
              ...
              <EmptyStateFirstTime onCreate={() => navigate("/azienda/climatizzazione/nuovo")} />
              ...
              <>
                <ModuloBulkToolbar
                  tableName="clm_progetti"
                  companyId={companyId}
                  selectedIds={sel.selected}
                  statoOptions={(Object.keys(CLM_STATI_LABEL) as (keyof typeof CLM_STATI_LABEL)[]).map((k) => ({ value: k as string, label: CLM_STATI_LABEL[k].label }))}
                  onClear={sel.clear}
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {progettiFiltrati.map((p) => {
                        const statoCfg = CLM_STATI_LABEL[p.stato] ?? CLM_STATI_LABEL.bozza;
                        ...
                        return (
                          <TableRow
                            ...
                            onClick={() => navigate(`/azienda/climatizzazione/${p.id}/modifica`)}
                          >
                ...
                <div className="md:hidden divide-y divide-border">
                  {progettiFiltrati.map((p) => {
                    ...
                    return (
                      <RigaMobile
                        key={p.id}
                        to={`/azienda/climatizzazione/${p.id}/modifica`}
  ...
  return (
    ...
    <div className="p-10 text-center space-y-4 max-sm:space-y-3 max-sm:p-5">
      ...
      <div className="space-y-1">
        <h3 className="text-base font-semibold text-slate-900">Nessun progetto di climatizzazione</h3>
