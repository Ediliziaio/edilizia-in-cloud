/**
 * StepPdf — step "PDF" del wizard Climatizzazione (Task 22).
 *
 ...
 *  - bottoni "Anteprima PDF" / "Scarica PDF" → chiamano `useClimatizzazionePDF`
 *
...
export default function StepPdf({ progetto, computo, media, onIndietro, onVaiAlPasso }: Props) {
  ...
  const { downloadPDF, previewPDF, isGenerating } = useClimatizzazionePDF();
  ...
  const { data: company } = useQuery<ClmPdfCompany | null>({
    queryKey: ["clm-step-pdf-company", companyId],
    enabled: !!companyId,
    staleTime: 5 * 60 * 1000,
    queryFn: async () => {
          ...
          <div className="overflow-hidden rounded-xl border bg-white">
            ...
            <div className="px-4 py-3">
              <p className="text-sm font-semibold text-slate-900">
                {template?.cover_title?.trim() || "Preventivo di climatizzazione"}
              </p>
