 ...
 * via `form`/`onChange`: l'autosave del wizard li persiste su `clm_progetti`.
...
import { INCENTIVI_CLIMATIZZAZIONE, calcDetraibile, superaMassimale } from "@/lib/preventivi/incentivi";
...
export default function StepEconomia({ form, onChange, computo }: Props) {
  ...
  const { data: template } = useClmTemplatePdf();
            ...
            <CardContent className="space-y-3">
              ...
              <div className="space-y-3 max-sm:grid max-sm:grid-cols-2 max-sm:gap-2 max-sm:space-y-0">
                <PctField
                  ...
                  hint="In edilizia spesso 10% (climatizzazione) o 4% (prima casa)."
                />
                <PctField
                  id="clm-detrazione"
              ...
              />
              {/* Preset incentivi climatizzazione: 1-click → imposta detrazione + massimale di spesa */}
              <div>
                <p className="mb-1 text-[10px] text-muted-foreground max-sm:hidden">Incentivi rapidi (climatizzazione):</p>
                <div className="flex flex-wrap gap-1.5">
                  {INCENTIVI_CLIMATIZZAZIONE.map((inc) => {
                    const active =
