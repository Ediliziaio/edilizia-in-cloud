/**
 * StepMedia — foto, render e allegati del progetto Climatizzazione (Task 18).
 *
 * Upload su Supabase Storage + riga in `clm_progetti_media` (via
 ...
 *   Il path resta `{company_id}/climatizzazione/{progetto_id}/{uuid}.{ext}`: le
 *   policy guardano la prima cartella, l'azienda su cui si lavora. In
 *   `clm_progetti_media.url` va il riferimento "progetti-media/<path>",
 *   non un link: si firma quando serve (anteprima qui sotto, PDF in
 *   useClimatizzazionePDF), quindi non scade. Vedi src/lib/storage/fileRiservati.ts.
 *
...
import type { ClmProgettoMedia } from "@/types/climatizzazione";
...
export default function StepMedia({ progettoId, media }: Props) {
  ...
  const handleFiles = async (files: FileList | null) => {
    ...
    try {
      for (const file of Array.from(files)) {
        ...
        const storagePath = `${companyId}/climatizzazione/${progettoId}/${crypto.randomUUID()}.${ext}`;
  ...
  const updateMedia = async (
    m: ClmProgettoMedia,
    patch: Partial<Pick<ClmProgettoMedia, "tipo" | "caption">>,
  ) => {
    try {
  ...
  // ─── Elimina ───────────────────────────────────────────────────────────────
  const remove = async (m: ClmProgettoMedia) => {
