/**
 * StepComputo — lo step "Computo metrico" del wizard Climatizzazione (Task 17).
 *
...
import { useSaveComputo, useEffectiveCompanyId } from "@/hooks/useClimatizzazioneProgetto";
import { useListinoVociSearch } from "@/hooks/useClimatizzazioneListino";
import type { ClmComputoVoce, ClmUnitaMisura } from "@/types/climatizzazione";
import type { ComputoVoceLocal } from "@/types/computo";
import ComputoEditor from "@/components/climatizzazione/ComputoEditor/ComputoEditor";
...
const LISTINO_SETTINGS_HREF =
  "/azienda/climatizzazione/listino";
...
export default function StepComputo({ progettoId, initialComputo, scontoPct, ivaPct, prezzoManuale = null, model }: Props) {
  ...
  const [computo, setComputo] = useState<ClmComputoVoce[]>(() => initialComputo);
  ...
  const handleChange = (next: ClmComputoVoce[]) => {
    setComputo(next);
    setDirty(true);
  };

  // Import AI: le voci estratte (già riviste nel modale) → ClmComputoVoce, accodate.
  const handleImportVoci = (estratte: ComputoVoceLocal[]) => {
    if (estratte.length === 0) return;
    const base = computo.length;
    const nuove: ClmComputoVoce[] = estratte.map((v, i) => ({
      id:
  ...
  const toPayload = (rows: ClmComputoVoce[]) =>
    rows.map((v, i) => ({
        ...
        <div className="flex items-start gap-2.5 rounded-xl border border-blue-100 bg-blue-50/60 px-3 py-2.5 max-sm:hidden">
          ...
          <div className="flex-1 text-[11px] text-blue-900">
            ...
            <p className="text-blue-800/80">
              ...
              velocizzare, popola il listino climatizzazione nelle impostazioni.
            </p>
            <Link
            ...
            >
              <Settings2 className="h-3 w-3" /> Vai al listino climatizzazione
            </Link>
          </div>
        </div>
      )}
...
// Coercizione dell'unità di misura estratta dall'AI (testo libero) → enum ClmUnitaMisura.
function coerceUnita(u: string | null | undefined): ClmUnitaMisura {
