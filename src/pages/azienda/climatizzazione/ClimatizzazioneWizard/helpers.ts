 ...
 * `@/lib/climatizzazione/calcoli`.
 */
import type { ClmProgetto, ClmComputoVoce, ClmStato } from "@/types/climatizzazione";

export type ClmWizardStepKey =
  | "cliente"
  | "immobile"
  | "computo"
  | "media"
...
export interface ClmWizardStepDef {
  key: ClmWizardStepKey;
  label: string;
}

/** Sequenza degli step del wizard (ordine = rendering nello stepper). */
export const CLM_WIZARD_STEPS: readonly ClmWizardStepDef[] = [
...
export const CLM_STATI_LABEL: Record<ClmStato, { label: string; className: string }> = {
...
export function stepCompletion(
  progetto: Partial<ClmProgetto> | null | undefined,
  computo: ClmComputoVoce[] | null | undefined,
): Record<ClmWizardStepKey, boolean> {
  const p = progetto ?? {};
  const righe = computo ?? [];
  return {
