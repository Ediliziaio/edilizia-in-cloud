/**
 * ClimatizzazioneWizard — shell a step per creare/modificare un progetto di
 * climatizzazione. Modellato su SerramentiWizard:
...
import {
  useClimatizzazioneProgetto,
  useUpsertProgetto,
  getClmTemplatePdf,
} from "@/hooks/useClimatizzazioneProgetto";
import type { ClmProgetto } from "@/types/climatizzazione";
import {
  CLM_WIZARD_STEPS, CLM_STATI_LABEL, stepCompletion,
  compactText, type ClmWizardStepKey,
} from "./ClimatizzazioneWizard/helpers";
import type { ClmFormPatch } from "./ClimatizzazioneWizard/types";
...
import StepCliente from "./ClimatizzazioneWizard/StepCliente";
import StepImmobile from "./ClimatizzazioneWizard/StepImmobile";
import StepComputo from "./ClimatizzazioneWizard/StepComputo";
import StepMedia from "./ClimatizzazioneWizard/StepMedia";
import StepEconomia from "./ClimatizzazioneWizard/StepEconomia";
import StepPdf from "./ClimatizzazioneWizard/StepPdf";

const STEP_ICONS: Record<ClmWizardStepKey, React.FC<React.SVGProps<SVGSVGElement>>> = {
  cliente: User,
  immobile: Home,
...
const ETICHETTA_BREVE_PASSO: Partial<Record<ClmWizardStepKey, string>> = { immobile: "Impianto" };

export default function ClimatizzazioneWizard() {
  ...
  const modelSupport = useSupportoModelloPreventivo("climatizzazione");
  ...
  const { data: ultimaBozza } = useQuery({
    queryKey: ["clm-ultima-bozza", user?.id, effectiveCompany?.id],
  ...
  const [currentStep, setCurrentStep] = useState<ClmWizardStepKey>("cliente");
  const [creating, setCreating] = useState(false);

  const { data: detail, isLoading, isError, refetch } = useClimatizzazioneProgetto(id);
  ...
  const savedModel = useMemo<{ snapshot: ReturnType<typeof leggiModelloPreventivo>; error: string | null }>(() => {
    try { return { snapshot: detail ? leggiModelloPreventivo("climatizzazione", detail.progetto.modello_snapshot, detail.progetto.company_id) : null, error: null }; }
    catch (error) { return { snapshot: null, error: error instanceof Error ? error.message : "Modello non valido" }; }
  }, [detail]);
  const model = interventoDelModulo("climatizzazione", isNew ? requestedModel : savedModel.snapshot?.modelId);

  // Local form state (campi del progetto).
  const [form, setForm] = useState<ClmFormPatch>(() => model ? { tipo_intervento: TIPO_INTERVENTO_DEL_MODELLO.climatizzazione[model.id] } : {});
  const createInput = async (): Promise<ClmFormPatch> => {
    ...
    const [{ createFullClmTemplate, isFullClmModuleId }, { loadLocalClmTemplate }, { sincronizzaModelliAzienda }] = await Promise.all([
      import("@/lib/moduli-vendita/fullClmModules"), import("@/lib/moduli-vendita/localClmTemplates"),
      import("@/lib/moduli-vendita/archivioModelli"),
    ]);
    if (!isFullClmModuleId(model.id)) throw new Error("Questo intervento non ha un modello completo. Nessuna offerta generica è stata creata.");
    ...
    const base = await getClmTemplatePdf(companyId);
    const source = loadLocalClmTemplate(companyId, model.id)?.template ?? createFullClmTemplate(base, model.id);
    return { ...form, tipo_intervento: TIPO_INTERVENTO_DEL_MODELLO.climatizzazione[model.id], modello_snapshot: creaModelloPreventivo("climatizzazione", companyId, model.id, source) };
  };
  ...
  const onChange = <K extends keyof ClmFormPatch>(key: K, value: ClmFormPatch[K]) => {
  ...
  const currentStepIndex = useMemo(
    () => CLM_WIZARD_STEPS.findIndex((s) => s.key === currentStep),
    [currentStep],
  );
  const progress = useMemo(
    () => Math.round(((currentStepIndex + 1) / CLM_WIZARD_STEPS.length) * 100),
    [currentStepIndex],
  );
  ...
  const handleSaveAndContinue = async () => {
    ...
    if (isNew) {
      ...
      try {
        ...
        navigate(`/azienda/climatizzazione/${created.id}/modifica`, { replace: true });
      } catch (e) {
    ...
    if (!ok) return;
    const idx = CLM_WIZARD_STEPS.findIndex((s) => s.key === currentStep);
    if (idx >= 0 && idx < CLM_WIZARD_STEPS.length - 1) {
      setCurrentStep(CLM_WIZARD_STEPS[idx + 1].key);
    } else {
      toast.success("Progetto salvato");
    }
  };

  const handleStepClick = async (target: ClmWizardStepKey) => {
  ...
  const handleBack = () => {
    if (currentStepIndex > 0) void handleStepClick(CLM_WIZARD_STEPS[currentStepIndex - 1].key);
  };
      ...
      <div className="container mx-auto p-4 max-w-4xl">
        <div className="rounded-lg border border-rose-200 bg-rose-50/40 p-6 text-center space-y-3">
          ...
          <div className="flex items-center gap-2 justify-center flex-wrap">
            ...
            <Button size="sm" variant="ghost" onClick={() => navigate("/azienda/climatizzazione")} className="gap-1">
  ...
  if (savedModel.error || (isNew && requestedModel && !model)) return <div role="alert" className="space-y-3 p-6"><h1 className="text-xl font-semibold">Intervento non disponibile</h1><p>{savedModel.error ?? "Il tipo di intervento richiesto non è riconosciuto."}</p><Button onClick={() => navigate("/azienda/marketing/preventivi?tab=moduli&area=climatizzazione")}>Scegli un intervento</Button></div>;

  const statoMeta = CLM_STATI_LABEL[(detail?.progetto.stato as ClmProgetto["stato"]) ?? "bozza"];
    ...
    <div className="pb-28 md:pb-20">
      ...
      <AlertDialog open={Boolean(isNew && !requestedModel && ultimaBozza && !resumeDismissed)}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={() => navigate(`/azienda/climatizzazione/${ultimaBozza!.id}/modifica`)}>
              Riprendi bozza
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              onClick={async () => {
                ...
                navigate("/azienda/climatizzazione");
              }}
            >
              Salva bozza ed esci
            </AlertDialogAction>
      ...
      <div className="sticky top-0 z-30 border-b bg-background/95 shadow-sm backdrop-blur max-md:-mx-3 max-md:-mt-3 max-md:-top-3 md:-top-6">
        <div className="container mx-auto flex max-w-6xl items-center gap-2 p-2.5 sm:gap-3 sm:p-3">
          <Button variant="ghost" size="icon" onClick={() => { if (isNew && dirty) { setExitDialogOpen(true); return; } navigate("/azienda/climatizzazione"); }} className="h-10 w-10 shrink-0">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[11px] text-muted-foreground truncate max-md:hidden">
              Step {currentStepIndex + 1} di {CLM_WIZARD_STEPS.length} · {CLM_WIZARD_STEPS[currentStepIndex]?.label}
            </p>
          </div>
        </div>
        ...
        <div className="md:hidden overflow-x-auto border-t bg-background/95 px-2 py-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          ...
          <nav className="flex min-h-0 w-full min-w-max gap-1" aria-label="Step progetto climatizzazione">
            {CLM_WIZARD_STEPS.map((s, idx) => {
          ...
          <main className="col-span-12 space-y-4 md:col-span-9 lg:col-span-10">
            ...
            <div className={cn("sticky bottom-[calc(env(safe-area-inset-bottom)-1.5rem)] z-40 flex items-center justify-between gap-2 rounded-2xl border bg-background/95 px-3 py-2.5 shadow-[0_-8px_20px_rgba(15,23,42,0.08)] backdrop-blur md:static md:mx-0 md:rounded-none md:border-0 md:bg-transparent md:px-0 md:py-2 md:shadow-none md:backdrop-blur-0", currentStep === "pdf" && id && detail && "max-md:hidden")}>
              ...
              >
                ...
                ) : currentStepIndex === CLM_WIZARD_STEPS.length - 1 ? (
                  <Save className="h-4 w-4" />
                ) : (
                  <ArrowRight className="h-4 w-4" />
                )}
                {isNew ? "Crea e continua" :
                  currentStepIndex === CLM_WIZARD_STEPS.length - 1 ? "Salva" : "Salva e continua"}
              </Button>
            </div>
          </main>
        </div>
...
function StepComingSoon({ step }: { step: ClmWizardStepKey }) {
  const meta = CLM_WIZARD_STEPS.find((s) => s.key === step);
