/**
 * StepMedia — foto, render e allegati del progetto Pavimenti (Task 18).
 *
 * Upload su Supabase Storage + riga in `pav_progetti_media` (via
...
import type { PavProgettoMedia } from "@/types/pavimenti";
...
export default function StepMedia({ progettoId, media }: Props) {
  ...
  const handleFiles = async (files: FileList | null) => {
    ...
    try {
      for (const file of Array.from(files)) {
        ...
        const storagePath = `${companyId}/pavimenti/${progettoId}/${crypto.randomUUID()}.${ext}`;
  ...
  const updateMedia = async (
    m: PavProgettoMedia,
    patch: Partial<Pick<PavProgettoMedia, "tipo" | "caption">>,
  ) => {
    try {
  ...
  // ─── Elimina ───────────────────────────────────────────────────────────────
  const remove = async (m: PavProgettoMedia) => {
