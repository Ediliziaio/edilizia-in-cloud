 ...
 * `@/lib/pavimenti/calcoli`.
 */
import type { PavProgetto, PavComputoVoce, PavStato } from "@/types/pavimenti";

export type PavWizardStepKey =
  | "cliente"
  | "immobile"
  | "computo"
  | "media"
...
export interface PavWizardStepDef {
  key: PavWizardStepKey;
  label: string;
}

/** Sequenza degli step del wizard (ordine = rendering nello stepper). */
export const PAV_WIZARD_STEPS: readonly PavWizardStepDef[] = [
  { key: "cliente", label: "Cliente" },
  { key: "immobile", label: "Dati intervento" },
...
export const PAV_STATI_LABEL: Record<PavStato, { label: string; className: string }> = {
...
export function stepCompletion(
  progetto: Partial<PavProgetto> | null | undefined,
  computo: PavComputoVoce[] | null | undefined,
): Record<PavWizardStepKey, boolean> {
  const p = progetto ?? {};
  const righe = computo ?? [];
  return {
