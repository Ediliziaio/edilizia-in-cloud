/**
 * StepComputo — lo step "Computo metrico" del wizard Pavimenti (Task 17).
 *
...
import { useSaveComputo, useEffectiveCompanyId } from "@/hooks/usePavimentiProgetto";
import { useListinoVociSearch } from "@/hooks/usePavimentiListino";
...
export default function StepComputo({ progettoId, initialComputo, scontoPct, ivaPct, prezzoManuale = null, model }: Props) {
  ...
  const [computo, setComputo] = useState<PavComputoVoce[]>(() => initialComputo);
  ...
  const handleChange = (next: PavComputoVoce[]) => {
    setComputo(next);
    setDirty(true);
  };

  // Import AI: le voci estratte (già riviste nel modale) → PavComputoVoce, accodate.
  const handleImportVoci = (estratte: ComputoVoceLocal[]) => {
    if (estratte.length === 0) return;
    const base = computo.length;
    const nuove: PavComputoVoce[] = estratte.map((v, i) => ({
      id:
  ...
  const toPayload = (rows: PavComputoVoce[]) =>
    rows.map((v, i) => ({
        ...
        <div className="flex items-start gap-2.5 rounded-xl border border-blue-100 bg-blue-50/60 px-3 py-2.5">
          ...
          <div className="flex-1 text-[11px] text-blue-900">
            ...
            <p className="text-blue-800/80">
              ...
              velocizzare, popola il listino pavimenti nelle impostazioni.
            </p>
            <Link
            ...
            >
              <Settings2 className="h-3 w-3" /> Vai al listino pavimenti
            </Link>
          </div>
        </div>
      )}
...
// Coercizione dell'unità di misura estratta dall'AI (testo libero) → enum PavUnitaMisura.
function coerceUnita(u: string | null | undefined): PavUnitaMisura {
