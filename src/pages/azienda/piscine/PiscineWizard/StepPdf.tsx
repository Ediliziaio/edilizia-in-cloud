/**
 * StepPdf — step "PDF" del wizard Piscine (Task 22).
 *
 ...
 *  - bottoni "Anteprima PDF" / "Scarica PDF" → chiamano `usePiscinePDF`
 *
...
import { calcTotaliComputo } from "@/lib/piscine/calcoli";
import {
  usePisTemplatePdf,
...
const TEMPLATE_SETTINGS_HREF =
  "/azienda/impostazioni/template-preventivi?tab=moduli-vendita&modulo=piscine";

export default function StepPdf({ progetto, computo, media }: Props) {
  const isMobile = useIsMobile();
  const companyId = useEffectiveCompanyId();
  const { data: template } = usePisTemplatePdf();
  const { downloadPDF, previewPDF, isGenerating } = usePiscinePDF();
  ...
  const { data: company } = useQuery<PisPdfCompany | null>({
    queryKey: ["pis-step-pdf-company", companyId],
    enabled: !!companyId,
    staleTime: 5 * 60 * 1000,
    queryFn: async () => {
          ...
          <div className="overflow-hidden rounded-xl border bg-white">
            ...
            <div className="px-4 py-3">
              <p className="text-sm font-semibold text-slate-900">
                {template?.cover_title?.trim() || "Preventivo di piscine"}
              </p>
