 ...
 * `@/lib/piscine/calcoli`.
 */
import type { PisProgetto, PisComputoVoce, PisStato } from "@/types/piscine";

export type PisWizardStepKey =
  | "cliente"
  | "immobile"
  | "computo"
  | "media"
...
export interface PisWizardStepDef {
  key: PisWizardStepKey;
  label: string;
}

/** Sequenza degli step del wizard (ordine = rendering nello stepper). */
export const PIS_WIZARD_STEPS: readonly PisWizardStepDef[] = [
  { key: "cliente", label: "Cliente" },
  { key: "immobile", label: "Dati piscina" },
...
export const PIS_STATI_LABEL: Record<PisStato, { label: string; className: string }> = {
...
export function stepCompletion(
  progetto: Partial<PisProgetto> | null | undefined,
  computo: PisComputoVoce[] | null | undefined,
): Record<PisWizardStepKey, boolean> {
  const p = progetto ?? {};
  const righe = computo ?? [];
  return {
