...
export default function PiscineIndex() {
  ...
  const handleClone = async (sourceId: string) => {
    ...
    try {
      ...
      setCloneOpen(false);
      navigate(`/azienda/piscine/${nuovo.id}/modifica`);
    } catch (e) {
        ...
        >
          <PilloleFiltro
            ...
            scelte={[
              ...
              ...(Object.keys(PIS_STATI_LABEL) as PisStato[]).map((k) => ({ value: k, label: PIS_STATI_LABEL[k].label, n: progetti.filter((p) => p.stato === k).length })),
            ]}
          />
        </PannelloFiltri>
        ...
        <Card>
          <CardContent className="p-0">
            {isLoading ? (
              ...
              <>
                <ModuloBulkToolbar
                  tableName="pis_progetti"
                  companyId={companyId}
                  selectedIds={sel.selected}
                  statoOptions={(Object.keys(PIS_STATI_LABEL) as (keyof typeof PIS_STATI_LABEL)[]).map((k) => ({ value: k as string, label: PIS_STATI_LABEL[k].label }))}
                  onClear={sel.clear}
                ...
                <div className="md:hidden divide-y divide-border">
                  {progettiFiltrati.map((p) => {
                    ...
                    return (
                      <RigaMobile
                        key={p.id}
                        to={`/azienda/piscine/${p.id}/modifica`}
