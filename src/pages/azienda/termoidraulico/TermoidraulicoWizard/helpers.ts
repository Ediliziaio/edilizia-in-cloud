 ...
 * `@/lib/termoidraulico/calcoli`.
 */
import type { IdrProgetto, IdrComputoVoce, IdrStato } from "@/types/termoidraulico";

export type IdrWizardStepKey =
  | "cliente"
  | "immobile"
  | "computo"
  | "media"
...
export interface IdrWizardStepDef {
  key: IdrWizardStepKey;
  label: string;
}

/** Sequenza degli step del wizard (ordine = rendering nello stepper). */
export const IDR_WIZARD_STEPS: readonly IdrWizardStepDef[] = [
...
export const IDR_STATI_LABEL: Record<IdrStato, { label: string; className: string }> = {
...
export function stepCompletion(
  progetto: Partial<IdrProgetto> | null | undefined,
  computo: IdrComputoVoce[] | null | undefined,
): Record<IdrWizardStepKey, boolean> {
  const p = progetto ?? {};
  const righe = computo ?? [];
  return {
