/**
 * StepComputo — lo step "Computo metrico" del wizard Termoidraulico (Task 17).
 *
...
import { useSaveComputo, useEffectiveCompanyId } from "@/hooks/useTermoidraulicoProgetto";
import { useListinoVociSearch } from "@/hooks/useTermoidraulicoListino";
import type { IdrComputoVoce, IdrUnitaMisura } from "@/types/termoidraulico";
import type { ComputoVoceLocal } from "@/types/computo";
import ComputoEditor from "@/components/termoidraulico/ComputoEditor/ComputoEditor";
...
const LISTINO_SETTINGS_HREF =
  "/azienda/termoidraulico/listino";
...
export default function StepComputo({ progettoId, initialComputo, scontoPct, ivaPct, prezzoManuale = null, model }: Props) {
  ...
  const [computo, setComputo] = useState<IdrComputoVoce[]>(() => initialComputo);
  ...
  const handleChange = (next: IdrComputoVoce[]) => {
    setComputo(next);
    setDirty(true);
  };

  // Import AI: le voci estratte (già riviste nel modale) → IdrComputoVoce, accodate.
  const handleImportVoci = (estratte: ComputoVoceLocal[]) => {
    if (estratte.length === 0) return;
    const base = computo.length;
    const nuove: IdrComputoVoce[] = estratte.map((v, i) => ({
      id:
  ...
  const toPayload = (rows: IdrComputoVoce[]) =>
    rows.map((v, i) => ({
        ...
        <div className="flex items-start gap-2.5 rounded-xl border border-blue-100 bg-blue-50/60 px-3 py-2.5">
          ...
          <div className="flex-1 text-[11px] text-blue-900">
            ...
            <p className="text-blue-800/80">
              ...
              velocizzare, popola il listino termoidraulico nelle impostazioni.
            </p>
            <Link
            ...
            >
              <Settings2 className="h-3 w-3" /> Vai al listino termoidraulico
            </Link>
          </div>
        </div>
      )}
...
// Coercizione dell'unità di misura estratta dall'AI (testo libero) → enum IdrUnitaMisura.
function coerceUnita(u: string | null | undefined): IdrUnitaMisura {
