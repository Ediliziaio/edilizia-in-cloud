/**
 * StepMedia — foto, render e allegati del progetto Termoidraulico (Task 18).
 *
 * Upload su Supabase Storage + riga in `idr_progetti_media` (via
 ...
 *   Il path resta `{company_id}/termoidraulico/{progetto_id}/{uuid}.{ext}`: le
 *   policy guardano la prima cartella, l'azienda su cui si lavora. In
 *   `idr_progetti_media.url` va il riferimento "progetti-media/<path>",
 *   non un link: si firma quando serve (anteprima qui sotto, PDF in
 *   useTermoidraulicoPDF), quindi non scade. Vedi src/lib/storage/fileRiservati.ts.
 *
...
import type { IdrProgettoMedia } from "@/types/termoidraulico";
...
export default function StepMedia({ progettoId, media }: Props) {
  ...
  const handleFiles = async (files: FileList | null) => {
    ...
    try {
      for (const file of Array.from(files)) {
        ...
        const storagePath = `${companyId}/termoidraulico/${progettoId}/${crypto.randomUUID()}.${ext}`;
  ...
  const updateMedia = async (
    m: IdrProgettoMedia,
    patch: Partial<Pick<IdrProgettoMedia, "tipo" | "caption">>,
  ) => {
    try {
  ...
  // ─── Elimina ───────────────────────────────────────────────────────────────
  const remove = async (m: IdrProgettoMedia) => {
