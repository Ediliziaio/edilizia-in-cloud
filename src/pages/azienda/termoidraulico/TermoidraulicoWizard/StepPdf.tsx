/**
 * StepPdf — step "PDF" del wizard Termoidraulico (Task 22).
 *
 ...
 *  - bottoni "Anteprima PDF" / "Scarica PDF" → chiamano `useTermoidraulicoPDF`
 *
...
import {
  useTermoidraulicoPDF,
  type IdrPdfCompany,
} from "@/hooks/useTermoidraulicoPDF";
...
export default function StepPdf({ progetto, computo, media }: Props) {
  ...
  const { data: company } = useQuery<IdrPdfCompany | null>({
    queryKey: ["idr-step-pdf-company", companyId],
    enabled: !!companyId,
    staleTime: 5 * 60 * 1000,
    queryFn: async () => {
          ...
          <div className="overflow-hidden rounded-xl border bg-white">
            ...
            <div className="px-4 py-3">
              <p className="text-sm font-semibold text-slate-900">
                {template?.cover_title?.trim() || "Preventivo di termoidraulico"}
              </p>
