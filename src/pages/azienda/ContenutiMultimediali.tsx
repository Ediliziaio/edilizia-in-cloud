...
export default function ContenutiMultimediali() {
  ...
  const sourceWarnings = mediaLoadResult.warnings;
  ...
  const filteredItems = useMemo(
    () => visibleItems.filter((item) => mediaLibraryItemMatchesTab(item, activeTab) && mediaLibraryItemMatchesSearch(item, query)),
    [activeTab, query, visibleItems],
  );
            ...
            {TABS.map((tab) => (
              <button
                ...
                className={cn(
                  ...
                  activeTab === tab ? "bg-orange-50 font-semibold text-orange-700" : "text-muted-foreground hover:bg-muted hover:text-foreground",
                )}
              >
        ...
        <Card className="min-w-0">
          <CardHeader className="gap-3 pb-3">
            <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
              <CardTitle className="text-base">{TAB_LABELS[activeTab]}</CardTitle>
