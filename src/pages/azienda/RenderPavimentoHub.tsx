...
import {
  Grid3X3, Plus, GalleryHorizontalEnd, Zap, Clock, CheckCircle2, XCircle,
...
const HOW_IT_WORKS = [
  {
    ...
    desc: "Foto dall'alto o prospettica della stanza con il pavimento attuale. JPG, PNG o WEBP.",
  },
  {
    icon: Wand2,
    title: "2. Analisi AI",
    desc: "L'AI identifica tipo di stanza, pavimento attuale, dimensioni e battiscopa.",
  },
  {
    icon: Sparkles,
    title: "3. Configura",
    desc: "Scegli materiale, schema di posa, colore, fuga e battiscopa desiderati.",
  },
  {
    icon: Share2,
    title: "4. Render & condividi",
    desc: "Ottieni un'immagine fotorealistica del nuovo pavimento e condividila col cliente.",
  },
];
...
export default function RenderPavimentoHub() {
  ...
  const { data: sessions = [], isLoading: loadingSessions } = useQuery({
    queryKey: ["render-pavimento-sessions", companyId],
    queryFn: async () => {
      if (!companyId) return [];
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-center gap-2 text-sm text-muted-foreground">
        ...
        <span>/</span>
        <span className="text-foreground">Pavimento</span>
      </div>

      {/* ── Hero ─────────────────────────────────────────────────────────── */}
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-amber-500/90 to-amber-700 p-6 sm:p-8 text-white">
        ...
        <div className="relative flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <h1 className="text-2xl sm:text-3xl font-bold text-white leading-tight">
              Visualizza il nuovo pavimento
              <br className="hidden sm:block" /> nella stanza reale
            </h1>
            <p className="mt-2 text-white/80 text-sm max-w-md">
              Carica una foto, configura il pavimento desiderato e ottieni un render fotorealistico
          ...
          <div className="flex flex-col gap-2 shrink-0">
            ...
            <Button
              size="lg"
              className="bg-white text-amber-700 hover:bg-white/90 font-semibold gap-2 shadow-lg"
              onClick={() => navigate("/azienda/render/pavimento/new")}
            >
            ...
            <Button
              ...
              onClick={() => navigate("/azienda/render/pavimento/gallery")}
            >
        ...
        {loadingSessions ? (
            ...
            <CardContent className="py-14 flex flex-col items-center gap-4 text-center">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">
                  Carica una foto della stanza e configura il nuovo pavimento
                </p>
              </div>
              <Button onClick={() => navigate("/azienda/render/pavimento/new")}>
          ...
          <div className="space-y-2">
            {sessions.map((session) => {
              ...
              const pavConfig = session.config as { tipo?: string; colore_nome?: string } | null;

              return (
                <Card
                  key={session.id}
                  className="cursor-pointer hover:border-amber-500/40 transition-colors"
                  onClick={() => navigate(`/azienda/render/pavimento/gallery/${session.id}`)}
                >
                  <CardContent className="p-3 flex items-center gap-3">
                      ...
                      <div className="flex items-center gap-1.5 flex-wrap">
                        ...
                        {pavConfig?.tipo && (
                          <Badge variant="outline" className="text-xs h-5 capitalize">
                            {pavConfig.tipo.replace(/_/g, " ")}
                          </Badge>
                        )}
                        {pavConfig?.colore_nome && (
                    ...
                    {session.status === "processing" && (
                      <Zap className="h-4 w-4 text-amber-600 animate-pulse shrink-0" />
                    )}
                  </CardContent>
                </Card>
              );
