/**
 * StepMedia — foto, render e allegati del progetto Bagni (Task 18).
 *
 * Upload su Supabase Storage + riga in `bgn_progetti_media` (via
 ...
 *   Il path resta `{company_id}/bagni/{progetto_id}/{uuid}.{ext}`: le
 *   policy guardano la prima cartella, l'azienda su cui si lavora. In
 *   `bgn_progetti_media.url` va il riferimento "progetti-media/<path>",
 *   non un link: si firma quando serve (anteprima qui sotto, PDF in
 *   useBagniPDF), quindi non scade. Vedi src/lib/storage/fileRiservati.ts.
 *
...
import type { BgnProgettoMedia } from "@/types/bagni";
...
export default function StepMedia({ progettoId, media }: Props) {
  ...
  const handleFiles = async (files: FileList | null) => {
    ...
    try {
      for (const file of Array.from(files)) {
        ...
        const storagePath = `${companyId}/bagni/${progettoId}/${crypto.randomUUID()}.${ext}`;
  ...
  const updateMedia = async (
    m: BgnProgettoMedia,
    patch: Partial<Pick<BgnProgettoMedia, "tipo" | "caption">>,
  ) => {
    try {
  ...
  // ─── Elimina ───────────────────────────────────────────────────────────────
  const remove = async (m: BgnProgettoMedia) => {
