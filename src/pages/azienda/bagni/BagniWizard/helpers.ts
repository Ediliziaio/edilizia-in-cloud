 ...
 * `@/lib/bagni/calcoli`.
 */
import type { BgnProgetto, BgnComputoVoce, BgnStato } from "@/types/bagni";

export type BgnWizardStepKey =
  | "cliente"
  | "immobile"
  | "computo"
  | "media"
...
export interface BgnWizardStepDef {
  key: BgnWizardStepKey;
  label: string;
}

/** Sequenza degli step del wizard (ordine = rendering nello stepper). */
export const BGN_WIZARD_STEPS: readonly BgnWizardStepDef[] = [
  { key: "cliente", label: "Cliente" },
  { key: "immobile", label: "Dati bagno" },
...
export const BGN_STATI_LABEL: Record<BgnStato, { label: string; className: string }> = {
...
export function stepCompletion(
  progetto: Partial<BgnProgetto> | null | undefined,
  computo: BgnComputoVoce[] | null | undefined,
): Record<BgnWizardStepKey, boolean> {
  const p = progetto ?? {};
  const righe = computo ?? [];
  return {
