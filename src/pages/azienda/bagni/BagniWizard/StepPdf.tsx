/**
 * StepPdf — step "PDF" del wizard Bagni (Task 22).
 *
 ...
 *  - bottoni "Anteprima PDF" / "Scarica PDF" → chiamano `useBagniPDF`
 *
...
import {
  useBagniPDF,
  type BgnPdfCompany, renderBgnPreviewBlobUrl } from "@/hooks/useBagniPDF";
import type { BgnProgetto, BgnComputoVoce, BgnProgettoMedia } from "@/types/bagni";
...
export default function StepPdf({ progetto, computo, media }: Props) {
  ...
  const { data: template } = useBgnTemplatePdf();
  const { downloadPDF, previewPDF, isGenerating } = useBagniPDF();
  ...
  const { data: company } = useQuery<BgnPdfCompany | null>({
    queryKey: ["bgn-step-pdf-company", companyId],
    enabled: !!companyId,
    staleTime: 5 * 60 * 1000,
    queryFn: async () => {
    ...
    <div className="space-y-3">
      ...
      {progetto.id && progetto.company_id && (
        <InviaFirmaCard
          ...
          generaPdfBlob={async () => {
            const url = await renderBgnPreviewBlobUrl(payload);
          ...
          <div className="overflow-hidden rounded-xl border bg-white">
            ...
            <div className="px-4 py-3">
              <p className="text-sm font-semibold text-slate-900">
                {template?.cover_title?.trim() || "Preventivo di bagni"}
              </p>
