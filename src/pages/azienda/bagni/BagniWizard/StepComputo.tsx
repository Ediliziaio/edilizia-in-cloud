/**
 * StepComputo — lo step "Computo metrico" del wizard Bagni (Task 17).
 *
...
import { ManodoperaLookup } from "@/components/bagni/ManodoperaLookup";
...
export default function StepComputo({ progettoId, initialComputo, scontoPct, ivaPct }: Props) {
  ...
  const toPayload = (rows: BgnComputoVoce[]) =>
    rows.map((v, i) => ({
        ...
        <div className="flex items-start gap-2.5 rounded-xl border border-blue-100 bg-blue-50/60 px-3 py-2.5">
          ...
          <div className="flex-1 text-[11px] text-blue-900">
            ...
            <p className="text-blue-800/80">
              ...
              velocizzare, popola il listino bagni nelle impostazioni.
            </p>
            <Link
            ...
            >
              <Settings2 className="h-3 w-3" /> Vai al listino bagni
            </Link>
          </div>
        </div>
      )}
