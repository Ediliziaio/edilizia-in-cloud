/**
 * BagniIndex — landing del modulo Bagni.
 *
 ...
 * Bagni non richiede ancora i 20+ filtri avanzati dei serramenti).
 */
...
import { BGN_STATI_LABEL } from "./BagniWizard/helpers";
import type { BgnStato } from "@/types/bagni";
import { Checkbox } from "@/components/ui/checkbox";
import { useEffectiveCompanyId } from "@/hooks/useEffectiveCompanyId";
import { useTableSelection } from "@/hooks/useTableSelection";
import { ModuloBulkToolbar } from "@/components/moduli/ModuloBulkToolbar";

export default function BagniIndex() {
  const navigate = useNavigate();
  const companyId = useEffectiveCompanyId();
  const sel = useTableSelection();
  const { data: progetti = [], isLoading, isError, refetch } = useBagniProgetti();
  ...
  const handleClone = async (sourceId: string) => {
    ...
    try {
      ...
      setCloneOpen(false);
      navigate(`/azienda/bagni/${nuovo.id}/modifica`);
    } catch (e) {
      ...
      >
        ...
        <div className="relative max-w-[1400px] mx-auto px-3 sm:px-8 py-4 sm:py-8 flex items-center justify-between flex-wrap gap-3 sm:gap-4 max-sm:flex-nowrap max-sm:py-3">
          ...
          <div className="flex flex-wrap gap-2 w-full sm:w-auto max-sm:w-auto max-sm:shrink-0 max-sm:flex-nowrap">
            ...
            <Button
              onClick={() => navigate("/azienda/bagni/nuovo")}
        ...
        >
          <PilloleFiltro
            ...
            scelte={[
              ...
              ...(Object.keys(BGN_STATI_LABEL) as BgnStato[]).map((k) => ({ value: k, label: BGN_STATI_LABEL[k].label, n: progetti.filter((p) => p.stato === k).length })),
            ]}
          />
        </PannelloFiltri>
          ...
          <CardContent className="p-2.5 sm:p-3 flex flex-wrap items-center gap-2">
            <div className="relative flex-1 min-w-[160px] sm:min-w-[200px] w-full sm:w-auto">
              ...
              <Input
                ...
                aria-label="Cerca progetto bagni"
              />
            </div>
            <Select value={filtroStato} onValueChange={setFiltroStato}>
              ...
              <SelectContent>
                ...
                {(Object.keys(BGN_STATI_LABEL) as BgnStato[]).map((k) => (
                  <SelectItem key={k} value={k}>{BGN_STATI_LABEL[k].label}</SelectItem>
                ))}
              </SelectContent>
            </Select>
        ...
        <Card>
          <CardContent className="p-0">
            {isLoading ? (
              ...
              <EmptyStateFirstTime onCreate={() => navigate("/azienda/bagni/nuovo")} />
              ...
              <>
                <ModuloBulkToolbar
                  tableName="bgn_progetti"
                  companyId={companyId}
                  selectedIds={sel.selected}
                  statoOptions={(Object.keys(BGN_STATI_LABEL) as (keyof typeof BGN_STATI_LABEL)[]).map((k) => ({ value: k as string, label: BGN_STATI_LABEL[k].label }))}
                  onClear={sel.clear}
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {progettiFiltrati.map((p) => {
                        const statoCfg = BGN_STATI_LABEL[p.stato] ?? BGN_STATI_LABEL.bozza;
                ...
                <div className="md:hidden divide-y divide-border">
                  {progettiFiltrati.map((p) => {
                    ...
                    return (
                      <RigaMobile
                        key={p.id}
                        to={`/azienda/bagni/${p.id}/modifica`}
  ...
  return (
    ...
    <div className="p-10 text-center space-y-4 max-sm:space-y-3 max-sm:p-5">
      ...
      <div className="space-y-1">
        <h3 className="text-base font-semibold text-slate-900">Nessun progetto di bagni</h3>
