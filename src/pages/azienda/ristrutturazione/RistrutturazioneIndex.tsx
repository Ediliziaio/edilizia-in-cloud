/**
 * RistrutturazioneIndex — landing del modulo Ristrutturazione.
 *
 ...
 * Ristrutturazione non richiede ancora i 20+ filtri avanzati dei serramenti).
 */
...
import {
  useRistrutturazioneProgetti,
  useDeleteProgetto,
  useClonaProgetto,
} from "@/hooks/useRistrutturazioneProgetto";
import { RST_STATI_LABEL } from "./RistrutturazioneWizard/helpers";
import type { RstStato } from "@/types/ristrutturazione";
import { Checkbox } from "@/components/ui/checkbox";
import { useEffectiveCompanyId } from "@/hooks/useEffectiveCompanyId";
import { useTableSelection } from "@/hooks/useTableSelection";
import { ModuloBulkToolbar } from "@/components/moduli/ModuloBulkToolbar";

export default function RistrutturazioneIndex() {
  const navigate = useNavigate();
  const companyId = useEffectiveCompanyId();
  const sel = useTableSelection();
  const { data: progetti = [], isLoading, isError, refetch } = useRistrutturazioneProgetti();
  ...
  const handleClone = async (sourceId: string) => {
    ...
    try {
      ...
      setCloneOpen(false);
      navigate(`/azienda/ristrutturazione/${nuovo.id}/modifica`);
    } catch (e) {
        ...
        <div className="relative max-w-[1400px] mx-auto px-3 sm:px-8 py-4 sm:py-8 flex items-center justify-between flex-wrap gap-3 sm:gap-4 max-sm:flex-nowrap max-sm:py-3">
          <div className="min-w-0 flex-1">
            ...
            <h1 className="text-xl sm:text-3xl font-bold tracking-tight flex items-center gap-2 max-sm:text-lg">
              ...
              <span className="truncate">Ristrutturazione</span>
            </h1>
            <p className="hidden sm:block text-sm text-blue-100 mt-1">
              Computo metrico e preventivi di ristrutturazione sotto controllo.
            </p>
          </div>
        ...
        >
          <PilloleFiltro
            ...
            scelte={[
              ...
              ...(Object.keys(RST_STATI_LABEL) as RstStato[]).map((k) => ({ value: k, label: RST_STATI_LABEL[k].label, n: progetti.filter((p) => p.stato === k).length })),
            ]}
          />
        </PannelloFiltri>
          ...
          <CardContent className="p-2.5 sm:p-3 flex flex-wrap items-center gap-2">
            <div className="relative flex-1 min-w-[160px] sm:min-w-[200px] w-full sm:w-auto">
              ...
              <Input
                ...
                aria-label="Cerca progetto ristrutturazione"
              />
            </div>
            <Select value={filtroStato} onValueChange={setFiltroStato}>
              ...
              <SelectContent>
                ...
                {(Object.keys(RST_STATI_LABEL) as RstStato[]).map((k) => (
                  <SelectItem key={k} value={k}>{RST_STATI_LABEL[k].label}</SelectItem>
                ))}
              </SelectContent>
            </Select>
        ...
        <Card>
          <CardContent className="p-0">
            {isLoading ? (
              ...
              <EmptyStateFirstTime onCreate={() => navigate("/azienda/ristrutturazione/nuovo")} />
              ...
              <>
                <ModuloBulkToolbar
                  tableName="rst_progetti"
                  companyId={companyId}
                  selectedIds={sel.selected}
                  statoOptions={(Object.keys(RST_STATI_LABEL) as (keyof typeof RST_STATI_LABEL)[]).map((k) => ({ value: k as string, label: RST_STATI_LABEL[k].label }))}
                  onClear={sel.clear}
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {progettiFiltrati.map((p) => {
                        const statoCfg = RST_STATI_LABEL[p.stato] ?? RST_STATI_LABEL.bozza;
                        ...
                        return (
                          <TableRow
                            ...
                            onClick={() => navigate(`/azienda/ristrutturazione/${p.id}/modifica`)}
                          >
                ...
                <div className="md:hidden divide-y divide-border">
                  {progettiFiltrati.map((p) => {
                    ...
                    return (
                      <RigaMobile
                        key={p.id}
                        to={`/azienda/ristrutturazione/${p.id}/modifica`}
  ...
  return (
    ...
    <div className="p-10 text-center space-y-4 max-sm:space-y-3 max-sm:p-5">
      ...
      <div className="space-y-1">
        <h3 className="text-base font-semibold text-slate-900">Nessun progetto di ristrutturazione</h3>
