/**
 * StepPdf — step "PDF" del wizard Ristrutturazione (Task 22).
 *
 ...
 *  - bottoni "Anteprima PDF" / "Scarica PDF" → chiamano `useRistrutturazionePDF`
 *
...
export default function StepPdf({ progetto, computo, media, onIndietro, onVaiAlPasso }: Props) {
  ...
  const { data: company } = useQuery<RstPdfCompany | null>({
    queryKey: ["rst-step-pdf-company", companyId],
    enabled: !!companyId,
    staleTime: 5 * 60 * 1000,
    queryFn: async () => {
  ...
  return (
    <div className="space-y-3 max-md:flex max-md:flex-col max-md:gap-3 max-md:space-y-0">
