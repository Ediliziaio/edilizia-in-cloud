/**
 * StepMedia — foto, render e allegati del progetto Ristrutturazione (Task 18).
 *
 * Upload su Supabase Storage + riga in `rst_progetti_media` (via
 ...
 *   Il path resta `{company_id}/ristrutturazione/{progetto_id}/{uuid}.{ext}`: le
 *   policy guardano la prima cartella, l'azienda su cui si lavora. In
 *   `rst_progetti_media.url` va il riferimento "progetti-media/<path>",
 *   non un link: si firma quando serve (anteprima qui sotto, PDF in
 *   useRistrutturazionePDF), quindi non scade. Vedi src/lib/storage/fileRiservati.ts.
 *
...
import type { RstProgettoMedia } from "@/types/ristrutturazione";
...
export default function StepMedia({ progettoId, media }: Props) {
  ...
  const handleFiles = async (files: FileList | null) => {
    ...
    try {
      for (const file of Array.from(files)) {
        ...
        const storagePath = `${companyId}/ristrutturazione/${progettoId}/${crypto.randomUUID()}.${ext}`;
  ...
  const updateMedia = async (
    m: RstProgettoMedia,
    patch: Partial<Pick<RstProgettoMedia, "tipo" | "caption">>,
  ) => {
    try {
  ...
  // ─── Elimina ───────────────────────────────────────────────────────────────
  const remove = async (m: RstProgettoMedia) => {
