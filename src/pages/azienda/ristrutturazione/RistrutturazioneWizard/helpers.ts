/**
 * RistrutturazioneWizard — helpers.
 *
 ...
 * `@/lib/ristrutturazione/calcoli`.
 */
import type { RstProgetto, RstComputoVoce, RstStato } from "@/types/ristrutturazione";

export type RstWizardStepKey =
  | "cliente"
  | "immobile"
  | "computo"
  | "media"
...
export interface RstWizardStepDef {
  key: RstWizardStepKey;
  label: string;
}

/** Sequenza degli step del wizard (ordine = rendering nello stepper). */
export const RST_WIZARD_STEPS: readonly RstWizardStepDef[] = [
...
export const RST_STATI_LABEL: Record<RstStato, { label: string; className: string }> = {
...
export function stepCompletion(
  progetto: Partial<RstProgetto> | null | undefined,
  computo: RstComputoVoce[] | null | undefined,
): Record<RstWizardStepKey, boolean> {
  const p = progetto ?? {};
  const righe = computo ?? [];
  return {
