/**
 * RistrutturazioneWizard — shell a step per creare/modificare un progetto di
 * ristrutturazione. Modellato su SerramentiWizard:
...
import {
  useRistrutturazioneProgetto,
  useUpsertProgetto,
  getRstTemplatePdf,
} from "@/hooks/useRistrutturazioneProgetto";
import type { RstProgetto } from "@/types/ristrutturazione";
import {
  RST_WIZARD_STEPS, RST_STATI_LABEL, stepCompletion,
  compactText, type RstWizardStepKey,
} from "./RistrutturazioneWizard/helpers";
...
export default function RistrutturazioneWizard() {
  ...
  const modelSupport = useSupportoModelloPreventivo("ristrutturazione");
  ...
  const { data: ultimaBozza } = useQuery({
    queryKey: ["rst-ultima-bozza", user?.id, effectiveCompany?.id],
  ...
  const [currentStep, setCurrentStep] = useState<RstWizardStepKey>("cliente");
  const [creating, setCreating] = useState(false);

  const { data: detail, isLoading, isError, refetch } = useRistrutturazioneProgetto(id);
  ...
  const savedModel = useMemo<{ snapshot: ReturnType<typeof leggiModelloPreventivo>; error: string | null }>(() => {
    try { return { snapshot: detail ? leggiModelloPreventivo("ristrutturazione", detail.progetto.modello_snapshot, detail.progetto.company_id) : null, error: null }; }
    catch (error) { return { snapshot: null, error: error instanceof Error ? error.message : "Modello non valido" }; }
  }, [detail]);
  const model = interventoDelModulo("ristrutturazione", isNew ? requestedModel : savedModel.snapshot?.modelId);

  // Local form state (campi del progetto).
  const [form, setForm] = useState<RstFormPatch>(() => model ? { tipo_intervento: TIPO_INTERVENTO_DEL_MODELLO.ristrutturazione[model.id] } : {});
  const createInput = async (): Promise<RstFormPatch> => {
    ...
    const [{ createFullRstTemplate, isFullRstModuleId }, { loadLocalRstTemplate }, { sincronizzaModelliAzienda }] = await Promise.all([
      import("@/lib/moduli-vendita/fullRstModules"), import("@/lib/moduli-vendita/localRstTemplates"),
      import("@/lib/moduli-vendita/archivioModelli"),
    ]);
    if (!isFullRstModuleId(model.id)) throw new Error("Questo intervento non ha un modello completo. Nessuna offerta generica è stata creata.");
    ...
    const base = await getRstTemplatePdf(companyId);
    const source = loadLocalRstTemplate(companyId, model.id)?.template ?? createFullRstTemplate(base, model.id);
    return { ...form, tipo_intervento: TIPO_INTERVENTO_DEL_MODELLO.ristrutturazione[model.id], modello_snapshot: creaModelloPreventivo("ristrutturazione", companyId, model.id, source) };
  };
  ...
  const onChange = <K extends keyof RstFormPatch>(key: K, value: RstFormPatch[K]) => {
  ...
  const currentStepIndex = useMemo(
    () => RST_WIZARD_STEPS.findIndex((s) => s.key === currentStep),
    [currentStep],
  );
  const progress = useMemo(
    () => Math.round(((currentStepIndex + 1) / RST_WIZARD_STEPS.length) * 100),
    [currentStepIndex],
  );
  ...
  const handleSaveAndContinue = async () => {
    ...
    if (isNew) {
      ...
      try {
        ...
        navigate(`/azienda/ristrutturazione/${created.id}/modifica`, { replace: true });
      } catch (e) {
    ...
    if (!ok) return;
    const idx = RST_WIZARD_STEPS.findIndex((s) => s.key === currentStep);
    if (idx >= 0 && idx < RST_WIZARD_STEPS.length - 1) {
      setCurrentStep(RST_WIZARD_STEPS[idx + 1].key);
    } else {
      toast.success("Progetto salvato");
    }
  };

  const handleStepClick = async (target: RstWizardStepKey) => {
      ...
      <div className="container mx-auto p-4 max-w-4xl">
        <div className="rounded-lg border border-rose-200 bg-rose-50/40 p-6 text-center space-y-3">
          ...
          <div className="flex items-center gap-2 justify-center flex-wrap">
            ...
            <Button size="sm" variant="ghost" onClick={() => navigate("/azienda/ristrutturazione")} className="gap-1">
  ...
  if (savedModel.error || (isNew && requestedModel && !model)) return <div role="alert" className="space-y-3 p-6"><h1 className="text-xl font-semibold">Intervento non disponibile</h1><p>{savedModel.error ?? "Il tipo di intervento richiesto non è riconosciuto."}</p><Button onClick={() => navigate("/azienda/marketing/preventivi?tab=moduli&area=ristrutturazioni")}>Scegli un intervento</Button></div>;

  const statoMeta = RST_STATI_LABEL[(detail?.progetto.stato as RstProgetto["stato"]) ?? "bozza"];
    ...
    <div className="pb-28 md:pb-20">
      ...
      <AlertDialog open={Boolean(isNew && !requestedModel && ultimaBozza && !resumeDismissed)}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={() => navigate(`/azienda/ristrutturazione/${ultimaBozza!.id}/modifica`)}>
              Riprendi bozza
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
        ...
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              ...
              onClick={() => navigate("/azienda/ristrutturazione")}
            >
              Esci senza salvare
            </AlertDialogAction>
            <AlertDialogAction
              onClick={async () => {
                ...
                navigate("/azienda/ristrutturazione");
              }}
            >
              Salva bozza ed esci
            </AlertDialogAction>
      ...
      <div className="sticky top-0 z-30 border-b bg-background/95 shadow-sm backdrop-blur max-md:-mx-3 max-md:-mt-3 max-md:-top-3 md:-top-6">
        <div className="container mx-auto flex max-w-6xl items-center gap-2 p-2.5 sm:gap-3 sm:p-3">
          <Button variant="ghost" size="icon" onClick={() => { if (isNew && dirty) { setExitDialogOpen(true); return; } navigate("/azienda/ristrutturazione"); }} className="h-10 w-10 shrink-0">
          ...
          <main className="col-span-12 space-y-4 md:col-span-9 lg:col-span-10">
            ...
            <div className={cn("sticky bottom-[calc(env(safe-area-inset-bottom)-1.5rem)] z-40 flex items-center justify-between gap-2 rounded-2xl border bg-background/95 px-3 py-2.5 shadow-[0_-8px_20px_rgba(15,23,42,0.08)] backdrop-blur md:static md:mx-0 md:rounded-none md:border-0 md:bg-transparent md:px-0 md:py-2 md:shadow-none md:backdrop-blur-0", currentStep === "pdf" && id && detail && "max-md:hidden")}>
              ...
              >
                ...
                ) : currentStepIndex === RST_WIZARD_STEPS.length - 1 ? (
                  <Save className="h-4 w-4" />
                ) : (
                  <ArrowRight className="h-4 w-4" />
                )}
                {isNew ? "Crea e continua" :
                  currentStepIndex === RST_WIZARD_STEPS.length - 1 ? "Salva" : "Salva e continua"}
              </Button>
            </div>
          </main>
        </div>
...
function StepComingSoon({ step }: { step: RstWizardStepKey }) {
  const meta = RST_WIZARD_STEPS.find((s) => s.key === step);
