/**
 * GbpOAuthCallbackPage — pagina di atterraggio dopo Google Business Profile OAuth.
 *
 * 2026-05-27: identico pattern a CalendarOAuthCallbackPage. Necessario perché
 * il gateway Supabase forza Content-Type:text/plain + CSP:sandbox su edge
 * function con verify_jwt=false → l'HTML che la edge function serviva veniva
 * mostrato come testo grezzo (script inline bloccato dal CSP) → window.opener.
 * postMessage non veniva mai eseguito → il parent restava bloccato in
 * "Connessione in corso..." (problema identico a quello già risolto per
 * Google Calendar).
 *
 * Flow:
 *   1. Edge function gbp-oauth?action=callback fa 302 redirect a:
 *      https://app.ediliziaincloud.com/azienda/impostazioni/integrazioni/gbp-callback?status=ok|error&message=...
 *   2. Questa pagina riceve i query param, fa postMessage al parent (window.opener)
 *      su multiple trusted origins, poi window.close().
 *   3. Il parent (GbpConnectionCard) riceve il messaggio e invalida la query.
 *
 * Il payload deve matchare il listener: { source: "gbp-oauth", status, message }.
 */
...
function postResultToOpener(status: "ok" | "error", message: string | null) {
  ...
  const payload = { source: "gbp-oauth", status, message };
...
export default function GbpOAuthCallbackPage() {
      ...
      <div className="w-full max-w-md rounded-3xl border border-blue-100 bg-white p-8 text-center shadow-xl">
        {status === "ok" ? (
          <>
            ...
            <h1 className="text-xl font-semibold text-slate-900">Google Business Profile collegato</h1>
