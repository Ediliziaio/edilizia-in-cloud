/**
 * StepMedia — foto, render e allegati del progetto Tetti (Task 18).
 *
 * Upload su Supabase Storage + riga in `tet_progetti_media` (via
 ...
 *   Il path resta `{company_id}/tetti/{progetto_id}/{uuid}.{ext}`: le
 *   policy guardano la prima cartella, l'azienda su cui si lavora. In
 *   `tet_progetti_media.url` va il riferimento "progetti-media/<path>",
 *   non un link: si firma quando serve (anteprima qui sotto, PDF in
 *   useTettiPDF), quindi non scade. Vedi src/lib/storage/fileRiservati.ts.
 *
...
import type { TetProgettoMedia } from "@/types/tetti";
...
export default function StepMedia({ progettoId, media }: Props) {
  ...
  const handleFiles = async (files: FileList | null) => {
    ...
    try {
      for (const file of Array.from(files)) {
        ...
        const storagePath = `${companyId}/tetti/${progettoId}/${crypto.randomUUID()}.${ext}`;
  ...
  const updateMedia = async (
    m: TetProgettoMedia,
    patch: Partial<Pick<TetProgettoMedia, "tipo" | "caption">>,
  ) => {
    try {
  ...
  // ─── Elimina ───────────────────────────────────────────────────────────────
  const remove = async (m: TetProgettoMedia) => {
