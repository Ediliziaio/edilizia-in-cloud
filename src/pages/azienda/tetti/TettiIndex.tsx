/**
 * TettiIndex — landing del modulo Tetti.
 *
 ...
 * Tetti non richiede ancora i 20+ filtri avanzati dei serramenti).
 */
...
export default function TettiIndex() {
  ...
  const handleClone = async (sourceId: string) => {
    ...
    try {
      ...
      setCloneOpen(false);
      navigate(`/azienda/tetti/${nuovo.id}/modifica`);
    } catch (e) {
      ...
      >
        ...
        <div className="relative max-w-[1400px] mx-auto px-3 sm:px-8 py-4 sm:py-8 flex items-center justify-between flex-wrap gap-3 sm:gap-4 max-sm:flex-nowrap max-sm:py-3">
          ...
          <div className="flex flex-wrap gap-2 w-full sm:w-auto max-sm:w-auto max-sm:shrink-0 max-sm:flex-nowrap">
            ...
            <Button
              onClick={() => navigate("/azienda/tetti/nuovo")}
        ...
        >
          <PilloleFiltro
            ...
            scelte={[
              ...
              ...(Object.keys(TET_STATI_LABEL) as TetStato[]).map((k) => ({ value: k, label: TET_STATI_LABEL[k].label, n: progetti.filter((p) => p.stato === k).length })),
            ]}
          />
        </PannelloFiltri>
          ...
          <CardContent className="p-2.5 sm:p-3 flex flex-wrap items-center gap-2">
            <div className="relative flex-1 min-w-[160px] sm:min-w-[200px] w-full sm:w-auto">
              ...
              <Input
                ...
                aria-label="Cerca progetto tetti"
              />
            </div>
            <Select value={filtroStato} onValueChange={setFiltroStato}>
              ...
              <SelectContent>
                ...
                {(Object.keys(TET_STATI_LABEL) as TetStato[]).map((k) => (
                  <SelectItem key={k} value={k}>{TET_STATI_LABEL[k].label}</SelectItem>
                ))}
              </SelectContent>
            </Select>
        ...
        <Card>
          <CardContent className="p-0">
            {isLoading ? (
              ...
              <EmptyStateFirstTime onCreate={() => navigate("/azienda/tetti/nuovo")} />
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {progettiFiltrati.map((p) => {
                        const statoCfg = TET_STATI_LABEL[p.stato] ?? TET_STATI_LABEL.bozza;
                ...
                <div className="md:hidden divide-y divide-border">
                  {progettiFiltrati.map((p) => {
                    ...
                    return (
                      <RigaMobile
                        key={p.id}
                        to={`/azienda/tetti/${p.id}/modifica`}
  ...
  return (
    ...
    <div className="p-10 text-center space-y-4 max-sm:space-y-3 max-sm:p-5">
      ...
      <div className="space-y-1">
        <h3 className="text-base font-semibold text-slate-900">Nessun progetto di tetti</h3>
