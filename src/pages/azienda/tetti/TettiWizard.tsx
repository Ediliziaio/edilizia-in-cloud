/**
 * TettiWizard — shell a step per creare/modificare un progetto di
 * tetti. Modellato su SerramentiWizard:
...
import {
  TET_WIZARD_STEPS, TET_STATI_LABEL, stepCompletion,
  compactText, type TetWizardStepKey,
} from "./TettiWizard/helpers";
...
const ETICHETTA_BREVE_PASSO: Partial<Record<TetWizardStepKey, string>> = { immobile: "Tetto" };

export default function TettiWizard() {
  ...
  const { data: ultimaBozza } = useQuery({
    queryKey: ["tet-ultima-bozza", user?.id, effectiveCompany?.id],
  ...
  const [currentStep, setCurrentStep] = useState<TetWizardStepKey>("cliente");
  ...
  const onChange = <K extends keyof TetFormPatch>(key: K, value: TetFormPatch[K]) => {
  ...
  const currentStepIndex = useMemo(
    () => TET_WIZARD_STEPS.findIndex((s) => s.key === currentStep),
    [currentStep],
  );
  ...
  const handleSaveAndContinue = async () => {
    ...
    if (!ok) return;
    const idx = TET_WIZARD_STEPS.findIndex((s) => s.key === currentStep);
    if (idx >= 0 && idx < TET_WIZARD_STEPS.length - 1) {
      setCurrentStep(TET_WIZARD_STEPS[idx + 1].key);
    } else {
      toast.success("Progetto salvato");
    }
  };

  const handleStepClick = async (target: TetWizardStepKey) => {
  ...
  const handleBack = () => {
    if (currentStepIndex > 0) void handleStepClick(TET_WIZARD_STEPS[currentStepIndex - 1].key);
  };
      ...
      <div className="container mx-auto p-4 max-w-4xl">
        <div className="rounded-lg border border-rose-200 bg-rose-50/40 p-6 text-center space-y-3">
          ...
          <div className="flex items-center gap-2 justify-center flex-wrap">
            ...
            <Button size="sm" variant="ghost" onClick={() => navigate("/azienda/tetti")} className="gap-1">
    ...
    <div className="pb-28 md:pb-20">
      ...
      <AlertDialog open={Boolean(isNew && !requestedModel && ultimaBozza && !resumeDismissed)}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={() => navigate(`/azienda/tetti/${ultimaBozza!.id}/modifica`)}>
              Riprendi bozza
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
        ...
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              ...
              onClick={() => navigate("/azienda/tetti")}
            >
              Esci senza salvare
            </AlertDialogAction>
            <AlertDialogAction
      ...
      <div className="sticky top-0 z-30 border-b bg-background/95 shadow-sm backdrop-blur">
        <div className="container mx-auto flex max-w-6xl items-center gap-2 p-2.5 sm:gap-3 sm:p-3">
          <Button variant="ghost" size="icon" onClick={() => { if (isNew && dirty) { setExitDialogOpen(true); return; } navigate("/azienda/tetti"); }} className="h-10 w-10 shrink-0">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[11px] text-muted-foreground truncate max-md:hidden">
              Step {currentStepIndex + 1} di {TET_WIZARD_STEPS.length} · {TET_WIZARD_STEPS[currentStepIndex]?.label}
            </p>
          </div>
        </div>
        ...
        <div className="md:hidden overflow-x-auto border-t bg-background/95 px-2 py-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          ...
          <nav className="flex min-h-0 w-full min-w-max gap-1" aria-label="Step progetto tetti">
            {TET_WIZARD_STEPS.map((s, idx) => {
          ...
          <main className="col-span-12 space-y-4 md:col-span-9 lg:col-span-10">
            ...
            <div className={cn("fixed inset-x-3 bottom-[calc(5.5rem+env(safe-area-inset-bottom))] z-40 flex items-center justify-between gap-2 rounded-2xl border bg-background/95 px-3 py-2.5 shadow-[0_-8px_20px_rgba(15,23,42,0.08)] backdrop-blur md:static md:mx-0 md:rounded-none md:border-0 md:bg-transparent md:px-0 md:py-2 md:shadow-none md:backdrop-blur-0", currentStep === "pdf" && id && detail && "max-md:hidden")}>
              ...
              >
                ...
                ) : currentStepIndex === TET_WIZARD_STEPS.length - 1 ? (
                  <Save className="h-4 w-4" />
                ) : (
                  <ArrowRight className="h-4 w-4" />
                )}
                {isNew ? "Crea e continua" :
                  currentStepIndex === TET_WIZARD_STEPS.length - 1 ? "Salva" : "Salva e continua"}
              </Button>
            </div>
          </main>
        </div>
...
function StepComingSoon({ step }: { step: TetWizardStepKey }) {
  const meta = TET_WIZARD_STEPS.find((s) => s.key === step);
