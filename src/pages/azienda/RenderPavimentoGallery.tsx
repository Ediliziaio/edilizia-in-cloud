...
export default function RenderPavimentoGallery() {
  ...
  const filtered = sessions.filter((item) => {
    ...
    const pavConfig = item.config as { tipo?: string; colore_nome?: string } | null;
    return (
      (pavConfig?.tipo ?? "").toLowerCase().includes(s) ||
      (pavConfig?.colore_nome ?? "").toLowerCase().includes(s) ||
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        <Button variant="ghost" size="icon" onClick={() => navigate("/azienda/render/pavimento")}>
          <ArrowLeft className="h-4 w-4" />
        </Button>
        <div className="flex-1">
          <h1 className="text-xl font-bold flex items-center gap-2">
            <GalleryHorizontalEnd className="h-5 w-5 text-amber-600" />
            Galleria Pavimenti
          </h1>
          <p className="text-sm text-muted-foreground">{filtered.length} render completati</p>
        </div>
        <Button className="bg-amber-600 hover:bg-amber-700" onClick={() => navigate("/azienda/render/pavimento/new")}>
          ...
          <CardContent className="py-16 flex flex-col items-center gap-4 text-center">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-1">
                I render pavimento completati appariranno qui
              </p>
            </div>
            <Button className="bg-amber-600 hover:bg-amber-700" onClick={() => navigate("/azienda/render/pavimento/new")}>
              <Plus className="h-4 w-4 mr-2" />
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {filtered.map((item) => {
            ...
            return (
              <div
                key={item.id}
                className="group relative aspect-video rounded-lg overflow-hidden cursor-pointer border hover:border-amber-500/50 transition-all hover:shadow-md bg-muted"
                onClick={() => navigate(`/azienda/render/pavimento/gallery/${item.id}`)}
              >
                {resultUrl ? (
                  <img
                    src={resultUrl}
