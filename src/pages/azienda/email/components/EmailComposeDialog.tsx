...
export function EmailComposeDialog({ open, onOpenChange, context, companyIdOverride, orderId }: EmailComposeDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !isSending && onOpenChange(o)}>
      {/* dvh (non vh): su iOS Safari/tastiera aperta il footer con "Invia" finiva fuori dal viewport visibile */}
      <DialogContent className="flex h-[calc(100dvh-2rem)] max-h-[90dvh] w-[calc(100vw-1rem)] max-w-4xl flex-col overflow-hidden rounded-2xl border-blue-100 p-0 shadow-2xl">
