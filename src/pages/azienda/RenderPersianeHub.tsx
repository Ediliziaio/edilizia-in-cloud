...
const HOW_IT_WORKS = [
  {
    ...
    desc: "Foto frontale della facciata con le persiane attuali. JPG, PNG o WEBP.",
  },
  {
    icon: Wand2,
    title: "2. Analisi AI",
    desc: "L'AI identifica automaticamente le persiane, il materiale e lo stato.",
  },
  {
    icon: PanelLeftClose,
    title: "3. Configura",
    desc: "Scegli tipo, materiale, colore RAL e stato di apertura desiderati.",
  },
  {
    icon: Share2,
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-center gap-2 text-sm text-muted-foreground">
        <Link to="/azienda/render" className="hover:text-foreground">
          Render AI
        </Link>
        <span>/</span>
        <span className="text-foreground">Persiane</span>
      </div>

      {/* ── Hero ─────────────────────────────────────────────────────────────── */}
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-green-600/90 to-green-700 p-6 sm:p-8 text-white">
        ...
        <div className="relative flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <h1 className="text-2xl sm:text-3xl font-bold text-white leading-tight">
              Visualizza le nuove persiane
              <br className="hidden sm:block" /> sulla facciata reale
            </h1>
            <p className="mt-2 text-white/80 text-sm max-w-md">
              Carica una foto, configura le persiane desiderate e ottieni un render
              fotorealistico in pochi secondi. Perfetto per preventivi e presentazioni.
            </p>
          </div>
          <div className="flex flex-col gap-2 shrink-0">
            ...
            <Button
              size="lg"
              className="bg-white text-green-700 hover:bg-white/90 font-semibold gap-2 shadow-lg"
        ...
        {loadingSessions ? (
          ...
          <Card>
            <CardContent className="py-14 flex flex-col items-center gap-4 text-center">
              ...
              <div>
                <p className="font-medium">Nessun render persiane ancora</p>
                <p className="text-sm text-muted-foreground mt-1">
                  Carica una foto della facciata e configura le tue persiane
                </p>
              </div>
              <Button
                onClick={() => navigate("/azienda/render/persiane/new")}
          ...
          <div className="space-y-2">
            {normalizedSessions.map((session) => {
              ...
              return (
                <Card
                  key={session.id}
                  className="cursor-pointer hover:border-green-600/40 transition-colors"
                  onClick={() =>
                    navigate(`/azienda/render/persiane/gallery/${session.id}`)
                  }
                >
                  <CardContent className="p-3 flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5 flex-wrap">
                        <Badge
                          variant={
                            cfg.color as "default" | "secondary" | "destructive"
                          }
