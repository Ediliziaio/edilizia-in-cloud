/**
 * ElettricoIndex — landing del modulo Elettrico.
 *
 ...
 * Elettrico non richiede ancora i 20+ filtri avanzati dei serramenti).
 */
...
import {
  useElettricoProgetti,
  useDeleteProgetto,
  useClonaProgetto,
} from "@/hooks/useElettricoProgetto";
import { ELE_STATI_LABEL } from "./ElettricoWizard/helpers";
import type { EleStato } from "@/types/elettrico";
import { Checkbox } from "@/components/ui/checkbox";
import { useEffectiveCompanyId } from "@/hooks/useEffectiveCompanyId";
import { useTableSelection } from "@/hooks/useTableSelection";
import { ModuloBulkToolbar } from "@/components/moduli/ModuloBulkToolbar";

export default function ElettricoIndex() {
  const navigate = useNavigate();
  const companyId = useEffectiveCompanyId();
  const sel = useTableSelection();
  const { data: progetti = [], isLoading, isError, refetch } = useElettricoProgetti();
  ...
  const handleClone = async (sourceId: string) => {
    ...
    try {
      ...
      setCloneOpen(false);
      navigate(`/azienda/elettrico/${nuovo.id}/modifica`);
    } catch (e) {
      ...
      >
        ...
        <div className="relative max-w-[1400px] mx-auto px-3 sm:px-8 py-4 sm:py-8 flex items-center justify-between flex-wrap gap-3 sm:gap-4 max-sm:flex-nowrap max-sm:py-3">
          ...
          <div className="flex flex-wrap gap-2 w-full sm:w-auto max-sm:w-auto max-sm:shrink-0 max-sm:flex-nowrap">
            ...
            <Button
              onClick={() => navigate("/azienda/elettrico/nuovo")}
        ...
        >
          <PilloleFiltro
            ...
            scelte={[
              ...
              ...(Object.keys(ELE_STATI_LABEL) as EleStato[]).map((k) => ({ value: k, label: ELE_STATI_LABEL[k].label, n: progetti.filter((p) => p.stato === k).length })),
            ]}
          />
        </PannelloFiltri>
          ...
          <CardContent className="p-2.5 sm:p-3 flex flex-wrap items-center gap-2">
            <div className="relative flex-1 min-w-[160px] sm:min-w-[200px] w-full sm:w-auto">
              ...
              <Input
                ...
                aria-label="Cerca progetto elettrico"
              />
            </div>
            <Select value={filtroStato} onValueChange={setFiltroStato}>
              ...
              <SelectContent>
                ...
                {(Object.keys(ELE_STATI_LABEL) as EleStato[]).map((k) => (
                  <SelectItem key={k} value={k}>{ELE_STATI_LABEL[k].label}</SelectItem>
                ))}
              </SelectContent>
            </Select>
        ...
        <Card>
          <CardContent className="p-0">
            {isLoading ? (
              ...
              <EmptyStateFirstTime onCreate={() => navigate("/azienda/elettrico/nuovo")} />
              ...
              <>
                <ModuloBulkToolbar
                  tableName="ele_progetti"
                  companyId={companyId}
                  selectedIds={sel.selected}
                  statoOptions={(Object.keys(ELE_STATI_LABEL) as (keyof typeof ELE_STATI_LABEL)[]).map((k) => ({ value: k as string, label: ELE_STATI_LABEL[k].label }))}
                  onClear={sel.clear}
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {progettiFiltrati.map((p) => {
                        const statoCfg = ELE_STATI_LABEL[p.stato] ?? ELE_STATI_LABEL.bozza;
                        ...
                        return (
                          <TableRow
                            ...
                            onClick={() => navigate(`/azienda/elettrico/${p.id}/modifica`)}
                          >
                ...
                <div className="md:hidden divide-y divide-border">
                  {progettiFiltrati.map((p) => {
                    ...
                    return (
                      <RigaMobile
                        key={p.id}
                        to={`/azienda/elettrico/${p.id}/modifica`}
  ...
  return (
    ...
    <div className="p-10 text-center space-y-4 max-sm:space-y-3 max-sm:p-5">
      ...
      <div className="space-y-1">
        <h3 className="text-base font-semibold text-slate-900">Nessun progetto di elettrico</h3>
