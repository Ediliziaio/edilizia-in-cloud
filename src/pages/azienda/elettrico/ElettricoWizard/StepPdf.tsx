/**
 * StepPdf — step "PDF" del wizard Elettrico (Task 22).
 *
 ...
 *  - bottoni "Anteprima PDF" / "Scarica PDF" → chiamano `useElettricoPDF`
 *
...
import { calcTotaliComputo } from "@/lib/elettrico/calcoli";
import {
  useEleTemplatePdf,
...
import {
  useElettricoPDF,
  type ElePdfCompany, renderElePreviewBlobUrl } from "@/hooks/useElettricoPDF";
import type { EleProgetto, EleComputoVoce, EleProgettoMedia } from "@/types/elettrico";
...
export default function StepPdf({ progetto, computo, media }: Props) {
  ...
  const { data: template } = useEleTemplatePdf();
  const { downloadPDF, previewPDF, isGenerating } = useElettricoPDF();
  ...
  const { data: company } = useQuery<ElePdfCompany | null>({
    queryKey: ["ele-step-pdf-company", companyId],
    enabled: !!companyId,
    staleTime: 5 * 60 * 1000,
    queryFn: async () => {
    ...
    <div className="space-y-3">
      ...
      {progetto.id && progetto.company_id && (
        <InviaFirmaCard
          ...
          generaPdfBlob={async () => {
            const url = await renderElePreviewBlobUrl(payload);
          ...
          <div className="overflow-hidden rounded-xl border bg-white">
            ...
            <div className="px-4 py-3">
              <p className="text-sm font-semibold text-slate-900">
                {template?.cover_title?.trim() || "Preventivo di elettrico"}
              </p>
