/**
 * StepComputo — lo step "Computo metrico" del wizard Elettrico (Task 17).
 *
...
import { useSaveComputo, useEffectiveCompanyId } from "@/hooks/useElettricoProgetto";
import { useListinoVociSearch } from "@/hooks/useElettricoListino";
import type { EleComputoVoce, EleUnitaMisura } from "@/types/elettrico";
import type { ComputoVoceLocal } from "@/types/computo";
import ComputoEditor from "@/components/elettrico/ComputoEditor/ComputoEditor";

interface Props {
  progettoId: string;
  initialComputo: EleComputoVoce[];
  scontoPct: number;
  ivaPct: number;
}

const LISTINO_SETTINGS_HREF =
  "/azienda/impostazioni/template-preventivi?tab=moduli-vendita&modulo=elettrico";
...
export default function StepComputo({ progettoId, initialComputo, scontoPct, ivaPct }: Props) {
  ...
  const [computo, setComputo] = useState<EleComputoVoce[]>(() => initialComputo);
  ...
  const handleChange = (next: EleComputoVoce[]) => {
    setComputo(next);
    setDirty(true);
  };

  // Import AI: le voci estratte (già riviste nel modale) → EleComputoVoce, accodate.
  const handleImportVoci = (estratte: ComputoVoceLocal[]) => {
    if (estratte.length === 0) return;
    const base = computo.length;
    const nuove: EleComputoVoce[] = estratte.map((v, i) => ({
      id:
  ...
  const toPayload = (rows: EleComputoVoce[]) =>
    rows.map((v, i) => ({
        ...
        <div className="flex items-start gap-2.5 rounded-xl border border-blue-100 bg-blue-50/60 px-3 py-2.5">
          ...
          <div className="flex-1 text-[11px] text-blue-900">
            ...
            <p className="text-blue-800/80">
              ...
              velocizzare, popola il listino elettrico nelle impostazioni.
            </p>
            <Link
            ...
            >
              <Settings2 className="h-3 w-3" /> Vai al listino elettrico
            </Link>
          </div>
        </div>
      )}
...
// Coercizione dell'unità di misura estratta dall'AI (testo libero) → enum EleUnitaMisura.
function coerceUnita(u: string | null | undefined): EleUnitaMisura {
