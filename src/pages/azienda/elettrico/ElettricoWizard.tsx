 ...
 * elettrico. Modellato su SerramentiWizard:
...
export default function ElettricoWizard() {
  ...
  const { data: ultimaBozza } = useQuery({
    queryKey: ["ele-ultima-bozza", user?.id],
      ...
      <div className="container mx-auto p-4 max-w-4xl">
        <div className="rounded-lg border border-rose-200 bg-rose-50/40 p-6 text-center space-y-3">
          ...
          <div className="flex items-center gap-2 justify-center flex-wrap">
            ...
            <Button size="sm" variant="ghost" onClick={() => navigate("/azienda/elettrico")} className="gap-1">
    ...
    <div className="pb-28 md:pb-20">
        ...
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              ...
              onClick={() => navigate("/azienda/elettrico")}
            >
              Esci senza salvare
            </AlertDialogAction>
            <AlertDialogAction
      ...
      <div className="sticky top-0 z-30 border-b bg-background/95 shadow-sm backdrop-blur">
        <div className="container mx-auto flex max-w-6xl items-center gap-2 p-2.5 sm:gap-3 sm:p-3">
          <Button variant="ghost" size="icon" onClick={() => { if (isNew && dirty) { setExitDialogOpen(true); return; } navigate("/azienda/elettrico"); }} className="h-10 w-10 shrink-0">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[11px] text-muted-foreground truncate">
              Step {currentStepIndex + 1} di {ELE_WIZARD_STEPS.length} · {ELE_WIZARD_STEPS[currentStepIndex]?.label}
            </p>
          </div>
        </div>
        ...
        <div className="md:hidden overflow-x-auto border-t bg-background/95 px-3 py-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          <nav className="flex min-w-max gap-2" aria-label="Step progetto elettrico">
            {ELE_WIZARD_STEPS.map((s, idx) => {
          ...
          <main className="col-span-12 space-y-4 md:col-span-9 lg:col-span-10">
            ...
            <div className="fixed inset-x-0 bottom-0 z-40 flex items-center justify-between gap-2 border-t bg-background/95 px-3 py-3 pb-[calc(0.75rem+env(safe-area-inset-bottom))] shadow-[0_-8px_20px_rgba(15,23,42,0.08)] backdrop-blur md:static md:mx-0 md:border-t-0 md:bg-transparent md:px-0 md:py-2 md:shadow-none md:backdrop-blur-0">
              ...
              >
                ...
                ) : currentStepIndex === ELE_WIZARD_STEPS.length - 1 ? (
                  <Save className="h-4 w-4" />
                ) : (
                  <ArrowRight className="h-4 w-4" />
                )}
                {isNew ? "Crea e continua" :
                  currentStepIndex === ELE_WIZARD_STEPS.length - 1 ? "Salva" : "Salva e continua"}
              </Button>
            </div>
          </main>
        </div>
