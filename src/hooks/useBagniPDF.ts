/**
 * useBagniPDF — download/preview del PDF cliente del preventivo
 * Bagni (Task 21).
 *
...
import { getBgnTemplatePdf } from "@/hooks/useBagniProgetto";
...
import type {
  BgnProgetto, BgnComputoVoce, BgnProgettoMedia, BgnTemplatePdf,
} from "@/types/bagni";

// ─── Tipi pre-fetch arricchimento ───────────────────────────────────────────

export interface BgnPdfCompany {
  name?: string | null;
...
export interface BgnPdfCapitolo {
  nome: string;
  voci: BgnComputoVoce[];
  subtotale: number;
  costo: number;
...
export interface BgnPdfEnriched {
  progetto: BgnProgetto;
  template: BgnTemplatePdf;
  company: BgnPdfCompany | null;
  capitoli: BgnPdfCapitolo[];
  totali: BgnPdfTotali;
  media: BgnProgettoMedia[];
  computoOptions: BgnPdfComputoOptions;
}
...
export interface BgnPdfComputoOptions {
...
export const DEFAULT_COMPUTO_OPTIONS: BgnPdfComputoOptions = {
  livello: "dettagliato",
  mostraPrezzi: true,
  mostraQta: true,
...
export interface BgnPdfPayload {
  progetto: BgnProgetto;
  ...
  pdfOptions?: BgnPdfComputoOptions;
}
...
// ─── Enrich ──────────────────────────────────────────────────────────────────
export async function enrichBagniPdf(opts: BgnPdfPayload): Promise<BgnPdfEnriched> {
  ...
  const template = await templateDelPreventivo("bagni", progetto, opts.template, getBgnTemplatePdf);

  // 2) Company (anagrafica per intestazione/contatti). Best-effort.
  let company: BgnPdfCompany | null = opts.company ?? null;
  ...
  const capMap = new Map<string, BgnPdfCapitolo>();
  const order: string[] = [];
  ...
  const [inlinedLogo, inlinedChiSiamo, immaginiModello] = await Promise.all([
    ...
    immaginiDelModello("bagni", template as unknown as Record<string, unknown>, company?.logo_chiaro_url ?? null, companyId),
  ]);
  ...
  const inlinedCompany: BgnPdfCompany | null = company
  ...
  const inlinedMedia: BgnProgettoMedia[] = imageMedia
 ...
 */
export async function renderBgnPreviewBlobUrl(opts: BgnPdfPayload): Promise<string> {
  const enriched = await enrichBagniPdf(opts);
  const [{ pdf }, { BagniPDF }, React] = await Promise.all([
    import("@react-pdf/renderer"),
    import("@/components/bagni/BagniPDF"),
    import("react"),
  ]);
  const element = React.createElement(BagniPDF, enriched);
  const blob = await pdf(element as unknown as ReactElement<DocumentProps>).toBlob();
  return URL.createObjectURL(blob);
}

// ─── Hook ──────────────────────────────────────────────────────────────────
function buildFilename(progetto: BgnProgetto): string {
  const cliente = [progetto.cliente_nome, progetto.cliente_cognome].filter(Boolean).join("_") || "cliente";
  return `Preventivo-${progetto.code ?? "bagni"}-${cliente}.pdf`;
}

export function useBagniPDF() {
  const [isGenerating, setIsGenerating] = useState(false);

  const downloadPDF = async (opts: BgnPdfPayload): Promise<{ ok: boolean }> => {
    ...
    try {
      ...
      console.error("Errore generazione PDF bagni:", err);
  ...
  const previewPDF = async (opts: BgnPdfPayload): Promise<void> => {
    setIsGenerating(true);
    try {
      if (opts.computo.length === 0) {
        toast.error("Computo vuoto", {
      ...
      console.error("Errore preview PDF bagni:", err);
      const msg = err instanceof Error ? err.message : "Errore sconosciuto";
      toast.error("Errore nella generazione del PDF", { description: msg });
    } finally {
      setIsGenerating(false);
