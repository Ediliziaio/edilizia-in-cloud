 ...
 * Pavimenti (tabelle `pav_listino_capitoli` / `pav_listino_voci`).
 *
 * NB: le tabelle `pav_*` provengono dalla migrazione LOCALE
...
import { buildSeedRows } from "@/lib/pavimenti/seedListino";
import { calcPrezzoVoce } from "@/lib/pavimenti/calcoli";
import type {
  PavListinoCapitolo,
  PavListinoVoce,
  PavUnitaMisura,
...
// ─── Query keys ────────────────────────────────────────────────────────────
const K = {
  capitoli: (companyId: string | null) => ["pav-listino-capitoli", companyId] as const,
  voci: (companyId: string | null, capitoloId?: string | null) =>
    ["pav-listino-voci", companyId, capitoloId ?? "all"] as const,
  articoliSearch: (companyId: string | null, term: string) =>
    ["pav-prefill-articoli", companyId, term] as const,
  tariffeSearch: (companyId: string | null, term: string) =>
    ["pav-prefill-tariffe", companyId, term] as const,
  vociSearch: (companyId: string | null, term: string) =>
    ["pav-listino-voci-search", companyId, term] as const,
};
...
export function useListinoCapitoli() {
  ...
  return useQuery<PavListinoCapitolo[]>({
    queryKey: K.capitoli(companyId),
    enabled: !!companyId,
    staleTime: 60_000,
    queryFn: async () => {
      const { data, error } = await sb()
        .from("pav_listino_capitoli")
        .select("*")
        .eq("company_id", companyId!)
        .order("ordine", { ascending: true })
        .order("created_at", { ascending: true });
      if (error) throw new Error(error.message);
      return (data ?? []) as PavListinoCapitolo[];
    },
  });
}
...
export function useListinoVoci(capitoloId?: string | null) {
  const companyId = useEffectiveCompanyId();
  return useQuery<PavListinoVoce[]>({
    queryKey: K.voci(companyId, capitoloId),
    enabled: !!companyId,
    staleTime: 60_000,
    queryFn: async () => {
      let q = sb()
        .from("pav_listino_voci")
        .select("*")
        .eq("company_id", companyId!);
...
export function useUpsertCapitolo() {
  ...
  return useMutation({
    mutationFn: async (payload: CapitoloPayload): Promise<PavListinoCapitolo> => {
      ...
      const q = payload.id
        ? sb()
            ...
            .select()
            .single()
        : sb().from("pav_listino_capitoli").insert(row).select().single();
      const { data, error } = await q;
      if (error) throw new Error(error.message);
      return data as PavListinoCapitolo;
    },
...
export function useUpsertVoce() {
  ...
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async (payload: VocePayload): Promise<PavListinoVoce> => {
      if (!companyId) throw new Error("Company non disponibile");
      ...
      const q = payload.id
        ? sb()
            ...
            .select()
            .single()
        : sb().from("pav_listino_voci").insert(row).select().single();
      const { data, error } = await q;
      if (error) throw new Error(error.message);
      return data as PavListinoVoce;
    },
    onSuccess: () => {
      void qc.invalidateQueries({ queryKey: ["pav-listino-voci", companyId] });
    },
  });
}
...
export function useImportSeedListino() {
  ...
  return useMutation({
    mutationFn: async (): Promise<{ capitoli: number; voci: number }> => {
      ...
      const { error: vErr } = await sb().from("pav_listino_voci").insert(voci);
      if (vErr) throw new Error(vErr.message);
...
/**
 * Adozione di voci da un prezzario regionale ufficiale nel listino PAVIMENTI
 * (`pav_listino_voci`). Gemello pav-scoped di `useAdottaPrezzario` di
 * `@/lib/prezzario/queries` (che scrive sul listino Ristrutturazione): qui le
 * voci finiscono nel listino Pavimenti e si invalidano le query key `pav-listino-*`.
 */
export function useAdottaPrezzario() {
  ...
  return useMutation({
    mutationFn: async (input: AdottaPrezzarioInput): Promise<{ inserite: number; senzaUnita: number }> => {
      ...
      const { error } = await sb().from("pav_listino_voci").insert(rows);
      if (error) throw new Error(error.message);
    ...
    onSuccess: () => {
      void qc.invalidateQueries({ queryKey: ["pav-listino-voci", companyId] });
      void qc.invalidateQueries({ queryKey: ["pav-listino-capitoli", companyId] });
    },
  });
}
...
const UM_MAP: Record<string, PavUnitaMisura> = {
  pz: "cad",
  cad: "cad",
  mq: "mq",
  ml: "ml",
...
/** Normalizza una UM sorgente (articoli/tariffe) verso `PavUnitaMisura`. */
export function mapUnitaMisura(raw: string | null | undefined): PavUnitaMisura {
  if (!raw) return "cad";
...
/**
 * Ricerca typeahead nel listino lavorazioni aziendale (`pav_listino_voci`) per
 * il picker del computo. Joina `pav_listino_capitoli` per esporre il nome del
  ...
  return useQuery<ListinoVoceSearchOption[]>({
    ...
    queryFn: async () => {
      let q = sb()
        .from("pav_listino_voci")
        .select(
          "id, descrizione, codice, unita_misura, prezzo_unitario, costo_materiali, costo_manodopera, fonte, capitolo:pav_listino_capitoli(nome)",
        )
        .eq("company_id", companyId!);
      const t = termineDiRicerca(term);
      ...
      return ((data ?? []) as Record<string, unknown>[]).map((d) => {
        ...
        return {
          ...
          unita_misura: ((d.unita_misura as string) ?? "cad") as PavUnitaMisura,
