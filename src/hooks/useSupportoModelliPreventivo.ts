...
import { MODULI_CON_MODELLI, TABELLA_PREVENTIVI, type ModuloConModelloPreventivo } from "@/lib/moduli/modelloPreventivo";
...
/** I preventivatori pronti a salvare il modello dell'intervento (di serie i sette edili). */
export function useSupportoModelliPreventivo(moduli: readonly ModuloConModelloPreventivo[] = MODULI_CON_MODELLI) {
  const companyId = useEffectiveCompanyId();
  const query = useQuery({
    queryKey: ["supporto-modelli-preventivo", companyId, moduli.join(",")],
    enabled: !!companyId, staleTime: 60_000, retry: false,
    queryFn: async (): Promise<Partial<Record<ModuloConModelloPreventivo, boolean>>> => Object.fromEntries(
      await Promise.all(moduli.map(async (modulo) => [modulo, await haColonnaModello(TABELLA_PREVENTIVI[modulo], companyId!)] as const)),
    ),
  });
  return {
    supportato: (modulo: ModuloConModelloPreventivo) => query.data?.[modulo] === true,
...
export function useSupportoModelloPreventivo(modulo: ModuloConModelloPreventivo) {
