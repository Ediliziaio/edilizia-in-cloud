 ...
 * Bagni (tabelle `bgn_listino_capitoli` / `bgn_listino_voci`).
 *
 * NB: le tabelle `bgn_*` provengono dalla migrazione LOCALE
...
import { buildSeedRows } from "@/lib/bagni/seedListino";
import { calcPrezzoVoce } from "@/lib/bagni/calcoli";
import type {
  BgnListinoCapitolo,
  BgnListinoVoce,
  BgnUnitaMisura,
...
// ─── Query keys ────────────────────────────────────────────────────────────
const K = {
  capitoli: (companyId: string | null) => ["bgn-listino-capitoli", companyId] as const,
  voci: (companyId: string | null, capitoloId?: string | null) =>
    ["bgn-listino-voci", companyId, capitoloId ?? "all"] as const,
  articoliSearch: (companyId: string | null, term: string) =>
    ["bgn-prefill-articoli", companyId, term] as const,
  tariffeSearch: (companyId: string | null, term: string) =>
    ["bgn-prefill-tariffe", companyId, term] as const,
  vociSearch: (companyId: string | null, term: string) =>
    ["bgn-listino-voci-search", companyId, term] as const,
};
...
export function useListinoCapitoli() {
  ...
  return useQuery<BgnListinoCapitolo[]>({
    queryKey: K.capitoli(companyId),
    enabled: !!companyId,
    staleTime: 60_000,
    queryFn: async () => {
      const { data, error } = await sb()
        .from("bgn_listino_capitoli")
        .select("*")
        .eq("company_id", companyId!)
        .order("ordine", { ascending: true })
        .order("created_at", { ascending: true });
      if (error) throw new Error(error.message);
      return (data ?? []) as BgnListinoCapitolo[];
    },
  });
}
...
export function useListinoVoci(capitoloId?: string | null) {
  const companyId = useEffectiveCompanyId();
  return useQuery<BgnListinoVoce[]>({
    queryKey: K.voci(companyId, capitoloId),
    enabled: !!companyId,
    staleTime: 60_000,
    queryFn: async () => {
      let q = sb()
        .from("bgn_listino_voci")
        .select("*")
        .eq("company_id", companyId!);
...
export function useUpsertCapitolo() {
  ...
  return useMutation({
    mutationFn: async (payload: CapitoloPayload): Promise<BgnListinoCapitolo> => {
      ...
      const q = payload.id
        ? sb()
            ...
            .select()
            .single()
        : sb().from("bgn_listino_capitoli").insert(row).select().single();
      const { data, error } = await q;
      if (error) throw new Error(error.message);
      return data as BgnListinoCapitolo;
    },
...
export function useUpsertVoce() {
  ...
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async (payload: VocePayload): Promise<BgnListinoVoce> => {
      if (!companyId) throw new Error("Company non disponibile");
      ...
      const q = payload.id
        ? sb()
            ...
            .select()
            .single()
        : sb().from("bgn_listino_voci").insert(row).select().single();
      const { data, error } = await q;
      if (error) throw new Error(error.message);
      return data as BgnListinoVoce;
    },
...
export function useImportSeedListino() {
  ...
  return useMutation({
    mutationFn: async (): Promise<{ capitoli: number; voci: number }> => {
      ...
      const { error: vErr } = await sb().from("bgn_listino_voci").insert(voci);
      if (vErr) throw new Error(vErr.message);
...
const UM_MAP: Record<string, BgnUnitaMisura> = {
  pz: "cad",
  cad: "cad",
  mq: "mq",
  ml: "ml",
...
/** Normalizza una UM sorgente (articoli/tariffe) verso `BgnUnitaMisura`. */
export function mapUnitaMisura(raw: string | null | undefined): BgnUnitaMisura {
  if (!raw) return "cad";
...
/**
 * Ricerca typeahead nel listino lavorazioni aziendale (`bgn_listino_voci`) per
 * il picker del computo. Joina `bgn_listino_capitoli` per esporre il nome del
  ...
  return useQuery<ListinoVoceSearchOption[]>({
    ...
    queryFn: async () => {
      let q = sb()
        .from("bgn_listino_voci")
        .select(
          "id, descrizione, codice, unita_misura, prezzo_unitario, costo_materiali, costo_manodopera, fonte, capitolo:bgn_listino_capitoli(nome)",
        )
        .eq("company_id", companyId!);
      const t = termineDiRicerca(term);
      ...
      return ((data ?? []) as Record<string, unknown>[]).map((d) => {
        ...
        return {
          ...
          unita_misura: ((d.unita_misura as string) ?? "cad") as BgnUnitaMisura,
