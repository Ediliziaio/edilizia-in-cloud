 ...
 * Climatizzazione (tabelle `clm_listino_capitoli` / `clm_listino_voci`).
 *
 * NB: le tabelle `clm_*` provengono dalla migrazione LOCALE
...
import { buildSeedRows } from "@/lib/climatizzazione/seedListino";
import { calcPrezzoVoce } from "@/lib/climatizzazione/calcoli";
import type {
  ClmListinoCapitolo,
  ClmListinoVoce,
  ClmUnitaMisura,
...
// ─── Query keys ────────────────────────────────────────────────────────────
const K = {
  capitoli: (companyId: string | null) => ["clm-listino-capitoli", companyId] as const,
  voci: (companyId: string | null, capitoloId?: string | null) =>
    ["clm-listino-voci", companyId, capitoloId ?? "all"] as const,
  articoliSearch: (companyId: string | null, term: string) =>
    ["clm-prefill-articoli", companyId, term] as const,
  tariffeSearch: (companyId: string | null, term: string) =>
    ["clm-prefill-tariffe", companyId, term] as const,
  vociSearch: (companyId: string | null, term: string) =>
    ["clm-listino-voci-search", companyId, term] as const,
};
...
export function useListinoCapitoli() {
  ...
  return useQuery<ClmListinoCapitolo[]>({
    queryKey: K.capitoli(companyId),
    enabled: !!companyId,
    staleTime: 60_000,
    queryFn: async () => {
      const { data, error } = await sb()
        .from("clm_listino_capitoli")
        .select("*")
        .eq("company_id", companyId!)
        .order("ordine", { ascending: true })
        .order("created_at", { ascending: true });
      if (error) throw new Error(error.message);
      return (data ?? []) as ClmListinoCapitolo[];
    },
  });
}
...
export function useListinoVoci(capitoloId?: string | null) {
  const companyId = useEffectiveCompanyId();
  return useQuery<ClmListinoVoce[]>({
    queryKey: K.voci(companyId, capitoloId),
    enabled: !!companyId,
    staleTime: 60_000,
    queryFn: async () => {
      let q = sb()
        .from("clm_listino_voci")
        .select("*")
        .eq("company_id", companyId!);
...
export function useUpsertCapitolo() {
  ...
  return useMutation({
    mutationFn: async (payload: CapitoloPayload): Promise<ClmListinoCapitolo> => {
      ...
      const q = payload.id
        ? sb()
            ...
            .select()
            .single()
        : sb().from("clm_listino_capitoli").insert(row).select().single();
      const { data, error } = await q;
      if (error) throw new Error(error.message);
      return data as ClmListinoCapitolo;
    },
...
export function useUpsertVoce() {
  ...
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async (payload: VocePayload): Promise<ClmListinoVoce> => {
      if (!companyId) throw new Error("Company non disponibile");
      ...
      const q = payload.id
        ? sb()
            ...
            .select()
            .single()
        : sb().from("clm_listino_voci").insert(row).select().single();
      const { data, error } = await q;
      if (error) throw new Error(error.message);
      return data as ClmListinoVoce;
    },
    onSuccess: () => {
      void qc.invalidateQueries({ queryKey: ["clm-listino-voci", companyId] });
    },
  });
}
...
export function useImportSeedListino() {
  ...
  return useMutation({
    mutationFn: async (): Promise<{ capitoli: number; voci: number }> => {
      ...
      const { error: vErr } = await sb().from("clm_listino_voci").insert(voci);
      if (vErr) throw new Error(vErr.message);
...
/**
 * Adozione di voci da un prezzario regionale ufficiale nel listino CLIMATIZZAZIONE
 * (`clm_listino_voci`). Gemello clm-scoped di `useAdottaPrezzario` di
 * `@/lib/prezzario/queries` (che scrive sul listino Ristrutturazione): qui le
 * voci finiscono nel listino Climatizzazione e si invalidano le query key `clm-listino-*`.
 */
export function useAdottaPrezzario() {
  ...
  return useMutation({
    mutationFn: async (input: AdottaPrezzarioInput): Promise<{ inserite: number }> => {
      ...
      const { error } = await sb().from("clm_listino_voci").insert(rows);
      if (error) throw new Error(error.message);
      return { inserite: rows.length };
    },
    onSuccess: () => {
      void qc.invalidateQueries({ queryKey: ["clm-listino-voci", companyId] });
      void qc.invalidateQueries({ queryKey: ["clm-listino-capitoli", companyId] });
    },
  });
}
...
const UM_MAP: Record<string, ClmUnitaMisura> = {
  pz: "cad",
  cad: "cad",
  mq: "mq",
  ml: "ml",
...
/** Normalizza una UM sorgente (articoli/tariffe) verso `ClmUnitaMisura`. */
export function mapUnitaMisura(raw: string | null | undefined): ClmUnitaMisura {
  if (!raw) return "cad";
...
/**
 * Ricerca typeahead nel listino lavorazioni aziendale (`clm_listino_voci`) per
 * il picker del computo. Joina `clm_listino_capitoli` per esporre il nome del
  ...
  return useQuery<ListinoVoceSearchOption[]>({
    ...
    queryFn: async () => {
      let q = sb()
        .from("clm_listino_voci")
        .select(
          "id, descrizione, codice, unita_misura, prezzo_unitario, costo_materiali, costo_manodopera, fonte, capitolo:clm_listino_capitoli(nome)",
        )
        .eq("company_id", companyId!);
      const t = termineDiRicerca(term);
      ...
      return ((data ?? []) as Record<string, unknown>[]).map((d) => {
        ...
        return {
          ...
          unita_misura: ((d.unita_misura as string) ?? "cad") as ClmUnitaMisura,
