/**
 * usePavimentiProgetto — CRUD del progetto (hub) del verticale
 * Pavimenti + computo metrico + media.
 *
 * Tabelle: `pav_progetti`, `pav_computo_voci`, `pav_progetti_media`.
 *
 * NB: come per `useListinoLavorazioni.ts`, le tabelle `pav_*` provengono dalla
 * migrazione LOCALE `20271101020000_pav_modulo.sql` e NON sono ancora
...
import { calcRigaImporto, calcTotaliComputo, type ComputoRigaInput } from "@/lib/pavimenti/calcoli";
import {
...
// Tipi pav_* non rigenerati: cast unico, riusato in tutto il file.
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const sb = () => supabase as any;

// ─── Query keys ────────────────────────────────────────────────────────────
const K = {
  progetti: (companyId: string | null) => ["pav-progetti", companyId] as const,
  progetto: (id: string | undefined) => ["pav-progetto", id ?? "none"] as const,
  template: (companyId: string | null) => ["pav-template", companyId] as const,
};

/** Dettaglio progetto: hub + computo voci (ordinate) + media. */
export interface PavProgettoDetail {
...
// ─── Lista progetti ──────────────────────────────────────────────────────────
export function usePavimentiProgetti() {
  const companyId = useEffectiveCompanyId();
  return useQuery<PavProgetto[]>({
    ...
    queryFn: async () => {
      ...
      return (data ?? []) as PavProgetto[];
    },
  });
}

// ─── Singolo progetto (+ computo + media) ─────────────────────────────────────
export function usePavimentiProgetto(id: string | undefined) {
  return useQuery<PavProgettoDetail | null>({
    ...
    queryFn: async () => {
      const [
        ...
        sb().from("pav_progetti").select("*").eq("id", id!).maybeSingle(),
        sb()
          .from("pav_computo_voci")
          .select("*")
          .eq("progetto_id", id!)
          .order("ordine", { ascending: true })
          .order("created_at", { ascending: true }),
        sb()
          .from("pav_progetti_media")
          .select("*")
          .eq("progetto_id", id!)
          .order("ordine", { ascending: true }),
      ]);
      ...
      return {
        progetto: progetto as PavProgetto,
        computo: (computo ?? []) as PavComputoVoce[],
        media: (media ?? []) as PavProgettoMedia[],
      };
    },
  });
}
...
// ─── Upsert progetto ──────────────────────────────────────────────────────────
export type PavProgettoPatch = Partial<Omit<PavProgetto, "id" | "company_id">>;

/**
 * Genera un codice progressivo leggibile per il progetto (es. PAV-2026-007).
 * La tabella `pav_progetti.code` non ha trigger DB: lo deriviamo client-side
...
async function generateProgettoCode(companyId: string): Promise<string> {
  ...
  try {
    ...
    const n = (count ?? 0) + 1;
    return `PAV-${year}-${String(n).padStart(3, "0")}`;
  } catch {
    return `PAV-${year}-${Date.now().toString().slice(-5)}`;
  }
}
...
export function useUpsertProgetto() {
  ...
  return useMutation({
    mutationFn: async (
      input: PavProgettoPatch & { id?: string },
    ): Promise<PavProgetto> => {
      ...
      leggiModelloPreventivo("pavimenti", patch.modello_snapshot, companyId);
...
export function useDeleteProgetto() {
  ...
  return useMutation({
    mutationFn: async (id: string): Promise<void> => {
      if (!companyId) throw new Error("Company non disponibile");
      // pav_computo_voci / pav_progetti_media hanno FK ON DELETE CASCADE.
...
export function useClonaProgetto() {
  ...
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async (sourceId: string): Promise<PavProgetto> => {
      if (!companyId) throw new Error("Company non disponibile");
      ...
      const [{ data: src, error: sErr }, { data: voci, error: vErr }] = await Promise.all([
        sb().from("pav_progetti").select("*").eq("id", sourceId).eq("company_id", companyId).maybeSingle(),
        sb().from("pav_computo_voci").select("*").eq("progetto_id", sourceId).eq("company_id", companyId)
          .order("ordine", { ascending: true }),
      ]);
      ...
      const { data: nuovo, error: cErr } = await sb()
        ...
        .insert({
          ...
          ...(src.modello_snapshot ? { modello_snapshot: leggiModelloPreventivo("pavimenti", src.modello_snapshot, companyId) } : {}),
      ...
      if (voci && voci.length > 0) {
        ...
        const { error: iErr } = await sb().from("pav_computo_voci").insert(rows);
        if (iErr) throw new Error(iErr.message);
      }

      return nuovo as PavProgetto;
    },
    onSuccess: () => {
      void qc.invalidateQueries({ queryKey: K.progetti(companyId) });
    },
...
export function useUpsertMedia(progettoId: string | undefined) {
  ...
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async (payload: MediaPayload): Promise<PavProgettoMedia> => {
      if (!companyId) throw new Error("Company non disponibile");
      if (!progettoId) throw new Error("Progetto id mancante");
      const row = {
        progetto_id: progettoId,
      ...
      const q = payload.id
        ? sb()
            ...
            .select()
            .single()
        : sb().from("pav_progetti_media").insert(row).select().single();
      const { data, error } = await q;
      if (error) throw new Error(error.message);
      return data as PavProgettoMedia;
    },
    onSuccess: () => {
      void qc.invalidateQueries({ queryKey: K.progetto(progettoId) });
    },
...
// Default coerenti con la migrazione `pav_template_pdf` (color_* + show_*).
// Le liste jsonb si normalizzano sempre ad array per non rompere `.map` in UI/PDF.
const PAV_TEMPLATE_DEFAULTS = {
...
/**
 * True se l'errore Supabase indica che la tabella `pav_template_pdf` (o lo schema
 * pav_*) NON esiste ancora sul DB → il modulo Pavimenti non è stato ancora
 * pubblicato (migrazione `20271101020000_pav_modulo.sql` non applicata sul
 ...
 */
export function isPavModuleNotPublished(
  ...
  return (
    code === "PGRST205" ||
    code === "42P01" ||
    /could not find the table|schema cache|does not exist|pav_template_pdf|pav_progetti/i.test(msg)
  );
}

const asArray = <T,>(v: unknown): T[] => (Array.isArray(v) ? (v as T[]) : []);

/** Normalizza una riga grezza del DB nel tipo `PavTemplatePdf` (liste sempre array). */
export function normalizeTemplate(row: Record<string, unknown> | null, companyId: string): PavTemplatePdf {
  const r = row ?? {};
  return {
    id: (r.id as string) ?? "",
    company_id: (r.company_id as string) ?? companyId,
    logo_url: (r.logo_url as string | null) ?? null,
    color_primary: (r.color_primary as string | null) ?? PAV_TEMPLATE_DEFAULTS.color_primary,
    color_secondary: (r.color_secondary as string | null) ?? PAV_TEMPLATE_DEFAULTS.color_secondary,
    color_accent: (r.color_accent as string | null) ?? PAV_TEMPLATE_DEFAULTS.color_accent,
    color_text: (r.color_text as string | null) ?? PAV_TEMPLATE_DEFAULTS.color_text,
    chi_siamo: (r.chi_siamo as string | null) ?? null,
    chi_siamo_foto_url: (r.chi_siamo_foto_url as string | null) ?? null,
    esigenze: asArray<PavListItem>(r.esigenze),
    soluzione: asArray<PavListItem>(r.soluzione),
    usp: asArray<PavListItem>(r.usp),
    testimonianze: asArray<PavTestimonianza>(r.testimonianze),
    cronoprogramma: asArray<PavCronoFase>(r.cronoprogramma),
    ...
    show_chi_siamo: (r.show_chi_siamo as boolean | null) ?? PAV_TEMPLATE_DEFAULTS.show_chi_siamo,
    show_cronoprogramma: (r.show_cronoprogramma as boolean | null) ?? PAV_TEMPLATE_DEFAULTS.show_cronoprogramma,
    show_margine: (r.show_margine as boolean | null) ?? PAV_TEMPLATE_DEFAULTS.show_margine,
    ...
    cover_logo_position: (r.cover_logo_position as PavTemplatePdf["cover_logo_position"]) ?? "top_left",
    ...
    garanzie: asArray<PavListItem>(r.garanzie),
    faq: asArray<PavFaqItem>(r.faq),
    percorso: asArray<PavListItem>(r.percorso),
    ...
    cover_text_align: (r.cover_text_align as PavTemplatePdf["cover_text_align"]) ?? "left",
 ...
 */
export async function getPavTemplatePdf(companyId: string): Promise<PavTemplatePdf> {
  const { data, error } = await sb()
    .from("pav_template_pdf")
    .select("*")
    .eq("company_id", companyId)
  ...
  if (error) {
    ...
    if (isPavModuleNotPublished(error)) return normalizeTemplate(null, companyId);
    throw new Error(error.message);
  }
  return normalizeTemplate(data as Record<string, unknown> | null, companyId);
}

export function usePavTemplatePdf() {
  const companyId = useEffectiveCompanyId();
  return useQuery<PavTemplatePdf>({
    queryKey: K.template(companyId),
    enabled: !!companyId,
    staleTime: 5 * 60 * 1000,
    queryFn: () => getPavTemplatePdf(companyId!),
  });
}

/** Template di default (in-memory) per quando il modulo non è ancora pubblicato. */
export function getDefaultPavTemplatePdf(companyId: string | null): PavTemplatePdf {
  return normalizeTemplate(null, companyId ?? "");
}

/**
 * Probe leggera (HEAD) per sapere se il modulo Pavimenti è pubblicato sul
 * DB (tabella `pav_template_pdf` esistente). `false` ⇒ l'editor mostra i default
 ...
 */
export function usePavBackendReady() {
  const companyId = useEffectiveCompanyId();
  return useQuery<boolean>({
    queryKey: ["pav-backend-ready", companyId] as const,
    enabled: !!companyId,
    staleTime: 5 * 60 * 1000,
    retry: false,
    queryFn: async () => {
      // GET (non HEAD): su 404 il body PostgREST espone code/message, così
      // `isPavModuleNotPublished` riconosce la tabella mancante. Una HEAD non ha
      ...
      const { error } = await sb()
        ...
        .limit(1);
      if (error) {
        if (isPavModuleNotPublished(error)) return false;
        throw new Error(error.message);
      }
      return true;
    },
...
export type PavTemplatePatch = Partial<Omit<PavTemplatePdf, "id" | "company_id">>;

export function useUpsertPavTemplatePdf() {
  const companyId = useEffectiveCompanyId();
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async (patch: PavTemplatePatch): Promise<PavTemplatePdf> => {
      if (!companyId) throw new Error("Company non disponibile");
      ...
      const { data, error } = await sb()
        ...
        .single();
      if (error) {
        if (isPavModuleNotPublished(error)) {
          throw new Error(
            "Il modulo Pavimenti non è ancora pubblicato sul database: applica la migrazione (pubblica il modulo) per salvare il template.",
          );
        }
        throw new Error(error.message);
      }
