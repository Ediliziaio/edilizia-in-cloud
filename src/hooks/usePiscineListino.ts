 ...
 * Piscine (tabelle `pis_listino_capitoli` / `pis_listino_voci`).
 *
 * NB: le tabelle `pis_*` provengono dalla migrazione LOCALE
...
import { buildSeedRows } from "@/lib/piscine/seedListino";
import { calcPrezzoVoce } from "@/lib/piscine/calcoli";
import type {
  PisListinoCapitolo,
  PisListinoVoce,
  PisUnitaMisura,
...
// ─── Query keys ────────────────────────────────────────────────────────────
const K = {
  capitoli: (companyId: string | null) => ["pis-listino-capitoli", companyId] as const,
  voci: (companyId: string | null, capitoloId?: string | null) =>
    ["pis-listino-voci", companyId, capitoloId ?? "all"] as const,
  articoliSearch: (companyId: string | null, term: string) =>
    ["pis-prefill-articoli", companyId, term] as const,
  tariffeSearch: (companyId: string | null, term: string) =>
    ["pis-prefill-tariffe", companyId, term] as const,
  vociSearch: (companyId: string | null, term: string) =>
    ["pis-listino-voci-search", companyId, term] as const,
};
...
export function useListinoCapitoli() {
  ...
  return useQuery<PisListinoCapitolo[]>({
    queryKey: K.capitoli(companyId),
    enabled: !!companyId,
    staleTime: 60_000,
    queryFn: async () => {
      const { data, error } = await sb()
        .from("pis_listino_capitoli")
        .select("*")
        .eq("company_id", companyId!)
        .order("ordine", { ascending: true })
        .order("created_at", { ascending: true });
      if (error) throw new Error(error.message);
      return (data ?? []) as PisListinoCapitolo[];
    },
  });
}
...
export function useListinoVoci(capitoloId?: string | null) {
  const companyId = useEffectiveCompanyId();
  return useQuery<PisListinoVoce[]>({
    queryKey: K.voci(companyId, capitoloId),
    enabled: !!companyId,
    staleTime: 60_000,
    queryFn: async () => {
      let q = sb()
        .from("pis_listino_voci")
        .select("*")
        .eq("company_id", companyId!);
...
export function useUpsertCapitolo() {
  ...
  return useMutation({
    mutationFn: async (payload: CapitoloPayload): Promise<PisListinoCapitolo> => {
      ...
      const q = payload.id
        ? sb()
            ...
            .select()
            .single()
        : sb().from("pis_listino_capitoli").insert(row).select().single();
      const { data, error } = await q;
      if (error) throw new Error(error.message);
      return data as PisListinoCapitolo;
    },
...
export function useUpsertVoce() {
  ...
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async (payload: VocePayload): Promise<PisListinoVoce> => {
      if (!companyId) throw new Error("Company non disponibile");
      ...
      const q = payload.id
        ? sb()
            ...
            .select()
            .single()
        : sb().from("pis_listino_voci").insert(row).select().single();
      const { data, error } = await q;
      if (error) throw new Error(error.message);
      return data as PisListinoVoce;
    },
    onSuccess: () => {
      void qc.invalidateQueries({ queryKey: ["pis-listino-voci", companyId] });
    },
  });
}
...
export function useImportSeedListino() {
  ...
  return useMutation({
    mutationFn: async (): Promise<{ capitoli: number; voci: number }> => {
      ...
      const { error: vErr } = await sb().from("pis_listino_voci").insert(voci);
      if (vErr) throw new Error(vErr.message);
...
/**
 * Adozione di voci da un prezzario regionale ufficiale nel listino PISCINE
 * (`pis_listino_voci`). Gemello pis-scoped di `useAdottaPrezzario` di
 * `@/lib/prezzario/queries` (che scrive sul listino Ristrutturazione): qui le
 * voci finiscono nel listino Piscine e si invalidano le query key `pis-listino-*`.
 */
export function useAdottaPrezzario() {
  ...
  return useMutation({
    mutationFn: async (input: AdottaPrezzarioInput): Promise<{ inserite: number }> => {
      ...
      const { error } = await sb().from("pis_listino_voci").insert(rows);
      if (error) throw new Error(error.message);
    ...
    onSuccess: () => {
      void qc.invalidateQueries({ queryKey: ["pis-listino-voci", companyId] });
      void qc.invalidateQueries({ queryKey: ["pis-listino-capitoli", companyId] });
    },
  });
}
...
const UM_MAP: Record<string, PisUnitaMisura> = {
  pz: "cad",
  cad: "cad",
  mq: "mq",
  ml: "ml",
...
/** Normalizza una UM sorgente (articoli/tariffe) verso `PisUnitaMisura`. */
export function mapUnitaMisura(raw: string | null | undefined): PisUnitaMisura {
  if (!raw) return "cad";
...
/**
 * Ricerca typeahead nel listino lavorazioni aziendale (`pis_listino_voci`) per
 * il picker del computo. Joina `pis_listino_capitoli` per esporre il nome del
  ...
  return useQuery<ListinoVoceSearchOption[]>({
    ...
    queryFn: async () => {
      let q = sb()
        .from("pis_listino_voci")
        .select(
          "id, descrizione, codice, unita_misura, prezzo_unitario, costo_materiali, costo_manodopera, fonte, capitolo:pis_listino_capitoli(nome)",
        )
        .eq("company_id", companyId!);
      const t = termineDiRicerca(term);
      ...
      return ((data ?? []) as Record<string, unknown>[]).map((d) => {
        ...
        return {
          ...
          unita_misura: ((d.unita_misura as string) ?? "cad") as PisUnitaMisura,
