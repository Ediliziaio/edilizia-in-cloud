/**
 * useTermoidraulicoPDF — download/preview del PDF cliente del preventivo
 * Termoidraulico (Task 21).
 *
...
export interface IdrPdfCompany {
  name?: string | null;
...
export interface IdrPdfCapitolo {
  nome: string;
  voci: IdrComputoVoce[];
  subtotale: number;
  costo: number;
...
export interface IdrPdfEnriched {
  progetto: IdrProgetto;
  template: IdrTemplatePdf;
  company: IdrPdfCompany | null;
  capitoli: IdrPdfCapitolo[];
  totali: IdrPdfTotali;
  media: IdrProgettoMedia[];
  computoOptions: IdrPdfComputoOptions;
}
...
export interface IdrPdfComputoOptions {
...
export const DEFAULT_COMPUTO_OPTIONS: IdrPdfComputoOptions = {
  livello: "dettagliato",
  mostraPrezzi: true,
  mostraQta: true,
...
// ─── Enrich ──────────────────────────────────────────────────────────────────
export async function enrichTermoidraulicoPdf(opts: IdrPdfPayload): Promise<IdrPdfEnriched> {
  ...
  const capMap = new Map<string, IdrPdfCapitolo>();
  const order: string[] = [];
  ...
  const [inlinedLogo, inlinedChiSiamo, immaginiModello] = await Promise.all([
    ...
    immaginiDelModello("termoidraulico", template as unknown as Record<string, unknown>, company?.logo_chiaro_url ?? null, companyId),
  ]);
  ...
  const inlinedCompany: IdrPdfCompany | null = company
...
// ─── Hook ──────────────────────────────────────────────────────────────────
function buildFilename(progetto: IdrProgetto): string {
  const cliente = [progetto.cliente_nome, progetto.cliente_cognome].filter(Boolean).join("_") || "cliente";
  return `Preventivo-${progetto.code ?? "termoidraulico"}-${cliente}.pdf`;
}

export function useTermoidraulicoPDF() {
  const [isGenerating, setIsGenerating] = useState(false);

  const downloadPDF = async (opts: IdrPdfPayload): Promise<{ ok: boolean }> => {
    ...
    try {
      ...
      console.error("Errore generazione PDF termoidraulico:", err);
  ...
  const previewPDF = async (opts: IdrPdfPayload): Promise<void> => {
    setIsGenerating(true);
    try {
      if (opts.computo.length === 0) {
        toast.error("Computo vuoto", {
      ...
      console.error("Errore preview PDF termoidraulico:", err);
      const msg = err instanceof Error ? err.message : "Errore sconosciuto";
      toast.error("Errore nella generazione del PDF", { description: msg });
    } finally {
      setIsGenerating(false);
