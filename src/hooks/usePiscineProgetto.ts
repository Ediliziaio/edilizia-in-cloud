/**
 * usePiscineProgetto — CRUD del progetto (hub) del verticale
 * Piscine + computo metrico + media.
 *
 * Tabelle: `pis_progetti`, `pis_computo_voci`, `pis_progetti_media`.
 *
 * NB: come per `useListinoLavorazioni.ts`, le tabelle `pis_*` provengono dalla
 * migrazione LOCALE `20271101020000_pis_modulo.sql` e NON sono ancora
...
import { calcRigaImporto, calcTotaliComputo, type ComputoRigaInput } from "@/lib/piscine/calcoli";
import {
...
// ─── Query keys ────────────────────────────────────────────────────────────
const K = {
  progetti: (companyId: string | null) => ["pis-progetti", companyId] as const,
  progetto: (id: string | undefined) => ["pis-progetto", id ?? "none"] as const,
  template: (companyId: string | null) => ["pis-template", companyId] as const,
};

/** Dettaglio progetto: hub + computo voci (ordinate) + media. */
export interface PisProgettoDetail {
...
// ─── Lista progetti ──────────────────────────────────────────────────────────
export function usePiscineProgetti() {
  const companyId = useEffectiveCompanyId();
  return useQuery<PisProgetto[]>({
    ...
    queryFn: async () => {
      ...
      return (data ?? []) as PisProgetto[];
    },
  });
}

// ─── Singolo progetto (+ computo + media) ─────────────────────────────────────
export function usePiscineProgetto(id: string | undefined) {
  return useQuery<PisProgettoDetail | null>({
    ...
    queryFn: async () => {
      const [
        ...
        sb().from("pis_progetti").select("*").eq("id", id!).maybeSingle(),
        sb()
      ...
      return {
        progetto: progetto as PisProgetto,
        computo: (computo ?? []) as PisComputoVoce[],
        media: (media ?? []) as PisProgettoMedia[],
      };
    },
  });
}
...
// ─── Upsert progetto ──────────────────────────────────────────────────────────
export type PisProgettoPatch = Partial<Omit<PisProgetto, "id" | "company_id">>;

/**
 * Genera un codice progressivo leggibile per il progetto (es. PIS-2026-007).
 * La tabella `pis_progetti.code` non ha trigger DB: lo deriviamo client-side
...
async function generateProgettoCode(companyId: string): Promise<string> {
  ...
  try {
    ...
    const n = (count ?? 0) + 1;
    return `PIS-${year}-${String(n).padStart(3, "0")}`;
  } catch {
    return `PIS-${year}-${Date.now().toString().slice(-5)}`;
  }
}
...
export function useUpsertProgetto() {
  ...
  return useMutation({
    mutationFn: async (
      input: PisProgettoPatch & { id?: string },
    ): Promise<PisProgetto> => {
      ...
      leggiModelloPreventivo("piscine", patch.modello_snapshot, companyId);
...
export function useDeleteProgetto() {
  ...
  return useMutation({
    mutationFn: async (id: string): Promise<void> => {
      if (!companyId) throw new Error("Company non disponibile");
      // pis_computo_voci / pis_progetti_media hanno FK ON DELETE CASCADE.
    ...
    mutationFn: async (sourceId: string): Promise<PisProgetto> => {
      ...
      const { data: nuovo, error: cErr } = await sb()
        ...
        .insert({
          ...
          ...(src.modello_snapshot ? { modello_snapshot: leggiModelloPreventivo("piscine", src.modello_snapshot, companyId) } : {}),
...
export function useUpsertMedia(progettoId: string | undefined) {
  ...
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async (payload: MediaPayload): Promise<PisProgettoMedia> => {
      if (!companyId) throw new Error("Company non disponibile");
      if (!progettoId) throw new Error("Progetto id mancante");
      const row = {
        progetto_id: progettoId,
      ...
      const q = payload.id
        ? sb()
            ...
            .select()
            .single()
        : sb().from("pis_progetti_media").insert(row).select().single();
      const { data, error } = await q;
      if (error) throw new Error(error.message);
      return data as PisProgettoMedia;
    },
    onSuccess: () => {
      void qc.invalidateQueries({ queryKey: K.progetto(progettoId) });
    },
...
// Default coerenti con la migrazione `pis_template_pdf` (color_* + show_*).
// Le liste jsonb si normalizzano sempre ad array per non rompere `.map` in UI/PDF.
const PIS_TEMPLATE_DEFAULTS = {
...
/**
 * True se l'errore Supabase indica che la tabella `pis_template_pdf` (o lo schema
 * pis_*) NON esiste ancora sul DB → il modulo Piscine non è stato ancora
 * pubblicato (migrazione `20271101020000_pis_modulo.sql` non applicata sul
 ...
 */
export function isPisModuleNotPublished(
  ...
  return (
    code === "PGRST205" ||
    code === "42P01" ||
    /could not find the table|schema cache|does not exist|pis_template_pdf|pis_progetti/i.test(msg)
  );
}

const asArray = <T,>(v: unknown): T[] => (Array.isArray(v) ? (v as T[]) : []);

/** Normalizza una riga grezza del DB nel tipo `PisTemplatePdf` (liste sempre array). */
export function normalizeTemplate(row: Record<string, unknown> | null, companyId: string): PisTemplatePdf {
  const r = row ?? {};
  return {
    id: (r.id as string) ?? "",
    company_id: (r.company_id as string) ?? companyId,
    logo_url: (r.logo_url as string | null) ?? null,
    color_primary: (r.color_primary as string | null) ?? PIS_TEMPLATE_DEFAULTS.color_primary,
    color_secondary: (r.color_secondary as string | null) ?? PIS_TEMPLATE_DEFAULTS.color_secondary,
    color_accent: (r.color_accent as string | null) ?? PIS_TEMPLATE_DEFAULTS.color_accent,
    color_text: (r.color_text as string | null) ?? PIS_TEMPLATE_DEFAULTS.color_text,
    chi_siamo: (r.chi_siamo as string | null) ?? null,
    chi_siamo_foto_url: (r.chi_siamo_foto_url as string | null) ?? null,
    esigenze: asArray<PisListItem>(r.esigenze),
    soluzione: asArray<PisListItem>(r.soluzione),
    usp: asArray<PisListItem>(r.usp),
    testimonianze: asArray<PisTestimonianza>(r.testimonianze),
    cronoprogramma: asArray<PisCronoFase>(r.cronoprogramma),
    ...
    show_chi_siamo: (r.show_chi_siamo as boolean | null) ?? PIS_TEMPLATE_DEFAULTS.show_chi_siamo,
    show_cronoprogramma: (r.show_cronoprogramma as boolean | null) ?? PIS_TEMPLATE_DEFAULTS.show_cronoprogramma,
    show_margine: (r.show_margine as boolean | null) ?? PIS_TEMPLATE_DEFAULTS.show_margine,
    ...
    cover_logo_position: (r.cover_logo_position as PisTemplatePdf["cover_logo_position"]) ?? "top_left",
    ...
    garanzie: asArray<PisListItem>(r.garanzie),
    faq: asArray<PisFaqItem>(r.faq),
    percorso: asArray<PisListItem>(r.percorso),
 ...
 */
export async function getPisTemplatePdf(companyId: string): Promise<PisTemplatePdf> {
  const { data, error } = await sb()
    .from("pis_template_pdf")
    .select("*")
    .eq("company_id", companyId)
  ...
  if (error) {
    ...
    if (isPisModuleNotPublished(error)) return normalizeTemplate(null, companyId);
    throw new Error(error.message);
  }
  return normalizeTemplate(data as Record<string, unknown> | null, companyId);
}

export function usePisTemplatePdf() {
  const companyId = useEffectiveCompanyId();
  return useQuery<PisTemplatePdf>({
    queryKey: K.template(companyId),
    enabled: !!companyId,
    staleTime: 5 * 60 * 1000,
    queryFn: () => getPisTemplatePdf(companyId!),
  });
}

/** Template di default (in-memory) per quando il modulo non è ancora pubblicato. */
export function getDefaultPisTemplatePdf(companyId: string | null): PisTemplatePdf {
  return normalizeTemplate(null, companyId ?? "");
}

/**
 * Probe leggera (HEAD) per sapere se il modulo Piscine è pubblicato sul
 * DB (tabella `pis_template_pdf` esistente). `false` ⇒ l'editor mostra i default
 ...
 */
export function usePisBackendReady() {
  const companyId = useEffectiveCompanyId();
  return useQuery<boolean>({
    queryKey: ["pis-backend-ready", companyId] as const,
    enabled: !!companyId,
    staleTime: 5 * 60 * 1000,
    retry: false,
    queryFn: async () => {
      // GET (non HEAD): su 404 il body PostgREST espone code/message, così
      // `isPisModuleNotPublished` riconosce la tabella mancante. Una HEAD non ha
      ...
      const { error } = await sb()
        ...
        .limit(1);
      if (error) {
        if (isPisModuleNotPublished(error)) return false;
        throw new Error(error.message);
      }
      return true;
    },
...
export type PisTemplatePatch = Partial<Omit<PisTemplatePdf, "id" | "company_id">>;

export function useUpsertPisTemplatePdf() {
  const companyId = useEffectiveCompanyId();
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async (patch: PisTemplatePatch): Promise<PisTemplatePdf> => {
      if (!companyId) throw new Error("Company non disponibile");
      ...
      const { data, error } = await sb()
        ...
        .single();
      if (error) {
        if (isPisModuleNotPublished(error)) {
          throw new Error(
            "Il modulo Piscine non è ancora pubblicato sul database: applica la migrazione (pubblica il modulo) per salvare il template.",
          );
        }
        throw new Error(error.message);
      }
