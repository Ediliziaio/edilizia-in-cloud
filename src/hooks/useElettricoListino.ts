 ...
 * Elettrico (tabelle `ele_listino_capitoli` / `ele_listino_voci`).
 *
 * NB: le tabelle `ele_*` provengono dalla migrazione LOCALE
...
// ─── Query keys ────────────────────────────────────────────────────────────
const K = {
  capitoli: (companyId: string | null) => ["ele-listino-capitoli", companyId] as const,
  voci: (companyId: string | null, capitoloId?: string | null) =>
    ["ele-listino-voci", companyId, capitoloId ?? "all"] as const,
  articoliSearch: (companyId: string | null, term: string) =>
    ["ele-prefill-articoli", companyId, term] as const,
  tariffeSearch: (companyId: string | null, term: string) =>
    ["ele-prefill-tariffe", companyId, term] as const,
  vociSearch: (companyId: string | null, term: string) =>
    ["ele-listino-voci-search", companyId, term] as const,
};
...
export function useListinoCapitoli() {
  ...
  return useQuery<EleListinoCapitolo[]>({
    queryKey: K.capitoli(companyId),
    enabled: !!companyId,
    staleTime: 60_000,
    queryFn: async () => {
      const { data, error } = await sb()
        .from("ele_listino_capitoli")
        .select("*")
        .eq("company_id", companyId!)
        .order("ordine", { ascending: true })
        .order("created_at", { ascending: true });
      if (error) throw new Error(error.message);
      return (data ?? []) as EleListinoCapitolo[];
    },
  });
}
...
export function useListinoVoci(capitoloId?: string | null) {
  const companyId = useEffectiveCompanyId();
  return useQuery<EleListinoVoce[]>({
    queryKey: K.voci(companyId, capitoloId),
    enabled: !!companyId,
    staleTime: 60_000,
    queryFn: async () => {
      let q = sb()
        .from("ele_listino_voci")
        .select("*")
        .eq("company_id", companyId!);
...
export function useUpsertCapitolo() {
  ...
  return useMutation({
    mutationFn: async (payload: CapitoloPayload): Promise<EleListinoCapitolo> => {
      ...
      const q = payload.id
        ? sb()
            ...
            .select()
            .single()
        : sb().from("ele_listino_capitoli").insert(row).select().single();
      const { data, error } = await q;
      if (error) throw new Error(error.message);
      return data as EleListinoCapitolo;
    },
...
export function useUpsertVoce() {
  ...
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async (payload: VocePayload): Promise<EleListinoVoce> => {
      if (!companyId) throw new Error("Company non disponibile");
      ...
      const q = payload.id
        ? sb()
            ...
            .select()
            .single()
        : sb().from("ele_listino_voci").insert(row).select().single();
      const { data, error } = await q;
      if (error) throw new Error(error.message);
      return data as EleListinoVoce;
    },
    onSuccess: () => {
      void qc.invalidateQueries({ queryKey: ["ele-listino-voci", companyId] });
    },
  });
}
...
export function useImportSeedListino() {
  ...
  return useMutation({
    mutationFn: async (): Promise<{ capitoli: number; voci: number }> => {
      ...
      const { error: vErr } = await sb().from("ele_listino_voci").insert(voci);
      if (vErr) throw new Error(vErr.message);
...
/**
 * Adozione di voci da un prezzario regionale ufficiale nel listino ELETTRICO
 * (`ele_listino_voci`). Gemello ele-scoped di `useAdottaPrezzario` di
 * `@/lib/prezzario/queries` (che scrive sul listino Ristrutturazione): qui le
 * voci finiscono nel listino Elettrico e si invalidano le query key `ele-listino-*`.
 */
export function useAdottaPrezzario() {
  ...
  return useMutation({
    mutationFn: async (input: AdottaPrezzarioInput): Promise<{ inserite: number }> => {
      ...
      const { error } = await sb().from("ele_listino_voci").insert(rows);
      if (error) throw new Error(error.message);
      return { inserite: rows.length };
    },
    onSuccess: () => {
      void qc.invalidateQueries({ queryKey: ["ele-listino-voci", companyId] });
      void qc.invalidateQueries({ queryKey: ["ele-listino-capitoli", companyId] });
    },
  });
}
...
const UM_MAP: Record<string, EleUnitaMisura> = {
  pz: "cad",
  cad: "cad",
  mq: "mq",
  ml: "ml",
...
/** Normalizza una UM sorgente (articoli/tariffe) verso `EleUnitaMisura`. */
export function mapUnitaMisura(raw: string | null | undefined): EleUnitaMisura {
  if (!raw) return "cad";
...
/**
 * Ricerca typeahead nel listino lavorazioni aziendale (`ele_listino_voci`) per
 * il picker del computo. Joina `ele_listino_capitoli` per esporre il nome del
    ...
    queryFn: async () => {
      ...
      return ((data ?? []) as Record<string, unknown>[]).map((d) => {
        ...
        return {
          ...
          unita_misura: ((d.unita_misura as string) ?? "cad") as EleUnitaMisura,
