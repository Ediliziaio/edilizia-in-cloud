...
import { calcRigaImporto, calcTotaliComputo } from "@/lib/termoidraulico/calcoli";
import type {
  IdrProgetto,
  IdrComputoVoce,
  IdrProgettoMedia,
...
function normalizeTemplate(row: Record<string, unknown> | null, companyId: string): IdrTemplatePdf {
  const r = row ?? {};
  return {
    id: (r.id as string) ?? "",
