/**
 * useTermoidraulicoProgetto — CRUD del progetto (hub) del verticale
 * Termoidraulico + computo metrico + media.
 *
 * Tabelle: `idr_progetti`, `idr_computo_voci`, `idr_progetti_media`.
 *
 * NB: come per `useListinoLavorazioni.ts`, le tabelle `idr_*` provengono dalla
 * migrazione LOCALE `20271101020000_idr_modulo.sql` e NON sono ancora
...
// Tipi idr_* non rigenerati: cast unico, riusato in tutto il file.
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const sb = () => supabase as any;

// ─── Query keys ────────────────────────────────────────────────────────────
const K = {
  progetti: (companyId: string | null) => ["idr-progetti", companyId] as const,
  progetto: (id: string | undefined) => ["idr-progetto", id ?? "none"] as const,
  template: (companyId: string | null) => ["idr-template", companyId] as const,
};

/** Dettaglio progetto: hub + computo voci (ordinate) + media. */
export interface IdrProgettoDetail {
  progetto: IdrProgetto;
  computo: IdrComputoVoce[];
  media: IdrProgettoMedia[];
}
...
// ─── Lista progetti ──────────────────────────────────────────────────────────
export function useTermoidraulicoProgetti() {
  const companyId = useEffectiveCompanyId();
  return useQuery<IdrProgetto[]>({
...
export function useTermoidraulicoProgetto(id: string | undefined) {
  return useQuery<IdrProgettoDetail | null>({
    ...
    queryFn: async () => {
      const [
        ...
        sb().from("idr_progetti").select("*").eq("id", id!).maybeSingle(),
        sb()
          .from("idr_computo_voci")
          .select("*")
          .eq("progetto_id", id!)
          .order("ordine", { ascending: true })
          .order("created_at", { ascending: true }),
        sb()
          .from("idr_progetti_media")
          .select("*")
          .eq("progetto_id", id!)
          .order("ordine", { ascending: true }),
      ]);
      ...
      return {
        progetto: progetto as IdrProgetto,
        computo: (computo ?? []) as IdrComputoVoce[],
        media: (media ?? []) as IdrProgettoMedia[],
      };
    },
  });
}
...
// ─── Upsert progetto ──────────────────────────────────────────────────────────
export type IdrProgettoPatch = Partial<Omit<IdrProgetto, "id" | "company_id">>;

/**
 * Genera un codice progressivo leggibile per il progetto (es. IDR-2026-007).
 * La tabella `idr_progetti.code` non ha trigger DB: lo deriviamo client-side
...
async function generateProgettoCode(companyId: string): Promise<string> {
  ...
  try {
    ...
    const n = (count ?? 0) + 1;
    return `IDR-${year}-${String(n).padStart(3, "0")}`;
  } catch {
    return `IDR-${year}-${Date.now().toString().slice(-5)}`;
  }
}
...
export function useUpsertProgetto() {
  ...
  return useMutation({
    mutationFn: async (
      input: IdrProgettoPatch & { id?: string },
    ): Promise<IdrProgetto> => {
      ...
      leggiModelloPreventivo("termoidraulico", patch.modello_snapshot, companyId);
...
export function useDeleteProgetto() {
  ...
  return useMutation({
    mutationFn: async (id: string): Promise<void> => {
      if (!companyId) throw new Error("Company non disponibile");
      // idr_computo_voci / idr_progetti_media hanno FK ON DELETE CASCADE.
...
export function useClonaProgetto() {
  ...
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async (sourceId: string): Promise<IdrProgetto> => {
      if (!companyId) throw new Error("Company non disponibile");
      ...
      const [{ data: src, error: sErr }, { data: voci, error: vErr }] = await Promise.all([
        sb().from("idr_progetti").select("*").eq("id", sourceId).eq("company_id", companyId).maybeSingle(),
        sb().from("idr_computo_voci").select("*").eq("progetto_id", sourceId).eq("company_id", companyId)
          .order("ordine", { ascending: true }),
      ]);
      ...
      if (voci && voci.length > 0) {
        ...
        const { error: iErr } = await sb().from("idr_computo_voci").insert(rows);
        if (iErr) throw new Error(iErr.message);
      }

      return nuovo as IdrProgetto;
    },
    onSuccess: () => {
      void qc.invalidateQueries({ queryKey: K.progetti(companyId) });
    },
...
export function useUpsertMedia(progettoId: string | undefined) {
  ...
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async (payload: MediaPayload): Promise<IdrProgettoMedia> => {
      if (!companyId) throw new Error("Company non disponibile");
      if (!progettoId) throw new Error("Progetto id mancante");
      const row = {
        progetto_id: progettoId,
      ...
      const q = payload.id
        ? sb()
            ...
            .select()
            .single()
        : sb().from("idr_progetti_media").insert(row).select().single();
      const { data, error } = await q;
      if (error) throw new Error(error.message);
      return data as IdrProgettoMedia;
    },
    onSuccess: () => {
      void qc.invalidateQueries({ queryKey: K.progetto(progettoId) });
    },
...
// Default coerenti con la migrazione `idr_template_pdf` (color_* + show_*).
// Le liste jsonb si normalizzano sempre ad array per non rompere `.map` in UI/PDF.
const IDR_TEMPLATE_DEFAULTS = {
...
/**
 * True se l'errore Supabase indica che la tabella `idr_template_pdf` (o lo schema
 * idr_*) NON esiste ancora sul DB → il modulo Termoidraulico non è stato ancora
 * pubblicato (migrazione `20271101020000_idr_modulo.sql` non applicata sul
 ...
 */
export function isIdrModuleNotPublished(
  ...
  return (
    code === "PGRST205" ||
    code === "42P01" ||
    /could not find the table|schema cache|does not exist|idr_template_pdf|idr_progetti/i.test(msg)
  );
}

const asArray = <T,>(v: unknown): T[] => (Array.isArray(v) ? (v as T[]) : []);

/** Normalizza una riga grezza del DB nel tipo `IdrTemplatePdf` (liste sempre array). */
export function normalizeTemplate(row: Record<string, unknown> | null, companyId: string): IdrTemplatePdf {
  const r = row ?? {};
  return {
    id: (r.id as string) ?? "",
    company_id: (r.company_id as string) ?? companyId,
    logo_url: (r.logo_url as string | null) ?? null,
    color_primary: (r.color_primary as string | null) ?? IDR_TEMPLATE_DEFAULTS.color_primary,
    color_secondary: (r.color_secondary as string | null) ?? IDR_TEMPLATE_DEFAULTS.color_secondary,
    color_accent: (r.color_accent as string | null) ?? IDR_TEMPLATE_DEFAULTS.color_accent,
    color_text: (r.color_text as string | null) ?? IDR_TEMPLATE_DEFAULTS.color_text,
    chi_siamo: (r.chi_siamo as string | null) ?? null,
    chi_siamo_foto_url: (r.chi_siamo_foto_url as string | null) ?? null,
    esigenze: asArray<IdrListItem>(r.esigenze),
    soluzione: asArray<IdrListItem>(r.soluzione),
    usp: asArray<IdrListItem>(r.usp),
    testimonianze: asArray<IdrTestimonianza>(r.testimonianze),
    cronoprogramma: asArray<IdrCronoFase>(r.cronoprogramma),
    ...
    show_chi_siamo: (r.show_chi_siamo as boolean | null) ?? IDR_TEMPLATE_DEFAULTS.show_chi_siamo,
    show_cronoprogramma: (r.show_cronoprogramma as boolean | null) ?? IDR_TEMPLATE_DEFAULTS.show_cronoprogramma,
    show_margine: (r.show_margine as boolean | null) ?? IDR_TEMPLATE_DEFAULTS.show_margine,
    ...
    cover_logo_position: (r.cover_logo_position as IdrTemplatePdf["cover_logo_position"]) ?? "top_left",
    ...
    garanzie: asArray<IdrListItem>(r.garanzie),
    faq: asArray<IdrFaqItem>(r.faq),
    percorso: asArray<IdrListItem>(r.percorso),
    ...
    cover_text_align: (r.cover_text_align as IdrTemplatePdf["cover_text_align"]) ?? "left",
 ...
 */
export async function getIdrTemplatePdf(companyId: string): Promise<IdrTemplatePdf> {
  const { data, error } = await sb()
    .from("idr_template_pdf")
    .select("*")
    .eq("company_id", companyId)
  ...
  if (error) {
    ...
    if (isIdrModuleNotPublished(error)) return normalizeTemplate(null, companyId);
    throw new Error(error.message);
  }
  return normalizeTemplate(data as Record<string, unknown> | null, companyId);
}

export function useIdrTemplatePdf() {
  const companyId = useEffectiveCompanyId();
  return useQuery<IdrTemplatePdf>({
    queryKey: K.template(companyId),
    enabled: !!companyId,
    staleTime: 5 * 60 * 1000,
    queryFn: () => getIdrTemplatePdf(companyId!),
  });
}

/** Template di default (in-memory) per quando il modulo non è ancora pubblicato. */
export function getDefaultIdrTemplatePdf(companyId: string | null): IdrTemplatePdf {
  return normalizeTemplate(null, companyId ?? "");
}

/**
 * Probe leggera (HEAD) per sapere se il modulo Termoidraulico è pubblicato sul
 * DB (tabella `idr_template_pdf` esistente). `false` ⇒ l'editor mostra i default
 ...
 */
export function useIdrBackendReady() {
  const companyId = useEffectiveCompanyId();
  return useQuery<boolean>({
    queryKey: ["idr-backend-ready", companyId] as const,
    enabled: !!companyId,
    staleTime: 5 * 60 * 1000,
    retry: false,
    queryFn: async () => {
      // GET (non HEAD): su 404 il body PostgREST espone code/message, così
      // `isIdrModuleNotPublished` riconosce la tabella mancante. Una HEAD non ha
      ...
      const { error } = await sb()
        ...
        .limit(1);
      if (error) {
        if (isIdrModuleNotPublished(error)) return false;
        throw new Error(error.message);
      }
      return true;
    },
...
export type IdrTemplatePatch = Partial<Omit<IdrTemplatePdf, "id" | "company_id">>;

export function useUpsertIdrTemplatePdf() {
  const companyId = useEffectiveCompanyId();
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async (patch: IdrTemplatePatch): Promise<IdrTemplatePdf> => {
      if (!companyId) throw new Error("Company non disponibile");
      ...
      const { data, error } = await sb()
        ...
        .single();
      if (error) {
        if (isIdrModuleNotPublished(error)) {
          throw new Error(
            "Il modulo Termoidraulico non è ancora pubblicato sul database: applica la migrazione (pubblica il modulo) per salvare il template.",
          );
        }
        throw new Error(error.message);
      }
