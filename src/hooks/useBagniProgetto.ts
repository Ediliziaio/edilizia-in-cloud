...
export function useUpsertProgetto() {
  ...
  return useMutation({
    mutationFn: async (
      ...
      leggiModelloPreventivo("bagni", patch.modello_snapshot, companyId);
    ...
    mutationFn: async (sourceId: string): Promise<BgnProgetto> => {
      ...
      const { data: nuovo, error: cErr } = await sb()
        ...
        .insert({
          ...
          ...(src.modello_snapshot ? { modello_snapshot: leggiModelloPreventivo("bagni", src.modello_snapshot, companyId) } : {}),
...
export function normalizeTemplate(row: Record<string, unknown> | null, companyId: string): BgnTemplatePdf {
  const r = row ?? {};
  return {
    id: (r.id as string) ?? "",
