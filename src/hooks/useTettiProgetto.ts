/**
 * useTettiProgetto — CRUD del progetto (hub) del verticale
 * Tetti + computo metrico + media.
 *
 * Tabelle: `tet_progetti`, `tet_computo_voci`, `tet_progetti_media`.
 *
 * NB: come per `useListinoLavorazioni.ts`, le tabelle `tet_*` provengono dalla
...
import { readTetQuoteModel } from "@/lib/tetti/quoteModel";
import type {
  TetProgetto,
  TetComputoVoce,
  TetProgettoMedia,
...
// ─── Query keys ────────────────────────────────────────────────────────────
const K = {
  progetti: (companyId: string | null) => ["tet-progetti", companyId] as const,
  progetto: (id: string | undefined) => ["tet-progetto", id ?? "none"] as const,
  template: (companyId: string | null) => ["tet-template", companyId] as const,
};

/** Dettaglio progetto: hub + computo voci (ordinate) + media. */
export interface TetProgettoDetail {
...
// ─── Lista progetti ──────────────────────────────────────────────────────────
export function useTettiProgetti() {
  const companyId = useEffectiveCompanyId();
  return useQuery<TetProgetto[]>({
...
export function useTettiProgetto(id: string | undefined) {
  return useQuery<TetProgettoDetail | null>({
    ...
    queryFn: async () => {
      const [
        ...
        sb().from("tet_progetti").select("*").eq("id", id!).maybeSingle(),
        sb()
      ...
      return {
        progetto: progetto as TetProgetto,
        computo: (computo ?? []) as TetComputoVoce[],
        media: (media ?? []) as TetProgettoMedia[],
      };
    },
  });
}
...
// ─── Upsert progetto ──────────────────────────────────────────────────────────
export type TetProgettoPatch = Partial<Omit<TetProgetto, "id" | "company_id">>;

/**
 * Genera un codice progressivo leggibile per il progetto (es. TET-2026-007).
 * La tabella `tet_progetti.code` non ha trigger DB: lo deriviamo client-side
...
async function generateProgettoCode(companyId: string): Promise<string> {
  ...
  try {
    ...
    const n = (count ?? 0) + 1;
    return `TET-${year}-${String(n).padStart(3, "0")}`;
  } catch {
    return `TET-${year}-${Date.now().toString().slice(-5)}`;
  }
}
...
export function useUpsertProgetto() {
  ...
  return useMutation({
    mutationFn: async (
      input: TetProgettoPatch & { id?: string },
    ): Promise<TetProgetto> => {
      ...
      readTetQuoteModel(patch.modello_snapshot, companyId);
...
export function useDeleteProgetto() {
  ...
  return useMutation({
    mutationFn: async (id: string): Promise<void> => {
      if (!companyId) throw new Error("Company non disponibile");
      // tet_computo_voci / tet_progetti_media hanno FK ON DELETE CASCADE.
...
export function useClonaProgetto() {
  ...
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async (sourceId: string): Promise<TetProgetto> => {
      if (!companyId) throw new Error("Company non disponibile");
      ...
      const [{ data: src, error: sErr }, { data: voci, error: vErr }] = await Promise.all([
        sb().from("tet_progetti").select("*").eq("id", sourceId).eq("company_id", companyId).maybeSingle(),
        sb().from("tet_computo_voci").select("*").eq("progetto_id", sourceId).eq("company_id", companyId)
          .order("ordine", { ascending: true }),
      ]);
      ...
      const { data: nuovo, error: cErr } = await sb()
        ...
        .insert({
          ...
          ...(src.modello_snapshot ? { modello_snapshot: readTetQuoteModel(src.modello_snapshot, companyId) } : {}),
...
export function useUpsertMedia(progettoId: string | undefined) {
  ...
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async (payload: MediaPayload): Promise<TetProgettoMedia> => {
      if (!companyId) throw new Error("Company non disponibile");
      if (!progettoId) throw new Error("Progetto id mancante");
      const row = {
        progetto_id: progettoId,
      ...
      const q = payload.id
        ? sb()
            ...
            .select()
            .single()
        : sb().from("tet_progetti_media").insert(row).select().single();
      const { data, error } = await q;
      if (error) throw new Error(error.message);
      return data as TetProgettoMedia;
    },
    onSuccess: () => {
      void qc.invalidateQueries({ queryKey: K.progetto(progettoId) });
    },
...
// Default coerenti con la migrazione `tet_template_pdf` (color_* + show_*).
// Le liste jsonb si normalizzano sempre ad array per non rompere `.map` in UI/PDF.
const TET_TEMPLATE_DEFAULTS = {
...
/**
 * True se l'errore Supabase indica che la tabella `tet_template_pdf` (o lo schema
 * tet_*) NON esiste ancora sul DB → il modulo Tetti non è stato ancora
 ...
 */
export function isTetModuleNotPublished(
  ...
  return (
    code === "PGRST205" ||
    code === "42P01" ||
    /could not find the table|schema cache|does not exist|tet_template_pdf|tet_progetti/i.test(msg)
  );
}

const asArray = <T,>(v: unknown): T[] => (Array.isArray(v) ? (v as T[]) : []);

/** Normalizza una riga grezza del DB nel tipo `TetTemplatePdf` (liste sempre array). */
export function normalizeTemplate(row: Record<string, unknown> | null, companyId: string): TetTemplatePdf {
  const r = row ?? {};
  return {
    id: (r.id as string) ?? "",
    company_id: (r.company_id as string) ?? companyId,
    logo_url: (r.logo_url as string | null) ?? null,
    color_primary: (r.color_primary as string | null) ?? TET_TEMPLATE_DEFAULTS.color_primary,
    color_secondary: (r.color_secondary as string | null) ?? TET_TEMPLATE_DEFAULTS.color_secondary,
    color_accent: (r.color_accent as string | null) ?? TET_TEMPLATE_DEFAULTS.color_accent,
    color_text: (r.color_text as string | null) ?? TET_TEMPLATE_DEFAULTS.color_text,
    chi_siamo: (r.chi_siamo as string | null) ?? null,
    chi_siamo_foto_url: (r.chi_siamo_foto_url as string | null) ?? null,
    esigenze: asArray<TetListItem>(r.esigenze),
    soluzione: asArray<TetListItem>(r.soluzione),
    usp: asArray<TetListItem>(r.usp),
    testimonianze: asArray<TetTestimonianza>(r.testimonianze),
    cronoprogramma: asArray<TetCronoFase>(r.cronoprogramma),
    ...
    show_chi_siamo: (r.show_chi_siamo as boolean | null) ?? TET_TEMPLATE_DEFAULTS.show_chi_siamo,
    show_cronoprogramma: (r.show_cronoprogramma as boolean | null) ?? TET_TEMPLATE_DEFAULTS.show_cronoprogramma,
    show_margine: (r.show_margine as boolean | null) ?? TET_TEMPLATE_DEFAULTS.show_margine,
    ...
    cover_logo_position: (r.cover_logo_position as TetTemplatePdf["cover_logo_position"]) ?? "top_left",
    ...
    garanzie: asArray<TetListItem>(r.garanzie),
    faq: asArray<TetFaqItem>(r.faq),
    percorso: asArray<TetListItem>(r.percorso),
    ...
    cover_text_align: (r.cover_text_align as TetTemplatePdf["cover_text_align"]) ?? "left",
    // Cover parity (preset 1-click) — default difensivi allineati a sr_template_pdf.
    cover_bg_color: (r.cover_bg_color as string | null) ?? null,
    cover_eyebrow: (r.cover_eyebrow as string | null) ?? null,
    cover_eyebrow_size: (r.cover_eyebrow_size as number | null) ?? null,
    cover_subtitle_size: (r.cover_subtitle_size as number | null) ?? null,
    cover_logo_size: (r.cover_logo_size as number | null) ?? null,
    cover_overlay_style: (r.cover_overlay_style as TetTemplatePdf["cover_overlay_style"]) ?? "flat",
    cover_text_vertical: (r.cover_text_vertical as TetTemplatePdf["cover_text_vertical"]) ?? "bottom",
    cover_decoration_style: (r.cover_decoration_style as TetTemplatePdf["cover_decoration_style"]) ?? "square",
    cover_show_decoration: (r.cover_show_decoration as boolean | null) ?? true,
    cover_show_client_card: (r.cover_show_client_card as boolean | null) ?? true,
 ...
 */
export async function getTetTemplatePdf(companyId: string): Promise<TetTemplatePdf> {
  const { data, error } = await sb()
    .from("tet_template_pdf")
    .select("*")
    .eq("company_id", companyId)
  ...
  if (error) {
    ...
    if (isTetModuleNotPublished(error)) return normalizeTemplate(null, companyId);
    throw new Error(error.message);
  }
  return normalizeTemplate(data as Record<string, unknown> | null, companyId);
}

export function useTetTemplatePdf() {
  const companyId = useEffectiveCompanyId();
  return useQuery<TetTemplatePdf>({
    queryKey: K.template(companyId),
    enabled: !!companyId,
    staleTime: 5 * 60 * 1000,
    queryFn: () => getTetTemplatePdf(companyId!),
  });
}

/** Template di default (in-memory) per quando il modulo non è ancora pubblicato. */
export function getDefaultTetTemplatePdf(companyId: string | null): TetTemplatePdf {
  return normalizeTemplate(null, companyId ?? "");
}

/**
 * Probe leggera (HEAD) per sapere se il modulo Tetti è pubblicato sul
 * DB (tabella `tet_template_pdf` esistente). `false` ⇒ l'editor mostra i default
 ...
 */
export function useTetBackendReady() {
  const companyId = useEffectiveCompanyId();
  return useQuery<boolean>({
    queryKey: ["tet-backend-ready", companyId] as const,
    enabled: !!companyId,
    staleTime: 5 * 60 * 1000,
    retry: false,
    queryFn: async () => {
      // GET (non HEAD): su 404 il body PostgREST espone code/message, così
      // `isTetModuleNotPublished` riconosce la tabella mancante. Una HEAD non ha
      ...
      const { error } = await sb()
        ...
        .limit(1);
      if (error) {
        if (isTetModuleNotPublished(error)) return false;
        throw new Error(error.message);
      }
      return true;
    },
...
export type TetTemplatePatch = Partial<Omit<TetTemplatePdf, "id" | "company_id">>;

export function useUpsertTetTemplatePdf() {
  const companyId = useEffectiveCompanyId();
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async (patch: TetTemplatePatch): Promise<TetTemplatePdf> => {
      if (!companyId) throw new Error("Company non disponibile");
      ...
      const { data, error } = await sb()
        ...
        .single();
      if (error) {
        if (isTetModuleNotPublished(error)) {
          throw new Error(
            "Il modulo Tetti non è ancora pubblicato sul database: applica la migrazione (pubblica il modulo) per salvare il template.",
          );
        }
        throw new Error(error.message);
      }
