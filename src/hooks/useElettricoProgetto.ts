/**
 * useElettricoProgetto — CRUD del progetto (hub) del verticale
 * Elettrico + computo metrico + media.
 *
 * Tabelle: `ele_progetti`, `ele_computo_voci`, `ele_progetti_media`.
 *
 * NB: come per `useListinoLavorazioni.ts`, le tabelle `ele_*` provengono dalla
 * migrazione LOCALE `20271101020000_ele_modulo.sql` e NON sono ancora
...
// Tipi ele_* non rigenerati: cast unico, riusato in tutto il file.
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const sb = () => supabase as any;

// ─── Query keys ────────────────────────────────────────────────────────────
const K = {
  progetti: (companyId: string | null) => ["ele-progetti", companyId] as const,
  progetto: (id: string | undefined) => ["ele-progetto", id ?? "none"] as const,
  template: (companyId: string | null) => ["ele-template", companyId] as const,
};

/** Dettaglio progetto: hub + computo voci (ordinate) + media. */
export interface EleProgettoDetail {
...
// ─── Lista progetti ──────────────────────────────────────────────────────────
export function useElettricoProgetti() {
  const companyId = useEffectiveCompanyId();
  return useQuery<EleProgetto[]>({
    ...
    queryFn: async () => {
      ...
      return (data ?? []) as EleProgetto[];
    },
  });
}

// ─── Singolo progetto (+ computo + media) ─────────────────────────────────────
export function useElettricoProgetto(id: string | undefined) {
  return useQuery<EleProgettoDetail | null>({
    ...
    queryFn: async () => {
      const [
        ...
        sb().from("ele_progetti").select("*").eq("id", id!).maybeSingle(),
        sb()
          .from("ele_computo_voci")
          .select("*")
          .eq("progetto_id", id!)
          .order("ordine", { ascending: true })
          .order("created_at", { ascending: true }),
        sb()
          .from("ele_progetti_media")
          .select("*")
          .eq("progetto_id", id!)
          .order("ordine", { ascending: true }),
      ]);
      ...
      return {
        progetto: progetto as EleProgetto,
        computo: (computo ?? []) as EleComputoVoce[],
        media: (media ?? []) as EleProgettoMedia[],
      };
    },
  });
}
...
// ─── Upsert progetto ──────────────────────────────────────────────────────────
export type EleProgettoPatch = Partial<Omit<EleProgetto, "id" | "company_id">>;

/**
 * Genera un codice progressivo leggibile per il progetto (es. ELE-2026-007).
 * La tabella `ele_progetti.code` non ha trigger DB: lo deriviamo client-side
...
async function generateProgettoCode(companyId: string): Promise<string> {
  ...
  try {
    ...
    const n = (count ?? 0) + 1;
    return `ELE-${year}-${String(n).padStart(3, "0")}`;
  } catch {
    return `ELE-${year}-${Date.now().toString().slice(-5)}`;
  }
}
...
export function useUpsertProgetto() {
  ...
  return useMutation({
    mutationFn: async (
      input: EleProgettoPatch & { id?: string },
    ): Promise<EleProgetto> => {
      ...
      leggiModelloPreventivo("elettrico", patch.modello_snapshot, companyId);
...
export function useDeleteProgetto() {
  ...
  return useMutation({
    mutationFn: async (id: string): Promise<void> => {
      if (!companyId) throw new Error("Company non disponibile");
      // ele_computo_voci / ele_progetti_media hanno FK ON DELETE CASCADE.
...
export function useClonaProgetto() {
  ...
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async (sourceId: string): Promise<EleProgetto> => {
      if (!companyId) throw new Error("Company non disponibile");
      ...
      const [{ data: src, error: sErr }, { data: voci, error: vErr }] = await Promise.all([
        sb().from("ele_progetti").select("*").eq("id", sourceId).eq("company_id", companyId).maybeSingle(),
        sb().from("ele_computo_voci").select("*").eq("progetto_id", sourceId).eq("company_id", companyId)
          .order("ordine", { ascending: true }),
      ]);
      ...
      const { data: nuovo, error: cErr } = await sb()
        ...
        .insert({
          ...
          ...(src.modello_snapshot ? { modello_snapshot: leggiModelloPreventivo("elettrico", src.modello_snapshot, companyId) } : {}),
      ...
      if (voci && voci.length > 0) {
        ...
        const { error: iErr } = await sb().from("ele_computo_voci").insert(rows);
        if (iErr) throw new Error(iErr.message);
      }

      return nuovo as EleProgetto;
    },
    onSuccess: () => {
      void qc.invalidateQueries({ queryKey: K.progetti(companyId) });
    },
...
export function useUpsertMedia(progettoId: string | undefined) {
  ...
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async (payload: MediaPayload): Promise<EleProgettoMedia> => {
      if (!companyId) throw new Error("Company non disponibile");
      if (!progettoId) throw new Error("Progetto id mancante");
      const row = {
        progetto_id: progettoId,
      ...
      const q = payload.id
        ? sb()
            ...
            .select()
            .single()
        : sb().from("ele_progetti_media").insert(row).select().single();
      const { data, error } = await q;
      if (error) throw new Error(error.message);
      return data as EleProgettoMedia;
    },
    onSuccess: () => {
      void qc.invalidateQueries({ queryKey: K.progetto(progettoId) });
    },
...
// Default coerenti con la migrazione `ele_template_pdf` (color_* + show_*).
// Le liste jsonb si normalizzano sempre ad array per non rompere `.map` in UI/PDF.
const ELE_TEMPLATE_DEFAULTS = {
...
/**
 * True se l'errore Supabase indica che la tabella `ele_template_pdf` (o lo schema
 * ele_*) NON esiste ancora sul DB → il modulo Elettrico non è stato ancora
 * pubblicato (migrazione `20271101020000_ele_modulo.sql` non applicata sul
 ...
 */
export function isEleModuleNotPublished(
  ...
  return (
    code === "PGRST205" ||
    code === "42P01" ||
    /could not find the table|schema cache|does not exist|ele_template_pdf|ele_progetti/i.test(msg)
  );
}
...
export function normalizeTemplate(row: Record<string, unknown> | null, companyId: string): EleTemplatePdf {
  ...
  return {
    ...
    color_primary: (r.color_primary as string | null) ?? ELE_TEMPLATE_DEFAULTS.color_primary,
    color_secondary: (r.color_secondary as string | null) ?? ELE_TEMPLATE_DEFAULTS.color_secondary,
    color_accent: (r.color_accent as string | null) ?? ELE_TEMPLATE_DEFAULTS.color_accent,
    color_text: (r.color_text as string | null) ?? ELE_TEMPLATE_DEFAULTS.color_text,
    chi_siamo: (r.chi_siamo as string | null) ?? null,
    chi_siamo_foto_url: (r.chi_siamo_foto_url as string | null) ?? null,
    esigenze: asArray<EleListItem>(r.esigenze),
    soluzione: asArray<EleListItem>(r.soluzione),
    usp: asArray<EleListItem>(r.usp),
    testimonianze: asArray<EleTestimonianza>(r.testimonianze),
    cronoprogramma: asArray<EleCronoFase>(r.cronoprogramma),
    ...
    show_chi_siamo: (r.show_chi_siamo as boolean | null) ?? ELE_TEMPLATE_DEFAULTS.show_chi_siamo,
    show_cronoprogramma: (r.show_cronoprogramma as boolean | null) ?? ELE_TEMPLATE_DEFAULTS.show_cronoprogramma,
    show_margine: (r.show_margine as boolean | null) ?? ELE_TEMPLATE_DEFAULTS.show_margine,
    ...
    cover_logo_position: (r.cover_logo_position as EleTemplatePdf["cover_logo_position"]) ?? "top_left",
    ...
    garanzie: asArray<EleListItem>(r.garanzie),
    faq: asArray<EleFaqItem>(r.faq),
    percorso: asArray<EleListItem>(r.percorso),
 ...
 */
export async function getEleTemplatePdf(companyId: string): Promise<EleTemplatePdf> {
  const { data, error } = await sb()
    .from("ele_template_pdf")
    .select("*")
    .eq("company_id", companyId)
  ...
  if (error) {
    ...
    if (isEleModuleNotPublished(error)) return normalizeTemplate(null, companyId);
    throw new Error(error.message);
  }
  return normalizeTemplate(data as Record<string, unknown> | null, companyId);
}

export function useEleTemplatePdf() {
  const companyId = useEffectiveCompanyId();
  return useQuery<EleTemplatePdf>({
    queryKey: K.template(companyId),
    enabled: !!companyId,
    staleTime: 5 * 60 * 1000,
    queryFn: () => getEleTemplatePdf(companyId!),
  });
}

/** Template di default (in-memory) per quando il modulo non è ancora pubblicato. */
export function getDefaultEleTemplatePdf(companyId: string | null): EleTemplatePdf {
  return normalizeTemplate(null, companyId ?? "");
}

/**
 * Probe leggera (HEAD) per sapere se il modulo Elettrico è pubblicato sul
 * DB (tabella `ele_template_pdf` esistente). `false` ⇒ l'editor mostra i default
 ...
 */
export function useEleBackendReady() {
  const companyId = useEffectiveCompanyId();
  return useQuery<boolean>({
    queryKey: ["ele-backend-ready", companyId] as const,
    enabled: !!companyId,
    staleTime: 5 * 60 * 1000,
    retry: false,
    queryFn: async () => {
      // GET (non HEAD): su 404 il body PostgREST espone code/message, così
      // `isEleModuleNotPublished` riconosce la tabella mancante. Una HEAD non ha
      ...
      const { error } = await sb()
        ...
        .limit(1);
      if (error) {
        if (isEleModuleNotPublished(error)) return false;
        throw new Error(error.message);
      }
      return true;
    },
...
export type EleTemplatePatch = Partial<Omit<EleTemplatePdf, "id" | "company_id">>;

export function useUpsertEleTemplatePdf() {
  const companyId = useEffectiveCompanyId();
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async (patch: EleTemplatePatch): Promise<EleTemplatePdf> => {
      if (!companyId) throw new Error("Company non disponibile");
      ...
      const { data, error } = await sb()
        ...
        .single();
      if (error) {
        if (isEleModuleNotPublished(error)) {
          throw new Error(
            "Il modulo Elettrico non è ancora pubblicato sul database: applica la migrazione (pubblica il modulo) per salvare il template.",
          );
        }
        throw new Error(error.message);
      }
