/**
 * usePiscinePDF — download/preview del PDF cliente del preventivo
 * Piscine (Task 21).
 *
...
import type {
  PisProgetto, PisComputoVoce, PisProgettoMedia, PisTemplatePdf,
} from "@/types/piscine";

// ─── Tipi pre-fetch arricchimento ───────────────────────────────────────────

export interface PisPdfCompany {
  name?: string | null;
...
export interface PisPdfCapitolo {
  nome: string;
  voci: PisComputoVoce[];
  subtotale: number;
  costo: number;
...
export interface PisPdfEnriched {
  progetto: PisProgetto;
  template: PisTemplatePdf;
  company: PisPdfCompany | null;
  capitoli: PisPdfCapitolo[];
  totali: PisPdfTotali;
  media: PisProgettoMedia[];
  computoOptions: PisPdfComputoOptions;
}
...
export interface PisPdfComputoOptions {
...
export const DEFAULT_COMPUTO_OPTIONS: PisPdfComputoOptions = {
  livello: "dettagliato",
  mostraPrezzi: true,
  mostraQta: true,
...
export interface PisPdfPayload {
  ...
  pdfOptions?: PisPdfComputoOptions;
}
...
async function enrichForPdf(opts: PisPdfPayload): Promise<PisPdfEnriched> {
  ...
  const capMap = new Map<string, PisPdfCapitolo>();
  const order: string[] = [];
  ...
  const inlinedMedia: PisProgettoMedia[] = imageMedia.map((m, i) => ({
    ...m,
...
// ─── Hook ──────────────────────────────────────────────────────────────────
function buildFilename(progetto: PisProgetto): string {
  const cliente = [progetto.cliente_nome, progetto.cliente_cognome].filter(Boolean).join("_") || "cliente";
  return `Preventivo-${progetto.code ?? "piscine"}-${cliente}.pdf`;
}

export function usePiscinePDF() {
  const [isGenerating, setIsGenerating] = useState(false);

  const downloadPDF = async (opts: PisPdfPayload): Promise<{ ok: boolean }> => {
    ...
    try {
      ...
      console.error("Errore generazione PDF piscine:", err);
  ...
  const previewPDF = async (opts: PisPdfPayload): Promise<void> => {
    setIsGenerating(true);
    try {
      if (opts.computo.length === 0) {
        toast.error("Computo vuoto", {
      ...
      console.error("Errore preview PDF piscine:", err);
      const msg = err instanceof Error ? err.message : "Errore sconosciuto";
      toast.error("Errore nella generazione del PDF", { description: msg });
    } finally {
      setIsGenerating(false);
