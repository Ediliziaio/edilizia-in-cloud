/**
 * useTettiPDF — download/preview del PDF cliente del preventivo
 * Tetti (Task 21).
 *
...
export interface TetPdfCompany {
  name?: string | null;
...
export interface TetPdfCapitolo {
  nome: string;
  voci: TetComputoVoce[];
  subtotale: number;
  costo: number;
...
export interface TetPdfEnriched {
  progetto: TetProgetto;
  template: TetTemplatePdf;
  company: TetPdfCompany | null;
  capitoli: TetPdfCapitolo[];
  totali: TetPdfTotali;
  media: TetProgettoMedia[];
  computoOptions: TetPdfComputoOptions;
}
...
export interface TetPdfComputoOptions {
...
export const DEFAULT_COMPUTO_OPTIONS: TetPdfComputoOptions = {
  livello: "dettagliato",
  mostraPrezzi: true,
  mostraQta: true,
...
export interface TetPdfPayload {
  progetto: TetProgetto;
  ...
  pdfOptions?: TetPdfComputoOptions;
}
...
export async function enrichTettiPdf(opts: TetPdfPayload): Promise<TetPdfEnriched> {
  ...
  const capMap = new Map<string, TetPdfCapitolo>();
  const order: string[] = [];
  ...
  const [inlinedLogo, inlinedChiSiamo, immaginiModello] = await Promise.all([
    ...
    immaginiDelModello("tetti", template as unknown as Record<string, unknown>, company?.logo_chiaro_url ?? null, companyId),
  ]);
  ...
  const inlinedCompany: TetPdfCompany | null = company
  ...
  const inlinedMedia: TetProgettoMedia[] = imageMedia
...
// ─── Hook ──────────────────────────────────────────────────────────────────
function buildFilename(progetto: TetProgetto): string {
  const cliente = [progetto.cliente_nome, progetto.cliente_cognome].filter(Boolean).join("_") || "cliente";
  return `Preventivo-${progetto.code ?? "tetti"}-${cliente}.pdf`;
}

export function useTettiPDF() {
  const [isGenerating, setIsGenerating] = useState(false);

  const downloadPDF = async (opts: TetPdfPayload): Promise<{ ok: boolean }> => {
    ...
    try {
      ...
      console.error("Errore generazione PDF tetti:", err);
  ...
  const previewPDF = async (opts: TetPdfPayload): Promise<void> => {
    setIsGenerating(true);
    try {
      if (opts.computo.length === 0) {
        toast.error("Computo vuoto", {
      ...
      console.error("Errore preview PDF tetti:", err);
      const msg = err instanceof Error ? err.message : "Errore sconosciuto";
      toast.error("Errore nella generazione del PDF", { description: msg });
    } finally {
      setIsGenerating(false);
