/**
 * useClimatizzazionePDF — download/preview del PDF cliente del preventivo
 * Climatizzazione (Task 21).
 *
...
import { getClmTemplatePdf } from "@/hooks/useClimatizzazioneProgetto";
...
import { calcTotaliComputo, type ComputoRigaInput } from "@/lib/climatizzazione/calcoli";
import { calcDetraibile } from "@/lib/preventivi/incentivi";
import type {
  ClmProgetto, ClmComputoVoce, ClmProgettoMedia, ClmTemplatePdf,
} from "@/types/climatizzazione";

// ─── Tipi pre-fetch arricchimento ───────────────────────────────────────────

export interface ClmPdfCompany {
  name?: string | null;
...
export interface ClmPdfCapitolo {
  nome: string;
  voci: ClmComputoVoce[];
  subtotale: number;
  costo: number;
...
export interface ClmPdfEnriched {
  ...
  company: ClmPdfCompany | null;
  capitoli: ClmPdfCapitolo[];
  totali: ClmPdfTotali;
  media: ClmProgettoMedia[];
  computoOptions: ClmPdfComputoOptions;
}
...
export interface ClmPdfComputoOptions {
...
export const DEFAULT_COMPUTO_OPTIONS: ClmPdfComputoOptions = {
  livello: "dettagliato",
  mostraPrezzi: true,
  mostraQta: true,
...
export interface ClmPdfPayload {
  /** Local module preview: explicit template, no database reads or media signing. */
  localOnly?: boolean;
  progetto: ClmProgetto;
  ...
  pdfOptions?: ClmPdfComputoOptions;
}
...
// ─── Enrich ──────────────────────────────────────────────────────────────────
export async function enrichClimatizzazionePdf(opts: ClmPdfPayload): Promise<ClmPdfEnriched> {
  ...
  const capMap = new Map<string, ClmPdfCapitolo>();
  const order: string[] = [];
  ...
  const [inlinedLogo, inlinedChiSiamo, immaginiModello] = await Promise.all([
    ...
    immaginiDelModello("climatizzazione", { ...template, ...(opts.localOnly ? { company_id: null } : {}) } as unknown as Record<string, unknown>, company?.logo_chiaro_url ?? null, opts.localOnly ? null : companyId),
  ]);
  ...
  const inlinedCompany: ClmPdfCompany | null = company
  ...
  const linkMedia = opts.localOnly ? imageMedia.map(m => m.url.startsWith("data:image/") || (m.url.startsWith("/") && !m.url.startsWith("//")) ? m.url : null) : await linkFileRiservati(imageMedia.map((m) => m.url));
  const inlinedUrls = await mapWithConcurrency(imageMedia, 4, async (_m, i) => toDataUrl(linkMedia[i]));
  const inlinedMedia: ClmProgettoMedia[] = imageMedia
 ...
 */
export async function renderClmPreviewBlobUrl(opts: ClmPdfPayload): Promise<string> {
  const enriched = await enrichClimatizzazionePdf(opts);
  const [{ pdf }, { ClimatizzazionePDF }, React] = await Promise.all([
    import("@react-pdf/renderer"),
    import("@/components/climatizzazione/ClimatizzazionePDF"),
    import("react"),
  ]);
  const element = React.createElement(ClimatizzazionePDF, enriched);
  const blob = await pdf(element as unknown as ReactElement<DocumentProps>).toBlob();
  return URL.createObjectURL(blob);
}

// ─── Hook ──────────────────────────────────────────────────────────────────
function buildFilename(progetto: ClmProgetto): string {
  const cliente = [progetto.cliente_nome, progetto.cliente_cognome].filter(Boolean).join("_") || "cliente";
  return `Preventivo-${progetto.code ?? "climatizzazione"}-${cliente}.pdf`;
}

export function useClimatizzazionePDF() {
  const [isGenerating, setIsGenerating] = useState(false);

  const downloadPDF = async (opts: ClmPdfPayload): Promise<{ ok: boolean }> => {
    ...
    try {
      ...
      console.error("Errore generazione PDF climatizzazione:", err);
  ...
  const previewPDF = async (opts: ClmPdfPayload): Promise<void> => {
    setIsGenerating(true);
    try {
      if (opts.computo.length === 0) {
        toast.error("Computo vuoto", {
      ...
      console.error("Errore preview PDF climatizzazione:", err);
      const msg = err instanceof Error ? err.message : "Errore sconosciuto";
      toast.error("Errore nella generazione del PDF", { description: msg });
    } finally {
      setIsGenerating(false);
