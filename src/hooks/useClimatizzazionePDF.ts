...
// ─── Enrich ──────────────────────────────────────────────────────────────────
async function enrichForPdf(opts: ClmPdfPayload): Promise<ClmPdfEnriched> {
  ...
  const inlinedTemplate: ClmTemplatePdf = {
    ...template,
  ...
  const inlinedMedia: ClmProgettoMedia[] = imageMedia.map((m, i) => ({
    ...m,
