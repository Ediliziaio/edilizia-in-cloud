/**
 * useClimatizzazioneProgetto — CRUD del progetto (hub) del verticale
 * Climatizzazione + computo metrico + media.
 *
 * Tabelle: `clm_progetti`, `clm_computo_voci`, `clm_progetti_media`.
 *
 * NB: come per `useListinoLavorazioni.ts`, le tabelle `clm_*` provengono dalla
 * migrazione LOCALE `20271101020000_clm_modulo.sql` e NON sono ancora
...
import { calcRigaImporto, calcTotaliComputo, type ComputoRigaInput } from "@/lib/climatizzazione/calcoli";
import {
...
// Tipi clm_* non rigenerati: cast unico, riusato in tutto il file.
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const sb = () => supabase as any;

// ─── Query keys ────────────────────────────────────────────────────────────
const K = {
  progetti: (companyId: string | null) => ["clm-progetti", companyId] as const,
  progetto: (id: string | undefined) => ["clm-progetto", id ?? "none"] as const,
  template: (companyId: string | null) => ["clm-template", companyId] as const,
};

/** Dettaglio progetto: hub + computo voci (ordinate) + media. */
export interface ClmProgettoDetail {
  progetto: ClmProgetto;
  computo: ClmComputoVoce[];
  media: ClmProgettoMedia[];
}
...
// ─── Lista progetti ──────────────────────────────────────────────────────────
export function useClimatizzazioneProgetti() {
  const companyId = useEffectiveCompanyId();
  return useQuery<ClmProgetto[]>({
    ...
    queryFn: async () => {
      ...
      return (data ?? []) as ClmProgetto[];
    },
  });
}

// ─── Singolo progetto (+ computo + media) ─────────────────────────────────────
export function useClimatizzazioneProgetto(id: string | undefined) {
  return useQuery<ClmProgettoDetail | null>({
    ...
    queryFn: async () => {
      const [
        ...
        sb().from("clm_progetti").select("*").eq("id", id!).maybeSingle(),
        sb()
          .from("clm_computo_voci")
          .select("*")
          .eq("progetto_id", id!)
          .order("ordine", { ascending: true })
          .order("created_at", { ascending: true }),
        sb()
          .from("clm_progetti_media")
          .select("*")
          .eq("progetto_id", id!)
          .order("ordine", { ascending: true }),
      ]);
      ...
      return {
        progetto: progetto as ClmProgetto,
        computo: (computo ?? []) as ClmComputoVoce[],
        media: (media ?? []) as ClmProgettoMedia[],
      };
    },
  });
}
...
// ─── Upsert progetto ──────────────────────────────────────────────────────────
export type ClmProgettoPatch = Partial<Omit<ClmProgetto, "id" | "company_id">>;

/**
 * Genera un codice progressivo leggibile per il progetto (es. CLM-2026-007).
 * La tabella `clm_progetti.code` non ha trigger DB: lo deriviamo client-side
...
async function generateProgettoCode(companyId: string): Promise<string> {
  ...
  try {
    ...
    return `CLM-${year}-${Date.now().toString().slice(-5)}`;
  }
}
...
export function useUpsertProgetto() {
  ...
  return useMutation({
    mutationFn: async (
      input: ClmProgettoPatch & { id?: string },
    ): Promise<ClmProgetto> => {
      ...
      leggiModelloPreventivo("climatizzazione", patch.modello_snapshot, companyId);
...
export function useDeleteProgetto() {
  ...
  return useMutation({
    mutationFn: async (id: string): Promise<void> => {
      if (!companyId) throw new Error("Company non disponibile");
      // clm_computo_voci / clm_progetti_media hanno FK ON DELETE CASCADE.
...
export function useClonaProgetto() {
  ...
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async (sourceId: string): Promise<ClmProgetto> => {
      if (!companyId) throw new Error("Company non disponibile");
      ...
      const [{ data: src, error: sErr }, { data: voci, error: vErr }] = await Promise.all([
        sb().from("clm_progetti").select("*").eq("id", sourceId).eq("company_id", companyId).maybeSingle(),
        sb().from("clm_computo_voci").select("*").eq("progetto_id", sourceId).eq("company_id", companyId)
          .order("ordine", { ascending: true }),
      ]);
      ...
      const { data: nuovo, error: cErr } = await sb()
        ...
        .insert({
          ...
          ...(src.modello_snapshot ? { modello_snapshot: leggiModelloPreventivo("climatizzazione", src.modello_snapshot, companyId) } : {}),
...
export function useUpsertMedia(progettoId: string | undefined) {
  ...
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async (payload: MediaPayload): Promise<ClmProgettoMedia> => {
      if (!companyId) throw new Error("Company non disponibile");
      if (!progettoId) throw new Error("Progetto id mancante");
      const row = {
        progetto_id: progettoId,
      ...
      const q = payload.id
        ? sb()
            ...
            .select()
            .single()
        : sb().from("clm_progetti_media").insert(row).select().single();
      const { data, error } = await q;
      if (error) throw new Error(error.message);
      return data as ClmProgettoMedia;
    },
    onSuccess: () => {
      void qc.invalidateQueries({ queryKey: K.progetto(progettoId) });
    },
...
// Default coerenti con la migrazione `clm_template_pdf` (color_* + show_*).
// Le liste jsonb si normalizzano sempre ad array per non rompere `.map` in UI/PDF.
const CLM_TEMPLATE_DEFAULTS = {
...
/**
 * True se l'errore Supabase indica che la tabella `clm_template_pdf` (o lo schema
 * clm_*) NON esiste ancora sul DB → il modulo Climatizzazione non è stato ancora
 * pubblicato (migrazione `20271101020000_clm_modulo.sql` non applicata sul
 ...
 */
export function isClmModuleNotPublished(
  ...
  return (
    code === "PGRST205" ||
    code === "42P01" ||
    /could not find the table|schema cache|does not exist|clm_template_pdf|clm_progetti/i.test(msg)
  );
}

const asArray = <T,>(v: unknown): T[] => (Array.isArray(v) ? (v as T[]) : []);

/** Normalizza una riga grezza del DB nel tipo `ClmTemplatePdf` (liste sempre array). */
export function normalizeTemplate(row: Record<string, unknown> | null, companyId: string): ClmTemplatePdf {
  const r = row ?? {};
  return {
    id: (r.id as string) ?? "",
    company_id: (r.company_id as string) ?? companyId,
    logo_url: (r.logo_url as string | null) ?? null,
    color_primary: (r.color_primary as string | null) ?? CLM_TEMPLATE_DEFAULTS.color_primary,
    color_secondary: (r.color_secondary as string | null) ?? CLM_TEMPLATE_DEFAULTS.color_secondary,
    color_accent: (r.color_accent as string | null) ?? CLM_TEMPLATE_DEFAULTS.color_accent,
    color_text: (r.color_text as string | null) ?? CLM_TEMPLATE_DEFAULTS.color_text,
    chi_siamo: (r.chi_siamo as string | null) ?? null,
    chi_siamo_foto_url: (r.chi_siamo_foto_url as string | null) ?? null,
    esigenze: asArray<ClmListItem>(r.esigenze),
    soluzione: asArray<ClmListItem>(r.soluzione),
    usp: asArray<ClmListItem>(r.usp),
    testimonianze: asArray<ClmTestimonianza>(r.testimonianze),
    cronoprogramma: asArray<ClmCronoFase>(r.cronoprogramma),
    ...
    show_chi_siamo: (r.show_chi_siamo as boolean | null) ?? CLM_TEMPLATE_DEFAULTS.show_chi_siamo,
    show_cronoprogramma: (r.show_cronoprogramma as boolean | null) ?? CLM_TEMPLATE_DEFAULTS.show_cronoprogramma,
    show_margine: (r.show_margine as boolean | null) ?? CLM_TEMPLATE_DEFAULTS.show_margine,
    ...
    cover_logo_position: (r.cover_logo_position as ClmTemplatePdf["cover_logo_position"]) ?? "top_left",
    ...
    garanzie: asArray<ClmListItem>(r.garanzie),
    faq: asArray<ClmFaqItem>(r.faq),
    percorso: asArray<ClmListItem>(r.percorso),
    ...
    cover_text_align: (r.cover_text_align as ClmTemplatePdf["cover_text_align"]) ?? "left",
 ...
 */
export async function getClmTemplatePdf(companyId: string): Promise<ClmTemplatePdf> {
  const { data, error } = await sb()
    .from("clm_template_pdf")
    .select("*")
    .eq("company_id", companyId)
  ...
  if (error) {
    ...
    if (isClmModuleNotPublished(error)) return normalizeTemplate(null, companyId);
    throw new Error(error.message);
  }
  return normalizeTemplate(data as Record<string, unknown> | null, companyId);
}

export function useClmTemplatePdf() {
  const companyId = useEffectiveCompanyId();
  return useQuery<ClmTemplatePdf>({
    queryKey: K.template(companyId),
    enabled: !!companyId,
    staleTime: 5 * 60 * 1000,
    queryFn: () => getClmTemplatePdf(companyId!),
  });
}

/** Template di default (in-memory) per quando il modulo non è ancora pubblicato. */
export function getDefaultClmTemplatePdf(companyId: string | null): ClmTemplatePdf {
  return normalizeTemplate(null, companyId ?? "");
}

/**
 * Probe leggera (HEAD) per sapere se il modulo Climatizzazione è pubblicato sul
 * DB (tabella `clm_template_pdf` esistente). `false` ⇒ l'editor mostra i default
 ...
 */
export function useClmBackendReady() {
  const companyId = useEffectiveCompanyId();
  return useQuery<boolean>({
    queryKey: ["clm-backend-ready", companyId] as const,
    enabled: !!companyId,
    staleTime: 5 * 60 * 1000,
    retry: false,
    queryFn: async () => {
      // GET (non HEAD): su 404 il body PostgREST espone code/message, così
      // `isClmModuleNotPublished` riconosce la tabella mancante. Una HEAD non ha
      ...
      const { error } = await sb()
        ...
        .limit(1);
      if (error) {
        if (isClmModuleNotPublished(error)) return false;
        throw new Error(error.message);
      }
      return true;
    },
...
export type ClmTemplatePatch = Partial<Omit<ClmTemplatePdf, "id" | "company_id">>;

export function useUpsertClmTemplatePdf() {
  const companyId = useEffectiveCompanyId();
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async (patch: ClmTemplatePatch): Promise<ClmTemplatePdf> => {
      if (!companyId) throw new Error("Company non disponibile");
      ...
      const { data, error } = await sb()
        ...
        .single();
      if (error) {
        if (isClmModuleNotPublished(error)) {
          throw new Error(
            "Il modulo Climatizzazione non è ancora pubblicato sul database: applica la migrazione (pubblica il modulo) per salvare il template.",
          );
        }
        throw new Error(error.message);
      }
