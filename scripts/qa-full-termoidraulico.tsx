...
mock.module("@/hooks/useTermoidraulicoProgetto", () => ({ getIdrTemplatePdf: () => { throw new Error("Template must be passed explicitly in QA"); } }));
...
const { createFullIdrTemplate, isFullIdrModuleId } = await import("../src/lib/moduli-vendita/fullIdrModules");
const { buildIdrModulePreview } = await import("../src/lib/moduli-vendita/fullIdrModules");
const { enrichTermoidraulicoPdf } = await import("../src/hooks/useTermoidraulicoPDF");
const { TermoidraulicoPDF } = await import("../src/components/termoidraulico/TermoidraulicoPDF");
const output = path.resolve(process.argv[2] || "../full-module-qa");
await mkdir(output, { recursive: true });
const moduleId = process.argv[3] || "ibrido";
if (!isFullIdrModuleId(moduleId)) throw new Error("Unknown Termoidraulico module");
const module = { id: moduleId };
const template = createFullIdrTemplate({
...
} as IdrTemplatePdf, module.id);
for (const variant of ["base", "sconto-100", "prezzo-manuale", "molte-voci"] as const) {
  const data = buildIdrModulePreview("qa-company", template, module.id);
  ...
  const enriched = await enrichTermoidraulicoPdf({ ...data, company: { name: "Impresa esempio", email: "info@example.invalid" } });
  const filename = path.join(output, `termoidraulico-${module.id}-${variant}.pdf`);
  await writeFile(filename, await renderToBuffer(<TermoidraulicoPDF {...enriched} />));
  const extracted = execFileSync("pdftotext", [filename, "-"], { encoding: "utf8" });
  const text = extracted.replace(/\s+/g, " ");
  const pagesWithImages = new Set(execFileSync("pdfimages", ["-list", filename], { encoding: "utf8" })
  ...
  for (const heading of ["Scelta per te", "Organizzare i lavori", "Funzione per funzione", "Da conservare", "Rese riconoscibili", "Pronti a partire"]) {
    if (!renderedPages.some((page, i) => page.includes(heading) && pagesWithImages.has(i + 1))) {
      throw new Error(`Missing actual photo in PDF: ${heading}`);
    }
  }
