-- ════════════════════════════════════════════════════════════════════════════
-- FIX KB External Sources tab — script monolitico paste-and-run
-- Generato 2026-05-09T15:37:09Z
-- Da incollare nel SQL Editor di Supabase Studio (project rsbrguhkodgnqfomrevo)
-- ════════════════════════════════════════════════════════════════════════════

-- ─── 1/2: 20260508250000_kb_multilang_and_external_sources.sql ─────────────
-- ═══════════════════════════════════════════════════════════════════════════
...
-- ─── 2/2: 20260508260000_kb_seed_external_sources.sql ──────────────────────
-- ═══════════════════════════════════════════════════════════════════════════
...
INSERT INTO supabase_migrations.schema_migrations (version, name, statements) VALUES
  ('20260508250000', 'kb_multilang_and_external_sources', ARRAY[]::text[]),
  ('20260508260000', 'kb_seed_external_sources',         ARRAY[]::text[])
ON CONFLICT (version) DO NOTHING;

-- ─── Verifica finale ────────────────────────────────────────────────────────
SELECT
  EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='public' AND table_name='ai_kb_external_sources') AS table_ok,
  EXISTS (SELECT 1 FROM information_schema.views WHERE table_schema='public' AND table_name='v_kb_external_sources_status') AS view_ok,
  EXISTS (SELECT 1 FROM pg_proc WHERE proname='kb_test_query_multilang' AND pronamespace='public'::regnamespace) AS rpc_multilang_ok,
  (SELECT COUNT(*)::int FROM public.ai_kb_external_sources) AS seed_rows;
