-- ════════════════════════════════════════════════════════════════════════════
-- MP-SILVIO-04 · Memoria operativa dei task (per-azienda)
-- Stato persistente dei compiti multi-step: aperti, passi fatti, cosa attende
-- conferma, cosa è bloccato. Memoria SNELLA: riferimenti, non prosa che cresce.
-- ════════════════════════════════════════════════════════════════════════════
CREATE TABLE IF NOT EXISTS public.silvio_task (
  ...
  origine_id     uuid,
  contesto       jsonb NOT NULL DEFAULT '{}'::jsonb,   -- SOLO riferimenti (email_id, fattura_id…)
...
CREATE INDEX IF NOT EXISTS idx_silvio_task_attivi ON public.silvio_task (company_id, stato)
