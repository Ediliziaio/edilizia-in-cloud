-- Fix: il trigger auto_create_scadenza_from_invoice() inseriva un valore esplicito
-- nella colonna scadenze.direction, che è GENERATED ALWAYS (calcolata da `tipo`:
-- CASE WHEN tipo='incasso_cliente' THEN 'entrata' ELSE 'uscita' END).
-- Postgres rifiuta l'INSERT con "cannot insert a non-DEFAULT value into column direction"
-- → ogni INSERT/UPDATE di public.invoices con scadenza falliva e rollbackava, rompendo
-- l'INTERO sync fatturazione (FIC/Aruba/...): fatture non aggiornate, scadenze mai create.
-- Fix: rimuovere `direction` (e la variabile v_direction) dall'INSERT — la calcola il DB.
...
BEGIN
  ...
  -- NB: NON inserire "direction": è GENERATED ALWAYS (la calcola il DB da `tipo`).
