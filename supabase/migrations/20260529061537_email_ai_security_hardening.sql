...
-- MP-EMAIL-AI · Hardening sicurezza (QA loop)
-- 1) FIX CRITICO: v_email_inbox_classified SECURITY INVOKER (rispetta RLS email_inbox).
--    DROP+CREATE perché email_inbox ha colonne nuove (embedding...) → e.* cambiato.
...
-- 2) Trigger functions: SET search_path = public
...
-- 3) Revoca EXECUTE da anon sulle RPC sensibili
