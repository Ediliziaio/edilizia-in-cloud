...
-- Prima: il client scaricava le righe grezze (orders, order_installments,
-- company_costs / marketing_contacts, appointments, marketing_opportunities)
-- e sommava in JS. Oltre al payload inutile, il cap righe di PostgREST
-- (default ~1000) TRONCAVA SILENZIOSAMENTE i totali sulle aziende grandi.
-- Le funzioni fanno GROUP BY mensile in Postgres: numeri esatti, ~KB di payload.
--
-- SECURITY INVOKER: girano coi permessi del chiamante → RLS si applica
-- normalmente (le stesse SELECT erano già permesse al client).
-- Bucket mensili in Europe/Rome per parità col vecchio client, che
-- bucketizzava con getMonth() nel fuso del browser (Italia).
...
as $$
  ...
  won as (
    -- "Contratti vinti" = opportunità status='won', bucket per updated_at
    -- (stessa definizione della KPI strip — vedi P1.4 nel client)
