-- ════════════════════════════════════════════════════════════════════════════
-- create_order_atomic — v2: persiste la SPINA MISURE sugli order_items
-- ----------------------------------------------------------------------------
-- Identica alla versione precedente, MA l'INSERT degli order_items ora porta
-- anche: family_id, axis_selections, misure_preventivo, measure_status — così
-- l'articolo su misura (creato anche da preventivo) conserva configurazione e
-- misura INIZIALE (measure_status='da_rilevare') fino al sopralluogo.
--
-- DIPENDENZA: richiede le colonne aggiunte da
--   20270630000000_order_items_measure_lifecycle.sql
-- (questa migration ha timestamp successivo → applicare in ordine: con
--  check_function_bodies=on il CREATE fallirebbe se le colonne non esistono).
-- I campi spina sono OPZIONALI: gli ordini senza prodotti su misura restano
-- identici (tutti i valori spina → NULL).
-- ════════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  LOOP
    ...
    INSERT INTO public.order_items (
      ...
      family_id, axis_selections, misure_preventivo, measure_status
    ) VALUES (
      ...
      -- ── spina misure: NULL se l'item non è su misura ──
