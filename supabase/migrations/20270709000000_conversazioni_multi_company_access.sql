-- =============================================================================
-- Conversazioni — accesso multi-azienda nelle RPC
-- =============================================================================
-- Bug: le RPC conversazioni_lista / conversazione_timeline / conversazioni_cerca
-- autorizzavano solo `p_company_id = get_user_company_id(auth.uid())` (azienda
-- PRIMARIA del profilo) o super_admin. Ma `get_user_company_id` NON considera
-- `multi_company_access` → un utente multi-azienda che apre l'inbox su una azienda
-- SECONDARIA riceveva "Accesso negato" → l'inbox mostrava "Inbox non ancora attivo".
--
-- Fix: helper `conversazioni_puo_accedere(company)` che accetta anche le aziende
-- raggiunte via multi_company_access (stesso criterio di requireCompanyAccess lato
-- edge function). Le 3 RPC usano il helper. CREATE OR REPLACE = idempotente.
-- =============================================================================
...
-- ── conversazioni_lista (auth via helper) ────────────────────────────────────
...
-- ── conversazione_timeline (auth via helper su company dell'entità) ──────────
...
-- ── conversazioni_cerca (auth via helper) ─────────────────────────────────────
