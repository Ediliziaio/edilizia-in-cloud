-- ============================================================================
-- Hardening cross-tenant RPC — Parte 2/3: guard nelle funzioni plpgsql.
-- ----------------------------------------------------------------------------
-- Inietta `PERFORM public.assert_company_access(p_company_id);` in testa al
-- corpo (dopo il primo BEGIN) di ogni funzione SECURITY DEFINER che restituisce
-- DATI o esegue scritture e che accetta p_company_id. Idempotente (salta le
-- funzioni gia' guardate). NON tocca i predicati booleani (is_*, *_allowed,
-- can_accountant_*) usati dentro le RLS, ne' i resolver di feature/piano
-- (resolve_company_feature/features, check_plan_limit) e i generatori di
-- progressivi: quelli sono a bassa sensibilita' e/o pericolosi da far sollevare.
-- ============================================================================
