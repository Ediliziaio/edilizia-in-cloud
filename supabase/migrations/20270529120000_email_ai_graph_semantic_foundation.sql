...
-- ────────────────────────────────────────────────────────────────────────────
-- Parte SICURA dello sprint strutturale: embedding (1536, allineato a brainEmbed
-- OpenAI text-embedding-3-small), audit accessi, estensione email_threads.
--
-- NON incluso (decisione Florin — vedi EMAIL-AI-MAP.md §6):
--   - Enum ruolo_aziendale (ceo/admin/reparto/operaio/readonly): i ruoli NON sono
--     su profiles ma nel sistema has_role/user_roles. Mappatura da concordare.
--   - thread_visibile() ACL per ruolo: dipende dalla decisione ruoli.
--   Per ora la ricerca è company-scoped (RLS esistente isola per company_id) +
--   esclude email is_personale. Nessuna regressione: è lo status quo + privacy.
-- ════════════════════════════════════════════════════════════════════════════
...
COMMENT ON COLUMN public.email_inbox.embedding IS 'MP-EMAIL-AI-04: embedding OpenAI text-embedding-3-small (1536). NULL=da generare. Mai per personali.';
COMMENT ON COLUMN public.email_inbox.is_personale IS 'MP-EMAIL-AI-04: marcata personale → esclusa da ricerca semantica e vista admin.';
