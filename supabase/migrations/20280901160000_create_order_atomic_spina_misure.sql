-- ============================================================================
-- create_order_atomic: la "spina misure" ARRIVA in tabella.
--
-- Il client (CreateOrder) manda da sempre per ogni articolo anche
-- product_code, family_id, axis_selections, misure_preventivo e
-- measure_status — ma l'INSERT dentro la funzione atomica non li ha mai
-- scritti: famiglia e misure si perdevano in silenzio su OGNI commessa
-- creata da questa strada (import contratto AI incluso, dove il listino
-- al mq calcola i prezzi proprio dalle misure).
--
-- Stessa funzione, INSERT order_items esteso con le 5 colonne. Nient'altro.
-- ============================================================================

CREATE OR REPLACE FUNCTION public.create_order_atomic(
  p_order_data jsonb,
  p_items jsonb DEFAULT '[]'::jsonb,
  p_salesperson jsonb DEFAULT NULL::jsonb,
