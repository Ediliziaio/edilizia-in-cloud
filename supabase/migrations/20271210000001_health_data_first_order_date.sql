-- ============================================================================
-- get_company_health_data: aggiunge first_order_date (MIN ordini).
-- Il KPI "giorni al primo ordine" usava last_order_date (MAX) → sovrastima
-- sistematica del time-to-first-value. Cambiare il tipo di ritorno di una
-- funzione TABLE richiede DROP + CREATE (CREATE OR REPLACE non basta).
-- Retro-compatibile: i campi esistenti restano identici, se ne aggiunge uno.
-- ============================================================================
DROP FUNCTION IF EXISTS public.get_company_health_data();

CREATE FUNCTION public.get_company_health_data()
RETURNS TABLE(
  company_id uuid,
  order_count bigint,
