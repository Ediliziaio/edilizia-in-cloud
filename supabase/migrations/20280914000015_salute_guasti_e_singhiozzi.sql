-- La pagina Salute contava come «job con errori» anche un solo giro fallito su
-- 1.440: l'11/09/2026 mostrava 16 job in rosso, di cui 11 erano lo stesso
-- singhiozzo del database del giorno prima (tutti alle 17:58) e i giri
-- successivi erano andati a buon fine. Ora ogni job porta l'esito dell'ultimo
-- giro: è un guasto solo se l'ultimo giro è fallito o se fallisce spesso
-- (almeno 3 volte e almeno il 5% dei giri in 48 ore); il resto sono
-- singhiozzi, contati a parte.
...
BEGIN
  ...
  FROM (SELECT j.jobname AS job, j.schedule, j.active AS attivo,
               ...
               (array_agg(d.status ORDER BY d.start_time DESC))[1] AS ultimo_esito
          FROM cron.job j
  ...
  SELECT COALESCE(jsonb_agg(t ORDER BY t.guasto DESC, t.falliti DESC), '[]'::jsonb) INTO v_falliti
  FROM (SELECT f.*, (f.ultimo_esito <> 'succeeded' OR (f.falliti >= 3 AND f.falliti * 20 >= f.esecuzioni)) AS guasto
          FROM (SELECT j.jobname AS job, count(*) FILTER (WHERE d.status <> 'succeeded') AS falliti,
                 ...
                 GROUP BY j.jobname
                HAVING count(*) FILTER (WHERE d.status <> 'succeeded') > 0) f) t;
  ...
  SELECT jsonb_build_object(
    ...
    'job_con_errori', (SELECT count(*) FROM jsonb_array_elements(v_falliti) e WHERE (e->>'guasto')::boolean),
    'job_singhiozzi', (SELECT count(*) FROM jsonb_array_elements(v_falliti) e WHERE NOT (e->>'guasto')::boolean),
