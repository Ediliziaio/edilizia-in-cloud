-- Gap #3 (Ciclo SDI fatturazione attiva): RPC read-only per Silvio.
-- Stato trasmissione SDI di fatture/note credito: per-stato (NS scartata, RC
-- consegnata, MC mancata consegna, AT trasmessa, null non inviata), elenco
-- SCARTATE (con errori → azionabili), conteggio da inviare, note credito recenti.
-- SECURITY DEFINER + scope company_id.
