-- get_email_campaign_recipients: dettaglio per-destinatario di una campagna
-- email (drill-down "vero gestionale" del tab Statistiche).
--
-- Ritorna una pagina di destinatari (email_logs ⋈ marketing_contacts) con
-- stato, timestamp degli eventi e total_rows (window) per la paginazione.
-- Filtri: stato (progressione: sent→delivered→opened→clicked, oppure
-- bounced/unsubscribed/spam/failed) e ricerca su email/nome.
--
-- Sicurezza: SECURITY DEFINER + guard super_admin O membro dell'azienda
-- (stesso pattern di email_enterprise_hardening).
...
AS $$
  ...
  WHERE el.company_id = p_company_id
    ...
    AND (
      ...
      OR p_company_id IN (
        -- profiles (user_roles non ha company_id; riferimento ambiguo col join)
