...
-- DM utente↔Silvio → il flag è di fatto per-utente. Additiva e idempotente.
-- (Già applicata in prod via Supabase MCP; questo file documenta lo stato del DB.)
ALTER TABLE public.internal_chat_channels
  ADD COLUMN IF NOT EXISTS silvio_pinned boolean NOT NULL DEFAULT false;

-- Ricrea la lista (aggiunge colonna `pinned` + ordina i fissati in cima).
