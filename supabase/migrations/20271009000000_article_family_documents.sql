-- Documenti / schede tecniche PDF per articolo di listino (article_families).
-- Multi-documento per prodotto (scheda tecnica, certificazioni, garanzia, …).
-- La colonna legacy article_families.pdf_scheda_url resta valida (singolo PDF):
-- questa tabella la affianca senza romperla.
...
-- RLS company-scoped (mirror di article_families)
...
CREATE POLICY afd_select ON public.article_family_documents
  FOR SELECT USING (
    company_id = get_user_company_id((SELECT auth.uid())) OR has_role((SELECT auth.uid()), 'super_admin'::app_role)
  );
...
-- ── Bucket PDF (pubblico come article-images: schede tecniche = materiale
--    commerciale, vanno mostrate nei preventivi/portale senza firma URL) ──────
INSERT INTO storage.buckets (id, name, public)
VALUES ('article-pdfs', 'article-pdfs', true)
ON CONFLICT (id) DO NOTHING;

-- Storage policies (mirror esatto di article-images, path {company_id}/…)
...
CREATE POLICY "Company users can upload article pdfs" ON storage.objects
  FOR INSERT WITH CHECK (
    bucket_id = 'article-pdfs' AND (storage.foldername(name))[1] = (get_user_company_id(auth.uid()))::text
  );

DROP POLICY IF EXISTS "Company users can update article pdfs" ON storage.objects;
CREATE POLICY "Company users can update article pdfs" ON storage.objects
  FOR UPDATE USING (
    bucket_id = 'article-pdfs' AND (storage.foldername(name))[1] = (get_user_company_id(auth.uid()))::text
  );

DROP POLICY IF EXISTS "Company users can delete article pdfs" ON storage.objects;
CREATE POLICY "Company users can delete article pdfs" ON storage.objects
  FOR DELETE USING (
    bucket_id = 'article-pdfs' AND (storage.foldername(name))[1] = (get_user_company_id(auth.uid()))::text
  );

DROP POLICY IF EXISTS "Super admins can manage all article pdfs" ON storage.objects;
CREATE POLICY "Super admins can manage all article pdfs" ON storage.objects
  FOR ALL USING (
    bucket_id = 'article-pdfs' AND has_role(auth.uid(), 'super_admin'::app_role)
  );
