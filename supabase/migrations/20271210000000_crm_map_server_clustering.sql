-- Mappa CRM — scala 100k+: centroidi provincia nel DB + RPC di clustering per viewport.
-- L aggregazione avviene lato server: il browser scarica poche centinaia di
-- celle/punti (per il riquadro visibile), mai tutti i 100k.
...
-- Cluster per griglia dentro un riquadro (bbox), precisione da zoom.
...
-- Punti singoli con dettaglio dentro un riquadro, cap p_limit (zoom alto + export).
