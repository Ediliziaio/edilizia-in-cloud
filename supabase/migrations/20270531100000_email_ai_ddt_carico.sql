-- ════════════════════════════════════════════════════════════════════════════
-- MP-EMAIL-AI-07 · Da DDT a carico di magazzino (bozza, mai movimento cieco)
-- ────────────────────────────────────────────────────────────────────────────
-- Riusa l'estrazione MP-06 (campi.righe sul DDT). Aggiunge la PROPOSTA di carico
-- con confronto vs ODA (purchase_orders/items): ordinato vs in bolla, scostamenti
-- evidenziati. La giacenza si aggiorna SOLO dopo conferma umana (step successivo).
-- ════════════════════════════════════════════════════════════════════════════
...
CREATE TABLE IF NOT EXISTS public.email_ddt_carico (
  ...
  -- [{descrizione,codice,qta_bolla,qta_ordine,scostamento,po_item_id,note}]
...
CREATE INDEX IF NOT EXISTS idx_email_ddt_carico_company_stato
  ON public.email_ddt_carico (company_id, stato, created_at DESC);
