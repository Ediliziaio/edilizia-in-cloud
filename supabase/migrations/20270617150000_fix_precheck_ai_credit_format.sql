-- BUG FIX: precheck_ai_credit usava format('...€%.2f...', ...) in stile C, ma il
-- format() di Postgres supporta solo %s/%I/%L/%%. Sui rami "saldo insufficiente" e
-- "hard cap superato" la funzione lanciava 22023 "unrecognized format() type
-- specifier", proprio quando un'azienda ha il credito basso. Il router AI
-- interpretava l'eccezione come "precheck non disponibile" e BLOCCAVA la chiamata
-- (errore fuorviante "Verifica credito AI non disponibile") invece di restituire il
-- corretto {ok:false, reason:'insufficient_balance'}.
-- Fix: %s + round(). Logica invariata.
