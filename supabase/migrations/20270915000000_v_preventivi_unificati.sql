-- ============================================================================
-- v_preventivi_unificati — vista di convergenza dei preventivi cross-modulo
-- ============================================================================
-- Unifica in un'unica sorgente normalizzata i tre tipi di preventivo:
...
--
-- Replica lato DB il merge che oggi UnifiedPreventiviList fa lato client
-- (stesse mappature di stato e stessa logica totale), così analytics/margini/
-- export/Silvio possono leggere TUTTI i preventivi da un solo posto, senza N
-- query e senza merge applicativo.
--
-- SICUREZZA: security_invoker = true → la vista eredita le RLS delle tabelle
-- sottostanti (isolamento per azienda garantito: ogni utente vede solo i
-- preventivi della propria azienda). NON usare security definer qui.
--
-- Mappa stato_unificato: bozza | in_corso | vinto | perso | altro
-- (mirror di mapClassicoStato / mapSerramentiStato / mapFotovoltaicoStato).
-- ============================================================================
...
COMMENT ON VIEW public.v_preventivi_unificati IS
  'Convergenza preventivi cross-modulo (classico/serramenti/fotovoltaico). security_invoker=true → eredita le RLS delle tabelle base (isolamento per azienda). Mirror server-side del merge di UnifiedPreventiviList.';
