-- ════════════════════════════════════════════════════════════════════════════
-- MP-SILVIO · Visibilità per AMBITO-DATI (segregazione per ruolo)
-- ────────────────────────────────────────────────────────────────────────────
-- Un dipendente marketing NON deve vedere il fatturato; un operaio NON deve
-- vedere dati finanziari. Ogni azione ha un `ambito`; una policy ruolo→ambiti
-- (server-side, non aggirabile dal tenant) decide cosa ciascun ruolo può
-- vedere/fare. Gate ESEGUITO dentro silvio_puo_eseguire → vale ovunque
-- (azioni, orchestratore, catalogo UI).
-- ════════════════════════════════════════════════════════════════════════════

ALTER TABLE public.silvio_azioni ADD COLUMN IF NOT EXISTS ambito text NOT NULL DEFAULT 'generale';

-- Classifica le azioni esistenti per ambito-dati
...
-- Policy ruolo→ambito (unione fra i ruoli posseduti, più permissivo). Hard-coded
-- server-side: il tenant non può ampliarla. admin/super = tutto.
...
-- Ambiti visibili all'utente corrente (per filtrare il catalogo dato al modello)
...
-- Re-crea silvio_puo_eseguire aggiungendo il GATE per ambito-dati
...
BEGIN
  ...
  IF NOT v_is_super THEN
    ...
    -- gate AMBITO-DATI (marketing non vede finanza, operai non vedono finanza, ecc.)
...
-- Catalogo: esponi anche l'ambito (return type cambia → serve DROP)
...
COMMENT ON FUNCTION public.silvio_ambito_consentito(text) IS 'MP-SILVIO sicurezza: visibilità dati per ruolo. Marketing/operai non vedono finanza. Non aggirabile dal tenant.';
