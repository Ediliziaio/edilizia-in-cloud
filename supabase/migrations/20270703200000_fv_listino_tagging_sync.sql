-- ═══════════════════════════════════════════════════════════════════════════
-- Listino come fonte unica: la macrocategoria viene classificata UNA volta
-- (tipologia preventivatore + componente FV) e i prodotti fluiscono da soli
-- nel preventivatore fotovoltaico (proiezione sincronizzata in articoli_native).
-- Caso reale: Green Energy Group — 638 prodotti a listino, 0 nel preventivatore.
-- ═══════════════════════════════════════════════════════════════════════════

-- 1) Classificazione della macrocategoria
...
-- Upsert della proiezione: serve un vincolo unico sul legame famiglia→componente.
...
-- 2) Sincronizzazione di UNA famiglia di listino verso articoli_native
...
BEGIN
  ...
  -- Fuori dal perimetro FV (macro non taggata / prodotto spento / eliminato):
  -- la proiezione, se esiste, viene disattivata (mai cancellata: può essere
  -- referenziata da preventivi esistenti).
  ...
  -- Parsing specs dal nome+descrizione (best-effort; l'azienda può correggere
  -- a mano e il valore manuale viene preservato ai sync successivi).
  ...
  DO UPDATE SET
    ...
    -- specs: il parse vince solo se trova qualcosa; il dato manuale resta.
...
-- 3) Trigger sui prodotti: ogni insert/modifica rilevante risincronizza.
...
-- Hard delete del prodotto: spegni la proiezione PRIMA che la FK vada a NULL.
...
-- 4) Trigger sulla macro: cambio di classificazione → risincronizza i prodotti.
...
-- 5) RPC di backfill/risincronizzazione manuale (bottone "Sincronizza ora").
...
DECLARE
  ...
  n int;
BEGIN
  -- Solo membri della company (o super_admin).
