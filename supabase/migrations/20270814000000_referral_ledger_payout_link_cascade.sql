-- Collega le righe di commissione (referral_commission_ledger) al payout che le
-- liquida e propaga lo stato del payout alle righe collegate.
--
-- PRIMA: generate_monthly_payouts portava le righe del ledger a 'approved' ma
-- NON impostava payout_id → quando l'admin segnava il payout 'paid', le righe
-- restavano 'approved' per sempre. Il partner vedeva "In attesa/Approvato" anche
-- dopo essere stato pagato. Ora:
--   1. generate_monthly_payouts imposta ledger.payout_id sulle righe approvate.
--   2. un trigger su referral_payouts propaga: paid → ledger 'paid';
--      rejected → ledger torna 'pending' (e si scollega) così è ri-pagabile.

-- ── 1. generate_monthly_payouts: linka il ledger al payout ────────────────────
...
BEGIN
  ...
  LOOP
    IF NOT EXISTS (
      ...
      -- Approva le righe del periodo E le collega a questo payout.
...
-- ── 2. Trigger di propagazione stato payout → ledger ─────────────────────────
...
AS $function$
BEGIN
  -- Payout pagato → le commissioni collegate diventano 'paid'.
  ...
  -- Payout rifiutato → le commissioni tornano 'pending' e si scollegano,
  -- così possono rientrare in un payout futuro.
