-- MP-CG-17 — Estendi cg_get_dettaglio_voce_mese: supporto mese=NULL (anno intero)
--
-- Quando p_mese è NULL, il filtro per mese viene rimosso, restituendo
-- TUTTI i movimenti dell'anno per la voce indicata. Permette il drill-down
-- dalle viste Confronto multi-anno cliccando su una colonna anno.

CREATE OR REPLACE FUNCTION public.cg_get_dettaglio_voce_mese(
  p_company_id uuid    DEFAULT NULL,
  p_anno       int     DEFAULT extract(year from current_date)::int,
  p_mese       int     DEFAULT NULL,        -- NULL = tutto l'anno
    ...
    WITH base AS (
      ...
      FROM (
        ...
        WHERE bt.company_id = v_company_id AND bt.amount < 0
          AND bt.id NOT IN (SELECT br.transaction_id FROM public.bank_reconciliations br WHERE br.invoice_id IS NOT NULL)
