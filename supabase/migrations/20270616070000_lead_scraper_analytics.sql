-- Lead Scraper · Analytics/ROI per fonte + utilizzo provider API
-- Viste read-only per il pannello analytics del super_admin. Usano
-- security_invoker così rispettano le policy RLS già presenti sulle tabelle.

-- ── ROI per fonte: imbuto + qualità media + contattabilità ───────────────────
...
-- ── Utilizzo provider API (per leggere il consumo / stimare i costi) ──────────
