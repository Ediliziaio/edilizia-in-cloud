...
-- MP-SILVIO-SA-PROBLEMS-01: centralina problemi cross-tenant + motore pacchetti (READ-ONLY)
...
-- mapping configurabile area → pacchetto AEDIX
...
INSERT INTO public.sa_problem_to_package (area, pacchetto) VALUES
 ('finanza','Finanza / Cashflow (CG)'),
 ('controllo_gestione','Controllo di Gestione'),
 ('marketing','Marketing Edile'),
 ('vendita','Vendita / Sales OS'),
 ('recruiting','Recruiting'),
 ('legale','Gestione Legale / TutelAI'),
 ('operativo','Upgrade piano EiC / moduli'),
 ('tecnico','Supporto tecnico / integrazioni'),
 ('prodotto','Upgrade piano EiC / moduli')
ON CONFLICT (area) DO NOTHING;

-- map alert_type → area problema
...
-- map area intel (sa_conversation_signals) → area problema
...
-- fan-out per una azienda: snapshot pulito da fonti esistenti (no LLM)
...
BEGIN
  ...
  -- fonte 2: segnali conversazione (difficolta/lamentela, ultimi 90gg)
  INSERT INTO public.sa_company_problems (company_id, area, problema, severita, fonte, da_quanti_giorni, dettaglio_anonimo)
  SELECT s.company_id, public.sa_signalarea_to_problemarea(s.area),
         'Dolore dichiarato in chat: ' || s.area,
  ...
  GROUP BY s.company_id, public.sa_signalarea_to_problemarea(s.area)
  ON CONFLICT (company_id, area, problema) DO NOTHING;

  RETURN (SELECT count(*) FROM public.sa_company_problems WHERE company_id = p_company_id);
END $$;

-- fan-out per tutte le aziende attive (chiamato dal cron)
...
-- problem-score per azienda (gravità × anzianità), distinto dal churn-score
CREATE OR REPLACE VIEW public.sa_company_problem_score AS
  SELECT company_id,
    LEAST(100, round(sum(
      (CASE severita WHEN 'critical' THEN 25 WHEN 'warning' THEN 10 ELSE 3 END)
      * (1 + coalesce(da_quanti_giorni,0) / 30.0)
    ))::int) AS problem_score,
    count(*)::int AS n_problemi,
    count(*) FILTER (WHERE severita = 'critical')::int AS n_critici
  FROM public.sa_company_problems
  GROUP BY company_id;

-- ── RPC readonly super_admin ────────────────────────────────────────────────
...
BEGIN
  ...
  FROM (
    SELECT jsonb_build_object(
      'company_id', co.id, 'azienda', co.name, 'problem_score', sc.problem_score,
      ...
      'problemi', (SELECT coalesce(jsonb_agg(jsonb_build_object('area', p.area, 'problema', p.problema,
                       'severita', p.severita, 'da_giorni', p.da_quanti_giorni,
                       'pacchetto', pk.pacchetto) ORDER BY CASE p.severita WHEN 'critical' THEN 0 WHEN 'warning' THEN 1 ELSE 2 END), '[]'::jsonb)
                   FROM public.sa_company_problems p
                   LEFT JOIN public.sa_problem_to_package pk ON pk.area = p.area AND pk.attivo
                   WHERE p.company_id = co.id
                     AND (coalesce(p_area,'tutte') = 'tutte' OR p.area = p_area)
                     AND (CASE p.severita WHEN 'critical' THEN 3 WHEN 'warning' THEN 2 ELSE 1 END) >= v_min
                   LIMIT 6)
    ) AS r
    FROM public.sa_company_problem_score sc
    JOIN public.companies co ON co.id = sc.company_id
    WHERE EXISTS (SELECT 1 FROM public.sa_company_problems p2
                  WHERE p2.company_id = co.id
    ...
    ORDER BY sc.problem_score DESC
...
BEGIN
  IF NOT public.is_super_admin(auth.uid()) THEN RAISE EXCEPTION 'forbidden' USING ERRCODE='42501'; END IF;
  SELECT jsonb_build_object(
    'company_id', co.id, 'azienda', co.name,
    'problem_score', coalesce(sc.problem_score,0), 'n_problemi', coalesce(sc.n_problemi,0), 'n_critici', coalesce(sc.n_critici,0),
    'top_problemi', (SELECT coalesce(jsonb_agg(jsonb_build_object('area', p.area, 'problema', p.problema,
                        'severita', p.severita, 'da_giorni', p.da_quanti_giorni, 'fonte', p.fonte,
                        'pacchetto', pk.pacchetto) ORDER BY CASE p.severita WHEN 'critical' THEN 0 WHEN 'warning' THEN 1 ELSE 2 END, p.da_quanti_giorni DESC), '[]'::jsonb)
                     FROM public.sa_company_problems p
                     LEFT JOIN public.sa_problem_to_package pk ON pk.area = p.area AND pk.attivo
  ...
  LEFT JOIN public.sa_company_problem_score sc ON sc.company_id = co.id
...
BEGIN
  ...
  FROM (
    SELECT pk.pacchetto, count(DISTINCT p.company_id)::int AS n
    FROM public.sa_company_problems p JOIN public.sa_problem_to_package pk ON pk.area = p.area AND pk.attivo
    GROUP BY pk.pacchetto
  ) t;
  RETURN v;
END $$;
...
-- cron: fan-out notturno 03:45 UTC (dopo l'intel delle 03:15)
