-- Tetti · Verticale preventivo (clone di Ristrutturazione, namespace tet_*).
-- Greenfield: un'unica migrazione con lo schema FINALE (base + builder + personalizzazione
-- + fonte) e gli adattamenti bagno (numero_falde, iva default 10). ADDITIVA/IDEMPOTENTE.
...
-- Progetto (hub) ──────────────────────────────────────────────────────────────
-- Adattamenti bagno: numero_falde (nuovo), iva_pct default 10 (tetti); superficie
...
COMMENT ON COLUMN public.tet_computo_voci.fonte IS 'Prezzario regionale di provenienza della voce (per citazione base d''asta). NULL = voce libera/listino.';
