-- La bolla dice il nome E il numero da cui e' partito il messaggio.
-- "da Giusy" non basta: chi legge deve sapere quale scheda WhatsApp e' stata
-- usata, perche' e' il numero che il destinatario si vede arrivare.
...
UNION ALL
 SELECT 'contatto'::text AS entita_tipo,
    ...
    NULLIF(TRIM(BOTH FROM CASE
        WHEN onum.display_name IS NOT NULL AND onum.numero IS NOT NULL
            THEN onum.display_name || ' · ' || onum.numero
