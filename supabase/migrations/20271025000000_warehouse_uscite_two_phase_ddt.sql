...
CREATE OR REPLACE FUNCTION public.register_warehouse_uscita(
  ...
  p_vettore jsonb DEFAULT NULL, p_note text DEFAULT NULL
)
...
CREATE OR REPLACE FUNCTION public.create_ddt_from_uscita(
  p_uscita_id uuid, p_ddt_extra jsonb DEFAULT NULL
)
...
DECLARE
  ...
  v_year integer := EXTRACT(YEAR FROM CURRENT_DATE)::integer;
  v_numero_ddt text; v_progressivo integer; v_documento_id uuid; v_indirizzo text;
BEGIN
  IF auth.uid() IS NULL THEN RAISE EXCEPTION 'Unauthorized'; END IF;
  IF v_company_id IS NULL THEN RAISE EXCEPTION 'No company context'; END IF;
  ...
  IF v_u.documento_id IS NOT NULL THEN
    RETURN QUERY SELECT v_u.documento_id, (SELECT numero FROM public.documenti_fiscali WHERE id = v_u.documento_id); RETURN;
  END IF;
