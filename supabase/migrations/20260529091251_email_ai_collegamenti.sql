...
CREATE POLICY email_collegamenti_staff_read ON public.email_collegamenti FOR SELECT TO authenticated
...
CREATE POLICY email_collegamenti_staff_write ON public.email_collegamenti FOR ALL TO authenticated
  USING (company_id = public.get_effective_company_id() AND public.is_email_staff_interno())
...
CREATE POLICY email_collegamenti_service_all ON public.email_collegamenti FOR ALL TO service_role USING (true) WITH CHECK (true);
DROP POLICY IF EXISTS email_collegamenti_super_admin ON public.email_collegamenti;
CREATE POLICY email_collegamenti_super_admin ON public.email_collegamenti FOR ALL TO authenticated USING (public.is_super_admin(auth.uid()));
