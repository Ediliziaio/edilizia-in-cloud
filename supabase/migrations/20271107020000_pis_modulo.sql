-- Piscine · Verticale preventivo (clone di Ristrutturazione, namespace pis_*).
-- Greenfield: un'unica migrazione con lo schema FINALE (base + builder + personalizzazione
-- + fonte) e gli adattamenti piscine (tipo piscina, tipo costruzione). ADDITIVA/IDEMPOTENTE.
-- RLS company-scoped (super_admin OR company match). LOCALE: applicata in pubblicazione.

-- Listino lavorazioni aziendale ──────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS public.pis_listino_capitoli (
...
CREATE TABLE IF NOT EXISTS public.pis_listino_voci (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  company_id uuid NOT NULL REFERENCES public.companies(id) ON DELETE CASCADE,
  capitolo_id uuid REFERENCES public.pis_listino_capitoli(id) ON DELETE SET NULL,
  codice text,
...
-- Progetto (hub) ──────────────────────────────────────────────────────────────
-- Adattamenti piscine: tipo_piscina + tipo_costruzione (interrata/fuori terra,
-- cemento/vetroresina/pannelli); superficie (specchio acqua) e tipo intervento
-- riusano immobile_superficie_mq / tipo_intervento (DRY). massimale_detrazione opz.
CREATE TABLE IF NOT EXISTS public.pis_progetti (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  company_id uuid NOT NULL REFERENCES public.companies(id) ON DELETE CASCADE,
  code text,
...
DO $$ BEGIN
  ALTER TABLE public.pis_progetti
    ADD CONSTRAINT pis_progetti_stato_chk CHECK (
...
CREATE TABLE IF NOT EXISTS public.pis_computo_voci (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  progetto_id uuid NOT NULL REFERENCES public.pis_progetti(id) ON DELETE CASCADE,
  company_id uuid NOT NULL REFERENCES public.companies(id) ON DELETE CASCADE,
...
COMMENT ON COLUMN public.pis_computo_voci.fonte IS 'Prezzario regionale di provenienza della voce (per citazione base d''asta). NULL = voce libera/listino.';

-- Media ──────────────────────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS public.pis_progetti_media (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  progetto_id uuid NOT NULL REFERENCES public.pis_progetti(id) ON DELETE CASCADE,
  company_id uuid NOT NULL REFERENCES public.companies(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS public.pis_template_pdf (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
...
-- Indici
CREATE INDEX IF NOT EXISTS idx_pis_progetti_company ON public.pis_progetti(company_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_pis_computo_progetto ON public.pis_computo_voci(progetto_id, ordine);
CREATE INDEX IF NOT EXISTS idx_pis_listino_voci_company ON public.pis_listino_voci(company_id, capitolo_id, ordine);
CREATE INDEX IF NOT EXISTS idx_pis_media_progetto ON public.pis_progetti_media(progetto_id, ordine);

-- RLS (super_admin OR company_id = get_user_company_id) su tutte le tabelle
DO $$
DECLARE t text;
BEGIN
  FOREACH t IN ARRAY ARRAY['pis_listino_capitoli','pis_listino_voci','pis_progetti','pis_computo_voci','pis_progetti_media','pis_template_pdf'] LOOP
    EXECUTE format('ALTER TABLE public.%I ENABLE ROW LEVEL SECURITY', t);
    EXECUTE format($q$DROP POLICY IF EXISTS "pis super admin all" ON public.%I$q$, t);
    EXECUTE format($q$CREATE POLICY "pis super admin all" ON public.%I FOR ALL
      USING (has_role(auth.uid(),'super_admin'::app_role)) WITH CHECK (has_role(auth.uid(),'super_admin'::app_role))$q$, t);
    EXECUTE format($q$DROP POLICY IF EXISTS "pis company members" ON public.%I$q$, t);
    EXECUTE format($q$CREATE POLICY "pis company members" ON public.%I FOR ALL TO authenticated
