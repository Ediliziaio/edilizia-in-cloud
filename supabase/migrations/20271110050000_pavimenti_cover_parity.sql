-- Pavimenti cover parity — porta su pav_template_pdf il sistema di
...
--
-- pav_template_pdf oggi ha solo i campi cover "legacy" (cover_title, cover_subtitle,
...
-- I vecchi campi cover_* restano per retrocompatibilità: l'editor/PDF preferiscono
-- pdf_cover_* e ripiegano su cover_* quando il nuovo valore è assente.
--
-- Idempotente: ADD COLUMN IF NOT EXISTS su ogni colonna → applicabile più volte.
ALTER TABLE public.pav_template_pdf
