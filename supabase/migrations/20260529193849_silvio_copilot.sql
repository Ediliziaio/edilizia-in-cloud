...
-- MP-SILVIO-COPILOT-01 · Blocco 6
CREATE OR REPLACE FUNCTION public.silvio_tool_guida_a(
  p_company_id uuid, p_operazione text, p_precompila boolean DEFAULT false, p_entita_id uuid DEFAULT NULL
) RETURNS jsonb LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path TO 'public' AS $$
DECLARE
  v_op text := lower(coalesce(p_operazione, ''));
  v_spiegazione text; v_route text; v_params jsonb := '{}'::jsonb;
BEGIN
  ...
  IF v_op ~ 'nota.*credito|storno|stornare' THEN
    ...
    v_route := '/azienda/fatturazione'; v_params := jsonb_build_object('action', 'nota-credito');
  ...
  ELSIF v_op ~ 'incass|prima nota|registra.*pagam+ento|registra.*incasso' THEN
    ...
    v_route := '/azienda/prima-nota'; v_params := jsonb_build_object('action', 'nuovo-incasso');
  ...
  ELSIF v_op ~ 'fattur' THEN
    ...
    v_route := '/azienda/fatturazione'; v_params := jsonb_build_object('action', 'nuova-fattura');
  ...
  END IF;
  RETURN jsonb_build_object('ok', true, 'operazione', p_operazione, 'spiegazione', v_spiegazione,
    'route_path', v_route, 'query_params', v_params, 'precompila', coalesce(p_precompila, false));
END $$;
...
CREATE POLICY silvio_decision_rules_company ON public.silvio_decision_rules FOR ALL TO authenticated
  USING (company_id = public.get_effective_company_id()) WITH CHECK (company_id = public.get_effective_company_id());
...
CREATE OR REPLACE FUNCTION public.silvio_match_decision_rule(
  p_company_id uuid, p_dominio text, p_payload jsonb
) RETURNS jsonb LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path TO 'public' AS $$
DECLARE
  r record; v_field text; v_op text; v_match boolean; v_lhs numeric; v_rhs numeric;
BEGIN
  ...
  LOOP
    ...
    BEGIN
      ...
      v_match := CASE v_op
        WHEN '<' THEN v_lhs < v_rhs WHEN '<=' THEN v_lhs <= v_rhs
        WHEN '>' THEN v_lhs > v_rhs WHEN '>=' THEN v_lhs >= v_rhs
        WHEN '!=' THEN v_lhs <> v_rhs ELSE v_lhs = v_rhs END;
    EXCEPTION WHEN OTHERS THEN
      v_match := CASE v_op WHEN '!=' THEN (p_payload->>v_field) <> (r.condizione->>'value')
                 ELSE (p_payload->>v_field) = (r.condizione->>'value') END;
    END;
...
CREATE OR REPLACE FUNCTION public.silvio_tool_salva_regola_decisionale(
  p_company_id uuid, p_user_id uuid, p_dominio text, p_condizione jsonb, p_azione text, p_origine text DEFAULT 'esplicito'
