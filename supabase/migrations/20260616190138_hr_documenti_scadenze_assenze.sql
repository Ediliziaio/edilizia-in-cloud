...
-- Funzione trigger updated_at (idempotente)
CREATE OR REPLACE FUNCTION public.hr_set_updated_at()
RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN NEW.updated_at = now(); RETURN NEW; END; $$;

-- Stato scadenza calcolato (valido / in_scadenza / scaduto / senza_scadenza)
...
-- ── Documenti / certificati / scadenze ──────────────────────────────────────
...
-- ── RLS: admin azienda full, dipendente self-read ───────────────────────────
...
-- ── Storage bucket privato per i file (contratti, certificati) ──────────────
INSERT INTO storage.buckets (id, name, public) VALUES ('hr-documenti','hr-documenti', false)
  ON CONFLICT (id) DO NOTHING;

-- Path convention: {company_id}/{hr_profilo_id}/{uuid}-{filename}
