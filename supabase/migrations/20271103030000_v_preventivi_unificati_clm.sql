-- v_preventivi_unificati — aggiunta del verticale Climatizzazione (clm_progetti).
...
-- pubblicazione del modulo Climatizzazione.
...
COMMENT ON VIEW public.v_preventivi_unificati IS
  'Convergenza preventivi cross-modulo: quotes (classico) + sr_progetti (serramenti) + fv_progetti (fotovoltaico) + rst_progetti (ristrutturazione) + bgn_progetti (bagni) + tet_progetti (tetti) + clm_progetti (climatizzazione). security_invoker eredita le RLS delle tabelle sorgente.';
