-- Sicurezza: 30 viste analitiche/finanziarie/cross-tenant erano SECURITY DEFINER
-- (bypassano RLS) e leggibili da anon/authenticated → leak reale (margini, ricavi,
-- costi, dati cross-tenant esposti a chiunque avesse la anon key).
-- Le converto a security_invoker: l'accesso ora rispetta privilegi + RLS del
-- chiamante. Le RPC admin (SECURITY DEFINER) le leggono come definer (bypass) →
-- nessuna regressione. Nessuna è usata via client Supabase (verificato grep
-- frontend + edge). Escluse le 3 pubbliche-by-design: ai_personas_public,
