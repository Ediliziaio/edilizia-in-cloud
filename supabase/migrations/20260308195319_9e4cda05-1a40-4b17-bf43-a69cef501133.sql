...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'openTickets',        (SELECT COUNT(*)::INT FROM tickets t
                           WHERE t.company_id = p_company_id
                             AND t.status NOT IN ('chiuso','closed','resolved','completato')),
  ...
  SELECT jsonb_build_object(
    ...
    'thisMonthOutflow', (SELECT COALESCE(SUM(amount),0) FROM company_costs
                         WHERE company_id = p_company_id AND due_date BETWEEN v_month_start AND v_month_end),
    ...
    'nextMonth',        (SELECT COALESCE(SUM(total_amount),0) FROM orders
                      ...
                      - (SELECT COALESCE(SUM(amount),0) FROM company_costs
                         WHERE company_id = p_company_id AND due_date BETWEEN v_month_end AND (v_month_end + INTERVAL '1 month'))
  ) INTO v_cash_flow;
  ...
  FROM (
    SELECT
      ...
      GREATEST(0, EXTRACT(DAY FROM (o.expected_date - v_now))::INT)     AS "daysLeft"
    FROM order_items oi
    JOIN orders o ON o.id = oi.order_id
    LEFT JOIN profiles p ON p.id = o.customer_id
    WHERE o.company_id = p_company_id
      AND o.expected_date IS NOT NULL
      AND o.expected_date BETWEEN v_now AND v_week_end
    ...
    'companyCosts', (
      SELECT COALESCE(jsonb_agg(cr ORDER BY cr."dueDate"), '[]'::jsonb) FROM (
        SELECT cc.name, cc.amount,
               ...
               GREATEST(0, EXTRACT(DAY FROM (cc.due_date - v_now))::INT) AS "daysLeft"
        FROM company_costs cc
        WHERE cc.company_id = p_company_id
          AND cc.is_paid = false
          AND cc.due_date BETWEEN v_now AND v_week_end
  ...
  FROM (
    SELECT
      ...
      COALESCE((SELECT SUM(amount) FROM company_costs
                WHERE company_id = p_company_id AND date_trunc('month', due_date) = m.month), 0) AS uscite
  ...
  -- AGING RECEIVABLES
  SELECT jsonb_build_object(
    'overdue',   COALESCE(SUM(CASE WHEN expected_date < v_now                              AND COALESCE(balance_amount,0)>0 THEN balance_amount ELSE 0 END),0),
    'thisWeek',  COALESCE(SUM(CASE WHEN expected_date BETWEEN v_now AND v_week_end          AND COALESCE(balance_amount,0)>0 THEN balance_amount ELSE 0 END),0),
    'thisMonth', COALESCE(SUM(CASE WHEN expected_date BETWEEN v_week_end AND v_month_end    AND COALESCE(balance_amount,0)>0 THEN balance_amount ELSE 0 END),0),
    'future',    COALESCE(SUM(CASE WHEN expected_date > v_month_end                         AND COALESCE(balance_amount,0)>0 THEN balance_amount ELSE 0 END),0)
  ) INTO v_aging
  FROM orders
  WHERE company_id = p_company_id;
