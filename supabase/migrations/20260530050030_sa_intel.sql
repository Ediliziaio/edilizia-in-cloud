...
CREATE POLICY sa_conv_signals_super ON public.sa_conversation_signals FOR SELECT TO authenticated USING (public.is_super_admin(auth.uid()));
...
CREATE TABLE IF NOT EXISTS public.sa_company_intel (
  ...
  top_aree jsonb NOT NULL DEFAULT '[]'::jsonb,
  dolore_principale text,
  intensita int NOT NULL DEFAULT 0,
  servizio_consigliato text,
  motivazione text,
  health_score int,
  priorita_contatto int NOT NULL DEFAULT 0
);
...
CREATE POLICY sa_company_intel_super ON public.sa_company_intel FOR SELECT TO authenticated USING (public.is_super_admin(auth.uid()));
...
RETURNS text LANGUAGE sql IMMUTABLE AS $$
  SELECT CASE p_area
    WHEN 'margini' THEN 'controllo_gestione' WHEN 'cashflow' THEN 'controllo_gestione'
    WHEN 'fatturazione' THEN 'controllo_gestione' WHEN 'marketing' THEN 'marketing_edile'
    WHEN 'vendita' THEN 'vendita' WHEN 'preventivi' THEN 'vendita'
    ELSE 'controllo_gestione' END;
$$;
...
BEGIN
  ...
  v_health := NULL; v_prio := v_intensita;
...
BEGIN
  ...
  FROM (SELECT jsonb_build_object('company_id', i.company_id, 'azienda', co.name, 'dolore_principale', i.dolore_principale,
          'servizio_consigliato', i.servizio_consigliato, 'intensita', i.intensita, 'priorita_contatto', i.priorita_contatto,
        ...
        WHERE (coalesce(p_servizio,'tutti') = 'tutti' OR i.servizio_consigliato = p_servizio) AND i.intensita > 0
        ORDER BY i.priorita_contatto DESC LIMIT GREATEST(coalesce(p_limite,15), 1)) q;
  RETURN v;
END $$;
...
BEGIN
  IF NOT public.is_super_admin(auth.uid()) THEN RAISE EXCEPTION 'forbidden' USING ERRCODE = '42501'; END IF;
  SELECT jsonb_build_object('company_id', i.company_id, 'azienda', co.name, 'aggiornato_al', i.aggiornato_al, 'top_aree', i.top_aree,
    'dolore_principale', i.dolore_principale, 'intensita', i.intensita, 'servizio_consigliato', i.servizio_consigliato,
    'motivazione', i.motivazione, 'health_score', i.health_score, 'priorita_contatto', i.priorita_contatto) INTO v
  FROM public.sa_company_intel i JOIN public.companies co ON co.id = i.company_id WHERE i.company_id = p_company_id;
...
BEGIN
  IF NOT public.is_super_admin(auth.uid()) THEN RAISE EXCEPTION 'forbidden' USING ERRCODE = '42501'; END IF;
  SELECT coalesce(jsonb_agg(jsonb_build_object('area', area, 'peso_totale', peso_tot, 'aziende', aziende) ORDER BY peso_tot DESC), '[]'::jsonb) INTO v
  FROM (SELECT area, sum(peso)::int AS peso_tot, count(DISTINCT company_id)::int AS aziende FROM public.sa_conversation_signals
        WHERE giorno > CURRENT_DATE - GREATEST(coalesce(p_giorni,30), 1) GROUP BY area) t;
  RETURN v;
END $$;
