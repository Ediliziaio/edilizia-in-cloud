...
CREATE TABLE IF NOT EXISTS public.silvio_audit (
  ...
  motivo        text,                              -- il "perché": trigger/richiesta/playbook
  origine       text,                              -- richiesta|trigger|playbook|canale
  origine_id    uuid,
  per_conto_di  uuid REFERENCES auth.users(id),    -- l'utente per cui Silvio ha agito
  approvata_da  uuid REFERENCES auth.users(id),    -- chi ha confermato (se 'confermata')
  reversibile   boolean NOT NULL DEFAULT false,
  ref_audit_id  uuid,                              -- per le voci 'annullata': punta all'azione annullata
...
-- append-only: nessun UPDATE/DELETE (l'undo è una NUOVA voce 'annullata')
...
-- ── Coda conferme: azioni 'conferma' in attesa, NESSUN effetto prima dell'ok ─
...
-- Risolvi una voce in coda (approva/rifiuta/modifica). NON esegue l'azione:
-- l'esecuzione la fa l'orchestratore (MP-SILVIO-02-exec) leggendo lo stato.
...
-- audit: staff legge la propria azienda; SOLO service_role scrive (no forge dallo staff)
