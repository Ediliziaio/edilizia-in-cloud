-- v_preventivi_unificati — aggiunta del verticale Tetti (tet_progetti).
...
-- ristrutturazione) e aggiungendo il branch Tetti con la STESSA logica CASE di
...
-- pubblicazione del modulo Tetti.
...
COMMENT ON VIEW public.v_preventivi_unificati IS
  'Convergenza preventivi cross-modulo: quotes (classico) + sr_progetti (serramenti) + fv_progetti (fotovoltaico) + rst_progetti (ristrutturazione) + bgn_progetti (bagni) + tet_progetti (tetti). security_invoker eredita le RLS delle tabelle sorgente.';
