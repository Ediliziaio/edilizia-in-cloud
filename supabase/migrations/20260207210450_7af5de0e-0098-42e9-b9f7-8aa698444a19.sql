...
ALTER TABLE public.suppliers ADD COLUMN IF NOT EXISTS vat_rate numeric DEFAULT 22;

-- Add vat_rate column to order_items table (inherits from supplier but can be overridden)
ALTER TABLE public.order_items ADD COLUMN IF NOT EXISTS vat_rate numeric DEFAULT 22;

-- Add vat_rate column to external_teams table (default VAT rate for the team)
ALTER TABLE public.external_teams ADD COLUMN IF NOT EXISTS vat_rate numeric DEFAULT 22;

-- Add vat_rate column to order_external_teams table (inherits from team but can be overridden)
ALTER TABLE public.order_external_teams ADD COLUMN IF NOT EXISTS vat_rate numeric DEFAULT 22;
