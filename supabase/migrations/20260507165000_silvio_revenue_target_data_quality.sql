-- Silvio revenue target hardening:
-- add explicit data-quality warnings so the AI does not treat missing company
-- setup data as a valid zero-cost cash scenario.
...
COMMENT ON FUNCTION public.silvio_tool_revenue_needed_next_month(uuid, date, numeric) IS
  'TOOL Silvio: calcola gap di cassa e scenari operativi distinguendo fatturato, incasso, costi variabili e qualita dati aziendali.';
