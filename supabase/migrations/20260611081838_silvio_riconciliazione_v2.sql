...
-- Round 2: forecast 90d + revenue_needed + commesse_attive via vista unificata
...
BEGIN
  ...
  FOR i IN 0..(p_weeks - 1) LOOP
    ...
    -- Incassi previsti: rate non pagate dalla VISTA UNIFICATA
    -- (rate DB > colonne legacy, mai entrambe), delay applicato.
    WITH expected_in AS (
    ...
    BEGIN
      ...
      WHERE company_id = p_company_id
        AND data_fattura + interval '60 days' BETWEEN v_week_start AND v_week_end
...
BEGIN
  ...
  SELECT COUNT(*) INTO v_open_costs_count
  FROM public.company_costs
  WHERE company_id = p_company_id AND COALESCE(is_paid, false) = false;

  SELECT COALESCE(SUM(amount), 0) INTO v_open_costs_month
  FROM public.company_costs
  WHERE company_id = p_company_id AND COALESCE(is_paid, false) = false
    AND due_date BETWEEN v_month_start AND v_month_end;

  SELECT COALESCE(SUM(amount), 0) INTO v_overdue_costs
  FROM public.company_costs
  WHERE company_id = p_company_id AND COALESCE(is_paid, false) = false
    AND due_date < CURRENT_DATE;

  SELECT COALESCE(SUM(amount), 0) INTO v_open_costs_no_date
  FROM public.company_costs
  WHERE company_id = p_company_id AND COALESCE(is_paid, false) = false
