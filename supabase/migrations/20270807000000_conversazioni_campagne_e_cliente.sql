-- =============================================================================
-- Conversazioni inbox — campagne email marketing + comunicazioni CLIENTE
-- =============================================================================
-- Gap chiusi (richiesta: "vedere in conversazioni se il cliente risponde,
-- aperture/cliccate, e tutte le comunicazioni anche sulla scheda cliente"):
--
--   8) Email di CAMPAGNA marketing inviate al contatto (email_logs ⋈
--      email_campaigns): prima le campagne non comparivano nella conversazione
--      (vivono in email_logs, non in email_outbox). Il testo include lo stato
--      di engagement: "· ✓ aperta" / "· ✓ cliccata".
--   9) Email IN ARRIVO dal CLIENTE (email_inbox ⋈ profiles per email):
--      prima la timeline cliente vedeva solo customer_messages (portale).
--  10) Email INVIATE al CLIENTE (email_outbox ⋈ profiles per email).
--  11) WhatsApp IN ARRIVO dal CLIENTE (whatsapp_messages per telefono).
--  12) SMS da/verso CLIENTE (sms_logs per telefono).
--
-- Filtro cliente: profiles.customer_type IS NOT NULL (i dipendenti non hanno
-- customer_type → esclusi). Le 7 branch esistenti sono riprodotte invariate
-- (CREATE OR REPLACE VIEW richiede la definizione completa).
-- =============================================================================
