-- ════════════════════════════════════════════════════════════════════════════
-- MP-ADS-04 GAP-2 · A/B test per campagne Meta (tracciamento)
-- ────────────────────────────────────────────────────────────────────────────
-- La tabella campaign_variants esistente è per le email CRM (crm_campaigns), NON
-- per le ads. Questa traccia gli A/B test tra una campagna base e la sua variante.
-- RLS rispecchia meta_campaigns (get_user_company_id + company_admin/super_admin).
-- ════════════════════════════════════════════════════════════════════════════
...
CREATE TABLE IF NOT EXISTS public.meta_ab_tests (
  ...
  variable            text NOT NULL,            -- creative | copy | audience | budget
  hypothesis          text,
  new_value           jsonb,
