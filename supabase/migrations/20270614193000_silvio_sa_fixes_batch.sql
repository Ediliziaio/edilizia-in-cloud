-- ════════════════════════════════════════════════════════════════════════════
-- FIX batch (audit senior): regole decisionali alias, SA-INTEL health,
-- SA-PROBLEMS +2 fonti (dead_letter+tickets), resolver destinatario outbound.
-- ════════════════════════════════════════════════════════════════════════════

-- FIX: alias dominio regole decisionali (utente 'preventivi' ↔ tool.domain 'crm')
...
-- FIX: SA-INTEL health reale + priorita = 80% dolore + 20% rischio churn
...
-- FIX: SA-PROBLEMS — aggiungi fonti automation_dead_letter (tecnico) + support_tickets (prodotto)
...
BEGIN
  ...
  SELECT d.company_id, 'tecnico', 'Automazioni/integrazioni fallite (' || count(*) || ')',
         ...
         'automation_dead_letter', GREATEST(0, (CURRENT_DATE - min(d.first_failed_at)::date)), left(max(d.error_message), 200)
  FROM public.automation_dead_letter d WHERE d.company_id = p_company_id AND d.resolved_at IS NULL
  GROUP BY d.company_id ON CONFLICT (company_id, area, problema) DO NOTHING;
  ...
  GROUP BY t.company_id ON CONFLICT (company_id, area, problema) DO NOTHING;

  RETURN (SELECT count(*) FROM public.sa_company_problems WHERE company_id = p_company_id);
END $$;

-- FIX: resolver destinatario outbound (service-role only, scoped company)
...
BEGIN
  ...
  IF p_dest_tipo = 'cliente' THEN
    SELECT email, coalesce(ragione_sociale, nome) INTO v_email, v_nome FROM public.anagrafiche_native WHERE id = p_dest_id AND company_id = p_company_id;
