-- MP-SILVIO-FOLLOWUP-3 — Promemoria: ASSEGNAZIONE a un collega + ORA.
--   • created_by: chi ha creato il promemoria (così resta visibile al creatore
--     anche se assegnato ad altri — via RLS).
--   • remind_at (timestamptz, opzionale): ora precisa di scadenza. Se valorizzata
--     il promemoria scatta a quell'ora; altrimenti vale la data (remind_on).
--   • Cron: da giornaliero a ogni 15 minuti, per onorare l'ora.
-- Additiva/idempotente. Non tocca silvio_tool_crea_promemoria.
...
-- RLS: il CREATORE vede sempre il promemoria (oltre a destinatario/null/admin).
...
-- promote: fa scattare i "timed" (remind_at<=now) e i "date-only" (remind_on<=oggi).
...
-- Cron: sostituisce il giornaliero con uno ogni 15 minuti.
DO $$
BEGIN
