-- ════════════════════════════════════════════════════════════════════════════
-- MP-SILVIO-SIMULATION-01 · Blocco 5 — simulazione what-if conversazionale
-- ────────────────────────────────────────────────────────────────────────────
-- ZERO nuovo motore: fonde i tool di simulazione esistenti in uno scenario unico.
-- silvio_tool_simula_scenario raccoglie i numeri base (cassa reale via
-- silvio_cashflow_forecast_90d, pipeline, carico squadre), applica le variabili
-- dell'ipotesi (nuova commessa, termini incasso, costo) come delta, ed estrae leve
-- concrete (rate non pagate anticipabili). La narrazione la fa Silvio nel loop.
-- SOLA LETTURA (STABLE): non scrive nulla → tool riskLevel 'safe'.
-- ════════════════════════════════════════════════════════════════════════════

CREATE OR REPLACE FUNCTION public.silvio_tool_simula_scenario(
  p_company_id uuid,
  p_ipotesi text,
  p_orizzonte integer DEFAULT 6,
...
DECLARE
  ...
  v_base_min numeric; v_base_fine numeric;
  v_scenario_min numeric; v_scenario_fine numeric;
  v_leve jsonb;
  v_overload int; v_free int;
...
BEGIN
  ...
  v_weeks := LEAST(v_mesi * 4 + 2, 52);                 -- mesi → settimane (cap 52)
  v_cf := public.silvio_cashflow_forecast_90d(p_company_id, v_weeks, true);
  BEGIN
    v_pipe := public.silvio_tool_get_pipeline_forecast(p_company_id, v_mesi * 30);
  ...
  -- costo: usa quello dato, altrimenti stima 65% del valore commessa (assunzione esplicitata in output)
  v_costo := coalesce(
    (p_variabili->>'costo_fornitori_eur')::numeric,
    CASE WHEN v_nuova > 0 THEN round(v_nuova * 0.65, 2) ELSE 0 END
  );
  v_net := v_nuova - v_costo;
  v_scenario_fine := v_base_fine + v_net;
  -- il minimo migliora solo se l'incasso entra entro l'orizzonte
  v_scenario_min := v_base_min + CASE
    WHEN coalesce((p_variabili->>'incasso_giorni')::int, 9999) <= v_mesi * 30 THEN v_net ELSE 0 END;

  -- leve concrete: top 3 rate non pagate (acconto/saldo) con scadenza nell'orizzonte
  BEGIN
    SELECT coalesce(jsonb_agg(e ORDER BY (e->>'importo')::numeric DESC), '[]'::jsonb)
      INTO v_leve
    FROM (
      SELECT e FROM (
        SELECT jsonb_build_object('cliente', coalesce(o.client_name, o.client_company, o.order_code),
                                  'tipo', 'acconto', 'importo', o.deposit_amount,
                                  'atteso_il', o.deposit_expected_date) AS e
        ...
        UNION ALL
        SELECT jsonb_build_object('cliente', coalesce(o.client_name, o.client_company, o.order_code),
                                  'tipo', 'saldo', 'importo', o.balance_amount,
                                  'atteso_il', o.balance_expected_date)
        FROM public.orders o
  ...
  RETURN jsonb_build_object(
    ...
    'scenario', jsonb_build_object(
      'nuova_commessa_eur', v_nuova,
      'costo_stimato_eur', v_costo,
      'impatto_netto_eur', v_net,
      'saldo_minimo_stimato_eur', v_scenario_min,
      'saldo_fine_periodo_stimato_eur', v_scenario_fine,
