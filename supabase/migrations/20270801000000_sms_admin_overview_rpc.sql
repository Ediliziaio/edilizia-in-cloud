-- sms_admin_overview(): aggregato P&L SMS per il pannello super admin.
-- Una sola chiamata al posto di N+1 query client-side; calcola i numeri
-- VERI (prima fatturato/costo/margine per tenant erano hardcoded a 0).
--
-- Fonti:
--   sms_log               → campagne + test (costo_cliente / costo_wholesale)
--   sms_messages          → transazionali (conteggio)
--   sms_wallet_transazioni→ ricavi transazionali (tipo addebito_sms)
--   sms_pricing_config    → costo wholesale stimato per i transazionali
--
-- Sicurezza: SECURITY DEFINER + guard is_super_admin(auth.uid()).
...
BEGIN
  ...
  -- o attività di invio — le aziende che inviano col mittente alfanumerico
  -- senza account dedicato devono comunque comparire.
  base AS (
