-- Estende silvio_tool_serie_grafico (company-scoped) con 3 nuove metriche:
--   preventivi_per_stato · lead_per_fonte · scadenze_incassi (incassi attesi futuri)
...
BEGIN
  ...
  ELSIF p_metric = 'scadenze_incassi' THEN
    ...
    agg AS (
      ...
      GROUP BY 1)
    SELECT jsonb_build_object('ok',true,'metric',p_metric,'titolo','Incassi attesi per scadenza','unita','€','x_label','mese',
      'data', coalesce(jsonb_agg(jsonb_build_object('label', mese_abbr[extract(month from months.m)::int]||' '||to_char(months.m,'YY'), 'value', round(coalesce(agg.v,0),2)) ORDER BY months.m), '[]'::jsonb))
    INTO res FROM months LEFT JOIN agg ON agg.m = months.m;

  ELSE
    res := jsonb_build_object('ok', false, 'error', 'metric non supportata',
      'metriche_disponibili', jsonb_build_array('fatturato_mensile','incassi_mensili','cantieri_per_stato','documenti_per_tipo','preventivi_per_stato','lead_per_fonte','scadenze_incassi'));
  END IF;
...
COMMENT ON FUNCTION public.silvio_tool_serie_grafico(uuid,text,int) IS 'Serie dati per i grafici di Silvio (company-scoped): fatturato/incassi mensili, commesse per stato, documenti per tipo, preventivi per stato, lead per fonte, scadenze incassi future.';
