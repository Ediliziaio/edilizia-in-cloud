...
BEGIN
  ...
  -- Recent orders (fixed: to_jsonb instead of row_to_jsonb)
  SELECT COALESCE(jsonb_agg(to_jsonb(sub)), '[]'::jsonb) INTO v_recent_orders
  FROM (
  ...
  -- Pending revenue, aging, cash flow
  FOR rec IN
  ...
  -- Urgent items (fixed: to_jsonb)
  SELECT COALESCE(jsonb_agg(to_jsonb(sub)), '[]'::jsonb) INTO v_urgent_items
  FROM (
  ...
  -- Weekly costs (fixed: to_jsonb)
  SELECT COALESCE(jsonb_agg(to_jsonb(sub)), '[]'::jsonb) INTO v_weekly_costs
  FROM (
  ...
  -- Upcoming works (fixed: to_jsonb)
  SELECT COALESCE(jsonb_agg(to_jsonb(sub)), '[]'::jsonb) INTO v_weekly_works
  FROM (
