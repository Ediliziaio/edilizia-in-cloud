-- Sync automatica fatture dai provider esterni (Fatture in Cloud / Aruba / ...) 2x/giorno.
-- billing_auto_sync_all() itera le integrazioni attive con auto_sync e chiama
-- billing-import in modalità cron (header x-cron-secret, stesso pattern degli altri cron
-- del progetto) per ciascuna company+provider. billing-import in modalità cron non richiede
-- un utente: legge company_id + provider dal body.
...
-- 07:00 e 13:00 ora italiana (estate CEST = UTC+2 -> 05:00 e 11:00 UTC; pg_cron gira in UTC).
do $$
begin
