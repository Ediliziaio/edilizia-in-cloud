...
-- FIX 1 (CRITICO): cg_get_cash_flow_prospettico filtrava direction 'in'/'out' e
--   status 'paid'/'cancelled' ma scadenze usa entrata/uscita + da_pagare/pagata
--   → leggeva ZERO scadenze da sempre.
-- FIX 2: rate attese commesse (order_installments non pagate) nelle entrate, con
--   dedup a livello ordine vs scadenze entrata aperte.
-- FIX 3: v_uscite_costi da company_costs non pagati (nessun trigger li porta in
--   scadenze → no doppio conteggio).
-- FIX 4: cg_cash_flow_manuali.ricorrente ora rispettato (vale dal suo mese in poi).
-- FIX 5: SP — mutui MLT da cg_loans.capitale_residuo (stub a 0 scritto prima che
--   cg_loans esistesse; il CF contava le rate, lo SP ignorava il debito).
