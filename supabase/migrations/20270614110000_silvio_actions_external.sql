-- ════════════════════════════════════════════════════════════════════════════
-- MP-SILVIO-ACTIONS-EXTERNAL-01 · Blocco 1 — esecuzione verso l'esterno
-- ────────────────────────────────────────────────────────────────────────────
-- Silvio può comporre+inviare un messaggio a un contatto reale (cliente/fornitore/
-- dipendente/lead) e rispondere a un thread email. SEMPRE su conferma umana:
-- i tool sono riskLevel 'yellow' → executeToolWithRouting crea un action_proposal,
-- l'invio reale avviene solo all'approvazione. Qui: tabella audit + 2 RPC che
-- validano azienda, registrano l'outbound (status='queued') per audit/consegna.
-- Stile coerente col template esistente silvio_tool_invia_proposal_cliente (RPC
-- leggera DB-side). NESSUN nuovo provider: la consegna riusa il sistema esistente.
-- ════════════════════════════════════════════════════════════════════════════
...
CREATE TABLE IF NOT EXISTS public.silvio_outbound_messages (
  ...
  corpo         text NOT NULL,
  scopo         text,
  thread_id     uuid,                 -- per rispondi_a_email
  doc_id        uuid,                 -- allegato opzionale
...
-- ── RPC: componi e invia messaggio (accoda + audit) ─────────────────────────
...
-- ── RPC: rispondi a thread email (accoda + audit) ───────────────────────────
