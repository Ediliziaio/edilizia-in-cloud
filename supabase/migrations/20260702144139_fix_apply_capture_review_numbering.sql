...
-- Fix silvio_tool_apply_capture_review: la numerazione usava
-- to_char(text, 5, '0') (firma inesistente → 42883). Ora PREV-YYYY-NNNN
-- con progressivo per anno calcolato sui soli digit finali del numero.
CREATE OR REPLACE FUNCTION public.silvio_tool_apply_capture_review(p_company_id uuid, p_run_id uuid, p_corrections jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- ─── 4. Insert quote ──────────────────────────────────────────────────────
  -- Numerazione PREV-YYYY-NNNN: progressivo per anno sui digit finali.
  SELECT 'PREV-' || to_char(now(), 'YYYY') || '-' ||
         LPAD((COALESCE(MAX((regexp_match(quote_number, '(\d+)$'))[1]::int), 0) + 1)::text, 4, '0')
    INTO v_quote_number
    FROM public.quotes
   WHERE company_id = p_company_id
     AND quote_number LIKE 'PREV-' || to_char(now(), 'YYYY') || '-%';
