...
-- anche per operatori senza lead "nuovi" (creati nel periodo).
--
-- Prima il SELECT finale era `FROM leads_periodo LEFT JOIN chiamate_periodo`:
-- un operatore con chiamate ma SENZA lead creati nel periodo non compariva proprio
-- e le sue chiamate non venivano contate (es. chiamata registrata a mano su una
-- vecchia opportunità). Fix: FULL OUTER JOIN tra lead e chiamate + COALESCE
-- sull'operatore, così qualsiasi operatore con attività (lead o chiamate) appare.
-- CTE invariati. Cast ::BIGINT sulle colonne-conteggio (SUM(...) restituisce numeric).
-- Applicata in prod via MCP (apply_migration) il 2026-06-30.
...
BEGIN
  ...
  SELECT
    ...
    p.email::TEXT,
    COALESCE(lp.assegnati, 0)::BIGINT,
    COALESCE(lp.lavorati, 0)::BIGINT,
    ROUND(100.0 * lp.lavorati / NULLIF(lp.assegnati, 0), 1),
    COALESCE(lp.contattati, 0)::BIGINT,
    ROUND(100.0 * lp.contattati / NULLIF(lp.lavorati, 0), 1),
    COALESCE(cp.nr_chiamate, 0)::BIGINT,
    ...
    COALESCE(cp.giorni_attivi, 0)::BIGINT
