-- =============================================================================
-- Conversazioni — Inbox unificato per ENTITÀ (contatto | cliente), stile GoHighLevel
-- =============================================================================
-- Aggrega in UN'UNICA timeline per entità i messaggi dei canali esistenti.
-- Due popolazioni distinte nell'app (verificato sui dati reali):
--   • CONTATTO  → marketing_contacts (lead/company/partner).
--                 Canali: email_inbox/outbox (per indirizzo), sms_logs (contact_id/
--                 telefono), messaging_messages⨝messaging_conversations (WhatsApp,
--                 per telefono_normalized che il webhook popola).
--   • CLIENTE   → profiles (orders.customer_id → profiles).
--                 Canali: customer_messages (log unico email/sms/whatsapp/nota
--                 del cliente; sender_role distingue in/out).
--
-- Approccio AGGREGATORE: nessuna duplicazione dei messaggi; li leggiamo dai sistemi
-- esistenti via VIEW, con discriminatore (entita_tipo, entita_id). Unica tabella
-- nuova = `conversazioni` (overlay di STATO: assegnazione, letto/non-letto, stato).
--
-- Sicurezza: RPC SECURITY DEFINER con filtro company (get_user_company_id /
-- is_super_admin). La view non è esposta direttamente ai client.
-- =============================================================================

-- ── 1. Tabella overlay di STATO conversazione ────────────────────────────────
CREATE TABLE IF NOT EXISTS public.conversazioni (
  id            uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  company_id    uuid NOT NULL,
  entita_tipo   text NOT NULL,                    -- 'contatto' | 'cliente'
  entita_id     uuid NOT NULL,                    -- marketing_contacts.id | profiles.id
  stato         text NOT NULL DEFAULT 'aperta',   -- aperta | in_attesa | chiusa
  assegnato_a   uuid,                             -- auth.users.id del gestore
  last_read_at  timestamptz,                      -- puntatore "letto fino a" (non-letti)
...
COMMENT ON TABLE public.conversazioni IS
  'Overlay di stato (assegnazione, letto/non-letto, stato) per le conversazioni unificate per entità (contatto|cliente). I messaggi vivono nei sistemi per-canale e sono uniti dalla view v_conversazioni_messaggi.';

-- ── 2. View unificata dei messaggi (entità contatto + cliente) ───────────────
CREATE OR REPLACE VIEW public.v_conversazioni_messaggi AS
-- ══ CONTATTO · EMAIL ricevute (il contatto ha scritto a noi) ══
SELECT
...
UNION ALL
-- ══ CONTATTO · EMAIL inviate (noi → contatto) ══
...
UNION ALL
-- ══ CONTATTO · WHATSAPP (messaging_messages ⨝ messaging_conversations, per telefono) ══
...
UNION ALL
-- ══ CLIENTE · customer_messages (log unico email/sms/whatsapp/nota del cliente) ══
SELECT
...
COMMENT ON VIEW public.v_conversazioni_messaggi IS
  'Stream unificato dei messaggi per entità (contatto=marketing_contacts via email/sms/whatsapp; cliente=profiles via customer_messages). Accesso solo via RPC SECURITY DEFINER.';

-- ── 3. RPC: timeline di una singola entità (pannello centrale) ────────────────
...
-- ── 4. RPC: lista conversazioni dell'azienda (sidebar) ───────────────────────
