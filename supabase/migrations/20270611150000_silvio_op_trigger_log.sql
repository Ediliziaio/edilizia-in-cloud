-- ════════════════════════════════════════════════════════════════════════════
-- MP-SILVIO-03 · Trigger autonomi — idempotenza
-- Silvio proattivo: osserva eventi deterministici e PROPONE (in coda) l'azione.
-- Il log garantisce che un trigger non scatti due volte sullo stesso evento.
-- ════════════════════════════════════════════════════════════════════════════
CREATE TABLE IF NOT EXISTS public.silvio_trigger_log (
  id            uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  company_id    uuid NOT NULL REFERENCES public.companies(id) ON DELETE CASCADE,
  tipo          text NOT NULL,            -- 'email_ferma' | 'scadenza_vicina' | ...
  chiave_evento text NOT NULL,            -- id dell'oggetto (email_id, scadenza_id…)
  esecuzione_id uuid,
