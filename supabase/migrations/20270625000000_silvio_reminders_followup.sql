...
-- L'utente (o Silvio dopo un'azione, es. un sollecito) crea un promemoria con
-- una data futura; un cron giornaliero promuove i promemoria scaduti ad ALERT
-- (riusa il push esistente: campanella + briefing). "Chiusura del cerchio".
...
CREATE TABLE IF NOT EXISTS public.silvio_reminders (
  ...
  cta_label text,
  cta_payload jsonb,
  status text NOT NULL DEFAULT 'pending',  -- pending | promoted | done | cancelled
...
-- 2) RPC: crea promemoria (usata dal tool Silvio crea_promemoria)
...
-- 3) RPC: promuove i promemoria scaduti ad alert (push: campanella + brief)
...
-- 4) Cron giornaliero (05:10 UTC) — promuove i promemoria scaduti
DO $$
BEGIN
