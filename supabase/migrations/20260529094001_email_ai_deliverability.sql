...
CREATE TABLE IF NOT EXISTS public.domini_invio (
  ...
  dmarc_policy text, dkim_selector text,
  ultimo_check_at timestamptz, warmup_stato text DEFAULT 'nuovo',
  created_at timestamptz NOT NULL DEFAULT now(), updated_at timestamptz NOT NULL DEFAULT now(),
  UNIQUE (company_id, dominio)
);
CREATE TABLE IF NOT EXISTS public.deliverability_metriche (
  company_id uuid NOT NULL REFERENCES public.companies(id) ON DELETE CASCADE,
  giorno date NOT NULL, inviate int NOT NULL DEFAULT 0, consegnate int NOT NULL DEFAULT 0,
  bounce int NOT NULL DEFAULT 0, spam_report int NOT NULL DEFAULT 0,
