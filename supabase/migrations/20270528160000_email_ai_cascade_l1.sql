...
-- ────────────────────────────────────────────────────────────────────────────
-- Estensione additiva ed idempotente di public.email_inbox per supportare la
-- cascata di classificazione MP-EMAIL-AI-01. Mai distruggere dati esistenti.
--
-- Cosa fa questa migration:
--   1. enum public.email_categoria_v2 con tassonomia MP (13 valori)
--   2. Colonne nuove su email_inbox: categoria, entita_tipo, entita_id,
--      confidenza, classificato_da, da_rivedere, headers, classificato_at
--   3. Tabella public.mittenti_noti (cache mittenti appresi)
--   4. RLS per mittenti_noti via get_effective_company_id()
--   5. Index per le query L1 (lookup mittente per email/dominio)
--
-- Cosa NON fa (volutamente):
--   - Non tocca ai_category / ai_priority / ai_summary / ai_extracted
--     (campi legacy email-triage-ai). UI userà COALESCE(categoria, ai_category).
--   - Non modifica RLS esistente di email_inbox (resta com'è).
--   - Non crea l'enum se già esiste (idempotente).
-- ════════════════════════════════════════════════════════════════════════════

BEGIN;

-- ─── 1) Enum tassonomia MP ──────────────────────────────────────────────────
-- 13 categorie operative impresa edile (MP §6)
DO $$
BEGIN
...
-- IF NOT EXISTS per idempotenza. Tutte nullable + default sensati per rollback safety.
ALTER TABLE public.email_inbox
  ADD COLUMN IF NOT EXISTS categoria       public.email_categoria_v2,
  ADD COLUMN IF NOT EXISTS entita_tipo     text,       -- 'cliente'|'fornitore'|'operaio'|...
  ADD COLUMN IF NOT EXISTS entita_id       uuid,       -- FK soft (no constraint per essere agnostici)
  ADD COLUMN IF NOT EXISTS confidenza      numeric(3,2), -- 0.00 .. 1.00
  ADD COLUMN IF NOT EXISTS classificato_da text,       -- 'regola'|'embedding'|'haiku'|'manuale'
  ADD COLUMN IF NOT EXISTS da_rivedere     boolean NOT NULL DEFAULT false,
  ADD COLUMN IF NOT EXISTS headers         jsonb,      -- header grezzi {from,list-unsubscribe,list-id,precedence,auto-submitted,return-path}
  ADD COLUMN IF NOT EXISTS classificato_at timestamptz;

-- Constraint check su classificato_da (idempotente)
DO $$
BEGIN
  IF NOT EXISTS (
...
-- Constraint check su entita_tipo (idempotente)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.check_constraints
...
-- Per query rapide "email da classificare" e "filtro UI per categoria"
...
-- ─── 4) Tabella mittenti_noti — cache apprendimento ─────────────────────────
-- Quando un mittente viene risolto (da regola, AI o correzione manuale),
-- lo memorizziamo qui. Le successive email di QUEL mittente vengono
-- classificate al Livello 1a senza alcun costo AI.
CREATE TABLE IF NOT EXISTS public.mittenti_noti (
  id               uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  company_id       uuid NOT NULL REFERENCES public.companies(id) ON DELETE CASCADE,
  email            text NOT NULL,                        -- normalizzato lowercase
  dominio          text,                                  -- normalizzato lowercase
  categoria        public.email_categoria_v2 NOT NULL,
  entita_tipo      text,
  entita_id        uuid,
  fonte            text NOT NULL,                         -- 'regola'|'haiku'|'manuale'
  hit_count        integer NOT NULL DEFAULT 1,            -- quante email questo mittente ha inviato
...
-- ─── 5) RLS mittenti_noti (replica pattern email_inbox) ─────────────────────
...
-- ─── 6) RPC upsert_mittente_noto — usata dal feedback loop manuale ─────────
-- Quando l'utente sposta un'email in altra categoria, chiamiamo questa per
-- aggiornare/inserire il mittente.
...
-- ─── 7) RPC reclassify_email_manuale — feedback loop completo ──────────────
-- Sposta un'email in altra categoria E aggiorna mittenti_noti in transazione.
...
BEGIN
  ...
  -- Recupera from_email + verifica scope
...
SELECT
  e.*,
  -- Categoria UI = nuova se presente, fallback a legacy
...
-- ─── 9) Stats helper — per STATUS.md % L1 hit rate ──────────────────────────
...
-- ─── 10) Commento documentale (auto-generato) ──────────────────────────────
COMMENT ON COLUMN public.email_inbox.categoria IS
  'MP-EMAIL-AI-01: categoria operativa risultante dalla cascata L1→L3. NULL = non ancora classificata.';
COMMENT ON COLUMN public.email_inbox.entita_tipo IS
  'MP-EMAIL-AI-01: tipo entità CRM linkata (cliente/fornitore/operaio/opportunita/ordine).';
COMMENT ON COLUMN public.email_inbox.entita_id IS
  'MP-EMAIL-AI-01: ID entità CRM linkata. FK soft (no constraint) per agnosticità.';
COMMENT ON COLUMN public.email_inbox.confidenza IS
  'MP-EMAIL-AI-01: 0..1 — regola=1.0, haiku=output modello, manuale=1.0.';
COMMENT ON COLUMN public.email_inbox.classificato_da IS
  'MP-EMAIL-AI-01: regola|embedding|haiku|manuale.';
COMMENT ON COLUMN public.email_inbox.da_rivedere IS
  'MP-EMAIL-AI-01: true se confidenza < soglia o ambigua → resta nella tab "Da fare".';
COMMENT ON COLUMN public.email_inbox.headers IS
  'MP-EMAIL-AI-01: header email grezzi (List-Unsubscribe, List-Id, Precedence, Auto-Submitted, Return-Path, ecc) per regole deterministiche.';

COMMENT ON TABLE public.mittenti_noti IS
  'MP-EMAIL-AI-01: cache mittenti già classificati. Permette al L1a di risolvere email future di quel mittente a costo zero.';
