...
begin
  ...
  pts as (
    select ('comp-'||c.id) as id, 'cliente' as tipo,
           ...
           case when c.operational_lat is not null then c.operational_lat
                else pc.lat + ((((hashtext(c.id::text) % 1000) + 1000) % 1000) / 1000.0 - 0.5) * 0.08 end as lat,
           case when c.operational_lng is not null then c.operational_lng
                else pc.lng + ((((hashtext(c.id::text||'L') % 1000) + 1000) % 1000) / 1000.0 - 0.5) * 0.11 end as lng,
    ...
    select ('mkt-'||m.id), case when sc2.contact_id is not null then 'cliente' else 'prospect' end,
           ...
           case when m.lat is not null then m.lat
                else pc.lat + ((((hashtext(m.id::text) % 1000) + 1000) % 1000) / 1000.0 - 0.5) * 0.08 end,
           case when m.lng is not null then m.lng
                else pc.lng + ((((hashtext(m.id::text||'L') % 1000) + 1000) % 1000) / 1000.0 - 0.5) * 0.11 end,
           (m.lat is not null),
