-- Dashboard marketing sulle regole di tutti gli altri numeri di vendita.
--
-- get_marketing_dashboard_stats contava a modo suo, e sbagliava:
--   · vinte e perse datate con updated_at (l'ultima modifica): per Renova le 14
--     vendite di agosto risultavano zero, e tutte a settembre;
--   · appuntamenti «svolti» = spunta is_completed, che nessuno usa: show rate e
--     tasso di chiusura sempre 0%, avviso «show rate sotto soglia» sempre acceso;
--   · cancellate contate; giorni del periodo presi nel fuso del server (UTC);
--   · la tabella dei commerciali ignorava il filtro della persona: chi vede solo
--     i propri lead vedeva fatturato e contratti di tutti i colleghi, e chiamando
--     la funzione senza filtro vedeva i numeri dell'azienda intera.
--
-- Qui le regole sono quelle di vendite_opportunita / vendite_appuntamenti
-- (20280915410001): vinte/perse per won_at/lost_at, appuntamenti del calendario
-- marketing senza annullati e riprogrammati, show rate = effettuati ÷
-- (effettuati + no-show), tasso di chiusura = vinte ÷ (vinte + perse). Chi vede
-- solo i propri lead è ristretto dentro la funzione, qualunque filtro arrivi.
-- Stessa firma e stessa forma del risultato: la pagina non cambia.
...
DECLARE
  ...
  -- i giorni ITALIANI del periodo, per le colonne data (appuntamenti, costi,
  -- andamento): v_date_from::date li prendeva nel fuso del server (UTC), e il
  -- primo del mese iniziava il giorno prima
  v_giorno_da date := (v_date_from AT TIME ZONE 'Europe/Rome')::date;
  v_giorno_a date := (v_date_to AT TIME ZONE 'Europe/Rome')::date;
  v_prev_giorno_da date := (v_prev_from AT TIME ZONE 'Europe/Rome')::date;
  v_prev_giorno_a date := ((v_prev_to - interval '1 microsecond') AT TIME ZONE 'Europe/Rome')::date;
  -- Current KPIs
  ...
  v_prev_appointments_no_show bigint;
...
BEGIN
  ...
  -- Chi vede solo i propri lead vede solo i propri numeri anche chiamando la
  -- funzione a mano: prima lo decideva la pagina, e la tabella dei commerciali
  -- il filtro non lo guardava nemmeno (tutti vedevano il fatturato di tutti).
  IF public.solo_assegnati_attivo() THEN
    p_assigned_user_ids := ARRAY[auth.uid()];
  END IF;
  -- CURRENT PERIOD KPIs
  SELECT COUNT(*) INTO v_leads_total
  FROM marketing_contacts mc
  WHERE mc.company_id = p_company_id AND mc.deleted_at IS NULL AND mc.created_at <= v_date_to
  ...
  FROM marketing_contacts mc
  WHERE mc.company_id = p_company_id AND mc.deleted_at IS NULL AND mc.created_at >= v_date_from AND mc.created_at <= v_date_to
    AND (p_assigned_user_ids IS NULL OR mc.assigned_to = ANY(p_assigned_user_ids))
    AND (p_sources IS NULL OR mc.source = ANY(p_sources));

  SELECT COUNT(DISTINCT mc.id) INTO v_contacts_worked
  FROM marketing_contacts mc
  WHERE mc.company_id = p_company_id AND mc.deleted_at IS NULL
    AND (p_assigned_user_ids IS NULL OR mc.assigned_to = ANY(p_assigned_user_ids))
    AND (p_sources IS NULL OR mc.source = ANY(p_sources))
    AND (
  ...
  FROM appointments a
  WHERE a.company_id = p_company_id AND a.appointment_date BETWEEN v_giorno_da AND v_giorno_a
    AND a.calendar_id IS NOT NULL AND NOT COALESCE(a.is_blocked_slot, false) AND COALESCE(a.status, '') NOT IN ('annullato', 'riprogrammato')
    AND (p_assigned_user_ids IS NULL OR a.assigned_to = ANY(p_assigned_user_ids));

  SELECT COUNT(*) FILTER (WHERE COALESCE(a.status, '') IN ('presentato', 'da_preventivare', 'preventivo_inviato', 'venduto', 'perso', 'non_qualificato', 'completato')),
         COUNT(*) FILTER (WHERE a.status = 'no_show')
  INTO v_appointments_done, v_appointments_no_show
  FROM appointments a
  WHERE a.company_id = p_company_id AND a.appointment_date BETWEEN v_giorno_da AND v_giorno_a
    AND a.calendar_id IS NOT NULL AND NOT COALESCE(a.is_blocked_slot, false)
    AND (p_assigned_user_ids IS NULL OR a.assigned_to = ANY(p_assigned_user_ids));

  SELECT COUNT(*), COALESCE(SUM(mo.value), 0)
  INTO v_contracts_won, v_revenue
  FROM marketing_opportunities mo
  WHERE mo.company_id = p_company_id AND mo.deleted_at IS NULL AND mo.status = 'won'
    AND mo.won_at >= v_date_from AND mo.won_at <= v_date_to
  ...
  FROM marketing_opportunities mo
  WHERE mo.company_id = p_company_id AND mo.deleted_at IS NULL AND mo.status = 'lost'
    AND mo.lost_at >= v_date_from AND mo.lost_at <= v_date_to
    AND (p_assigned_user_ids IS NULL OR mo.assigned_to = ANY(p_assigned_user_ids))
    AND (p_sources IS NULL OR mo.source = ANY(p_sources))
    AND (p_pipeline_id IS NULL OR mo.pipeline_id = p_pipeline_id);
  ...
  WHERE cc.company_id = p_company_id AND cc.date BETWEEN v_giorno_da AND v_giorno_a
  ...
  -- show rate = effettuati ÷ (effettuati + no-show); tasso di chiusura = vinte ÷
  -- (vinte + perse): le stesse regole di Sales OS e del report Venditori
  v_show_rate := CASE WHEN v_appointments_done + v_appointments_no_show > 0 THEN ROUND((v_appointments_done::numeric / (v_appointments_done + v_appointments_no_show)) * 100, 1) ELSE 0 END;
  v_close_rate := CASE WHEN v_contracts_won + v_contracts_lost > 0 THEN ROUND((v_contracts_won::numeric / (v_contracts_won + v_contracts_lost)) * 100, 1) ELSE 0 END;
  ...
  FROM marketing_opportunities mo
  WHERE mo.company_id = p_company_id AND mo.deleted_at IS NULL AND mo.status = 'open'
    AND (p_assigned_user_ids IS NULL OR mo.assigned_to = ANY(p_assigned_user_ids))
    AND (p_sources IS NULL OR mo.source = ANY(p_sources))
    AND (p_pipeline_id IS NULL OR mo.pipeline_id = p_pipeline_id);
  ...
  SELECT COALESCE(ROUND(AVG(EXTRACT(EPOCH FROM (mo.won_at - mo.created_at)) / 86400)::numeric, 1), 0)
  ...
  SELECT COALESCE(ROUND(AVG(EXTRACT(EPOCH FROM (mo.won_at - mc.created_at)) / 86400)::numeric, 1), 0)
  ...
  SELECT COALESCE(ROUND(percentile_cont(0.5) WITHIN GROUP (
    ORDER BY EXTRACT(EPOCH FROM (mo.won_at - mc.created_at)) / 86400
  )::numeric, 1), 0)
  ...
  FROM marketing_contacts mc
  WHERE mc.company_id = p_company_id AND mc.deleted_at IS NULL AND mc.created_at <= v_prev_to
    AND (p_assigned_user_ids IS NULL OR mc.assigned_to = ANY(p_assigned_user_ids))
    AND (p_sources IS NULL OR mc.source = ANY(p_sources));

  SELECT COUNT(*) INTO v_prev_leads_new
  FROM marketing_contacts mc
  WHERE mc.company_id = p_company_id AND mc.deleted_at IS NULL AND mc.created_at >= v_prev_from AND mc.created_at <= v_prev_to
    AND (p_assigned_user_ids IS NULL OR mc.assigned_to = ANY(p_assigned_user_ids))
    AND (p_sources IS NULL OR mc.source = ANY(p_sources));
  ...
  FROM appointments a
  WHERE a.company_id = p_company_id AND a.appointment_date BETWEEN v_prev_giorno_da AND v_prev_giorno_a
    AND a.calendar_id IS NOT NULL AND NOT COALESCE(a.is_blocked_slot, false) AND COALESCE(a.status, '') NOT IN ('annullato', 'riprogrammato')
    AND (p_assigned_user_ids IS NULL OR a.assigned_to = ANY(p_assigned_user_ids));

  SELECT COUNT(*) FILTER (WHERE COALESCE(a.status, '') IN ('presentato', 'da_preventivare', 'preventivo_inviato', 'venduto', 'perso', 'non_qualificato', 'completato')),
         COUNT(*) FILTER (WHERE a.status = 'no_show')
  INTO v_prev_appointments_done, v_prev_appointments_no_show
  FROM appointments a
  WHERE a.company_id = p_company_id AND a.appointment_date BETWEEN v_prev_giorno_da AND v_prev_giorno_a
    AND a.calendar_id IS NOT NULL AND NOT COALESCE(a.is_blocked_slot, false)
    AND (p_assigned_user_ids IS NULL OR a.assigned_to = ANY(p_assigned_user_ids));
  ...
  FROM marketing_opportunities mo
  WHERE mo.company_id = p_company_id AND mo.deleted_at IS NULL AND mo.status = 'won'
    AND mo.won_at >= v_prev_from AND mo.won_at <= v_prev_to
    AND (p_assigned_user_ids IS NULL OR mo.assigned_to = ANY(p_assigned_user_ids))
    AND (p_sources IS NULL OR mo.source = ANY(p_sources))
    AND (p_pipeline_id IS NULL OR mo.pipeline_id = p_pipeline_id);
  ...
  FROM marketing_opportunities mo
  WHERE mo.company_id = p_company_id AND mo.deleted_at IS NULL AND mo.status = 'lost'
    AND mo.lost_at >= v_prev_from AND mo.lost_at <= v_prev_to
    AND (p_assigned_user_ids IS NULL OR mo.assigned_to = ANY(p_assigned_user_ids))
    AND (p_sources IS NULL OR mo.source = ANY(p_sources))
    AND (p_pipeline_id IS NULL OR mo.pipeline_id = p_pipeline_id);
  ...
  FROM campaign_costs cc
  WHERE cc.company_id = p_company_id AND cc.date BETWEEN v_prev_giorno_da AND v_prev_giorno_a
    AND (p_sources IS NULL OR cc.source = ANY(p_sources));
  ...
  v_prev_sales_velocity := CASE
    WHEN v_prev_avg_lead_to_won_days > 0 AND v_prev_contracts_won > 0
    THEN ROUND(
      (v_open_opps_count * (ROUND((v_prev_contracts_won::numeric / (v_prev_contracts_won + v_prev_contracts_lost)) * 100, 1) / 100.0) *
  ...
  FROM (
    ...
    LEFT JOIN marketing_opportunities mo ON mo.stage_id = mps.id
      AND mo.company_id = p_company_id AND mo.deleted_at IS NULL
  ...
  WITH sales_users AS (
    ...
    WHERE p.company_id = p_company_id
      AND (p_assigned_user_ids IS NULL OR p.id = ANY(p_assigned_user_ids))
      AND (
        EXISTS (SELECT 1 FROM marketing_opportunities mo2 WHERE mo2.assigned_to = p.id AND mo2.company_id = p_company_id AND mo2.deleted_at IS NULL)
  ...
  apt_set_counts AS (
    ...
    WHERE a3.company_id = p_company_id AND a3.appointment_date BETWEEN v_giorno_da AND v_giorno_a AND a3.calendar_id IS NOT NULL AND NOT COALESCE(a3.is_blocked_slot, false) AND COALESCE(a3.status, '') NOT IN ('annullato', 'riprogrammato')
    GROUP BY a3.assigned_to
  ),
  apt_done_counts AS (
    SELECT a4.assigned_to AS user_id,
           COUNT(*) FILTER (WHERE COALESCE(a4.status, '') IN ('presentato', 'da_preventivare', 'preventivo_inviato', 'venduto', 'perso', 'non_qualificato', 'completato')) AS cnt,
           COUNT(*) FILTER (WHERE a4.status = 'no_show') AS no_show
    FROM appointments a4
    WHERE a4.company_id = p_company_id AND a4.appointment_date BETWEEN v_giorno_da AND v_giorno_a AND a4.calendar_id IS NOT NULL AND NOT COALESCE(a4.is_blocked_slot, false)
  ...
  opp_won_counts AS (
    ...
    WHERE mo3.company_id = p_company_id AND mo3.deleted_at IS NULL AND mo3.status = 'won' AND mo3.won_at >= v_date_from AND mo3.won_at <= v_date_to
  ...
  FROM (
    SELECT su.user_id, su.first_name || ' ' || su.last_name AS name,
      ...
      CASE WHEN COALESCE(adc.cnt, 0) + COALESCE(adc.no_show, 0) > 0 THEN ROUND((COALESCE(adc.cnt, 0)::numeric / (adc.cnt + adc.no_show)) * 100, 1) ELSE 0 END AS show_rate
    FROM sales_users su
  ...
  FROM (
    SELECT COALESCE(mc.source, 'Non specificata') AS source,
      ...
      END AS roi_pct
    FROM marketing_contacts mc
    LEFT JOIN marketing_opportunities mo ON mo.contact_id = mc.id AND mo.company_id = p_company_id AND mo.deleted_at IS NULL
      AND mo.status = 'won' AND mo.won_at >= v_date_from AND mo.won_at <= v_date_to
    LEFT JOIN (
  ...
  FROM (
    ...
    LEFT JOIN (
      ...
      WHERE a5.company_id = p_company_id AND a5.appointment_date BETWEEN v_giorno_da AND v_giorno_a AND a5.calendar_id IS NOT NULL AND NOT COALESCE(a5.is_blocked_slot, false) AND COALESCE(a5.status, '') NOT IN ('annullato', 'riprogrammato')
  ...
  SELECT jsonb_build_object(
    'stale_leads', (SELECT COUNT(*) FROM marketing_contacts mc WHERE mc.company_id = p_company_id AND mc.deleted_at IS NULL AND mc.created_at >= v_date_from AND mc.last_activity_at IS NULL AND mc.created_at < now() - interval '48 hours' AND (p_assigned_user_ids IS NULL OR mc.assigned_to = ANY(p_assigned_user_ids))),
    'stale_leads_2h', (SELECT COUNT(*) FROM marketing_contacts mc WHERE mc.company_id = p_company_id AND mc.deleted_at IS NULL AND mc.created_at >= v_date_from AND mc.last_activity_at IS NULL AND mc.created_at < now() - interval '2 hours' AND mc.created_at >= now() - interval '48 hours' AND (p_assigned_user_ids IS NULL OR mc.assigned_to = ANY(p_assigned_user_ids))),
    'stale_opportunities', (SELECT COUNT(*) FROM marketing_opportunities mo WHERE mo.company_id = p_company_id AND mo.deleted_at IS NULL AND mo.status = 'open' AND mo.updated_at < now() - interval '7 days' AND (p_assigned_user_ids IS NULL OR mo.assigned_to = ANY(p_assigned_user_ids))),
    -- appuntamenti passati senza esito: confermati (o vuoti) con la data passata,
    -- come vendite_appuntamenti; la spunta is_completed non l'ha mai usata nessuno
    'pending_appointments', (SELECT COUNT(*) FROM appointments a WHERE a.company_id = p_company_id AND a.calendar_id IS NOT NULL AND NOT COALESCE(a.is_blocked_slot, false) AND COALESCE(a.status, 'confermato') = 'confermato' AND a.appointment_date < (now() AT TIME ZONE 'Europe/Rome')::date AND (p_assigned_user_ids IS NULL OR a.assigned_to = ANY(p_assigned_user_ids))),
    'show_rate_below_threshold', (v_show_rate < 60 AND v_appointments_done + v_appointments_no_show >= 5),
  ...
  FROM (
    SELECT d.day::date AS day,
      (SELECT COUNT(*) FROM marketing_contacts mc WHERE mc.company_id = p_company_id AND mc.deleted_at IS NULL AND (mc.created_at AT TIME ZONE 'Europe/Rome')::date = d.day
        AND (p_assigned_user_ids IS NULL OR mc.assigned_to = ANY(p_assigned_user_ids))
      ...
      (SELECT COUNT(*) FROM marketing_opportunities mo WHERE mo.company_id = p_company_id AND mo.deleted_at IS NULL AND mo.status = 'won' AND (mo.won_at AT TIME ZONE 'Europe/Rome')::date = d.day
        AND (p_assigned_user_ids IS NULL OR mo.assigned_to = ANY(p_assigned_user_ids))
        AND (p_sources IS NULL OR mo.source = ANY(p_sources))
        AND (p_pipeline_id IS NULL OR mo.pipeline_id = p_pipeline_id)) AS won
    FROM generate_series(v_giorno_da, v_giorno_a, '1 day'::interval) d(day)
  ) t;
  ...
  result := jsonb_build_object(
    ...
    'kpi_prev', jsonb_build_object(
      ...
      'show_rate', CASE WHEN v_prev_appointments_done + v_prev_appointments_no_show > 0 THEN ROUND((v_prev_appointments_done::numeric / (v_prev_appointments_done + v_prev_appointments_no_show)) * 100, 1) ELSE 0 END,
      ...
      'close_rate', CASE WHEN v_prev_contracts_won + v_prev_contracts_lost > 0 THEN ROUND((v_prev_contracts_won::numeric / (v_prev_contracts_won + v_prev_contracts_lost)) * 100, 1) ELSE 0 END,
