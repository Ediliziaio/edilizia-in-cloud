...
--
-- RPC: lista / crea / rinomina / elimina. Tutte SECURITY DEFINER e scoped
-- all'utente autenticato. Additiva: NON tocca ensure_user_silvio_channel
-- (resta la "conversazione principale").
...
AS $$
  SELECT c.id,
   ...
   WHERE c.name = 'silvio-ai'
     ...
     -- Esclude il canale legacy della vecchia chat (FAB/SilvioChatSheet): Silvio AI
     -- mostra SOLO le conversazioni create qui, separate dalla chat originale.
