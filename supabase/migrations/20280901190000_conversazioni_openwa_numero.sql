-- Le bolle WhatsApp Locale dicono DA QUALE numero sono partite.
--
-- Nel centro Conversazioni ogni messaggio del canale mostrava solo "WHATSAPP":
-- con piu' numeri collegati non c'era modo di sapere chi avesse scritto, e
-- rispondere dal numero sbagliato confonde il destinatario (per lui e' una
-- persona diversa). Il nome del numero viaggia nel campo `oggetto`, che per i
-- messaggi non-email era sempre NULL, e la timeline lo mostra gia' accanto
-- all'etichetta del canale.
...
UNION ALL
 SELECT 'contatto'::text AS entita_tipo,
    ...
    NULLIF(TRIM(COALESCE(onum.display_name, onum.numero, ''::text)), ''::text) AS oggetto,
