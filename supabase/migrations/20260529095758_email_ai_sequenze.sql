...
CREATE TABLE IF NOT EXISTS public.sequenze_esecuzioni (
  ...
  entita_id           uuid,
  oauth_connection_id uuid REFERENCES public.email_oauth_connections(id) ON DELETE SET NULL,
  variabili           jsonb NOT NULL DEFAULT '{}'::jsonb,
  ancora_at           timestamptz NOT NULL DEFAULT now(),
...
BEGIN
  IF coalesce(NEW.mailbox_folder, '') = 'sent' THEN RETURN NEW; END IF;
  IF NEW.from_email IS NULL OR NEW.company_id IS NULL THEN RETURN NEW; END IF;
...
CREATE OR REPLACE FUNCTION public.sequenza_enroll(
  p_sequenza_id uuid, p_destinatario text, p_destinatario_nome text DEFAULT NULL,
  p_thread_id uuid DEFAULT NULL, p_entita_tipo text DEFAULT NULL, p_entita_id uuid DEFAULT NULL,
  p_variabili jsonb DEFAULT '{}'::jsonb, p_ancora_at timestamptz DEFAULT now(), p_oauth_connection_id uuid DEFAULT NULL
) RETURNS public.sequenze_esecuzioni LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE s public.sequenze; e public.sequenze_esecuzioni; v_offset int; v_conn uuid;
BEGIN
  ...
  IF p_destinatario IS NULL OR position('@' in p_destinatario) = 0 THEN RAISE EXCEPTION 'destinatario_non_valido'; END IF;
  SELECT * INTO e FROM public.sequenze_esecuzioni
   WHERE sequenza_id = p_sequenza_id AND lower(destinatario) = lower(p_destinatario)
     AND stato IN ('attiva','in_attesa_conferma','in_pausa') LIMIT 1;
...
RETURNS public.sequenze_esecuzioni LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE inv public.sequenze_invii; e public.sequenze_esecuzioni; s public.sequenze; v_next int; v_offset int;
BEGIN
...
COMMENT ON TABLE public.sequenze IS 'MP-EMAIL-AI-13: definizione sequenze in uscita. modalita_invio conferma|automatico. Approvazione testi obbligatoria prima di attiva.';
