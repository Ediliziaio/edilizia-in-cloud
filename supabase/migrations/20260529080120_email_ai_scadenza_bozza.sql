...
CREATE TABLE IF NOT EXISTS public.email_scadenza_bozza (
  ...
  descrizione text,
  controparte_id uuid,
  controparte_tipo text,
  dedup_scadenza_id uuid,
...
CREATE POLICY email_scad_bozza_staff_read ON public.email_scadenza_bozza FOR SELECT TO authenticated
...
CREATE POLICY email_scad_bozza_staff_update ON public.email_scadenza_bozza FOR UPDATE TO authenticated
  USING (company_id = public.get_effective_company_id() AND public.is_email_staff_interno())
...
CREATE POLICY email_scad_bozza_service_all ON public.email_scadenza_bozza FOR ALL TO service_role USING (true) WITH CHECK (true);
DROP POLICY IF EXISTS email_scad_bozza_super_admin ON public.email_scadenza_bozza;
CREATE POLICY email_scad_bozza_super_admin ON public.email_scadenza_bozza FOR ALL TO authenticated USING (public.is_super_admin(auth.uid()));

DROP TRIGGER IF EXISTS trg_email_scad_bozza_updated_at ON public.email_scadenza_bozza;
CREATE TRIGGER trg_email_scad_bozza_updated_at BEFORE UPDATE ON public.email_scadenza_bozza
...
BEGIN
  ...
  UPDATE public.email_scadenza_bozza SET stato='aggiunta', scadenza_creata_id=v_id, confirmed_by=auth.uid(), confirmed_at=now() WHERE id = p_bozza_id;
  RETURN v_id;
END $fn$;
