-- Elettrico · Verticale preventivo (clone di Ristrutturazione, namespace ele_*).
-- Greenfield: un'unica migrazione con lo schema FINALE (base + builder + personalizzazione
-- + fonte) e gli adattamenti elettrico (n. punti, livello impianto). ADDITIVA/IDEMPOTENTE.
-- RLS company-scoped (super_admin OR company match). LOCALE: applicata in pubblicazione.

-- Listino lavorazioni aziendale ──────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS public.ele_listino_capitoli (
...
CREATE TABLE IF NOT EXISTS public.ele_listino_voci (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  company_id uuid NOT NULL REFERENCES public.companies(id) ON DELETE CASCADE,
  capitolo_id uuid REFERENCES public.ele_listino_capitoli(id) ON DELETE SET NULL,
  codice text,
...
-- Progetto (hub) ──────────────────────────────────────────────────────────────
-- Adattamenti elettrico: numero_punti + livello_impianto (CEI 64-8 liv. 1/2/3);
-- superficie e tipo intervento riusano immobile_superficie_mq / tipo_intervento (DRY).
CREATE TABLE IF NOT EXISTS public.ele_progetti (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  company_id uuid NOT NULL REFERENCES public.companies(id) ON DELETE CASCADE,
  code text,
...
DO $$ BEGIN
  ALTER TABLE public.ele_progetti
    ADD CONSTRAINT ele_progetti_stato_chk CHECK (
...
CREATE TABLE IF NOT EXISTS public.ele_computo_voci (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  progetto_id uuid NOT NULL REFERENCES public.ele_progetti(id) ON DELETE CASCADE,
  company_id uuid NOT NULL REFERENCES public.companies(id) ON DELETE CASCADE,
...
COMMENT ON COLUMN public.ele_computo_voci.fonte IS 'Prezzario regionale di provenienza della voce (per citazione base d''asta). NULL = voce libera/listino.';

-- Media ──────────────────────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS public.ele_progetti_media (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  progetto_id uuid NOT NULL REFERENCES public.ele_progetti(id) ON DELETE CASCADE,
  company_id uuid NOT NULL REFERENCES public.companies(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS public.ele_template_pdf (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
...
-- Indici
CREATE INDEX IF NOT EXISTS idx_ele_progetti_company ON public.ele_progetti(company_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_ele_computo_progetto ON public.ele_computo_voci(progetto_id, ordine);
CREATE INDEX IF NOT EXISTS idx_ele_listino_voci_company ON public.ele_listino_voci(company_id, capitolo_id, ordine);
CREATE INDEX IF NOT EXISTS idx_ele_media_progetto ON public.ele_progetti_media(progetto_id, ordine);

-- RLS (super_admin OR company_id = get_user_company_id) su tutte le tabelle
DO $$
DECLARE t text;
BEGIN
  FOREACH t IN ARRAY ARRAY['ele_listino_capitoli','ele_listino_voci','ele_progetti','ele_computo_voci','ele_progetti_media','ele_template_pdf'] LOOP
    EXECUTE format('ALTER TABLE public.%I ENABLE ROW LEVEL SECURITY', t);
    EXECUTE format($q$DROP POLICY IF EXISTS "ele super admin all" ON public.%I$q$, t);
    EXECUTE format($q$CREATE POLICY "ele super admin all" ON public.%I FOR ALL
      USING (has_role(auth.uid(),'super_admin'::app_role)) WITH CHECK (has_role(auth.uid(),'super_admin'::app_role))$q$, t);
    EXECUTE format($q$DROP POLICY IF EXISTS "ele company members" ON public.%I$q$, t);
    EXECUTE format($q$CREATE POLICY "ele company members" ON public.%I FOR ALL TO authenticated
