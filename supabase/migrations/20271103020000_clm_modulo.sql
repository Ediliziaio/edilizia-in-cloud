-- Climatizzazione · Verticale preventivo (clone di Ristrutturazione, namespace clm_*).
-- Greenfield: un'unica migrazione con lo schema FINALE (base + builder + personalizzazione
-- + fonte) e gli adattamenti climatizzazione (n. unità interne, tipologia impianto). ADDITIVA/IDEMPOTENTE.
...
-- Progetto (hub) ──────────────────────────────────────────────────────────────
-- Adattamenti climatizzazione: numero_unita_interne + tipologia_impianto (split/
-- multisplit/vrf); superficie e tipo intervento riusano immobile_superficie_mq /
-- tipo_intervento (DRY). massimale_detrazione = tetto di spesa per gli incentivi.
...
DO $$ BEGIN
  ALTER TABLE public.clm_progetti
    ADD CONSTRAINT clm_progetti_stato_chk CHECK (
...
COMMENT ON COLUMN public.clm_computo_voci.fonte IS 'Prezzario regionale di provenienza della voce (per citazione base d''asta). NULL = voce libera/listino.';
