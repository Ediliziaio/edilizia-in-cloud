-- Pavimenti · Verticale preventivo (clone di Ristrutturazione, namespace pav_*).
-- Greenfield: un'unica migrazione con lo schema FINALE (base + builder + personalizzazione
-- + fonte) e gli adattamenti pavimenti (n. ambienti, tipo materiale). ADDITIVA/IDEMPOTENTE.
-- RLS company-scoped (super_admin OR company match). LOCALE: applicata in pubblicazione.

-- Listino lavorazioni aziendale ──────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS public.pav_listino_capitoli (
...
CREATE TABLE IF NOT EXISTS public.pav_listino_voci (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  company_id uuid NOT NULL REFERENCES public.companies(id) ON DELETE CASCADE,
  capitolo_id uuid REFERENCES public.pav_listino_capitoli(id) ON DELETE SET NULL,
  codice text,
...
-- Progetto (hub) ──────────────────────────────────────────────────────────────
-- Adattamenti pavimenti: numero_ambienti + tipo_materiale (gres/parquet/resina/
-- microcemento/pietra); superficie e tipo intervento riusano immobile_superficie_mq
-- / tipo_intervento (DRY). massimale_detrazione = tetto di spesa degli incentivi.
CREATE TABLE IF NOT EXISTS public.pav_progetti (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  company_id uuid NOT NULL REFERENCES public.companies(id) ON DELETE CASCADE,
  code text,
...
DO $$ BEGIN
  ALTER TABLE public.pav_progetti
    ADD CONSTRAINT pav_progetti_stato_chk CHECK (
...
CREATE TABLE IF NOT EXISTS public.pav_computo_voci (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  progetto_id uuid NOT NULL REFERENCES public.pav_progetti(id) ON DELETE CASCADE,
  company_id uuid NOT NULL REFERENCES public.companies(id) ON DELETE CASCADE,
...
COMMENT ON COLUMN public.pav_computo_voci.fonte IS 'Prezzario regionale di provenienza della voce (per citazione base d''asta). NULL = voce libera/listino.';

-- Media ──────────────────────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS public.pav_progetti_media (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  progetto_id uuid NOT NULL REFERENCES public.pav_progetti(id) ON DELETE CASCADE,
  company_id uuid NOT NULL REFERENCES public.companies(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS public.pav_template_pdf (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
...
-- Indici
CREATE INDEX IF NOT EXISTS idx_pav_progetti_company ON public.pav_progetti(company_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_pav_computo_progetto ON public.pav_computo_voci(progetto_id, ordine);
CREATE INDEX IF NOT EXISTS idx_pav_listino_voci_company ON public.pav_listino_voci(company_id, capitolo_id, ordine);
CREATE INDEX IF NOT EXISTS idx_pav_media_progetto ON public.pav_progetti_media(progetto_id, ordine);

-- RLS (super_admin OR company_id = get_user_company_id) su tutte le tabelle
DO $$
DECLARE t text;
BEGIN
  FOREACH t IN ARRAY ARRAY['pav_listino_capitoli','pav_listino_voci','pav_progetti','pav_computo_voci','pav_progetti_media','pav_template_pdf'] LOOP
    EXECUTE format('ALTER TABLE public.%I ENABLE ROW LEVEL SECURITY', t);
    EXECUTE format($q$DROP POLICY IF EXISTS "pav super admin all" ON public.%I$q$, t);
    EXECUTE format($q$CREATE POLICY "pav super admin all" ON public.%I FOR ALL
      USING (has_role(auth.uid(),'super_admin'::app_role)) WITH CHECK (has_role(auth.uid(),'super_admin'::app_role))$q$, t);
    EXECUTE format($q$DROP POLICY IF EXISTS "pav company members" ON public.%I$q$, t);
    EXECUTE format($q$CREATE POLICY "pav company members" ON public.%I FOR ALL TO authenticated
