...
-- Ogni conversazione è un canale silvio-ai (DM utente↔Silvio); la cartella è
-- per-utente. Additiva: nuova tabella + colonna nullable (FK SET NULL) + RPC
-- SECURITY DEFINER scoped ad auth.uid(). NON tocca il motore silvio-chat.

-- 1) Tabella cartelle (per-utente, dentro l'azienda)
...
-- 2) Colonna folder sul canale conversazione (FK → SET NULL alla cancellazione cartella)
...
-- 3) RPC: lista cartelle dell'utente (con conteggio conversazioni)
...
-- 6) RPC: elimina cartella (le conversazioni tornano "senza cartella" via FK SET NULL)
...
-- 7) RPC: sposta conversazione in cartella (p_folder_id NULL = togli dalla cartella)
...
-- 8) Ricrea lista conversazioni includendo folder_id
