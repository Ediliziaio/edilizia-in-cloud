...
// Render Pergole AI — Multi-Provider (OpenAI / Gemini)
...
const PERGOLA_TYPE: Record<string, string> = {
  ...
  legno_addossata: "wall-mounted laminated timber pergola with warm structural beams and realistic carpentry joints",
  legno_autoportante: "freestanding laminated timber pergola with robust posts, beams and realistic outdoor joinery",
};
...
const MATERIAL: Record<string, string> = {
  ...
  acciaio: "painted steel structure with slightly heavier sections, welded/bolted details and durable outdoor coating",
...
const COVER_STATE: Record<string, string> = {
  ...
  aperta: "roof cover open enough to reveal sky/light path through the structure",
  lamelle_15: "louvers tilted about 15 degrees, almost closed with narrow light gaps",
  lamelle_30: "louvers tilted about 30 degrees with directional light filtering",
...
function buildPergolaPrompt(session: Record<string, unknown>) {
  ...
  const wallMounted = bool(install.addossata_si_no, text(struttura.tipo).includes("addossata"));
  const tipoPergola = text(struttura.tipo, wallMounted ? "bioclimatica_addossata" : "bioclimatica_autoportante");
  ...
  const targetMap = {
    zone,
    target_description: zone === "custom" ? text(install.descrizione_zona, "user-described outdoor installation zone") : zone.replace(/_/g, " "),
    footprint: `${text(install.larghezza_apparente, "media")} width x ${text(install.profondita_apparente, "standard")} depth apparent footprint, scaled to the photographed outdoor area and never oversized`,
  ...
  const envelope = {
    structural_height: text(install.altezza_apparente, "standard buildable clear height"),
    post_count: postCount,
    post_positions: wallMounted ? ["front-left grounded post", "front-right grounded post"] : ["four grounded corner posts aligned to perspective"],
    anchoring_logic: `${text(install.ancoraggio_a_terra, zone === "giardino_relax" ? "prato_con_plinti" : "pavimento").replace(/_/g, " ")} anchoring with believable foot plates / concealed anchors / foundations`,
    facade_relation: wallMounted ? "explicit wall-mounted relation with credible facade attachment and waterproof rear junction" : "freestanding independent structure; no fake wall brackets",
    drainage_logic: "integrated water management: perimeter gutter/profile or slight pitch must make rainwater runoff plausible; downpipe only if visible/selected and coherent",
  };
  ...
  const pergolaSpec = `${PERGOLA_TYPE[tipoPergola] || tipoPergola}; ${MATERIAL[material] || material}; color ${colorName} (${colorHex}); cover ${COVER[coverType] || coverType}; state ${COVER_STATE[coverState] || coverState}`;
  ...
  if (operation === "replace_existing_awning_with_pergola") {
    ...
    replacements.push(`Replace the old shading system with the selected pergola: ${pergolaSpec}.`);
    conversions.push("No old awning arms, fabric, cassette or support brackets may remain.");
    ...
    replacements.push(`Install only the newly selected pergola system: ${pergolaSpec}.`);
    conversions.push("Do not mix old and new pergola structures.");
  } else if (operation === "recolor_only") {
    recolors.push(`Change only the existing pergola structure finish/color to ${colorName} (${colorHex}).`);
    conversions.push("Recolor-only: preserve exact footprint, post positions, beam geometry, cover type, side closures and structural proportions.");
  } else if (operation === "change_cover_only") {
    replacements.push(`Keep existing pergola structure and replace only the roof/cover system with ${COVER[coverType] || coverType}; ${COVER_STATE[coverState] || coverState}.`);
    conversions.push("Cover-only: do not move posts or beams; only cover, edge trims and drainage details may adapt.");
  } else if (operation === "add_side_closures") {
    additions.push(`Add side closures: ${SIDE[sideType] || sideType}; state ${sideState.replace(/_/g, " ")}.`);
    conversions.push("Side closure addition must not change structure, cover or footprint except required tracks/guides.");
  } else if (operation === "remove_side_closures") {
    removals.push("Remove side closures, screens, curtains, panels and visible tracks/guides where incompatible with an open pergola perimeter.");
    conversions.push("Restore clean open perimeter with no dangling rails or fabric.");
  } else if (operation === "change_open_state") {
    replacements.push(`Change only the cover/opening state: ${COVER_STATE[coverState] || coverState}.`);
    conversions.push("Open-state change preserves structure, footprint, material, side closures and mounting.");
  } else {
    additions.push(`Install a new ${tipoPergola.replace(/_/g, " ")} in the target zone: ${pergolaSpec}.`);
    conversions.push("Do not remove existing architecture; adapt only contact shadows and realistic installation junctions.");
  }

  if (sideType !== "nessuna" && operation !== "remove_side_closures") {
    additions.push(`Include side closure system only where selected: ${SIDE[sideType] || sideType}; state ${sideState.replace(/_/g, " ")}; avoid generic decorative curtains.`);
  }
  if (lighting !== "nessuna") {
    additions.push(`Add integrated lighting: ${LIGHTS[lighting] || lighting}; sparse, buildable and consistent with the pergola profiles.`);
  }
  if (text(arredo.gestisci_arredo, "mantieni") === "mantieni") {
    ...
    additions.push(`Add only sparse coherent outdoor furniture for ${text(arredo.uso_area, "relax").replace(/_/g, " ")} use below pergola, avoiding showroom staging.`);
  } else if (text(arredo.gestisci_arredo) === "rimuovi_superfluo") {
    removals.push("Declutter only small non-essential outdoor objects; do not remove primary functional furniture unless listed.");
  }
  ...
  const blocks: Record<string, string> = {
    ...
    B: `[BLOCK B - EXISTING OUTDOOR SCENE INVENTORY]\nOutdoor area: infer from photo (${zone.replace(/_/g, " ")}). Facade/openings: read all doors, windows, shutters, thresholds, eaves and gutters from the image. Existing paving, pool, parapets, garden, furniture, walls, fences and neighboring context must be preserved unless explicitly selected. Preserve original light and shadows.`,
    ...
    F: `[BLOCK F - PERGOLA STRUCTURE SPECIFICATION]\nTypology: ${tipoPergola}; ${PERGOLA_TYPE[tipoPergola] || tipoPergola}\nWall-mounted: ${wallMounted ? "yes" : "no, freestanding independent"}\nMaterial: ${MATERIAL[material] || material}\nColor/finish: ${colorName} (${colorHex})\nPosts and beams: grounded, proportional to span, believable joints, no floating or impossible structure.`,
    G: `[BLOCK G - ROOF / COVER SYSTEM SPECIFICATION]\nCover type: ${coverType}; ${COVER[coverType] || coverType}\nOpen state: ${COVER_STATE[coverState] || coverState}\nCover must read as selected system, not a generic canopy; align modules/blades/fabric/glass panels to the same perspective and light.`,
    H: `[BLOCK H - SIDE CLOSURES SPECIFICATION]\nSide closure: ${sideType}; ${SIDE[sideType] || sideType}; state ${sideState.replace(/_/g, " ")}. Must attach to posts/beams with tracks, guides or panels where relevant; no random curtains unless selected.`,
    I: `[BLOCK I - DRAINAGE / WATER MANAGEMENT RULES]\n${bullets([envelope.drainage_logic, "wall-mounted pergolas need plausible rear waterproof junction and front/side runoff logic", "glass, polycarbonate and opaque covers need visible edge profiles, seals and drainage-compatible slope", "bioclimatic louvers need integrated perimeter water channel logic", "fabric covers need believable tension/collection profile, not water-heavy sagging textile"])}`,
    J: `[BLOCK J - INSTALLATION REALISM RULES]\n${bullets([envelope.anchoring_logic, "wall attachments, post foot plates, profile transitions and contact shadows must be credible", "no blocked door/window operation unless explicitly accepted", "no collision with shutters, windows, eaves, gutters, parapets, pool edge or furniture", "new shadows and reflected light must match original sun direction and outdoor context"])}`,
    ...
    L: `[BLOCK L - REMOVAL / CONVERSION RULES]\n${bullets([...removals, ...conversions, "if replacing an existing awning/pergola, remove old brackets/cassettes/arms/posts/rails/supports completely", "patch facade and floor contact points cleanly", "do not leave hybrid old/new systems", "if recolor-only, do not alter geometry, cover type, footprint or post positions"])}`,
  ...
  const userPrompt = [blocks.B, blocks.C, blocks.D, blocks.E, blocks.F, blocks.G, blocks.H, blocks.I, blocks.J, blocks.K, blocks.L, blocks.M, blocks.N, blocks.O, blocks.P, notes ? `[ADDITIONAL USER NOTES]\n${notes}` : ""]
    .filter(Boolean)
    .join("\n\n");
  ...
  return {
    ...
    promptPayload: {
      ...
      technical_specification: { tipoPergola, material, coverType, coverState, sideType, sideState, lighting, colorName, colorHex },
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!allowed) {
      return new Response(JSON.stringify({ error: "forbidden", message: "Accesso negato alla sessione render pergola" }), {
        status: 403,
        headers: { ...CORS, "Content-Type": "application/json" },
      });
    }
    ...
    const { systemPrompt, userPrompt, promptVersion, promptPayload } = buildPergolaPrompt({ ...session, config: rawConfig });
    ...
    if (providerConfig.provider_key === "openai") {
      ...
      const geminiBody = {
        ...
        generationConfig: { responseModalities: ["IMAGE", "TEXT"], temperature: 0.62 },
      };
    ...
    const resultPath = `${session.company_id}/${session_id}/render_pergole_${Date.now()}.png`;
    const { error: uploadErr } = await supabase.storage.from("pergole-results").upload(resultPath, uint8, {
      contentType: "image/png",
      upsert: true,
    });
    if (uploadErr) throw new Error(`Errore upload risultato: ${uploadErr.message}`);

    const { data: publicUrlData } = supabase.storage.from("pergole-results").getPublicUrl(resultPath);
    ...
    return new Response(JSON.stringify({ success: true, session_id, result_url: resultUrl, prompt_version: promptVersion, prompt_char_count: finalProviderPrompt.length }), {
      status: 200,
      headers: { ...CORS, "Content-Type": "application/json" },
    });
