...
function resolveAccessRecipientAlias(data: Record<string, unknown>, key: string): unknown {
  ...
  const aliases: Record<string, unknown> = {
    ...
    // ── Email di sistema (58 copy riscritti) — alias verso i prop dei chiamanti ──
    "inviter.name": coalesce(data.inviterName, data.invitedByName),
    "accountant.name": coalesce(data.accountantName, data.recipientName, recipientName.full),
    "customer.name": coalesce(data.customerName, data.recipientName, recipientName.full),
    "sender.name": coalesce(data.senderName, data.fromName),
    "access.level": coalesce(data.accessLevel, data.accessModeLabel, data.access_level),
    "event.date": coalesce(data.eventDate, data.acceptedDate, data.changedDate, data.date),
    "event.time": coalesce(data.eventTime, data.acceptedTime, data.changedTime, data.time),
    "platform.support_email": coalesce(data.supportEmail, data.platformSupportEmail, data.replyTo, data.support_email),
    "platform.name": coalesce(data.platformName, data.platform_name),
    "otp.code": coalesce(data.otpCode, data.code, data.otp),
    "user.new_email": coalesce(data.newEmail, data.userNewEmail),
    "user.inactive_days": coalesce(data.inactiveDays),
    "referral.code": coalesce(data.referralCode, data.referral_code),
    "partner.tier": coalesce(data.partnerTier, data.tier, data.tierName),
    "partner.new_tier": coalesce(data.newTier, data.partnerNewTier),
    "partner.commission": coalesce(data.commissionLabel, data.partnerCommission),
    "partner.multiplier": coalesce(data.multiplier),
    "partner.active_companies": coalesce(data.activeCompanies, data.companyCount),
    "commission.amount": coalesce(data.commissionAmount, data.totalAmount),
    "payout.amount": coalesce(data.payoutAmount, data.amount),
    "payment.reference": coalesce(data.paymentReference, data.reference),
    "payment.amount": coalesce(data.paymentAmount, data.amountFormatted, data.amount),
    "payment.method": coalesce(data.paymentMethod),
    "period.month": coalesce(data.month, data.monthName, data.periodMonth),
    "period.month_year": coalesce(data.monthYear, data.monthName),
    "trial.days_left": coalesce(data.daysLeft, data.daysRemaining),
    "stats.orders": coalesce(data.ordersCount, data.numberOrders),
    "stats.customers": coalesce(data.customersCount, data.custCount),
    "stats.revenue": coalesce(data.revenueFormatted, data.revenue),
    ...
    "device.location": coalesce(data.deviceLocation, data.location),
    "document.name": coalesce(data.documentName),
    "document.type": coalesce(data.documentType),
    "document.expiry_date": coalesce(data.expiryDate, data.dueDate),
    "document.days_left": coalesce(data.documentDaysLeft, data.daysLeft),
    "task.name": coalesce(data.taskName),
    "invoice.number": coalesce(data.invoiceNumber),
    "sdi.reject_reason": coalesce(data.rejectReason),
    "inspection.date": coalesce(data.inspectionDate),
    "inspection.time": coalesce(data.inspectionTime),
    "site.address": coalesce(data.siteAddress),
    "feature.name": coalesce(data.featureName),
    "maintenance.date": coalesce(data.maintenanceDate),
    "maintenance.window": coalesce(data.maintenanceWindow),
    // Link CTA: il primo link utile passato dal chiamante (single-CTA = la maggior parte).
    "link_url_1": coalesce(
