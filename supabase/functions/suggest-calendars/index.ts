...
async function getDirections(
  ...
  if (apiKey) {
    try {
      ...
      const params = new URLSearchParams({ origin, destination, mode: "driving", language: "it", key: apiKey });
...
// Debug logger — collects entries only when debug=true
type DebugEntry = Record<string, unknown>;
function createDebugLogger(enabled: boolean) {
  const entries: DebugEntry[] = [];
  return {
    log: (entry: DebugEntry) => { if (enabled) entries.push({ ts: Date.now(), ...entry }); },
    entries,
  };
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { company_id, client_lat, client_lng, date, desired_start_time, debug } = body;
    ...
    dbg.log({ phase: "request", company_id, client_lat, client_lng, date, desired_start_time });
    ...
    dbg.log({ phase: "calendars_found", count: calendars.length, ids: calendars.map((c) => c.id) });
    ...
    dbg.log({ phase: "preferences", defaultMaxDailyKm, maxTravelMinutes, defaultDuration });
    ...
    dbg.log({ phase: "maps_provider", hasApiKey: !!apiKey });
    ...
    for (const cal of calendars) {
      ...
      if (!hasBase) {
        dbg.log({ phase: "calendar_skip", calendar_id: cal.id, reason: "no_base_coordinates" });
        suggestions.push({
          calendar_id: cal.id,
          calendar_name: cal.name,
          owner_name: cal.owner_id ? ownerProfiles[cal.owner_id] || null : null,
          travel_km: 0, travel_minutes: 0, daily_km_if_assigned: 0, max_daily_km: maxKm,
          suggested_times: [], reason: "Base non configurata", status: "WARNING", score: 9999,
          daily_route: [], is_estimate: false,
        });
        continue;
      }
      ...
      for (const a of appts) {
        if (a.lat != null && a.lng != null && !a.is_blocked_slot) {
          ...
          if (d < closestDist) { closestDist = d; closestAppt = a; }
        }
      }

      // Simulate adding new appointment
      ...
      for (const a of appts) {
        if (a.lat != null && a.lng != null) {
          if (!inserted && desired_start_time) {
            ...
            if (desired_start_time < aTime) { simWaypoints.push(newApptWaypoint); inserted = true; }
          }
      ...
      // Travel time to client from nearest point
      let travelToClient = baseToClient;
      let travelOrigin = "base";
      if (closestAppt && closestAppt.lat != null && closestAppt.lng != null) {
        ...
        if (fromClosest.total_minutes < baseToClient.total_minutes) {
          ...
          travelOrigin = `closest_appt:${closestAppt.id}`;
        }
      }
      ...
      const calAvail = (allAvailability || []).filter(
        (a) => a.calendar_id === cal.id &&
          ((a.specific_date === date) || (a.specific_date === null && a.day_of_week === dayOfWeek))
      );
      const availWindows = calAvail.map((a) => ({ start: timeToMinutes(a.start_time), end: timeToMinutes(a.end_time) }));
      ...
      if (desired_start_time) {
        ...
        const hasConflict = appts.some((a) => {
          ...
          const aEnd = a.appointment_end_time ? timeToMinutes(a.appointment_end_time.substring(0, 5)) : aStart + calDuration;
          return desiredMin < aEnd && desiredMin + calDuration > aStart;
        });
        if (!hasConflict) suggestedTimes.push(desired_start_time);
      }
      ...
      if (closestAppt && suggestedTimes.length < 3) {
        ...
        const hasConflict = busySlots.some((s) => afterClosest < s.end && afterClosest + calDuration > s.start);
        if (!hasConflict && !suggestedTimes.includes(t)) suggestedTimes.push(t);
      }

      // Determine status using extracted function
      const statusResult = determineStatus(
        travelToClient.total_minutes, maxTravelMinutes,
      ...
      const score = calculateScore(
        travelToClient.total_minutes, travelToClient.total_km,
        maxTravelMinutes, simRoute.total_km, maxKm
      );

      // Debug log for this calendar
      dbg.log({
        phase: "calendar_eval",
        calendar_id: cal.id,
        calendar_name: cal.name,
        origin: travelOrigin,
        origin_coords: travelOrigin === "base"
          ? { lat: baseLat, lng: baseLng }
          : closestAppt ? { lat: closestAppt.lat, lng: closestAppt.lng } : null,
        destination: { lat: client_lat, lng: client_lng },
        ...
        daily_km_current: currentRoute.total_km,
        daily_km_simulated: simRoute.total_km,
        max_daily_km: maxKm,
        existing_appointments: appts.length,
        slot_generation: {
          found: suggestedTimes.length,
          slots: suggestedTimes,
          avail_windows: availWindows.length,
      ...
      dailyRoute.push({ label: "Base", address: cal.base_formatted_address || company?.operational_address || "Sede" });
      for (const a of appts) {
        dailyRoute.push({
      ...
      dailyRoute.push({ label: "NUOVO", address: body.client_address || `${client_lat}, ${client_lng}` });
      dailyRoute.push({ label: "Base", address: cal.base_formatted_address || company?.operational_address || "Sede" });
    ...
    const responseBody: Record<string, unknown> = { suggestions: suggestions.slice(0, 5) };
