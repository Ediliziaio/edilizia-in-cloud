 ...
 *
 * Genera l'HTML completo del preventivo Fotovoltaico v2 (16 pagine
 ...
 *   - Renderizza il template HTML (16 pagine con SVG inline)
 ...
 *   tipo='vendita' → HTML 16 pagine completo (default)
...
import { renderFvPdfHtml, type FvPdfTemplateData } from "../_shared/fvHtmlTemplate.ts";
  ...
  try {
    ...
    const [calcRes, compRes, manodRes, servRes, companyRes] = await Promise.all([
      ...
      supabaseAdmin
        ...
        .select("categoria, descrizione, marca, modello, quantita, potenza_unitaria_w, capacita_kwh, garanzia_anni, prezzo_unitario_vendita")
        .eq("progetto_id", p.progetto_id)
        .order("ordinamento"),
      supabaseAdmin
      ...
      supabaseAdmin
        ...
        .select("descrizione, prezzo_vendita")
        .eq("progetto_id", p.progetto_id),
      supabaseAdmin
        .from("companies")
    ...
    const validoGiorni = 30; // default; future: lettura da template_pdf.valido_giorni
    ...
    const data: FvPdfTemplateData = {
      ...
      flows,
      componenti: (compRes.data ?? []).map((c: Record<string, unknown>) => ({
        categoria: String(c.categoria ?? ""),
        descrizione: String(c.descrizione ?? ""),
        marca: c.marca ? String(c.marca) : null,
        modello: c.modello ? String(c.modello) : null,
    ...
    // ── Upload su storage ──────────────────────────────────────────────────
    const filename = `${prog.numero}-${payloadTipo}.html`;
    const path = `${prog.company_id}/${prog.id}/${Date.now()}-${filename}`;
    const { error: errUp } = await supabaseAdmin.storage
      .from("fv-progetti")
      .upload(path, htmlBytes, { contentType: "text/html", upsert: false });
