...
const DEFAULT_DATA_GATHERING_CONCURRENCY = 3;
  ...
  try {
    ...
    if (!trigger_type && !override_playbook_id) {
      return errorResponse(
        "trigger_type o override_playbook_id required",
        400,
        cors,
      );
    }
    ...
    if (override_playbook_id) {
      const { data, error } = await supabaseAdmin
        ...
        .maybeSingle();
      if (error) {
        return errorResponse(`load override: ${error.message}`, 500, cors);
      }
    ...
    if (!playbook) {
      return errorResponse(
        `no playbook matches trigger=${trigger_type} source=${trigger_source_type} alert=${alert_type}`,
        404,
        cors,
      );
    }
    ...
    // ── 2. Load persona ──────────────────────────────────────────────────
    const { data: personaPermission, error: personaPermissionError } =
      await supabaseAdmin.rpc("can_user_use_persona", {
        p_user_id: userId,
        p_persona_key: persona_key,
      });
    if (personaPermissionError) {
      return errorResponse(
        `persona permission: ${personaPermissionError.message}`,
        500,
        cors,
      );
    }
    if (!personaPermission?.allowed) {
      return errorResponse(
        `persona ${persona_key} non autorizzata: ${
          personaPermission?.reason ?? "rbac_denied"
        }`,
        403,
        cors,
      );
    ...
    const { data: persona } = await supabaseAdmin
      ...
      .maybeSingle();
    if (!persona) {
      return errorResponse(`persona ${persona_key} not found`, 404, cors);
    }

    // ── 3. Data gathering a concorrenza controllata ──────────────────────
    const gathered: AnyObj = {};
    const gatherStart = Date.now();
    const dataGatheringConcurrency = readEnvInt(
      "SILVIO_PLAYBOOK_DATA_GATHERING_CONCURRENCY",
      DEFAULT_DATA_GATHERING_CONCURRENCY,
      1,
      8,
    );
    const results = await runLimited(
      playbook.data_gathering,
      dataGatheringConcurrency,
      async (step: AnyObj) => {
        const params: AnyObj = {
        ...
        try {
          ...
          if (error) {
            return [step.step, { error: error.message, rpc: step.rpc }];
          }
    ...
    if (playbook.kb_context_areas && playbook.kb_context_areas.length > 0) {
      try {
        ...
        if (OPENAI_KEY) {
          const queryText = user_request_text ??
            `${playbook.title} ${playbook.description ?? ""}`;
          const { embedding, tokens: providerTokens } =
            await generateQueryEmbedding(OPENAI_KEY, queryText);
          ...
          await chargeDirectAiCall({
            ...
            idempotencyKey: await buildStableAiIdempotencyKey(
              "playbook_rag_embedding",
              [
                company_id,
                userId,
                playbook.id,
            ...
            costRealUsd: providerTokens
              ? Math.max(
                ...
                (tokensIn / 1_000_000) *
                  Number(
                    Deno.env.get("AI_EMBEDDING_3_SMALL_USD_PER_1M_TOKENS") ??
                      "0.02",
                  ),
              )
    ...
    const userPrompt = playbook.ai_prompt_template
      ...
      .replace(
        "{diagnosis_questions}",
        "- " + (playbook.diagnosis_questions ?? []).join("\n- "),
      )
    ...
    const { prompt: systemPromptComplete } = await buildSystemPrompt(
      supabaseAdmin,
    ...
    try {
      ...
      return errorResponse(
        `AI router: ${e instanceof Error ? e.message : String(e)}`,
        502,
        cors,
      );
    }
    ...
    try {
      ...
      return errorResponse(
        `AI returned invalid JSON (parse: ${
    ...
    // Validation minima
    if (
      !parsed.situation || !Array.isArray(parsed.options) ||
      parsed.options.length === 0
    ) {
      return errorResponse(
        "AI proposal missing required fields (situation, options[])",
        502,
        cors,
      );
    }
    ...
    try {
      const { data: dlData, error: dlErr } = await supabaseAdmin.rpc(
      ...
      if (dlErr) {
        console.error("[playbook-execute] decision_log save:", dlErr.message);
    ...
    return jsonResponse(
      {
        ...
        proposal: parsed,
        gathered_summary: Object.fromEntries(
          Object.entries(gathered).map((
            [k, v],
          ) => [k, typeof v === "object" ? "ok" : v]),
        ),
        ai_meta: {
