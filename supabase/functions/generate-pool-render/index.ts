...
import { collectPoolReferenceImages } from "../../../shared/render-references/outdoorReferences.ts";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const renderJob = (async () => {
      ...
      try {
        const refsCondivise = collectPoolReferenceImages((() => { const c = asRecord(session.config); const p = asRecord(c.piscina); return { operazione: c.operazione as string | undefined, tipo: p.tipo as string | undefined }; })());
        if (refsCondivise.length > 0) {
          const fetched = await fetchSharedReferenceImages(refsCondivise, (entry) => console.log(JSON.stringify({ fn: "generate-pool-render", session_id, ...entry })));
        ...
        console.warn(JSON.stringify({ lvl: "warn", fn: "generate-pool-render", session_id, msg: "shared_references_threw", error: String((refErr as Error)?.message ?? refErr) }));
      }
