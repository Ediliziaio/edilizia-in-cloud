  ...
  try {
    ...
    await supabase
      ...
      .eq("id", session_id);
	    ...
	    const effectiveWidth = prepared.effective_width ?? target_width ?? undefined;
	    const effectiveHeight = prepared.effective_height ?? target_height ?? undefined;
    ...
    const finalProviderPrompt = `${systemPrompt}\n\n${userPrompt}`;
	    ...
	    const resultPath = `${session.company_id}/${session_id}/render_piscine_${Date.now()}.${uploadPayload.extension}`;
	    const { error: uploadErr } = await supabase.storage.from("piscine-results").upload(resultPath, uploadPayload.bytes, {
